...
const dir =
  'docs/external-beta/gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1'
const recordPath =
  `${dir}/gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1-record.json`
const sourceRecordPath =
  'docs/external-beta/gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-qa-rollup-1/gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-qa-rollup-1-record.json'
const integrationBase = '27d558b1868b24a14f076e9aa381bc8e3cae7e22'
...
const sourceDecision = 'qa_passed_gstreamer_mkvtoolnix_narrow_route_worker_noop_source_implementation_evidence'
const sourceExecution =
  'completed_docs_only_narrow_route_worker_noop_source_qa_rollup_no_route_worker_tool_or_media_execution'
const sourceReadiness = 'ready_for_guarded_narrow_route_worker_noop_source_dry_run'
const readiness = 'ready_for_guarded_narrow_route_worker_noop_source_dry_run_qa_rollup'
...
const packetFiles = [
  ...
  recordPath,
  'docs/activation-phase-rp-external-beta-gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1-results.md',
...
const implementationFiles = [
  'server/smoke/rp-external-beta-gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1.ts',
...
const requiredRunnerText = [
  "process.env[RP_EXTERNAL_BETA_GSTREAMER_MKVTOOLNIX_NARROW_ROUTE_WORKER_NOOP_SOURCE_DRY_RUN_CONFIRM_ENV] !== 'true'",
  'blocked_pending_gstreamer_mkvtoolnix_narrow_route_worker_noop_source_dry_run_confirmation',
...
if (
  ...
  'tsx server/smoke/rp-external-beta-gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1.ts'
) {
  fail('missing no-op source dry-run package script')
}
if (
  packageJson.scripts?.[
    'rp-external-beta-gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1:diagnostics'
  ] !==
  'node scripts/validation/rp-external-beta-gstreamer-mkvtoolnix-narrow-external-agent-runtime-guarded-route-worker-noop-source-dry-run-1-diagnostics.mjs'
) {
  fail('missing no-op source dry-run diagnostics package script')
}
...
if (record.sourceChain?.noopSourceQaRollupPr !== 1977) fail('source QA PR mismatch')
if (record.sourceChain?.noopSourceQaRollupMergeSha !== integrationBase) fail('source QA merge SHA mismatch')
if (record.sourceChain?.noopSourceQaDecision !== sourceDecision) fail('source QA decision mismatch')
if (record.sourceChain?.noopSourceQaExecution !== sourceExecution) fail('source QA execution mismatch')
if (record.sourceChain?.noopSourceQaReadiness !== sourceReadiness) fail('source QA readiness mismatch')
...
if (record.dryRun?.responseShapeStatus !== 'accepted_noop_source_contract') fail('response status mismatch')
for (const flag of ['runtimeEnabled', 'routeExecution', 'workerDispatch', 'workerExecution', 'toolExecution']) {
