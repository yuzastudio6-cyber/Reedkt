...
assert.ok((await client.toolRouting.summary({ level: 'premium' })).data?.includes('planning depth'))
assert.ok((await client.qwenRouting.summary({ level: 'ultra_premium' })).data?.includes('visual understanding'))
