...
assert.ok(repository.createToolRoutingSummary({ level: 'premium' }).data?.includes('planning depth'))
assert.ok(repository.createQwenRoutingSummary({ level: 'ultra_premium' }).data?.includes('visual understanding'))
