...
| Critical | Local/staging Supabase/RLS not executed | Access policies are not proven. | Cross-workspace data exposure. | Repair local migration chain, then run the existing first local executable SQL candidate; staging SQL requires later human approval. | Prompt 20I, then retry Prompt 20B SQL execution if local `supabase start` passes | No. | Yes |
| Critical | Remote/staging migration not validated | Schema chain may not apply cleanly. | Runtime data corruption or missing tables. | Validate migration chain in approved local environment first, then staging only with approval. | Prompt 20I, then Prompt 20B or another migration-chain follow-up depending on local result | No. | Yes |
...
| Medium | Local Supabase migration-chain blocker | Local SQL validation blocked. | Slower RLS iteration and no RLS execution evidence. | Prompt 20H repairs the Prompt 3-era core workspace/project compatibility blockers, but `supabase start` now fails in `202605180002_reeditpro_media_source_sequence.sql` because `public.media_assets.status` is missing before `idx_media_assets_project_status` is created. | Prompt 20I, then rerun Prompt 20B SQL execution if migration-chain validation passes | Yes for static planning only. | Yes before production beta |

## Decision

Prompt 20H repairs the `projects_current_edit_session_id_fkey`, owner-column, and chat-message edit-session compatibility blockers in the core workspace/project migration. Production beta still cannot proceed because local/staging Supabase/RLS has not executed. The next safe milestone is Prompt 20I - Local Supabase Migration Chain Repair Follow-Up for `202605180002_reeditpro_media_source_sequence.sql` before SQL/RLS execution is retried.
