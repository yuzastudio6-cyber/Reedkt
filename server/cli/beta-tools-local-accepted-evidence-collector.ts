...
export interface BetaToolsLocalAcceptedEvidenceCollectorEnv extends BetaToolsLocalAcceptedEvidenceBundleEnv {
  ...
  REEDITPRO_BETA_TOOLS_LOCAL_BUNDLE_REQUIRED_BOUNDED_ACCEPTED_TOOL_COUNT?: string
...
async function postCoreEvidence(
  ...
  const request = buildBetaToolsCoreRealCheckEvidenceRequest({
    ...
    REEDITPRO_BETA_TOOLS_ACCEPT_PRODUCTION_READINESS: effectiveProductionReadinessAcceptance(env) ? 'true' : undefined,
    REEDITPRO_BETA_TOOLS_CONFIRM_PRODUCTION_READINESS_ACCEPTANCE: effectiveProductionReadinessAcceptance(env) ? 'true' : undefined,
...
function parseRequiredCount(value: string | undefined, fallback: number): number {
  ...
  if (!Number.isFinite(parsed) || parsed < 0) {
    throw new Error('REEDITPRO_BETA_TOOLS_LOCAL_BUNDLE_REQUIRED_PRODUCT_READY_LOCAL_OSS_COUNT must be a non-negative integer.')
  }
  return parsed
}

function effectiveProductionReadinessAcceptance(env: BetaToolsLocalAcceptedEvidenceCollectorEnv): boolean {
  return parseBoolean(env.REEDITPRO_BETA_TOOLS_LOCAL_BUNDLE_ACCEPT_PRODUCTION_READINESS) ||
    parseBoolean(env.REEDITPRO_BETA_TOOLS_LOCAL_BUNDLE_ACCEPT_BOUNDED_ACCEPTED_EVIDENCE)
}
