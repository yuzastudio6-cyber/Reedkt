...
import { QWEN2_5_VL_58DZ_EXTERNAL_AGENT_WRAPPER_RERUN_RESULT } from '../../src/backend/mock/mock-qwen2-5-vl-58dz-external-agent-wrapper-rerun-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/qwen2-5-vl-7b-58dz-external-agent-wrapper-rerun-result.md'
const SPEC_PATH = 'src/backend/mock/mock-qwen2-5-vl-58dz-external-agent-wrapper-rerun-result.ts'
const SMOKE_PATH = 'server/smoke/qwen2-5-vl-58dz-external-agent-wrapper-rerun-result-smoke.ts'
const WRAPPER_PATH = 'server/cli/external-agent-tool-execute-qwen.ts'
const PACKAGE_SCRIPT = 'smoke:qwen2-5-vl-58dz-external-agent-wrapper-rerun-result'
const DECISION = 'qwen2_5_vl_58dz_external_agent_wrapper_rerun_passed_result_review_required'
const NEXT_PROMPT =
...
function scanForbiddenValues(value: unknown, prefix = 'qwen58dzWrapperRerunResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/qwen2-5-vl-58dz-external-agent-wrapper-rerun-result-smoke.ts',
...
for (const required of [
  ...
  'run id: `qwen58dw-20260701T230400`',
  'caller execution: `reeditpro-qwen2-5-vl-private-caller-fx8cw`',
  ...
  assert.equal(doc.includes(required), true, `Wrapper rerun result doc missing ${required}`)
}
...
const result = QWEN2_5_VL_58DZ_EXTERNAL_AGENT_WRAPPER_RERUN_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'qwen2_5_vl_58dz_external_agent_wrapper_rerun_result')
...
assert.equal(result.reviewedRun.runId, 'qwen58dw-20260701T230400')
assert.equal(result.reviewedRun.executionName, 'reeditpro-qwen2-5-vl-private-caller-fx8cw')
assert.equal(result.reviewedRun.jobExitCode, 0)
assert.equal(result.reviewedRun.selectedGpu, 'nvidia_l4')
assert.equal(result.reviewedRun.serviceGenerationBeforeRun, 48)
assert.equal(result.reviewedRun.serviceGenerationAfterRestore, 50)
...
assert.equal(result.blockedEvidence.brollStillQuotaBlocked, true)
