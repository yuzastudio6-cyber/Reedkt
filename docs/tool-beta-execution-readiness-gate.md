...
- Durable billing persistence remains backend-required before billable beta execution.
...
The current evidence is dry-run and source-of-truth only. External beta needs real bounded runtime checks, deployment/storage/security approvals, durable cost persistence, model/license approvals, monitoring, rollback, and support readiness.

The gate is intentionally strict: if any tool lacks coverage, readiness evidence, billing persistence, model approval, or product-ready acceptance, external beta and paid production remain blocked.
