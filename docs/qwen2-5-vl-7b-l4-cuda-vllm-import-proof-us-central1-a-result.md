...
The following preflight checks passed before the `us-central1-a` create attempt:
...
| Exact proof regional address before create | absent |
...
g2-standard-8 with 1 nvidia-l4 accelerator is currently unavailable in us-central1-a.
...
| Resource | Result |
| --- | --- |
| Instance `reeditpro-qwen2-5-vl-l4-proof` in `us-central1-a` | absent |
| Disk `reeditpro-qwen2-5-vl-l4-proof` in `us-central1-a` | absent |
| Reservation `reeditpro-qwen2-5-vl-l4-proof` in `us-central1-a` | absent |
...
The correct next step is a single approved retry in `us-central1-c`, the remaining same-region candidate identified by the stockout plan.
