# AI-VIDEO-BROLL-GEN-9N-NO-IDLE-L4-PROOF-EXECUTE-US-CENTRAL1-C

Run one bounded no-idle L4 VM lifecycle proof in `us-central1-c` with mandatory cleanup and no model inference.

Exact next prompt: `AI-VIDEO-BROLL-GEN-9N-NO-IDLE-L4-PROOF-EXECUTE-US-CENTRAL1-C: run bounded no-idle L4 VM lifecycle proof in us-central1-c with mandatory cleanup, no model inference`.

This future prompt may create at most one prompt-scoped no-public-IP `g2-standard-4` VM with one NVIDIA L4 in `us-central1-c` after repeated preflight passes. It must delete only the VM it creates and verify the proof instance, disk, static address, and reservation are absent before completion.
