...
| Revision and recovery | `private_executable` through accepted second review, superseded-history recovery, and pre-lease cancellation | An exact review-bound revision completes snapshot-v2 execution/acceptance. Fresh history-service instances reopen current and superseded QA-passed MP4 bytes. A separate authenticated cancellation route atomically releases a fully unused synthetic reservation before lease creation, including after packaging, under a shared cancellation/claim fence while preserving snapshot/job/package evidence. | Cancellation after lease/dispatch, deployed disaster recovery/retention, and real-user/browser handoff remain gated. |
...
14. **Completed for authenticated local/private pre-lease scope:** atomically cancel a fully unused synthetic reservation before any lease/dispatch exists, including after execution packaging, preserve immutable snapshot/job/package evidence, serialize cancellation against lease claim, and deny future active package authority. Post-lease worker-fenced cancellation remains gated.
