...
The legacy upload journey still reports its historical fail-closed seam, and the legacy standalone-credit browser journey remains blocked. Separately, the authenticated local/private canonical path now preserves unique incremental source identity and finalized media identity, synchronizes current Exact Edit Preferences into server authority, derives verified source/frame evidence, and can present and approve an exactly representable source-and-caption plan through frontend-safe coordinators. The named-edit browser can also request or recover one exact private execution handoff from the approved snapshot. That route returns only a bounded receipt and starts no work graph, worker, job, tool, provider, render, billing, wallet, or delivery action. The exact compiler now supports bounded duration-preserving source sequences, and a dedicated two-source fixture completes snapshot, synthetic reservation, lease, one-use dispatch, private composition, final QA, reconciliation, replay, downstream verification, and private download. The normal two-source/rich named-edit aggregate scenario still exits successfully with `blocked_by_exact_multi_source_and_rich_work_item_compilation`; it does not silently simplify the plan or approve it. Internal-service execution advancement, authenticated review mutations, revision submission, history opening, and deployed real-user infrastructure remain required.
...
| Authenticated private upload-to-review pipeline | narrow exact plan presentation/approval/handoff connected; exact ordered multi-source backend lifecycle proven; rich compilation remains blocked | The authenticated backend persists a checksum-protected tenant-scoped planning handoff and content-addressed publication candidate, then presents one server-owned canonical plan only when the plan is exactly representable by the bounded source-and-caption graph. Exact one-source and duration-preserving sequence candidates can be compiled; a two-source backend fixture passes the complete private canonical lifecycle. The named-edit browser binds the visible plan version/hash and estimate identity/hash/maximum to a frontend-safe approval coordinator, then can request a bounded private execution-handoff receipt from the exact approved snapshot. Approval derives jobs but starts no execution; handoff creation exposes no raw package/jobs/tools and likewise starts no execution. | Normal rich multi-source work-item compilation, named-edit work-graph advancement, review mutations, revision submission, history opening, and deployed real-user operations remain blocked. |
...
> ReeditPro has deterministic planning and canonical approval authority plus 50 exact private tool lifecycles. Its bounded compiler can publish one exact source or a duration-preserving ordered source sequence with one approved caption; a two-source fixture passes the complete canonical private lifecycle through final QA, reconciliation, replay, and private download. A named-edit browser can request a bounded private handoff from the exact approved snapshot without starting execution. The normal rich multi-source editor plan remains approval-blocked because its captions, transitions, audio/SFX/ducking, color, provider, and segment operations are not silently discarded. Named-edit work-graph/review mutations, deployed real-user infrastructure, external beta, public delivery, live billing, Supabase/RLS tenancy, distributed workers, and production promotion remain gated.
