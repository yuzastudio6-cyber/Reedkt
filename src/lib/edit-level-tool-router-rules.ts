...
export function createEditLevelToolRoute(input: {
  ...
  return {
    ...
    reason: hideInternalToolNamesInCopy(input.reason),
...
export function createNormalEditLevelToolRoutes(): EditLevelToolRoute[] {
  ...
  return [
    ...
    route(level, 'deepseek_tool_code', 'future_only', 'future_gated', 'Technical assembly support is not used for user reasoning and is unnecessary for Normal planning now.', 'Use deterministic summaries and existing renderer planning notes.', 'Technical assembly support is not used for user reasoning.'),
...
export function createPremiumEditLevelToolRoutes(): EditLevelToolRoute[] {
  ...
  return [
    ...
    route(level, 'storage_runtime', 'future_only', 'storage_required', 'Private storage runtime is future backend infrastructure.', 'Stay local/mock and do not read file bytes.', 'Storage runtime future-gated.'),
    route(level, 'deepseek_tool_code', 'future_only', 'future_gated', 'Technical assembly support remains renderer-draft support only, not user reasoning.', 'Use deterministic technical planning notes instead.', 'Future technical assembly support only.'),
...
export function createUltraPremiumEditLevelToolRoutes(): EditLevelToolRoute[] {
  ...
  return [
    ...
    route(level, 'storage_runtime', 'future_only', 'storage_required', 'Private storage runtime is future backend infrastructure.', 'Stay local/mock and do not read file bytes.', 'Storage runtime future-gated.'),
    route(level, 'deepseek_tool_code', 'future_only', 'future_gated', 'Technical assembly support is future renderer support only and must not reason with users.', 'Use deterministic technical planning notes instead.', 'Future renderer support only, not user reasoning.'),
...
export function createEditLevelToolRoutingPackage(
  ...
  return {
    ...
    warnings: [
      ...
      'No AI planning, visual understanding, technical assembly, asset service, backend, media, render, progress, Supabase, or credit operation is executed.',
    ],
    ...sideEffectFlags,
    sideEffectFlags,
  }
...
export function createEditLevelToolRouterBoundarySummary(): string[] {
  return [
    ...
    'Selecting a level does not call AI planning, visual understanding, technical assembly support, media assembly, transcript, audio analysis, render, or credits.',
  ]
}
