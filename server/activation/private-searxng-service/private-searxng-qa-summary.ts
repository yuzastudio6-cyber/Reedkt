...
import type {
  ...
  PrivateSearxngQaGate,
  PrivateSearxngQaSummary,
  PrivateSearxngQueryResponse,
  PrivateSearxngServiceValidation,
  PrivateSearxngSourceManifest,
...
export function buildPrivateSearxngQaSummary(input: {
  ...
  const gateMap: Record<string, PrivateSearxngQaGate> = {
    ...
    private_service_deployed_or_resolved: gate('private_service_deployed_or_resolved', input.serviceValidation.deployedOrResolved && input.serviceValidation.blockers.length === 0, 'Private SearXNG Cloud Run service is deployed or resolved without service blockers.'),
  ...
  const gates = privateSearxngQaGateIds.map((id) => gateMap[id])
  const blockers = Array.from(new Set([
    ...(input.preflightBlockers ?? []),
    ...input.serviceValidation.blockers,
    ...input.sourceManifest.blockers,
    ...gates.filter((item) => !item.passed).map((item) => `${item.gateId}: ${item.summary}`),
  ]))
  return {
