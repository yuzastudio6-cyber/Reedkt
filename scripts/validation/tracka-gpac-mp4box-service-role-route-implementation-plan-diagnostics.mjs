...
for (const file of changedFiles) {
  ...
  if (/^src\//.test(file) && !guardedRouteMockImplementationFiles.includes(file)) fail(`forbidden src path changed ${file}`)
  if (/^server\//.test(file) && !guardedRouteMockImplementationFiles.includes(file)) fail(`forbidden server path changed ${file}`)
