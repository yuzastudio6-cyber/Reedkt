...
const controlPlaneObjectPort = createCanonicalGcsSourceAnalysisJsonObjectPort({
  storage,
...
const receipt = await publishTrackAllSam31OrchestraQualification({
  ...
  runtimeReleaseRegistry: createCanonicalSam31GcpGpuRuntimeReleaseRegistry({
    storage,
  }),
  ...
  qualificationRegistry: createCanonicalSkillQualificationRegistry({
    objectPort: controlPlaneObjectPort,
  }),
})
