...
RP-TEST-01 adds a static local testing-readiness check. Run `npm.cmd run build`, `npm.cmd run lint`, and `npm.cmd run test:readiness` before local/staging test execution. The readiness script only reads repo files; it does not run Supabase, SQL, providers, tools, cloud, rendering, billing, media processing, or migrations.
