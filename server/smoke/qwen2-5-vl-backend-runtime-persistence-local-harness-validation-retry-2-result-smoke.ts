...
function assertFalseFlags(flags: JsonRecord) {
  for (const key of [
    'backendRuntimePersistenceLocalHarnessValidationRetry2Passed',
...
const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-local-harness-validation-retry-2-result.md')
for (const phrase of [
  DECISION,
  '`55430`',
  '`55431`',
...
const result = QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_LOCAL_HARNESS_VALIDATION_RETRY_2_RESULT
assert.equal(result.decision, DECISION)
assert.equal(
  result.upstreamBackendRuntimePersistenceBaselineMigrationFixDecision,
...
assert.equal(result.validationOutcome.status, 'blocked_current_edit_session_baseline_column')
assert.equal(result.validationOutcome.previousBaselineMigrationFixVerified, true)
assert.equal(result.validationOutcome.currentEditSessionBaselineFixRequired, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry2ResultRecorded, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry2Attempted, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceBaselineAmbiguousDescriptionFixVerified, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceCurrentEditSessionBaselineFixRequired, true)

const forbiddenDataFindings = scanValues({ result })
assert.deepEqual(
  forbiddenDataFindings,
  [],
  `Forbidden values in Qwen validation retry 2 data: ${forbiddenDataFindings.join('; ')}`,
)

console.log(JSON.stringify({
  ok: true,
  decision: DECISION,
  localHarnessValidationRetry2Attempted: result.validationOutcome.localHarnessValidationRetry2Attempted,
  localHarnessValidationRetry2Passed: result.validationOutcome.localHarnessValidationRetry2Passed,
  previousBaselineMigrationFixVerified: result.validationOutcome.previousBaselineMigrationFixVerified,
