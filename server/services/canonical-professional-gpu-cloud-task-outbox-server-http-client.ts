...
/** Server-only PostgREST transport for the durable Cloud Task outbox. */
export function createCanonicalProfessionalGpuCloudTaskOutboxServerHttpClient(
  input: {
  ...
  return Object.freeze({
    async rpc(
      ...
      parameters: Parameters<
        CanonicalProfessionalGpuCloudTaskOutboxRpcClient['rpc']
      >[1],
...
function invalid(reason: string): never {
  throw new ApiError(
    ...
    'The server-only professional GPU Cloud Task outbox is unavailable.',
    503,
    {
      reason,
