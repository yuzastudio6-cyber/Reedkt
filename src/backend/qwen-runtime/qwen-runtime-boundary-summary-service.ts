...
export function createQwenRuntimeBoundarySummary(readiness: QwenRuntimeReadiness): string {
  return [
    `Qwen 3.7 boundary status: ${readiness.context.gateStatus}.`,
...
export function createQwenSecretBoundarySummary(references: QwenSecretReference[]): string {
  return `Qwen secret boundary: ${references.length} symbolic reference(s), valueAccessed false, valuePrinted false, frontendVisible false.`
}

export function createQwenProviderBoundarySummary(readiness: QwenProviderReadiness): string {
  return `Qwen provider boundary: ${readiness.gateStatus}; clientCreated false, providerCallMade false, fallback available.`
}
...
export function createQwenRuntimeNextStepSummary(context?: QwenRuntimeBoundaryContext): string {
  ...
  return `Next step after owner review: RP-QWEN-02 - Backend Qwen 3.7 Adapter with fake transport first by default. Current gate: ${status}.`
}
