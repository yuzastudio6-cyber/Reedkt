...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_retry_attempt_result_blocked_by_fail_closed_external_agent_gate'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58DV-PRIVATE-INFERENCE-GATE-ALIGNMENT: align the fail-closed external-agent gate with the approved bounded retry attempt, no Cloud Run invocation/no inference/no generated assets'
}
  ...
  return {
    ...
    privateInvokeClient: {
      ...
      boundaryNotes: [
        ...
        'The retry attempt result is recorded as blocked by the fail-closed external-agent gate; the next step is gate alignment before any bounded retry attempt. Generated assets, published artifacts, signed URLs, beta, and production remain blocked now.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
