...
| Gap | Status | Notes |
| --- | --- | --- |
| Auth/profile/workspace bootstrap missing | Partially fixed | Google-first Supabase OAuth handoff, exact same-origin/base-path callback construction, protected-route recovery, email/password fallback, and local browser regression evidence now exist. The browser-to-private-Cloud-Run source contract adds safe API-origin validation, Google API Gateway JWT validation, a dedicated original-token revalidation header, exact gateway/token/Supabase-user matching, and noncredentialed exact-origin CORS. A confirmation-gated, rollback-aware staging activation workflow proves exact source/revision/secret-version inputs, asymmetric JWKS preflight, keyless gateway identity, exclusive service-level invocation, immutable image/config evidence, and route probes. Only after strict readiness can it upload a sanitized attempt-specific activation artifact. The Pages workflow accepts only the same successful run/SHA, validates that artifact, derives the exact gateway origin from it, locally verifies the compiled `/Reedkt/` sign-in surface, deploys, and rechecks the hosted route. Credential-free same-SHA CI verifies the exact successful Pages run and Google-first surface without tester credentials or a session. The owner-local headed verifier requires exact callback, expected Google identity, protected reload, authenticated private-gateway `/v1/projects` readback, sign-out, and post-sign-out denial without exporting credentials, tokens, browser storage, traces, screenshots, or video. A first-login workspace gap now has a guarded same-SHA backend bridge: the provisioning workflow accepts only the verifier's email hash, keeps the raw email/service-role key in protected staging secrets, refuses Auth creation/invitation and all writes until it observes a Google-linked identity, confirmed email, and prior Auth sign-in, reuses compatible profile/owned-workspace state, and emits hashed identifiers; the owner-interactive verifier separately proves the live browser session used Google. A separate workflow performs mutation-free identity/profile/workspace readback before the owner reruns that verifier. None of these workflows has been pushed or dispatched. A read-only 2026-07-17 check still finds the hosted app stale, the Cloud Run API private, and API Gateway disabled. Google provider/callback allowlists, live asymmetric signing-key compatibility, real Gmail session, gateway/IAM deployment, canonical RLS-backed tenancy, and two-user staging readback remain unproven. The compatibility bootstrap does not resolve the raw migration baseline. |
...
| Signed-in private edit journey | Privately verified | The 2026-07-20 v29 full internal regression passed 45/45. Its real local authenticated browser journey created a project and named edit, uploaded and backend-probed eight private sources, compiled exact preferences and Edit Brief authority, revised and approved one 4K-ceiling plan/estimate, completed the server-derived 27/27 work graph, loaded an integrity-bound 3840x2160 private review, preserved all eight source audio identities, and persisted acceptance. This is local/private evidence only; hosted Gmail sign-in, deployed gateway/API/storage/database/workers, real providers, customer billing, public delivery, external beta, and production remain unproven. |
...
| Worker/job queue runtime missing | Partially fixed | The canonical private package queue is checksum-protected and restart-readable. Its server-selected claim plus outbox insert, accepted-worker completion, accepted-worker pre-commit failure, and accepted-worker lease timeout each use cooperative cross-process same-host write-ahead commits with real process-exit recovery and exact replay. Completion, failure, and timeout are mutually exclusive. An expired accepted worker fences every later Cloud-dispatch attempt until the same accepted controller principal atomically reconciles timeout; retry/exhaustion is derived from the immutable attempt ceiling and never starts automatically. The three currently metered DeepFilterNet, Remotion, and FFmpeg workload profiles now persist an exact create-only attempt start before execution. A controller-owned package scanner selects expired attempts, finalizes internal cost only through immutable lease expiry, and invokes the atomic timeout transaction; unsupported, missing, tampered, or mismatched start/cost evidence fails closed without mutation. A strict database-neutral lifecycle port and locked seven-function server RPC transport now pass adversarial conformance and aggregate v14, while terminal receipts exclude customer price/credits/service fee/wallet/billing authority. No canonical SQL function or live RPC client has been activated. Real database durability, deployed worker-death/heartbeat observation and sweeping, live Cloud Tasks/Cloud Run dispatch and terminal callbacks, production metering profiles for other tools, worker execution, and production operations remain open. |
| Private Docker execution transport | Privately verified | All server-side private Docker subprocesses now use a shared credential-isolated CLI boundary with a fresh intentionally absent Docker config, no inherited Docker host/context, direct reviewed local Buildx for image builds, local image loading, and fail-closed push/remote-output/secret/SSH/builder overrides. The focused browser-graphics execution smoke and the current v29 `45/45` full internal pipeline passed. This is local/private evidence only; Artifact Registry, Google IAM, cloud build/worker execution, deployment, and production authority remain open. |
