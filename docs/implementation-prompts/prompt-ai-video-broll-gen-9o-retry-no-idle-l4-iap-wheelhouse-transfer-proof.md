# AI-VIDEO-BROLL-GEN-9O-RETRY-NO-IDLE-L4-IAP-WHEELHOUSE-TRANSFER-PROOF

Retry one bounded no-idle L4 VM lifecycle with private wheelhouse IAP transfer validation in `us-central1-c`, mandatory cleanup, and no model inference.

Exact next prompt: `AI-VIDEO-BROLL-GEN-9O-RETRY-NO-IDLE-L4-IAP-WHEELHOUSE-TRANSFER-PROOF: retry bounded no-idle L4 VM lifecycle with private wheelhouse IAP transfer validation in us-central1-c and mandatory cleanup, no model inference`.

This future prompt may create at most one prompt-scoped no-public-IP `g2-standard-4` VM with one NVIDIA L4 in `us-central1-c` after repeated preflight passes. It may validate IAP transfer of the already-approved private Python 3.12 wheelhouse only, then must delete only the VM it creates and verify the proof instance, disk, static address, and reservation are absent before completion.
...
- `us-central1-c` zone status;
- `g2-standard-4` visibility in `us-central1-c`;
- `nvidia_l4` visibility in `us-central1-c`;
...
- create at most one no-public-IP `g2-standard-4` VM with one NVIDIA L4 in `us-central1-c`;
...
- firewall, IAM, service-account, key, router, Cloud NAT, bucket, image, or Artifact Registry mutation;
- Docker;
