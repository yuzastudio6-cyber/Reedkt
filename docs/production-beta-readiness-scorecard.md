...
Phase 36J Signalsmith controlled real-media timing/stretch evidence, when present, counts only as one bounded approved private controlled sample using the Phase 36I Signalsmith runtime path. It does not approve broad media, arbitrary media, full-video processing, DeepFilterNet runtime, Demucs, OCR, VLM, provider calls, production, external beta, paid production, public output, Docker/Cloud Run/Cloud Build, IAM mutation, or Track A. Audio/timing tool-family beta status remains `phase-complete but tool-family incomplete` after a pass until Demucs provenance decision or explicit scoped block, rollback/support policy, and final beta-readiness gate pass.
