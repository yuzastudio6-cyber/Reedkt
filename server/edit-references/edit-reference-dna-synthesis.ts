...
export function synthesizeEditReferencePreferenceDNA(input: SynthesisInput): PreferenceDNAVersionRecord {
  ...
  const conflicts = createConflictSnapshots(synthesisEvidence)
...
export function calculateEditReferenceDNAContentDigest(
  version: Pick<
    ...
    | 'adaptedNotCopied'
  > & Partial<Pick<PreferenceDNAVersionRecord, 'reasoningProvenance' | 'providerCallMade' | 'modelCallMade'>>,
): string {
  const base = {
  ...
  return sha256(stableStringify(version.reasoningProvenance
    ? {
        ...base,
        reasoningProvenance: version.reasoningProvenance,
        providerCallMade: version.providerCallMade,
        modelCallMade: version.modelCallMade,
      }
    : base))
}
