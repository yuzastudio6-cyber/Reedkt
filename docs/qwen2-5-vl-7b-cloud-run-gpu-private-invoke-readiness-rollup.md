...
- backend runtime persistence credit estimates plan version baseline fix: blocked, compatibility column guard required
...
| Backend runtime persistence credit estimates plan version baseline fix | blocked backend runtime persistence credit estimates plan version baseline fix required | Retry 6 isolated the next active baseline prerequisite inside `202605180004_reeditpro_credits_approval_snapshots.sql`. | Fix the active baseline migration so `credit_estimates.edit_plan_version_id` exists before `idx_credit_estimates_project_plan` runs, without inventing edit plan versions, credit estimates, approval records, or credit movements. |
