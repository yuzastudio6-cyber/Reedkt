...
| Worker/job queue runtime missing | Partially fixed | The canonical private package queue is checksum-protected and restart-readable, and its server-selected claim plus dispatch-outbox insert now share a cooperative cross-process same-host write-ahead commit with real process-exit recovery. The distributed database transaction, live Cloud Tasks/Cloud Run dispatch, worker execution, and production operations remain open. |
