...
## 2026-06-03 Initial Rerun Result
...
## 2026-06-03 QA-Prefix Access Rerun Result

The narrow Phase 36J QA-prefix access fix was applied only to `gs://reeditpro-staging-reeditpro-qa-artifacts/activation/phase36j/controlled-real-media-timing-stretch/` for `reeditpro-stg-cpu-worker-sa@reeditpro.iam.gserviceaccount.com`.

- `roles/storage.objectCreator`: added with condition title `phase36j-signalsmith-qa-create`
- `roles/storage.objectViewer`: added with condition title `phase36j-signalsmith-qa-readback`
- Broad IAM: not granted
- Public access: unchanged and blocked

The worker upload path was changed to use direct GCS object uploads instead of `gcloud storage cp`, avoiding any need for broad bucket `storage.objects.list`. Because worker code changed, the linux/amd64 image was rebuilt and pushed with digest `sha256:b51665f616a7c109e39e1cb4e425129516f3fefa3c4cf2e3709bbe1385ed6246`.

The controlled Cloud Run Job rerun passed. Runtime smoke, Phase 36I evidence, controlled sample verification, bounded extraction, `1.10x`, `0.90x`, and optional `1.25x` stretch QA, private artifact upload, and readback all passed. Phase 36J is complete for the bounded internal controlled-sample scope, and audio/timing tool-family beta status is `phase-complete but tool-family incomplete`.
