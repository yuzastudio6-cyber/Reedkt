...
function DataSummary({ item }: { item: DataVizPlanItem }) {
  return (
    <div className="dataviz-data-summary">
      ...
      <span><strong>Safe wording</strong>{hideInternalToolNamesInCopy(item.dataPlan.safeWording)}</span>
      <span><strong>Example/fictional</strong>{item.dataPlan.mockData ? 'example' : item.dataPlan.fictionalData ? 'fictional' : 'no'}</span>
    </div>
  )
}
  ...
  return (
    <article className="dataviz-plan-item">
      ...
      <div className="understanding-chip-row">
        {item.toolIds.length > 0 && <span className="dataviz-tool-badge">{userFacingActivityList(item.toolIds)}</span>}
        <span className="dataviz-no-render-note">Chart render gated</span>
        {item.dataPlan.sourceNeeded && <span className="dataviz-source-needed-badge">Source needed</span>}
        {item.dataPlan.mockData && <span className="dataviz-mock-data-badge">Example data</span>}
      ...
      <div className="dataviz-plan-meta">
        ...
        <span><strong>Preferred activity</strong>{userFacingActivityLabel(item.preferredTool)}</span>
      ...
      <p className="dataviz-controlled-tool-note">{hideInternalToolNamesInCopy(item.whyNotAiVideo)}</p>

      <details className="understanding-section">
        <summary>Data nodes, fallback, QA, and readiness notes</summary>
        ...
        <div className="dataviz-layout-summary">
          ...
          <span><strong>Source notes</strong>{hideInternalToolNamesInCopy(item.dataPlan.notes.join(' '))}</span>
        </div>
        <ul>
        ...
        <p className="dataviz-no-render-note">{hideInternalToolNamesInCopy(item.workerNotes.join(' '))}</p>
      </details>
    </article>
  )
}
  ...
  return (
    <InlinePlanCardShell
      ...
      compactSummary={(
        <div className="compact-summary-row">
          ...
          <span className="compact-summary-chip">{userFacingActivityList(dataVizPlan.toolsPlanned)}</span>
      ...
      helper="ReeditPro uses controlled chart and diagram planning for exact labels, money flows, timelines, metrics, comparisons, and explanation graphics. Chart rendering remains approval-gated."
    ...
    >
      <div className="dataviz-plan-summary-grid">
        ...
        <span><strong>{dataVizPlan.toolsPlanned.length}</strong>readiness checks</span>
        <span><strong>{sourceNeeded}</strong>source-needed</span>
        <span><strong>{mockOrFictional}</strong>example/fictional</span>
      </div>

      <div className="understanding-chip-row">
        {dataVizPlan.toolsPlanned.map((toolId) => (
          <span className="dataviz-tool-badge" key={toolId}>{userFacingActivityLabel(toolId)}</span>
        ))}
        <span className="dataviz-controlled-tool-note">Controlled activity, not AI video</span>
        <span className="dataviz-no-render-note">Chart render gated</span>
        <span className="dataviz-no-render-note">Data verification gated</span>
      </div>

      <p>{hideInternalToolNamesInCopy(dataVizPlan.summary)}</p>
      ...
      <details className="understanding-section">
        ...
        <div className="dataviz-animation-summary">
          <span><strong>Rules</strong>{hideInternalToolNamesInCopy(dataVizPlan.globalRules.join(' '))}</span>
          <span><strong>QA</strong>{hideInternalToolNamesInCopy(dataVizPlan.qaChecks.join(' '))}</span>
          <span><strong>Limitations</strong>{hideInternalToolNamesInCopy(dataVizPlan.limitations.join(' '))}</span>
          <span><strong>Notes</strong>{hideInternalToolNamesInCopy(dataVizPlan.notes.join(' '))}</span>
        </div>
        <Badge accent="cyan">Chart and composition planning only</Badge>
      </details>
    </InlinePlanCardShell>
  )
}
