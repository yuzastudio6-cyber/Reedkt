...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 51)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceQaReportsApprovedPlanSnapshotFixRequired, true)
