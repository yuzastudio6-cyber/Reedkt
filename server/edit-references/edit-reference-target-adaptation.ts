...
export const EDIT_REFERENCE_TARGET_APPLICATION_VERSION = 'edit-reference-target-application-v1' as const
...
export function createEditReferenceTargetApplication(input: TargetApplicationInput): PreferenceApplicationRecord {
  ...
  const decisions = input.dnaVersion.rules.map((rule) => adaptRule(rule, input.targetContext, targetContextDigest))
  ...
  return {
    ...
    precedencePolicy: [...EDIT_REFERENCE_APPLICATION_PRECEDENCE_POLICY],
    summary,
    targetIdentityStatus: 'caller_confirmed_unverified',
...
function adaptRule(
  ...
  if (rule.kind === 'do_not_copy') {
    decision = 'blocked_from_transfer'
...
function createHintGroups(decisions: PreferenceApplicationAdaptedDecisionRecord[]): PreferenceApplicationHintGroupRecord[] {
  ...
  return layerIds.map((layerId) => {
    ...
    const adapted = layerDecisions.filter((record) => record.decision === 'adapted')
    const heldBack = layerDecisions.length - adapted.length
    ...
    return {
      ...
      layerId,
      title,
      summary: adapted.length
        ? `${adapted.length} target-specific direction${adapted.length === 1 ? '' : 's'} prepared${heldBack ? `; ${heldBack} reference-specific item${heldBack === 1 ? '' : 's'} held back` : ''}.`
...
function createApplicationSummary(
  ...
  const adapted = decisions.filter((record) => record.decision === 'adapted').length
  const heldBack = decisions.length - adapted
  return `${adapted} target-specific guidance item${adapted === 1 ? '' : 's'} prepared for “${target.editName}”; ${heldBack} reference-specific or excluded item${heldBack === 1 ? '' : 's'} held back.`
}
