...
After the local accepted evidence snapshot passes, operators must first deploy or verify the staging API service, then run `npm run beta:readiness:api-deployment-preflight`. The guarded manual workflow `.github/workflows/beta-readiness-api-staging-deploy.yml` is the source-controlled staging deploy path for this step: it runs only through `workflow_dispatch` from default branch `codex/reeditpro-web-ui-shell`, requires `DEPLOY_STAGING_BETA_READINESS_API`, fetches `source_ref=codex/sound-music-audio-1abc-checkpoint`, requires an exact source SHA, locks the service to `reeditpro-api-staging`, rejects `latest` image tags, uses Workload Identity, binds Secret Manager names only, and keeps `--no-allow-unauthenticated`. The preflight verifies the API URL, deployed source SHA, API image, service account, staging-only environment, and Secret Manager binding confirmations without running `gcloud` or touching the deployed backend.
