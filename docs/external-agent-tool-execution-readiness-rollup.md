...
| --- | --- | --- | --- | --- |
| `qwen2_5_vl_7b_instruct` | controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference gate alignment accepted explicit tool prompt required | ready for explicit tool gate | read-only gcloud token refresh, Cloud Run service describe, and private caller job describe now pass; bounded private inference retry planning, the 58DS retry gate, the 58DT retry attempt approval, the historical 58DU blocked retry result, and the 58DV gate alignment are recorded; Qwen may proceed only to the explicit 58DW bounded retry prompt | `QWEN2_5_VL_STACK_TOOL_58DW-PRIVATE-INFERENCE-BOUNDED-RETRY-PROMPT: run one bounded approved-fixture private inference retry through the persisted job and lease bridge, no generated assets/no mutation` |
...
1. Qwen: auth/service/job visibility is now verified, the bounded approved-fixture private inference retry plan/gate/approval are recorded, the historical 58DU blocked retry result is preserved, and the 58DV gate alignment is accepted; run only the explicit 58DW bounded retry prompt before any runtime action.
...
- `npm run external-agent-tool-execution-gate` provides a fail-closed static go/no-go report for external agents before runtime execution; `npm run external-agent-tool-execution-gate -- --require-go` returns success only for the current Qwen explicit-tool-gate state and still performs no runtime action.
...
- Qwen is the closest lane to controlled private model inference; auth/service/job visibility is now verified, the bounded private inference retry plan/gate/approval/historical result are recorded, and the 58DV gate alignment is accepted. Runtime remains blocked until the explicit 58DW bounded retry prompt repeats live checks and runs the single approved-fixture path.
