...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 89)
assert.equal(
...
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchExecutionAttemptRequired, true)
