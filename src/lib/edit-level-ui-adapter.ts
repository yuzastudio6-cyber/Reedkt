...
export function createEditLevelBoundarySummary(): string[] {
  return [
    ...
    'Rendering, private processing, activity execution, AI asset preparation, and credits remain approval-gated.',
    'Selecting a level does not start AI preparation, media processing, render/export, or credit spend.',
  ]
}
...
export function createEditLevelSelectedSummaryModel(
  ...
  return {
    ...
    boundarySummary: 'Selection only. Final charge policy is actual billable activity cost + ReEditPro service/edit fee; choosing this level does not start rendering, private processing, activity execution, reservations, or credit spend.',
  }
}
...
export async function loadEditLevelRecommendationForUI(
  ...
  return {
    ...
    warnings: [
      ...result.warnings,
      'Recommendation is based on current project context.',
    ],
  }
}
...
export async function loadEditLevelCardsForUI(input: {
  ...
  return {
    ...
    warnings: [
      ...
      'Cards are visible and product-facing, but selection remains estimate-only until approval.',
    ],
  }
}
