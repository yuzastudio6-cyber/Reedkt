...
assert.equal(brollGateRow?.noIdleLifecycleGate?.targetRegion, 'us-east4')
assert.equal(brollGateRow?.noIdleLifecycleGate?.targetZone, 'us-east4-c')
...
assert.equal(
  ...
  'npm run smoke:ai-video-broll-gen-9y-no-idle-l4-iap-wheelhouse-transfer-proof-us-east4-c-result',
)
