...
assert.equal(report.internalTestingMode.allowed, true, 'internal break/fix testing should be allowed without paid users')
assert.equal(report.internalTestingMode.signInTestingAllowed, true, 'signed-in internal testing should be allowed')
assert.equal(report.internalTestingMode.repeatedBreakFixTestingAllowed, true, 'repeated break/fix testing should be allowed')
assert.equal(report.internalTestingMode.paidUsersRequired, false, 'internal testing must not require paid users')
assert.equal(report.internalTestingMode.liveBillingAllowed, false, 'internal testing must not allow live billing')
assert.equal(report.internalTestingMode.externalUsersAllowed, false, 'internal testing must not allow external users')
assert.equal(report.internalTestingMode.realUserMediaAllowed, false, 'internal testing must not allow real user media by default')
assert.equal(report.internalTestingMode.mockOrTestCreditsOnly, true, 'internal testing must use mock/test credits only')
assert.ok(report.internalTestingMode.allowedScopes.includes('signed_in_internal_testing'), 'internal testing should include signed-in internal testing scope')
assert.ok(report.internalTestingMode.allowedScopes.includes('repeated_break_fix_iteration'), 'internal testing should include repeated break/fix scope')
assert.ok(report.internalTestingMode.blockedScopes.includes('paid_user_onboarding'), 'internal testing should block paid-user onboarding')
assert.ok(report.internalTestingMode.blockedScopes.includes('live_billing_or_stripe_charges'), 'internal testing should block live billing')
assert.ok(report.internalTestingMode.guardrails.includes('mock_or_test_credits_only'), 'internal testing should keep mock/test credit guardrail')
...
assert.equal(evidenceDrivenReport.internalTestingMode.allowed, true, 'complete evidence should preserve internal testing readiness')
assert.equal(evidenceDrivenReport.internalTestingMode.paidUsersRequired, false, 'complete evidence should not make paid users required for internal testing')
...
const blockedInternalTestingReport = buildBetaReadinessReport({ e2eDryRunPassed: false })
assert.equal(blockedInternalTestingReport.internalTestingMode.allowed, false, 'internal testing should fail closed when its own dry-run baseline fails')
assert.equal(blockedInternalTestingReport.goNoGo.externalBetaAllowed, false, 'external beta should remain blocked when internal baseline fails')
assert.ok(blockedInternalTestingReport.internalTestingMode.blockers.some((blocker) => blocker.includes('dry-run E2E')), 'internal testing blocker should name the missing internal baseline')
