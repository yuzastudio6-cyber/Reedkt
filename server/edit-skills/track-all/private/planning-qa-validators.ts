...
const planningQaInputSchema = z.object({
  ...
  preflightObservationPresent: z.boolean(),
  routeQualificationStatus: z.enum(['blocked', 'planning_qualified', 'internal_execution_qualified', 'production_qualified']),
...
export function deriveTrackAllPlanningQaFindings(raw: unknown) {
  ...
  const routeRequiresChunks = input.samWorkPlanned || input.decision === 'track_planar_region' ||
    input.decision === 'produce_track_graph'
  const chunkSafe = !input.executable || !routeRequiresChunks || input.chunkPlan.chunks.length > 0 && input.chunkPlan.chunks.length <= input.assignment.permissions.maximumChunks && input.chunkPlan.chunks.every((chunk) => contains(chunk.range, input.assignment.authorizedWriteRange))
  ...
  const rules = [
    ...
    finding({ qaKey: 'track_all.qa.sam_qualification', passed: !input.samWorkPlanned || ['internal_execution_qualified', 'production_qualified'].includes(input.routeQualificationStatus), warning: input.samWorkPlanned && input.routeQualificationStatus !== 'production_qualified', summary: 'SAM work remains behind exact independently resolved route qualification.', evidence: [input.manifest.manifestHash, input.routeQualificationStatus, input.promptStrategyEvidenceHash ?? { sam: 'not_planned' }, input.sessionLifecycleEvidenceHash], observations: { routeQualificationStatus: input.routeQualificationStatus } }),
    ...
    finding({ qaKey: 'track_all.qa.initialization_frame', passed: initializationSafe && (!routeRequiresChunks || input.preflightObservationPresent), summary: 'Initialization and chunk risks derive from measured in-range preflight evidence.', evidence: [input.initializationFrame ?? -1, input.assignment.authorizedWriteRange, input.preflightObservationPresent], observations: { initializationFrame: input.initializationFrame ?? 'not_required', preflightObservationPresent: input.preflightObservationPresent } }),
    ...
    finding({ qaKey: 'track_all.qa.multiplex_budget', passed: input.bucketCount === (input.expectedObjects === 0 ? 0 : Math.ceil(input.expectedObjects / input.bucketSize)) && input.bucketCount <= Math.ceil(input.assignment.permissions.maximumObjects / input.bucketSize), summary: 'Multiplex bucket count is profile-derived, explicit, and bounded.', evidence: [input.expectedObjects, input.bucketSize, input.bucketCount, input.sessionCount], observations: { bucketSize: input.bucketSize, bucketCount: input.bucketCount, sessionCount: input.sessionCount } }),
