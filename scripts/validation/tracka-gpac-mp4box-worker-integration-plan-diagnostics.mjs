...
const packetDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-worker-integration-plan'
const decisionText = 'tracka_gpac_mp4box_worker_integration_plan_passed_ready_for_route_contract_and_mock_worker_interface'
const executionText = 'completed_docs_only_worker_integration_plan_no_runtime_execution'
const priorDecisionText = 'tracka_gpac_mp4box_worker_contract_review_passed_ready_for_worker_integration_plan'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-WORKER-ROUTE-CONTRACT-1'
...
const packetFiles = [
  ...
  `${packetDir}/gpac-mp4box-worker-integration-plan-decision.json`,
  `${packetDir}/gpac-mp4box-worker-integration-plan-decision.md`,
  `${packetDir}/private-artifact-manifest.json`,
  `${packetDir}/readiness-report.json`,
  `${packetDir}/route-boundary.json`,
  `${packetDir}/route-boundary.md`,
  `${packetDir}/source-of-truth-audit.json`,
  `${packetDir}/source-of-truth-audit.md`,
  `${packetDir}/validation-results.md`,
  `${packetDir}/worker-integration-plan.json`,
...
const requiredText = [
  ...
  'cleanupPolicyRef',
  'auditRecordRef',
  'blocked_pending_worker_route_contract_private_artifact_policy_and_guarded_execution_packet',
...
const forbiddenPatterns = [
  ...
  /readyForWorkerImplementation"\s*:\s*true/i,
...
function fail(message) {
  console.error(`TRACKA-GPAC-MP4BOX-WORKER-INTEGRATION-PLAN-1 diagnostics failed: ${message}`)
  process.exit(1)
}
...
if (packageJson.scripts?.['tracka:gpac-mp4box-worker-integration-plan:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-worker-integration-plan-diagnostics.mjs') {
...
const priorDecision = json('docs/track-a/native-container-render-tools/gpac-mp4box-worker-contract-review/gpac-mp4box-worker-contract-review-decision.json')
if (priorDecision.decision !== priorDecisionText) fail('prior worker contract decision drift')
if (priorDecision.nextPrompt !== 'TRACKA-GPAC-MP4BOX-WORKER-INTEGRATION-PLAN-1') fail('prior worker contract next prompt drift')
if (priorDecision.productReadyLocalOssTools !== 0) fail('prior worker contract product-ready count drift')
...
const decision = json(`${packetDir}/gpac-mp4box-worker-integration-plan-decision.json`)
if (decision.decision !== decisionText) fail('worker integration decision drift')
if (decision.execution !== executionText) fail('worker integration execution drift')
if (decision.readiness?.routeContract !== 'ready') fail('route contract readiness drift')
if (decision.readiness?.mockWorkerInterface !== 'ready') fail('mock worker interface readiness drift')
if (decision.readiness?.workerImplementation !== 'blocked_pending_route_contract_mock_worker_interface_and_private_artifact_policy') fail('worker implementation readiness drift')
if (decision.readiness?.productRuntime !== 'blocked_pending_worker_route_contract_private_artifact_policy_and_guarded_execution_packet') fail('product runtime readiness drift')
...
const plan = json(`${packetDir}/worker-integration-plan.json`)
if (plan.decision !== decisionText) fail('plan decision drift')
if (plan.execution !== executionText) fail('plan execution drift')
if (plan.frontendDirectExecution !== false) fail('frontend direct execution drift')
for (const field of ['approvedSnapshotRef', 'approvalRecordRef', 'jobRef', 'workerLeaseRef', 'routeIdempotencyKey', 'privateInputManifestRef', 'privateArtifactManifestRef', 'privateArtifactChecksumRef', 'gpacMp4boxWorkerContractRef', 'gpacMp4boxRouteContractRef', 'cleanupPolicyRef', 'auditRecordRef']) {
  if (!plan.requiredFutureWorkerEnvelope?.includes(field)) fail(`missing worker envelope field ${field}`)
}
if (plan.readyForRouteContract !== true) fail('route contract readiness not true')
if (plan.readyForMockWorkerInterface !== true) fail('mock worker interface readiness not true')
if (plan.readyForProductRuntime !== false) fail('product runtime readiness not false')

const routeBoundary = json(`${packetDir}/route-boundary.json`)
for (const input of ['rawChat', 'frontendDirectFilePath', 'publicUrl', 'signedUrlAsSourceOfTruth', 'arbitraryPrivateMedia', 'userProvidedCommandString']) {
  if (!routeBoundary.forbiddenFutureInputs?.includes(input)) fail(`missing forbidden route input ${input}`)
}
for (const value of Object.values(routeBoundary.blockedInThisPhase ?? {})) {
  if (value !== true) fail('route boundary blocked scope drift')
}

const artifactPolicy = json(`${packetDir}/artifact-policy.json`)
if (artifactPolicy.privateInputManifestRequired !== true) fail('private input manifest policy drift')
if (artifactPolicy.privateArtifactManifestRequired !== true) fail('private artifact manifest policy drift')
...
if (!Array.isArray(artifactPolicy.generatedArtifactsCommitted) || artifactPolicy.generatedArtifactsCommitted.length !== 0) fail('generated artifact policy drift')

const readiness = json(`${packetDir}/readiness-report.json`)
if (readiness.routeContractReady !== true) fail('route contract readiness drift')
if (readiness.mockWorkerInterfaceReady !== true) fail('mock worker interface readiness drift')
for (const key of ['readyForWorkerImplementation', 'readyForWorkerExecution', 'readyForProductRuntime', 'readyForExternalBetaProductUse', 'readyForProduction']) {
...
console.log('TRACKA-GPAC-MP4BOX-WORKER-INTEGRATION-PLAN-1 diagnostics passed')
console.log(`Decision: ${decisionText}`)
console.log('Route contract readiness: ready')
console.log('Mock worker interface readiness: ready')
console.log('Product runtime: blocked_pending_worker_route_contract_private_artifact_policy_and_guarded_execution_packet')
