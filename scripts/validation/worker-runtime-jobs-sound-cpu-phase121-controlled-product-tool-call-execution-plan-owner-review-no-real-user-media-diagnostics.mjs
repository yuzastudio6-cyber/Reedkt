...
const nextPrompt =
  'WORKER_RUNTIME_JOBS-SOUND-CPU-PHASE122-CONTROLLED-PRODUCT-TOOL-CALL-EXECUTION-PROOF-NO-REAL-USER-MEDIA'
...
const docs = {
  ...
  blockers:
    'docs/worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-blocker-register-no-real-user-media.md',
  policy:
    'docs/worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-claim-policy-no-real-user-media.md',
  next:
...
const parsed = {
  ...
  blockers: parseJsonBlock(
    docs.blockers,
    'worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-blocker-register-no-real-user-media',
  ),
  policy: parseJsonBlock(
    docs.policy,
    'worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-claim-policy-no-real-user-media',
  ),
  next: parseJsonBlock(
    docs.next,
...
assert(parsed.source.sourceVerification.sourcePr === 2088, 'source PR mismatch')
assert(
  parsed.source.sourceVerification.sourceMergeCommit === '3eeabf9b66ad80c29da60a9e8cd38ddfe50c4f3c',
  'source merge mismatch',
)
assert(parsed.source.planResult.controlledProductToolCallExecutionPlanCreated === true, 'source plan missing')
...
assert(parsed.source.planResult.readyForControlledProductToolCallExecutionPlanOwnerReview === 15, 'source owner review count mismatch')
assert(parsed.source.planResult.readyForControlledProductToolCallExecutionProofToday === 0, 'source proof today widened')
assert(parsed.source.soundCpuTools.readyForControlledProductToolCallExecutionPlanOwnerReview === 15, 'source tool owner count mismatch')
...
assert(parsed.sourceBlockers.readyForControlledProductToolCallExecutionPlanOwnerReview === true, 'source blocker owner missing')
assert(parsed.sourceBlockers.readyForControlledProductToolCallExecutionProofToday === false, 'source blocker proof widened')
assert(parsed.sourcePolicy.nextGateMayRunControlledProductToolCallExecutionPlanOwnerReview === true, 'source policy owner missing')
assert(parsed.sourcePolicy.nextGateMayRunControlledProductToolCallExecutionProof === false, 'source policy proof widened')
...
assert(parsed.prompt.reviewScope.reviewControlledProductToolCallExecutionPlanOnly === true, 'prompt scope mismatch')
assert(parsed.prompt.reviewScope.allowedToolCount === 15, 'prompt tool count mismatch')
assert(parsed.prompt.reviewScope.mayCreateControlledProductToolCallExecutionProofNext === true, 'prompt proof next missing')
assert(parsed.prompt.reviewScope.allowControlledProductToolCallExecutionProofToday === false, 'prompt proof today widened')
...
assert(parsed.result.sourceVerification.sourcePr === 2090, 'result source PR mismatch')
assert(
  parsed.result.sourceVerification.sourceMergeCommit === '7f1f2f1b3278b0d1d1e702ce6016888a1647e290',
  'result source merge mismatch',
)
assert(parsed.result.sourceVerification.sourceDecision === sourceDecision, 'result source decision mismatch')
assert(parsed.result.ownerReview.controlledProductToolCallExecutionPlanAccepted === true, 'result plan acceptance missing')
assert(parsed.result.ownerReview.controlledProductToolCallExecutionProofMayProceedNext === true, 'result proof next missing')
...
assert(parsed.result.ownerReview.acceptedForControlledProductToolCallExecutionProofToday === false, 'result proof today widened')
assert(parsed.result.ownerReview.acceptedForProductToolCallExecutionToday === false, 'result product widened')
assert(parsed.result.soundCpuTools.controlledProductToolCallExecutionPlanOwnerReviewed === 15, 'result owner reviewed count mismatch')
assert(parsed.result.soundCpuTools.readyForControlledProductToolCallExecutionProofNoRealUserMedia === 15, 'result proof count mismatch')
...
assert(parsed.acceptance.acceptedPlanElements.controlledProductToolCallExecutionPlanCreated === true, 'acceptance plan missing')
...
assert(parsed.acceptance.acceptedForNextProofOnly.controlledProductToolCallExecutionProofMayProceed === true, 'acceptance proof next missing')
...
assert(parsed.blockers.readyForControlledProductToolCallExecutionProofNoRealUserMedia === true, 'blocker proof missing')
assert(parsed.blockers.readyForProductToolCallExecutionToday === false, 'blocker product widened')
assert(parsed.blockers.soundCpuToolsReadyForControlledProductToolCallExecutionProofNoRealUserMedia === 15, 'blocker proof count mismatch')
assert(parsed.blockers.soundCpuToolsReadyForProductToolCallExecutionToday === 0, 'blocker product count widened')

assert(parsed.policy.allowedClaims.controlledProductToolCallExecutionPlanOwnerReviewedClaimed === true, 'policy owner review missing')
assert(parsed.policy.allowedClaims.controlledProductToolCallExecutionProofMayProceedClaimed === true, 'policy proof next missing')
assert(parsed.policy.blockedClaims.controlledProductToolCallExecutionProofPassedClaimed === false, 'policy proof passed widened')
assert(parsed.policy.blockedClaims.productToolCallExecutionReadyClaimed === false, 'policy product widened')
assert(parsed.policy.nextGateMayRunControlledProductToolCallExecutionProof === true, 'policy proof next missing')
...
assert(parsed.next.proofScope.createControlledProductToolCallExecutionProofRunner === true, 'next proof runner missing')
assert(parsed.next.proofScope.runControlledProductToolCallExecutionProofCommand === true, 'next proof command missing')
  ...
  JSON.stringify(
    {
      ...
      sourceDecision,
      sourcePr: 2090,
      sourceMergeCommit: '7f1f2f1b3278b0d1d1e702ce6016888a1647e290',
      acceptedToolCount: 15,
      controlledProductToolCallExecutionProofMayProceedNext: true,
