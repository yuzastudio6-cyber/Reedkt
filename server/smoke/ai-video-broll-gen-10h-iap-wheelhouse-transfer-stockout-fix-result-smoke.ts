...
import {
  AI_VIDEO_BROLL_GEN_10H_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_10I_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST4_C_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10h-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-10h-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-10i-no-idle-l4-iap-wheelhouse-transfer-proof-us-west4-c.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-10h-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-10h-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10h-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
...
function scanForbiddenValues(value: unknown, prefix = 'broll10hStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10h-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  'Cloud Run or queued-job scale-to-zero remains a future architecture track',
  ...
  assert.equal(doc.includes(required), true, `10H stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `10I prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10H_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10h_iap_wheelhouse_transfer_stockout_fix_result')
...
assert.equal(result.selectedCapacityStrategy, 'bounded_same_region_cross_zone_no_idle_transfer_proof_retry')
assert.equal(result.selectedRetryZone, 'us-west4-c')
assert.equal(result.selectedRetryRegion, 'us-west4')
...
assert.equal(result.suggestedByPriorCreateResult, true)
...
assert.equal(result.readOnlyChecks.candidateZoneStatus, 'UP')
assert.equal(result.readOnlyChecks.machineTypeVisible, true)
assert.equal(result.readOnlyChecks.acceleratorVisible, true)
...
assert.equal(result.readOnlyChecks.regionalL4QuotaLimit, 1)
assert.equal(result.readOnlyChecks.regionalL4QuotaUsage, 0)
...
assert.equal(
  result.readOnlyChecks.candidates.some(
    (candidate) =>
      ...
      candidate.selected &&
      candidate.disposition === 'selected_prior_create_suggested_same_region_cross_zone_no_idle_transfer_proof' &&
...
assert.equal(
  result.readOnlyChecks.candidates.some(
    (candidate) =>
      candidate.zone === 'us-west4-a' &&
      candidate.disposition === 'rejected_recent_stockout' &&
...
assert.equal(result.futureRetryPreflightRequired.includes('us_west4_c_zone_status'), true)
assert.equal(result.futureRetryPreflightRequired.includes('us_west4_regional_nvidia_l4_quota'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10I_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST4_C_PROMPT)
  ...
  JSON.stringify(
    {
      ...
      sameRegionRetrySelected: result.sameRegionRetrySelected,
