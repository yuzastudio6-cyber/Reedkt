...
- Private invoke status: `backend_runtime_persistence_generation_requests_approved_snapshot_baseline_fix_required`
...
Retry 7 isolated the active migration blocker to `202605180005_reeditpro_generation_assets_jobs.sql`: `idx_generation_requests_project_snapshot` needs `generation_requests.approved_plan_snapshot_id` to exist before the local baseline can advance toward the Qwen draft SQL.
