...
# Builds the source-bound, control-only account-effective A100 rate publisher.
# It does not read prices, start a GPU/model job, or mutate customer credits.
...
readonly CONFIG='scripts/gcp/prod/cloudbuild-vertex-a100-rate-publisher.yaml'
readonly CONFIRMATION='build-weeditpro-vertex-a100-rate-publisher-v1'
...
[[ "${WEEDITPRO_CONFIRM_VERTEX_A100_RATE_PUBLISHER_IMAGE_BUILD:-}" \
...
printf '{"operation":"weeditpro_vertex_a100_rate_publisher_image_build_v1",'
...
printf '"immutableImage":"%s@%s",' \
  ...
  "${digest}"
printf '"billingPriceReadAttempted":false,"gpuOrModelRuntimeStarted":false,'
