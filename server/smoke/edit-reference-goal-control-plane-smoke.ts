...
assert.equal(status.currentGate, 'gate_0')
assert.deepEqual(status.completedGates, [])
assert.deepEqual(status.blockedGates, [])
assert.equal(status.latestCommit, null)
assert.equal(status.migrationBaseline, 21)
assert.equal(status.migrationCurrent, 21)
assert.equal(status.browserQa, 'not_started')
assert.equal(status.runtimeQa, 'not_started')
assert.equal(status.persistenceQa, 'not_started')
