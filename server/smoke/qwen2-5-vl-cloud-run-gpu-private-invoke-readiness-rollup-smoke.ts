...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 47)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceGeneratedAssetVersionsVersionBaselineFixRequired, true)
