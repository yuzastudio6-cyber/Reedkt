...
| Selective deterministic motion | `verified_private_runtime` for the bounded scalar renderer; `verified_source_contract` for role/activation divergence detection, environmental-particle admission, the subject-neutral particle kernel, closed PixiJS request materialization, and controlled transparent-sequence measurement | canonical motion compiler, independent sample-digest verification, Remotion sampler, actual pixel-displacement measurement, selected-scene selective-motion reconciliation, selected-scene environmental-particle admission, environmental-particle kernel, environmental-particle operation materialization, environmental-particle sequence observation | The current canonical compiler broadcasts a scene-level `rotate` verb to every component. The read-only reconciliation proves that static anchors, stable subjects, and environmental effects receive mechanical rotation even when only explicitly activated mechanical components should rotate. The environmental admission binds the exact selected component, MasterTiming phases, confirmed frame, procedural geometry, existing PixiJS identity, and required particle primitive. The adjacent kernel provides seven typed subject-neutral profiles plus deterministic, style-bound, motion-density-capped, one-shot particle state tracks. A process-bound materializer converts only a verified kernel into one closed, single-use private PixiJS request with exact canvas/frame lineage and a transparent frame-sequence policy; it leaves the candidate operation unregistered and non-dispatchable. A controlled non-promotable fixture now verifies byte-level RGBA PNG decoding, exact frame/alpha lineage, settled endpoints, temporal variation, and alpha-centroid movement through a byte-free observation receipt, but explicitly does not prove PixiJS execution or a qualified runtime artifact. Runtime materialization remains blocked because approved selected-scene evidence still has no immutable typed profile reference, the canonical full-segment five-phase domain conflicts with the narrower visual timing range required by the motion compiler, the current PixiJS runtime implements only one opaque 640×360 static PNG operation, and the current Remotion Living Frame overlay accepts one static PNG. Canonical owners must reconcile the phase domain, adopt the profile reference, qualify/register the bounded operation and Remotion adapter, and bind work/cost/asset/QA/private-review evidence. |
