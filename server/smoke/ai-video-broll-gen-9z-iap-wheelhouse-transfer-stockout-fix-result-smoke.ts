...
import {
  AI_VIDEO_BROLL_GEN_10A_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_B_PROMPT,
  AI_VIDEO_BROLL_GEN_9Z_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9z-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9z-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-10a-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-b.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-9z-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-9z-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9z-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
...
function scanForbiddenValues(value: unknown, prefix = 'broll9zStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9z-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  '`us-east4-c`',
  '`us-east1-b`',
  '| `us-east1-b` | `us-east1` | `UP` | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | selected |',
  '| `us-east4-b` | `us-east4` | `UP` | false | false | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | rejected_no_l4_shape_visible |',
  'Select `us-east1-b` as the next bounded no-idle IAP wheelhouse transfer proof target.',
  ...
  assert.equal(doc.includes(required), true, `9Z stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `10A prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9Z_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9z_iap_wheelhouse_transfer_stockout_fix_result')
...
assert.equal(result.selectedRetryZone, 'us-east1-b')
assert.equal(result.selectedRetryRegion, 'us-east1')
assert.equal(result.crossRegionRetrySelected, true)
assert.equal(result.sameRegionRetrySelected, false)
assert.equal(result.crossZoneRetrySelected, false)
...
assert.equal(
  result.readOnlyChecks.candidates.some(
    (candidate) =>
      candidate.zone === 'us-east1-b' &&
      candidate.selected &&
      candidate.disposition === 'selected_cross_region_retry' &&
...
assert.equal(
  result.readOnlyChecks.candidates.some(
    (candidate) =>
      candidate.zone === 'us-east4-b' &&
      candidate.disposition === 'rejected_no_l4_shape_visible' &&
...
assert.equal(result.futureRetryPreflightRequired.includes('us_east1_b_zone_status'), true)
assert.equal(result.futureRetryPreflightRequired.includes('us_east1_regional_nvidia_l4_quota'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10A_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_B_PROMPT)
