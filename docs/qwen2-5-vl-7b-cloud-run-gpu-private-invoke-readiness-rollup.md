...
- backend runtime persistence RLS function parameter baseline fix: blocked, `public.is_workspace_member(uuid)` parameter compatibility required
...
| Backend runtime persistence RLS function parameter baseline fix | blocked backend runtime persistence RLS function parameter baseline fix required | Retry 11 isolated the next blocker: older active baseline migrations created `public.is_workspace_member(uuid)` with input parameter `target_workspace_id`, while `202605180007_reeditpro_rls_policies.sql` attempts to replace it with input parameter `workspace_uuid`, causing `cannot change name of input parameter "target_workspace_id" (SQLSTATE 42P13)`. | Fix the RLS helper parameter compatibility without touching Supabase cloud, staging, production, live data, Cloud Run, Qwen inference, worker dispatch, generated assets, public artifacts, signed URLs, credit records, beta, or production readiness. |
