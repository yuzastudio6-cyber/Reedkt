...
| Critical | Local/staging Supabase/RLS not executed | Access policies are not proven. | Cross-workspace data exposure. | Repair local migration chain, then run the existing first local executable SQL candidate; staging SQL requires later human approval. | Prompt 20N, then retry Prompt 20B SQL execution if local `supabase start` passes | No. | Yes |
| Critical | Remote/staging migration not validated | Schema chain may not apply cleanly. | Runtime data corruption or missing tables. | Validate migration chain in approved local environment first, then staging only with approval. | Prompt 20N, then Prompt 20B or another migration-chain follow-up depending on local result | No. | Yes |
...
| Medium | Local Supabase migration-chain blocker | Local SQL validation blocked. | Slower RLS iteration and no RLS execution evidence. | Prompt 20M repairs the QA/export/audit blocker, but `supabase start` now fails in `202605180007_reeditpro_rls_policies.sql` because `is_workspace_member(workspace_uuid uuid)` attempts to rename an existing function input parameter from `target_workspace_id`. | Prompt 20N, then rerun Prompt 20B SQL execution if migration-chain validation passes | Yes for static planning only. | Yes before production beta |

## Decision

Prompt 20M repairs the QA/export/audit migration-chain blocker in `202605180006_reeditpro_qa_exports_audit.sql`. Production beta still cannot proceed because local/staging Supabase/RLS has not executed. The next safe milestone is Prompt 20N - Local Supabase Migration Chain Repair Follow-Up 7 for `202605180007_reeditpro_rls_policies.sql` before SQL/RLS execution is retried.
