...
The default repo state remains blocked because those approvals and real execution proofs are not present yet. Local SQL proof can reduce wallet-settlement and RLS/readback blockers from "unimplemented source" to "needs deployed staging/prod evidence", but it does not clear the platform gate by itself.
