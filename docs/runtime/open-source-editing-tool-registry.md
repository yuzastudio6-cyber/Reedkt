...
Current-milestone strict required tools are `ffmpeg`, `ffprobe`, and `remotion` because they back the already-passed staging render infrastructure and real-video upload-to-preview gates. RP-MEDIA-01 adds a media-analysis canary that overrides strict readiness to require only `ffmpeg` and `ffprobe`; Remotion is not required for that gate. Future tools remain optional/planned until a specific worker canary makes them required.
...
1. Dispatch and record the RP-MEDIA-01 media analysis worker canary after workflow visibility review.
