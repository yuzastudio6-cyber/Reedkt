...
import {
  TRACKA_THREE_TOOL_EXTERNAL_AGENT_WORKER_PROCESS_NOOP_INVOKE_PACKET,
  invokeThreeToolExternalAgentWorkerProcessNoop,
} from '../services/tracka-three-tool-external-agent-worker-process-noop-invoke-1'

const reportRoot = '/tmp/reeditpro-tracka-three-tool-external-agent-worker-process-noop-invoke-1'
const reportFileName = 'three-tool-external-agent-worker-process-noop-invoke-report.json'
const manifestFileName = 'three-tool-external-agent-worker-process-noop-invoke-manifest.json'
...
async function main(): Promise<void> {
  ...
  const result = invokeThreeToolExternalAgentWorkerProcessNoop()
  ...
  console.log(JSON.stringify({
    ...
    entrypointId: result.workerProcessNoop.entrypointId,
