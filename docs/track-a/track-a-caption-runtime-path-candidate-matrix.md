...
| --- | --- | --- | --- | --- |
| `local_ffmpeg_libass_runtime_path` | preferred minimal local runtime path | ffmpeg=`/opt/homebrew/bin/ffmpeg`; ffprobe=`/opt/homebrew/bin/ffprobe`; ass=`false`; subtitles=`false`; libass=`false` | `blocked_runtime_image_build_failed` | approval is metadata-only; no media input or output was used |
...
| `missing_runtime_path` | fallback blocker | `blocked_runtime_image_build_failed` | `blocked_runtime_image_build_failed` | used when local FFmpeg/FFprobe/filter support is not metadata-approved |
