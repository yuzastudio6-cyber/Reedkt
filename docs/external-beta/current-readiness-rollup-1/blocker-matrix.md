...
| Staging migration path | `blocked_clean_branch_remote_migration_history_has_untracked_versions` | clean branch migration-history reconciliation before any apply retry |
| Worker RPC 4R | `blocked_pending_clean_staging_migration_history_reconciliation` | source-aligned clean branch migration history, then apply/currentness and RPC readback |
