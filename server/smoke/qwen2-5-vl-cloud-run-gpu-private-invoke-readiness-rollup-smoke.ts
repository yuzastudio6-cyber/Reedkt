...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt: blocked, attempt required',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt | blocked controlled persisted worker dispatch runtime real-dispatch approved fixture private invoke attempt required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 121)
assert.equal(
...
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptRequired, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptRecorded, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptPassed, false)
...
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeAttempt, true)
assert.equal(
