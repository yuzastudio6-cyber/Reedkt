...
function ensureCpuModelGpuModelRouteProofSourceImage() {
  const absolutePath = absolute(cpuModelGpuModelRouteProofSourceImage)
  fs.mkdirSync(path.dirname(absolutePath), { recursive: true })
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      const foreground = x > 24 && x < 72 && y > 18 && y < 78
      row.push(foreground ? '235 50 70' : '25 45 75')
    }
    lines.push(row.join(' '))
  }
...
function cpuModelGpuModelRouteProofArgs() {
  return [
    'run',
    '--silent',
    'ai-graphics:external-agent-all21-controlled-route-execution-smoke',
...
function checkCpuModelGpuModelRouteProof(label, proof) {
  if (
    proof.decision !==
    'ai_graphics_external_agent_all21_controlled_route_execution_smoke_passed'
  ) {
  ...
  const attempts = Array.isArray(proof.scopedGpuModelLocalDevRouteAttempts)
    ? proof.scopedGpuModelLocalDevRouteAttempts
    : []
  for (const toolId of cpuModelGpuModelRouteProofTools) {
    const attempt = attempts.find((item) => item.requestedToolId === toolId)
    if (!attempt) {
      fail(`${label}_missing_attempt:${toolId}`)
      continue
    ...
    if (toolId !== 'transparent_background') {
      if (attempt.booleans?.scopedGpuModelRuntimeExecutionPerformed !== false) {
        fail(`${label}_${toolId}_runtime_execution_should_be_false_for_failed_diagnostic`)
      }
      if (result.externalAgentExecutionState !== 'failed_with_diagnostics') {
        fail(`${label}_${toolId}_state_not_failed_with_diagnostics:${result.externalAgentExecutionState}`)
      }
      if (result.localGpuModelRuntimeExecutionPerformed !== false) {
        fail(`${label}_${toolId}_local_runtime_should_be_false_for_failed_diagnostic`)
      }
      if (result.controlledAdapterExecutedNow !== false) {
        fail(`${label}_${toolId}_adapter_should_not_execute_for_failed_diagnostic`)
      }
      if (result.gpuRuntimeShouldStartNow !== false) {
        fail(`${label}_${toolId}_gpu_runtime_should_start_now_not_false`)
      }
      if (typeof result.failureDiagnostics !== 'string' ||
        !result.failureDiagnostics.includes('cpu_model_runtime_container_command_failed')) {
        fail(`${label}_${toolId}_missing_actionable_failure_reason`)
      }
      continue
    }
    if (attempt.booleans?.scopedGpuModelRuntimeExecutionPerformed !== true) {
...
function checkReadinessCpuSafeRouteProofSummary(label, report) {
  ...
  if (report.booleans?.agentCanExecuteGpuModelToolsNow !== true) {
    fail(`${label}_cpu_safe_report_gpu_model_tools_not_executable`)
  }
  if (report.booleans?.agentCanExecuteAll21ToolsNow !== false ||
  ...
  if (
    typeof report.executionScope?.agentExecutableToolCountNow !== 'number' ||
    report.executionScope.agentExecutableToolCountNow < 16
  ) {
    fail(`${label}_cpu_safe_execution_scope_not_proof_backed:${report.executionScope?.agentExecutableToolCountNow}`)
  }
}

function checkReadinessCpuModelRouteProofSummary(label, report) {
  const summary = report.cpuModelGpuModelRouteProof ?? {}
  const acceptedCpuModelExecutableTools = ['transparent_background']
  const expectedCombinedExecutableTools = [
    ...cpuSafeGpuModelRouteProofTools,
    ...acceptedCpuModelExecutableTools,
  ]
  const expectedRemainingBlockedTools = [
    'sam2',
    'birefnet',
    'real_esrgan',
    'rembg',
  ]
  if (summary.attempted !== true) fail(`${label}_cpu_model_summary_not_attempted`)
  if (summary.accepted !== true) fail(`${label}_cpu_model_summary_not_accepted`)
  if (!arrayMatches(summary.expectedToolIds, cpuModelGpuModelRouteProofTools)) {
    fail(`${label}_cpu_model_expected_tools_mismatch:${JSON.stringify(summary.expectedToolIds)}`)
  }
  if (!arrayMatches(summary.executableToolIds, acceptedCpuModelExecutableTools)) {
  ...
  for (const toolId of expectedCombinedExecutableTools) {
    if (!summary.combinedGpuExecutableToolIds?.includes(toolId)) {
  ...
  for (const toolId of expectedRemainingBlockedTools) {
    if (!summary.remainingGpuModelBlockedToolIds?.includes(toolId)) {
      fail(`${label}_cpu_model_missing_remaining_blocked_tool:${toolId}`)
    }
  }
  if (!arrayMatches(summary.rejectedAsTooSlowOrStillBlockedToolIds, [
    'real_esrgan',
    'rembg',
  ])) {
    fail(`${label}_cpu_model_rejected_or_slow_tools_mismatch:${JSON.stringify(summary.rejectedAsTooSlowOrStillBlockedToolIds)}`)
  }
  if (summary.gpuRuntimeShouldStartNow !== false ||
    summary.gpuRuntimeStartsIdle !== false ||
    summary.publicArtifactCreated !== false ||
  ...
  if (report.booleans?.agentCanExecuteGpuModelToolsNow !== true) {
    fail(`${label}_cpu_model_report_gpu_model_tools_not_executable`)
  }
  if (report.booleans?.agentCanExecuteAll21ToolsNow !== false ||
    report.booleans?.gpuRuntimeShouldStartNow !== false ||
    report.booleans?.runtimeReadyNow !== false ||
    report.booleans?.externalBetaReadyNow !== false ||
    report.booleans?.productionReadyNow !== false) {
    fail(`${label}_cpu_model_report_broad_gates_not_false`)
  }
  ...
  for (const toolId of expectedCombinedExecutableTools) {
    if (!executionScope.acceptedProofSubsetGpuToolIds?.includes(toolId)) {
      fail(`${label}_cpu_model_execution_scope_missing_proof_tool:${toolId}`)
    }
  }
}
...
function checkReport(label, report, expectedStatus = status) {
  if (report.decision !== decision) fail(`${label}_decision_mismatch`)
  if (report.status !== expectedStatus) {
    fail(`${label}_status_mismatch:${report.status}`)
  }
  const counts = report.counts ?? {}
  const acceptedProofSubsetGpuToolIds = Array.isArray(
    report.executionScope?.acceptedProofSubsetGpuToolIds,
  )
    ? report.executionScope.acceptedProofSubsetGpuToolIds
    : []
  const proofBackedGpuToolCount = acceptedProofSubsetGpuToolIds.length
  const proofBackedExecutableToolCount = 13 + proofBackedGpuToolCount
  const remainingGpuModelBlockedToolCount = 8 - proofBackedGpuToolCount
  const expectedCounts = {
    ...
    controlledExecutionRuntimePresentNowTools:
      proofBackedExecutableToolCount,
    agentCallableTools: 21,
    agentExecutableTools: proofBackedExecutableToolCount,
    agentExecutableToolsWithAcceptedProof:
      proofBackedExecutableToolCount,
    cpuStaticExecutableTools: 6,
    browserRuntimeExecutableTools: 7,
    gpuToolsWithValidRuntimeProof: proofBackedGpuToolCount,
    ...
    gpuModelBlockedWithReasonTools: remainingGpuModelBlockedToolCount,
    currentHostGpuProofBlockers: 0,
    blockedWithReasonTools: remainingGpuModelBlockedToolCount,
  ...
  if (proofBackedExecutableToolCount === 19) {
    requiredTrueBooleans.push(
      'nineteenToolsHaveAcceptedControlledExecutionProofNow',
    )
  }
  if (proofBackedExecutableToolCount === 17) {
    requiredTrueBooleans.push(
  ...
  const expectedScope = {
    ...
    agentCanExecuteGpuModelToolsNow: proofBackedGpuToolCount > 0,
    agentCanExecuteAll21ControlledToolsNow: false,
    agentExecutableToolCountNow: proofBackedExecutableToolCount,
    agentExecutableNonGpuToolCountNow: 13,
    agentExecutableGpuModelToolCountNow: proofBackedGpuToolCount,
    gpuModelBlockedToolCountNow: remainingGpuModelBlockedToolCount,
  ...
  for (const toolId of allTools) {
    ...
    if (gpuModelTools.includes(toolId)) {
      const acceptedProofBackedTool =
        acceptedProofSubsetGpuToolIds.includes(toolId)
      if (acceptedProofBackedTool) {
        if (row.readinessState !== 'executable') {
          fail(`${label}_${toolId}_gpu_state_not_executable:${row.readinessState}`)
        }
        if (row.executable !== true || row.executionPassed !== true) {
          fail(`${label}_${toolId}_gpu_not_proof_backed_executable`)
        }
        if (row.controlledExecutionRuntimePresentNow !== true) {
          fail(`${label}_${toolId}_gpu_controlled_runtime_not_present`)
        }
        if (row.controlledRouteProofBackedExecutable !== true) {
          fail(`${label}_${toolId}_gpu_missing_proof_backed_marker`)
        }
        if (row.fastestGpuModelUnlockCandidate !== false) {
          fail(`${label}_${toolId}_proof_backed_tool_still_marked_unlock_candidate`)
        }
      } else {
      ...
      if (acceptedProofBackedTool) {
        ...
        if (row.blockingPrerequisite !== null) {
          fail(`${label}_${toolId}_proof_backed_tool_has_blocker:${row.blockingPrerequisite}`)
        }
      } else {
      ...
      if (acceptedProofBackedTool) {
        if (row.currentBlockingPrerequisiteKey !== null) {
          fail(`${label}_${toolId}_proof_backed_current_blocker_not_null:${row.currentBlockingPrerequisiteKey}`)
        }
        if (row.currentBlockingReasonCode !== null) {
          fail(`${label}_${toolId}_proof_backed_reason_not_null:${row.currentBlockingReasonCode}`)
        }
      } else {
      ...
      if (acceptedProofBackedTool) {
        if (!Array.isArray(row.remainingPrivateRuntimeInputKeys) || row.remainingPrivateRuntimeInputKeys.length !== 0) {
          fail(`${label}_${toolId}_proof_backed_remaining_inputs_not_empty:${JSON.stringify(row.remainingPrivateRuntimeInputKeys)}`)
        }
      } else if (!arrayMatches(row.remainingPrivateRuntimeInputKeys, expectedRemainingInputKeys)) {
      ...
      if (acceptedProofBackedTool) {
        if (unlockPlan.length !== 0) {
          fail(`${label}_${toolId}_proof_backed_unlock_plan_not_empty:${unlockPlan.length}`)
        }
      } else if (unlockPlan.length !== expectedUnlockActions.length) {
      ...
      if (toolId === 'kornia' && !acceptedProofBackedTool) {
...
if (
  ...
  fail('fastest_gpu_unlock_candidate_not_cpu_model_closeout')
}
if (
  docs.fastestGpuModelUnlockCandidate?.recommendedBackend !==
  'docker_container_cpu_model_runtime'
) {
  fail('fastest_gpu_unlock_candidate_not_cpu_model_backend')
}
if (
  !String(docs.fastestGpuModelUnlockCandidate?.remainsBlockedUntil ?? '').includes(
    'Real-ESRGAN/rembg CPU-model route failure diagnostics',
  )
) {
  fail('fastest_gpu_unlock_candidate_missing_cpu_model_blocker')
}
if (
  !String(
    docs.fastestGpuModelUnlockCandidate?.nextExactCommand ?? '',
  ).includes('ai-graphics:external-agent-all21-controlled-route-execution-smoke')
) {
  fail('fastest_gpu_unlock_candidate_missing_cpu_model_route_command')
}
if (
  !String(
    docs.fastestGpuModelUnlockCandidate?.nextExactCommand ?? '',
  ).includes('--scoped-gpu-tool real_esrgan,rembg,transparent_background')
) {
  fail('fastest_gpu_unlock_candidate_missing_cpu_model_tool_scope')
}
if (
  !String(
    docs.fastestGpuModelUnlockCandidate?.nextExactCommand ?? '',
  ).includes('--scoped-gpu-timeout-ms 30000')
) {
  fail('fastest_gpu_unlock_candidate_missing_bounded_timeout')
}
if (
  !String(
    docs.fastestGpuModelUnlockCandidate?.nextExactReadinessWithPrivateProofCommand ?? '',
  ).includes('ai-graphics:external-agent-execution-readiness')
) {
  fail('fastest_gpu_unlock_candidate_missing_readiness_recheck_command')
}
for (const flag of [
  '--scoped-gpu-allow-cpu-model-runtime',
  '--scoped-gpu-real-esrgan-model',
  ...
  if (
    ...
    fail(`fastest_gpu_unlock_candidate_cpu_model_command_missing:${flag}`)
  }
}
...
const docsExpectedStatus =
  docs.cpuSafeGpuModelRouteProof?.accepted === true ||
  docs.cpuModelGpuModelRouteProof?.accepted === true
    ? privateProofStatus
    : status
checkReport('docs', docs, docsExpectedStatus)
if (docs.cpuSafeGpuModelRouteProof?.accepted === true) {
  checkReadinessCpuSafeRouteProofSummary('docs', docs)
}
if (docs.cpuModelGpuModelRouteProof?.accepted === true) {
  checkReadinessCpuModelRouteProofSummary('docs', docs)
}
...
checkReport('live_cpu_safe_route_proof', liveWithCpuSafeRouteProof, privateProofStatus)
checkReadinessCpuSafeRouteProofSummary(
  'live_cpu_safe_route_proof',
  liveWithCpuSafeRouteProof,
)
ensureCpuModelGpuModelRouteProofSourceImage()
const cpuModelGpuModelRouteProof = execFileJson(
  'npm',
  cpuModelGpuModelRouteProofArgs(),
)
checkCpuModelGpuModelRouteProof(
  'cpu_model_gpu_model_route_proof',
  cpuModelGpuModelRouteProof,
)
const cpuModelGpuModelRouteProofPacketPath = path.join(
  root,
  '.local-artifacts',
  'ai-graphics',
  'external-agent-execution-readiness',
...
fs.writeFileSync(
  cpuModelGpuModelRouteProofPacketPath,
  `${JSON.stringify(cpuModelGpuModelRouteProof, null, 2)}\n`,
)
const liveWithCpuSafeAndCpuModelRouteProof = execFileJson('npm', [
  'run',
  '--silent',
  runScriptName,
  '--',
...
checkReport(
  'live_cpu_safe_and_cpu_model_route_proof',
  liveWithCpuSafeAndCpuModelRouteProof,
  privateProofStatus,
)
checkReadinessCpuSafeRouteProofSummary(
  'live_cpu_safe_and_cpu_model_route_proof',
...
checkReadinessCpuModelRouteProofSummary(
  'live_cpu_safe_and_cpu_model_route_proof',
  liveWithCpuSafeAndCpuModelRouteProof,
)
...
for (const phrase of [
  ...
  '--attempt-cpu-model-gpu-model-route-proof',
  '--cpu-model-gpu-model-route-proof-packet',
  '--scoped-gpu-timeout-ms',
  'cpuModelGpuModelRouteProof',
  'agentCanExecute17ControlledRouteToolsWithCpuSafeAndCpuModelGpuModelRouteProofNow',
...
if (executionGate.counts?.externalAgentExecutableNowTools !== 17) {
  fail('execution_gate_executable_now_not_17')
}
...
for (const phrase of [
  ...
  'Tools investigated but still blocked on this host',
  'rembg',
  'real_esrgan',
  'transparent_background',
...
console.log(JSON.stringify({
  ...
  agentExecutableToolsWithCpuSafeAndCpuModelGpuModelRouteProof:
    docs.counts.agentExecutableToolsWithCpuSafeAndCpuModelGpuModelRouteProof,
  cpuModelGpuModelRouteProofAccepted:
    docs.booleans.cpuModelGpuModelRouteProofAccepted,
