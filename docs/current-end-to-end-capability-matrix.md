...
| Canonical private work graph | `private_executable` run-to-blocked orchestration | Authenticated package route derives topological order, executes only dependency-ready jobs, persists exact per-job outcomes, and reuses completed jobs across continuation runs | Required media/render/QA capabilities and terminal private-review assembly still block full journey completion. |
