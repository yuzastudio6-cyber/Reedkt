...
After the local accepted evidence snapshot passes, operators must first deploy or verify the staging API service, then run `npm run beta:readiness:api-deployment-preflight`. That preflight verifies the API URL, deployed source SHA, API image, service account, staging-only environment, and Secret Manager binding confirmations without running `gcloud` or touching the deployed backend.
