...
export function evaluateBetaReadinessGatePolicy(
  ...
  const internalDryRunBlockers = [
    ...
    ...(input.rawPromptSafetyPassed === false ? ['Raw prompt safety gate failed.'] : []),
    ...(input.secretSafetyPassed === false ? ['Secret safety gate failed.'] : []),
    ...(input.signedUrlSourceTruthBlocked === false ? ['Signed URLs must not be used as source truth.'] : []),
  ]
  ...
  const boundedToolExecutionBlockers = [
    ...
    ...(input.boundedToolExecutionReady ? [] : ['Bounded tool execution evidence is missing.']),
    ...(input.approvedSnapshotPolicyApproved ? [] : ['Approved plan snapshot policy is missing.']),
    ...(input.creditReservationPolicyApproved ? [] : ['Credit estimate/reservation policy is missing.']),
  ]
