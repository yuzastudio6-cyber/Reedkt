...
M15B uses the M10 FFmpeg readiness path for preview-only color correction and the optional OpenColorIO/OpenImageIO readiness declarations for color-management and frame transform scaffolds. OpenColorIO/OpenImageIO remain optional/manual-review gated until a later install/legal/readiness step approves production use.
