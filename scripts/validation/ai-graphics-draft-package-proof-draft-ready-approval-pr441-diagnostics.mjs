...
const baseRef = 'origin/codex/rp-ai-graphics-draft-package-proof-draft-ready-execution-pr433'
const expectedDecision = 'ai_graphics_draft_package_proof_pr441_draft_ready_approval_passed_with_warnings'
const allowedDecisions = new Set([
  expectedDecision,
  'ai_graphics_draft_package_proof_pr441_draft_ready_approval_passed',
  'blocked_pending_pr425_post_ready_state_review',
  'blocked_pending_pr433_post_ready_state_review',
  'blocked_pending_pr441_not_draft',
  'blocked_pending_pr441_not_mergeable_clean',
  'blocked_pending_pr441_head_changed_review',
  'blocked_pending_pr441_scope_mismatch',
  'blocked_pending_pr441_draft_ready_scope_review',
])

const requiredDocs = [
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441-source-lockfile.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441-matrix.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr425-post-ready-review-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr433-post-ready-review-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr441-draft-ready-approval.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-stack-order-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-boundary-review-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-blocked-use-register-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-decision-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-next-lane-recommendation-pr441.md',
  'docs/prompt-ai-graphics-draft-package-proof-draft-ready-approval-pr441-results.md',
  'docs/implementation-prompts/prompt-ai-graphics-draft-package-proof-draft-ready-approval-pr441.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441-matrix.json',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441-source-lockfile.json',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-stack-order-pr441.json',
]

const expectedSources = ['PR #564', 'PR #433', 'PR #425', 'PR #562', 'PR #561', 'PR #558', 'PR #556', 'PR #554', 'PR #552', 'PR #550', 'PR #548', 'PR #543', 'PR #536', 'PR #416', 'PR #441', 'PR #542', 'PR #544']
...
const allStackTools = [...pr425Tools, ...pr433Tools, ...pr441Tools]
const expectedOrder = [425, 433, 441]

const requiredTrueBooleans = [
  'pr441DraftReadyApprovalCompleted',
  'pr425PostReadyStateChecked',
  'pr433PostReadyStateChecked',
  'pr425StillOpenNonDraftClean',
  'pr433StillOpenNonDraftClean',
  'pr441Reviewed',
  'pr441DraftReadyApprovedForFutureExecution',
  'stackOrderAccepted',
  'readyForPr441DraftReadyExecution',
]
...
const forbiddenPatterns = [
  ['pr441_ready_now', /\b(pr441MarkedReadyNow|draftMarkedReadyNow)\b\s*[:=]\s*`?true`?/i],
  ['other_ready_now', /\banyOtherPrMarkedReadyNow\b\s*[:=]\s*`?true`?/i],
  ['source_prs_merged_now', /\b(prMergedNow|sourcePrsMergedNow|pr425MergedNow|pr433MergedNow)\b\s*[:=]\s*`?true`?/i],
  ...
  ['all_tools_installed', /\ball 21 tools (?:are )?installed\b/i],
...
for (const required of ['TRACK_B_MEDIA_OSS_STEWARD', 'Track A', 'PR #425 -> PR #433 -> PR #441', 'AI_GRAPHICS_DRAFT_PACKAGE_PROOF_DRAFT_READY_EXECUTION_PR441']) {
  if (!docsText.includes(required)) failures.push(`missing_required_text:${required}`)
}
for (const tool of pr441Tools) {
  if (!docsText.includes(tool)) failures.push(`missing_pr441_tool:${tool}`)
}
...
let stackOrder = {}
try {
  sourceLockfile = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441-source-lockfile.json')
  matrix = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-pr441-matrix.json')
  stackOrder = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-stack-order-pr441.json')
} catch (error) {
...
if (stackOrder.firstDraftReadyTarget !== 441) failures.push(`first_draft_ready_target_unexpected:${stackOrder.firstDraftReadyTarget}`)
...
if (rows[433]?.draft !== false || rows[433]?.state !== 'open' || rows[433]?.mergeable !== 'CLEAN' || rows[433]?.merged !== false) failures.push('pr433_post_ready_state_unexpected')
...
if (rows[441]?.canBeMarkedReadyInFutureExecution !== true) failures.push('pr441_future_approval_not_true')
if (rows[441]?.canBeMarkedReadyNow !== false || rows[441]?.draftMarkedReadyNow !== false) failures.push('pr441_now_flags_not_false')
if (rows[425]?.canBeMarkedReadyInFutureExecution !== false || rows[433]?.canBeMarkedReadyInFutureExecution !== false) failures.push('post_ready_prs_should_not_have_future_approval')
...
for (const source of ['pr564', 'pr562', 'pr561', 'pr558', 'pr556', 'pr554', 'pr552', 'pr550', 'pr548', 'pr543', 'pr536', 'pr441']) {
...
for (const source of ['pr425', 'pr433']) {
  const data = sourceLockfile.sources?.[source]
  if (!data || data.state !== 'open' || data.draft !== false || data.mergeable !== 'CLEAN' || data.merged !== false) failures.push(`source_lockfile_post_ready_unexpected:${source}`)
}
...
try {
  ...
  const expectedScript = 'node scripts/validation/ai-graphics-draft-package-proof-draft-ready-approval-pr441-diagnostics.mjs'
  if (packageAfter.scripts?.['ai-graphics:draft-package-proof-draft-ready-approval-pr441:diagnostics'] !== expectedScript) failures.push('missing_package_script')
} catch (error) {
...
if (failures.length) {
  console.error('AI graphics draft package proof PR441 draft-ready approval diagnostics failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('AI graphics draft package proof PR441 draft-ready approval diagnostics passed.')
