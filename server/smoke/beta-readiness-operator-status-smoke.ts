...
assert.ok(readyReport.nextActions.some((action) => action.includes('beta:readiness:owner-command-handoff')), 'ready report should name the owner command handoff CLI')
...
assert.ok(readyReport.nextActions.some((action) => action.includes('28321041675')), 'ready report should name the latest read-only prerequisite audit run')
assert.ok(readyReport.nextActions.some((action) => action.includes('28321557589')), 'ready report should name the latest owner-remediation workflow-scope-fix run')
assert.equal(readyReport.nextActions.some((action) => action.includes('28310853041')), false, 'ready report should not point at the stale owner-remediation partial-results run')
...
assert.ok(emptyReport.nextActions.some((action) => action.includes('beta:readiness:owner-command-handoff')), 'empty report should direct operator to owner command handoff')
