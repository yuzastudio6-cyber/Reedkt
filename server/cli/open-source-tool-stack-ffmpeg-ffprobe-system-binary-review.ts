import {
  ...
  writeOpenSourceToolStackFfmpegFfprobeSystemBinaryReviewArtifacts,
...
if (!execute) {
  console.log(summarizeOpenSourceToolStackFfmpegFfprobeSystemBinaryReview())
  process.exit(0)
}

if (!metadataOnly) {
  throw new Error('Use --metadata-only. This review may not install binaries, run probes, process media, or mutate containers.')
}
...
const reports = writeOpenSourceToolStackFfmpegFfprobeSystemBinaryReviewArtifacts()
console.log(summarizeOpenSourceToolStackFfmpegFfprobeSystemBinaryReview(reports))
