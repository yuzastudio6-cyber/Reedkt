...
check(
  ...
  'Private invoke client status must record the controlled persisted worker dispatch runtime real-dispatch transport attempt approval blocker.',
)
check(
...
check(
  docText.includes('approved_worker_integration_review_required') ||
    ...
    docText.includes('controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_attempt_approval_required') ||
    data.privateInvokeClient.currentStatus ===
      'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_attempt_approval_required',
...
check(
  docText.includes('Retry 14 verified that fix') &&
    ...
    docText.includes('QWEN2_5_VL_STACK_TOOL_58CU-CONTROLLED-PERSISTED-WORKER-DISPATCH-RUNTIME-REAL-DISPATCH-TRANSPORT-ATTEMPT-APPROVAL'),
  'Doc must record retry-14 evidence, the storage upload pipeline policy-comment fix, retry-15 pass, and controlled persisted worker dispatch runtime real-dispatch transport attempt approval blocker.',
)
check(
