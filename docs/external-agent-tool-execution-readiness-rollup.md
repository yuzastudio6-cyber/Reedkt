...
Decision: `external_agent_tool_execution_readiness_qwen_structured_output_fix_required_broll_quota_blocked`.
...
| --- | --- | --- | --- | --- |
| `qwen2_5_vl_7b_instruct` | controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference bounded retry executed, structured schema invalid | blocked | the 58DW bounded retry loaded Qwen and returned parseable JSON, but the fixture metadata failed the required schema; another runtime retry is blocked until structured-output generation is tightened | `QWEN2_5_VL_STACK_TOOL_58DW-FIX: tighten Qwen fixture structured-output generation after schema-invalid bounded retry, no generated assets/no mutation` |
...
1. Qwen: the 58DW bounded retry result is recorded as schema-invalid runtime evidence. Current read-only live preflight can still verify auth/service/job visibility, but external agents must not run another Qwen runtime retry until the 58DW-FIX structured-output prompt updates and validates the schema-constrained generation path.
...
- Qwen is the closest lane to controlled private model inference; the bounded private inference retry plan/gate/approval, 58DV gate alignment, and 58DW bounded retry result are recorded. The retry proved model load/inference reachability but failed the required structured fixture schema, so runtime is blocked pending 58DW-FIX; raw chat, direct Cloud Run invocation, unbounded inference, generated assets, Supabase mutation, signed URLs, credits, beta, and production remain blocked.
