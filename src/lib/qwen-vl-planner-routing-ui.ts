...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'backend_runtime_persistence_credit_approval_snapshots_baseline_fix_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58AE-BACKEND-RUNTIME-PERSISTENCE-BASELINE-CREDIT-APPROVAL-SNAPSHOTS-FIX: fix ReEditPro local baseline approved snapshot credit/approval columns for Qwen harness validation, no deploy/no cloud/no assets/no beta'
}
  ...
  return {
    ...
    privateInvokeClient: {
      ...
      boundaryNotes: [
        ...
        'The next runtime gate must fix the credit approval snapshots local baseline prerequisite; it must not touch Supabase cloud, staging, production, live data, Cloud Run, Qwen inference, worker dispatch, generated assets, public artifacts, signed URLs, beta, or production.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
