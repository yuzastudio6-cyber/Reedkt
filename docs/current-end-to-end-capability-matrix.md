...
| Private render and review journey | `private_executable` for bounded internal fixtures including exact ordered two-source/two-caption composition; narrow named-edit browser review and exact bounded revision continuation remain connected at the prior baseline | The canonical graph applies approved trims and exact caption frame ranges, renders the private H.264/AAC MP4, independently probes that exact dependency, proves lease/artifact lineage, reconciles and replays safely, and supports authenticated private download. The focused caption-track lifecycle passes through that private download boundary. The Remotion image must carry the aggregate SHA-256 of its current reviewed source tree before activation. | The post-change aggregate browser review/revision branch has not been rerun in this slice. The legacy upload-to-render route remains disabled. Rich plan semantics, structural preference-changing revisions, deployed real-user storage/workers, public delivery, and production promotion are not connected. |
