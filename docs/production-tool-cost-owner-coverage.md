...
RP-TOOLEXEC-02 does not change the registry-derived owner model. Its narrowly authorized private Playwright capture emits a nonbillable `infrastructure_runtime` event only after validated PNG bytes exist; unresolved or degraded browser-capture plans emit no actual-work event. The event keeps `serviceFeeIncluded = false`, `walletMutationExecuted = false`, and `settlementExecuted = false`.
