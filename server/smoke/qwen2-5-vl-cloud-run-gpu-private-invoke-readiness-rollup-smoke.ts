...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke preflight: blocked, preflight required',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke preflight | blocked controlled persisted worker dispatch runtime real-dispatch approved fixture private invoke preflight required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 119)
assert.equal(
...
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokePreflightRequired, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokePreflightRecorded, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokePreflightPassed, false)
...
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokePreflight, true)
assert.equal(
