...
| Audio, timing, music, and SFX manifests | `SOUND_MUSIC_AUDIO` | `server/routes/worker-routes.ts::future_audio_tool_route_placeholder` | `timing_aware_sound_cue_manifest_placeholder`, AudioFlux planning placeholder, SFX Director placeholder, music ducking placeholder, Track A/Track B/provider handoff placeholders from PR #371. | Audio processing, audio generation, SFX/music generation, FFmpeg/FFprobe, DeepFilterNet, Demucs, provider calls, worker execution, and public artifacts remain blocked. |
