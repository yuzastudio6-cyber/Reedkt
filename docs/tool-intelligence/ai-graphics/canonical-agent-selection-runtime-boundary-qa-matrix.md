# AI Graphics Canonical Agent Selection Runtime Boundary QA Matrix

Decision: `ai_graphics_canonical_agent_selection_runtime_boundary_qa_passed_with_warnings`

## Runtime Bucket QA
| Bucket | Applies To | QA Result |
| --- | --- | --- |
| planning_metadata_allowed_now | all 21 tools and all 12 capabilities | accepted with warning: Agent may select, rank, eliminate, and recommend tools for planning/study metadata only; no execution or artifacts. |
| cpu_static_execution_previously_validated_but_not_agent_executable_now | `d3`, `vega_lite`, `vega`, `satori`, `svgdotjs_svg_js`, `viz_js` | accepted with warning: CPU/static proof exists as evidence, but canonical agent selection may not execute these tools. |
| browser_chart_runtime_later | `echarts` | accepted with warning: Browser chart runtime approval is required before execution. |
| animation_runtime_later | `lottie_web`, `animejs` | accepted with warning: Animation manifest/runtime approval is required before execution. |
| browser_canvas_webgl_runtime_later | `three_js`, `pixi_js`, `konva`, `babylonjs` | accepted with warning: Browser/canvas/WebGL sandbox approval is required before execution. |
| model_cpu_gpu_runtime_later | `torch_torchvision`, `transformers`, `sam2`, `birefnet`, `real_esrgan`, `kornia`, `rembg`, `transparent_background` | accepted with warning: CPU import, model provenance, model-weight, and later GPU/runtime approvals are required before execution. |
| tool_route_handoff_later | `torch_torchvision`, `transformers`, `sam2`, `birefnet`, `real_esrgan`, `kornia`, `rembg`, `transparent_background`, `d3`, `echarts`, `vega_lite`, `vega`, `satori`, `svgdotjs_svg_js`, `viz_js`, `lottie_web`, `animejs`, `three_js`, `pixi_js`, `konva`, `babylonjs` | accepted with warning: Future Tool Route approval may consume canonical agent-selection schema as planning metadata only until execution is separately approved. |
...
| public_artifact_and_signed_url_later | `torch_torchvision`, `transformers`, `sam2`, `birefnet`, `real_esrgan`, `kornia`, `rembg`, `transparent_background`, `d3`, `echarts`, `vega_lite`, `vega`, `satori`, `svgdotjs_svg_js`, `viz_js`, `lottie_web`, `animejs`, `three_js`, `pixi_js`, `konva`, `babylonjs` | accepted with warning: Signed URLs, GCS/public artifacts, and public artifact creation remain blocked. |

## Tool QA
| Tool | Runtime Bucket | Planning Selection | Agent Execution | Required Next Approval |
| --- | --- | --- | --- | --- |
...
## Capability QA
| Capability | Runtime Bucket | Candidate Tools | Planning Selection | Agent Execution |
| --- | --- | --- | --- | --- |
