...
function assertFalseFlags(flags: JsonRecord) {
  for (const key of [
    'backendRuntimePersistenceLocalHarnessValidationRetry10Passed',
...
const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-local-harness-validation-retry-10-result.md')
for (const phrase of [
  DECISION,
  '`55430`',
  '`55431`',
  ...
  '`qa_reports_approved_plan_snapshot_id_missing`',
...
const result = QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_LOCAL_HARNESS_VALIDATION_RETRY_10_RESULT
assert.equal(result.decision, DECISION)
assert.equal(
  result.upstreamBackendRuntimePersistenceBaselineQaCheckResultsCheckColumnFixDecision,
...
assert.equal(result.attempt.previouslyFailedBaselineColumnPassed, 'qa_check_results.check')
assert.equal(result.attempt.previouslyFailedBaselineColumnSqlPassed, '"check" text')
assert.equal(
...
assert.equal(result.attempt.failedStatementCategory, 'qa_reports_approved_plan_snapshot_id_missing')
assert.equal(result.attempt.failedBaselineIndex, 'idx_qa_reports_project_snapshot')
assert.equal(result.attempt.missingBaselineColumnObserved, 'qa_reports.approved_plan_snapshot_id')
assert.equal(
...
assert.equal(result.validationOutcome.status, 'blocked_qa_reports_approved_plan_snapshot_id_missing')
assert.equal(result.validationOutcome.qaCheckResultsCheckReservedColumnFixVerified, true)
assert.equal(result.validationOutcome.qaReportsApprovedPlanSnapshotFixRequired, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry10ResultRecorded, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry10Attempted, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceQaCheckResultsCheckReservedColumnFixVerified, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceQaReportsApprovedPlanSnapshotFixRequired, true)

const forbiddenDataFindings = scanValues({ result })
assert.deepEqual(
  forbiddenDataFindings,
  [],
  `Forbidden values in Qwen validation retry 10 data: ${forbiddenDataFindings.join('; ')}`,
)

console.log(JSON.stringify({
  ok: true,
  decision: DECISION,
  localHarnessValidationRetry10Attempted: result.validationOutcome.localHarnessValidationRetry10Attempted,
  localHarnessValidationRetry10Passed: result.validationOutcome.localHarnessValidationRetry10Passed,
  qaCheckResultsCheckReservedColumnFixVerified:
    result.validationOutcome.qaCheckResultsCheckReservedColumnFixVerified,
  ...
  failedBaselineIndex: result.attempt.failedBaselineIndex,
  missingBaselineColumnObserved: result.attempt.missingBaselineColumnObserved,
