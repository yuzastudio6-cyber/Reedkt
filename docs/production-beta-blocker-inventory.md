...
| Critical | Local/staging Supabase/RLS not executed | Access policies are not proven. | Cross-workspace data exposure. | Repair local migration chain, then run the existing first local executable SQL candidate; staging SQL requires later human approval. | Prompt 20L, then retry Prompt 20B SQL execution if local `supabase start` passes | No. | Yes |
| Critical | Remote/staging migration not validated | Schema chain may not apply cleanly. | Runtime data corruption or missing tables. | Validate migration chain in approved local environment first, then staging only with approval. | Prompt 20L, then Prompt 20B or another migration-chain follow-up depending on local result | No. | Yes |
...
| Medium | Local Supabase migration-chain blocker | Local SQL validation blocked. | Slower RLS iteration and no RLS execution evidence. | Prompt 20K repairs the Prompt 5/6-era credit/approval compatibility blocker, but `supabase start` now fails in `202605180005_reeditpro_generation_assets_jobs.sql` because `public.generation_requests.approved_plan_snapshot_id` is missing before `idx_generation_requests_project_snapshot` is created. | Prompt 20L, then rerun Prompt 20B SQL execution if migration-chain validation passes | Yes for static planning only. | Yes before production beta |

## Decision

Prompt 20K repairs the credit/approval compatibility blocker in `202605180004_reeditpro_credits_approval_snapshots.sql`. Production beta still cannot proceed because local/staging Supabase/RLS has not executed. The next safe milestone is Prompt 20L - Local Supabase Migration Chain Repair Follow-Up 5 for `202605180005_reeditpro_generation_assets_jobs.sql` before SQL/RLS execution is retried.
