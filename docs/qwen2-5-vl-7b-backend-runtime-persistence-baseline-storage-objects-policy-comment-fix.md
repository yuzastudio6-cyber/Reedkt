# Qwen2.5-VL 7B Backend Runtime Persistence Baseline Storage Objects Policy Comment Fix

Decision: `qwen2_5_vl_backend_runtime_persistence_baseline_storage_objects_policy_comment_fix_recorded`.

This packet records a narrow active-baseline compatibility fix for Qwen backend runtime persistence local harness validation. Retry 13 proved the `storage.buckets` table-comment guard worked, then stopped before Qwen draft SQL in active migration `202605180008_reeditpro_storage_buckets_policies.sql`. The blocker was the documentation-only policy comment statement `comment on policy "reeditpro_project_members_read_project_objects" on storage.objects`, which failed because the local migration role is not owner of the Supabase platform relation: `must be owner of relation objects (SQLSTATE 42501)`.

The fix updates only `supabase/migrations/202605180008_reeditpro_storage_buckets_policies.sql`. The storage buckets remain private, bucket upserts remain unchanged, and `storage.objects` policy definitions remain unchanged. The policy comments are now wrapped in an ownership-safe `do $$` block that catches `insufficient_privilege` and raises a notice instead of aborting the local baseline load.
...
- Failed statement: `comment on policy "reeditpro_project_members_read_project_objects" on storage.objects`
- Repair: `guard_storage_objects_policy_comments_with_insufficient_privilege_notice`
...
- The active storage buckets policies migration no longer aborts solely because a local migration role cannot comment on policies attached to the Supabase-owned `storage.objects` platform relation.
