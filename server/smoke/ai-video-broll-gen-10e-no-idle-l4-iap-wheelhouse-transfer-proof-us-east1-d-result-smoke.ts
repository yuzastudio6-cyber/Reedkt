...
import {
  AI_VIDEO_BROLL_GEN_10E_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_D_RESULT,
  AI_VIDEO_BROLL_GEN_10F_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10e-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-d-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-10e-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-d-result.md'
const NEXT_PROMPT_PATH = 'docs/implementation-prompts/prompt-ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix.md'
const SPEC_PATH =
...
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10e-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-d-result'
const DECISION = 'ai_video_broll_gen_10e_us_east1_d_iap_wheelhouse_transfer_blocked_resource_pool_exhausted_cleanup_verified'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll10eResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10e-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-d-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `10E result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `10F prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10E_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_D_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10e_no_idle_l4_iap_wheelhouse_transfer_proof_us_east1_d_result')
assert.equal(result.attemptedZone, 'us-east1-d')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10F_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT)
