...
assert.equal(report.inputCounts.required, 66)
assert.equal(report.inputCounts.currentlyPendingInBlankEnvironment, 63)
...
assert.equal(status.inputCounts.required, 66)
assert.equal(status.inputCounts.currentlyPendingInBlankEnvironment, 63)
assert.equal(status.pendingInputs.length, 63)
assert.deepEqual(status.actionabilityCounts, {
  humanActionablePending: 48,
  autoFillablePending: 15,
  pendingPrefilledConstants: 11,
  pendingOperatorGeneratedIds: 4,
  pendingOwnerEvidenceNotes: 11,
  pendingOwnerApprovalConfirmations: 15,
  pendingOperatorConfirmations: 14,
...
assert.deepEqual(status.pendingValuePolicies, {
  ...
  operator_confirmation: 14,
  operator_unique_id: 4,
  prefilled_non_secret_constant: 11,
...
assert.equal(status.pendingInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_CONFIRM_DEPLOYED_ROUTE_PROOF'), true)
assert.equal(status.pendingInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX'), true)
...
assert.equal(status.humanActionablePendingInputs.length, 48)
assert.equal(status.autoFillablePendingInputs.length, 15)
...
assert.equal(status.humanActionablePendingInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_CONFIRM_DEPLOYED_ROUTE_PROOF' && input.valuePolicy === 'operator_confirmation'), true)
...
assert.equal(status.autoFillablePendingInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX'), true)
assert.equal(status.autoFillablePendingInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_APPROVED_SNAPSHOT_ID'), true)
assert.equal(status.autoFillablePendingInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_TOOL_EXECUTION_PLAN_PREFIX'), true)
...
assert.equal(autofill.inputCounts.autoFillablePending, 15)
assert.equal(autofill.inputCounts.generatedIdempotencyKeys, 4)
assert.equal(autofill.inputCounts.prefilledNonSecretConstants, 11)
...
assert.equal(autofill.autoFillableInputs.length, 15)
assert.equal(autofill.autoFillableInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX'), true)
assert.equal(autofill.autoFillableInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_APPROVED_SNAPSHOT_ID'), true)
assert.equal(autofill.autoFillableInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_TOOL_EXECUTION_PLAN_PREFIX'), true)
...
assert.equal(checklist.humanActionableInputs.length, 48)
...
assert.equal(checklist.humanActionableInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_CONFIRM_DEPLOYED_ROUTE_PROOF'), true)
...
assert.equal(checklist.humanActionableInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX'), false)
...
assert.equal(report.requiredInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_CONFIRM_DEPLOYED_ROUTE_PROOF'), true)
assert.equal(report.requiredInputs.some((input) => input.name === 'REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX'), true)
...
assert.ok(report.envTemplate.includes('REEDITPRO_BETA_TRACKB_AGENT_ROUTE_CONFIRM_DEPLOYED_ROUTE_PROOF="<operator confirmation: set to true only after this gate is intentionally accepted>"'))
assert.ok(report.envTemplate.includes('REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX="trackb-agent-route-proof-ee177046bfb0"'))
assert.ok(report.envTemplate.includes('REEDITPRO_BETA_TRACKB_AGENT_ROUTE_APPROVED_SNAPSHOT_ID="approved-snapshot-trackb-agent-route-proof-ee177046bfb0"'))
assert.ok(report.envTemplate.includes('REEDITPRO_BETA_TRACKB_AGENT_ROUTE_TOOL_EXECUTION_PLAN_PREFIX="tool-exec-trackb-agent-route-proof-ee177046bfb0"'))
...
assert.ok(report.envTemplate.includes('npm run beta:tools:trackb-agent-route-deployed-evidence-collector'))
assert.equal(report.envTemplate.includes('/path/to/local-only.env'), false)
assert.ok(autofill.envTemplate.includes('export REEDITPRO_BETA_TRACKB_AGENT_ROUTE_IDEMPOTENCY_PREFIX="trackb-agent-route-proof-ee177046bfb0"'))
...
assert.equal(applyAutofill.inputCounts.autoFillableInputs, 15)
...
assert.equal(applyAutofillAgain.inputCounts.unchangedInputs, 15)
...
assert.equal(bootstrap.inputCounts.assignedSafeInputs, 17)
assert.equal(bootstrap.inputCounts.commentedHumanInputs, 49)
...
assert.equal(autofillMarkdown.includes('Auto-fillable pending inputs: `15`'), true)
...
assert.equal(checklistMarkdown.includes('Human-actionable pending inputs: `48`'), true)
...
assert.equal(statusMarkdown.includes('Human-actionable pending inputs: `48`'), true)
assert.equal(statusMarkdown.includes('Auto-fillable pending inputs: `15`'), true)
