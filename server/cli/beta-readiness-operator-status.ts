...
function buildNextActions(
  ...
  actions.push('Before staging API deploy, a higher-privilege GCP owner must apply the exact staging IAM/runtime prerequisites directly: Artifact Registry writer for the deployer on us-central1/reeditpro-staging-workers, creation or selection of reeditpro-api-staging@reeditpro.iam.gserviceaccount.com, deployer iam.serviceAccountUser on that runtime account, approved Cloud Run deploy authority, and runtime secret accessor on only the fixed staging API secret entries. The guarded owner-remediation workflow beta-readiness-api-staging-owner-remediation.yml now reports partial results but current source truth run 28310853041 proves the deployer cannot self-remediate those owner-side permissions.')
