...
test.describe('editor mocked browser flow', () => {
  ...
  test('approves the mocked plan before progress and preview appear', async ({ page }) => {
    ...
    const previewSkillTrace = page.getByTestId('preview-ready-skill-trace')
    await expect(previewSkillTrace).toBeVisible()
    expect(await previewSkillTrace.evaluate((element) => getComputedStyle(element).display)).toBe('grid')
    ...
    const preparationStatus = page.getByTestId('private-review-preparation-status-card')
    await expect(preparationStatus).toBeVisible()
    await expect(preparationStatus).toContainText(/Preparation status/i)
    await expect(preparationStatus).toContainText(/Approved edit direction/i)
    await expect(preparationStatus).toContainText(/Why this direction was selected/i)
    await expect(preparationStatus).not.toContainText(/librosa|d3|three|gpac|mp4box|backend|source-truth/i)
    expect(await preparationStatus.evaluate((element) => getComputedStyle(element).display)).toBe('grid')
    expect(await preparationStatus.locator('.private-review-activity-grid').evaluate((element) => getComputedStyle(element).display)).toBe('grid')
