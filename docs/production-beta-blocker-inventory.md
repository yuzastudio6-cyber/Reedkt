...
| Critical | Local/staging Supabase/RLS not executed | Access policies are not proven. | Cross-workspace data exposure. | Repair arm64 Supabase CLI and `psql`, verify local DB URL, add first local SQL candidate, then run scoped local SQL; staging SQL requires later human approval. | Prompt 20D, then Prompt 20B | No. | Yes |
| Critical | Remote/staging migration not validated | Schema chain may not apply cleanly. | Runtime data corruption or missing tables. | Validate migration chain in approved local environment first, then staging only with approval. | Prompt 20D, Prompt 20B, then staging validation | No. | Yes |
...
| Medium | Supabase CLI/local SQL blocker | Local SQL validation blocked. | Slower RLS iteration and no RLS execution evidence. | Install compatible arm64 Supabase CLI or approved container path, provide `psql` or approved local SQL executor, verify a local DB URL, then add the first executable local SQL candidate. | Prompt 20D, then Prompt 20B | Yes for static planning only. | Yes before production beta |

## Decision

Prompt 20C adds the manual local Supabase setup package and clearer preflight/runner guidance. Local SQL/RLS execution remains blocked by wrong-architecture Supabase CLI error `-86`, missing `psql`, no verified local DB URL, and no local executable SQL candidates. Production beta still cannot proceed because local/staging Supabase/RLS has not executed. The next safe milestone is Prompt 20D - Manual Environment Setup Verification, followed by Prompt 20B - Local RLS First Executable Smoke Test once preflight reports `canRunLocalSql=true`.
