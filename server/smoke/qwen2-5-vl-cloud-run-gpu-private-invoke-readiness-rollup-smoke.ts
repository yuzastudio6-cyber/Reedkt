...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 65)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceActiveMigrationDeployPlanRequired, true)
