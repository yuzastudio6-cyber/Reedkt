import { existsSync, readFileSync } from 'node:fs';
import path from 'node:path';
import process from 'node:process';

const repoRoot = process.cwd();

const toolIds = [
  'remotion_graphics',
  'd3_dataviz',
  'three_js_visuals',
  'pixijs_canvas_graphics',
  'anime_js_motion',
  'lottie_web_overlays',
  'svg_js_vector_graphics',
  'echarts_dataviz',
  'vega_lite_dataviz',
  'viz_graphviz_diagrams',
  'satori_social_cards',
  'resvg_js_svg_rasterization',
];

const packageNames = [
  'remotion',
  'd3',
  'three',
  'pixi.js',
  'animejs',
  'lottie-web',
  '@svgdotjs/svg.js',
  'echarts',
  'vega',
  'vega-lite',
  '@viz-js/viz',
  'satori',
  '@resvg/resvg-js',
];

const requiredDocs = [
  'docs/ai-tools/creative-graphics-package-runtime-enablement.md',
  'docs/ai-tools/creative-graphics-package-runtime-matrix.md',
  'docs/ai-tools/creative-graphics-package-license-security-notes.md',
  'docs/ai-tools/creative-graphics-gd8-runtime-probe-evidence.md',
  'docs/prompt-gd-8-validation-results.md',
  'docs/implementation-prompts/prompt-gd-8-ai-tools-creative-graphics-package-runtime-enablement.md',
];

const requiredTrackers = [
  'PRODUCTION_FOUNDATION_STATUS.md',
  'docs/source-of-truth-map.md',
  'docs/production-milestone-plan.md',
  'docs/implementation-prompts/README.md',
  'docs/beta-readiness-scorecard.md',
  'docs/production-beta-blocker-inventory.md',
  'docs/ai-tools/creative-graphics-gd7-runtime-availability.md',
  'docs/ai-tools/creative-graphics-gd7-local-execution-evidence.md',
  'docs/ai-tools/creative-graphics-gd7-local-artifact-manifest-evidence.md',
  'docs/ai-tools/creative-graphics-gd7-qa-evidence.md',
  'docs/ai-tools/creative-graphics-future-prompt-sequence.md',
  'docs/ai-tools/creative-graphics-readiness-scorecard.md',
  'docs/ai-tools/creative-graphics-all-tools-readiness-matrix.md',
  'docs/ai-tools/creative-graphics-next-fixture-plan.md',
];

const requiredTerms = [
  'GD-8',
  'package_runtime_probe_mostly_passed_with_native_blocker',
  'package_runtime_probe_passed',
  'package_runtime_blocked',
  'needs_runtime_review',
  'generated_local_fixture_not_executed',
  'none; AI Tools creative graphics package runtime enablement only',
  'Supabase update required: `docs/status only`',
  'Supabase update status: `docs_only`',
  'Supabase environment touched: `none`',
  'SQL executed: `none`',
  'Migration deployed: `no`',
  'Prompt GD-8A - Package Runtime Fixes',
];

const runnerPath = 'scripts/fixtures/ai-tools/probe-creative-graphics-runtimes.mjs';
const failures = [];

function readFile(relativePath) {
  const absolutePath = path.join(repoRoot, relativePath);
  if (!existsSync(absolutePath)) {
    failures.push(`Missing required file: ${relativePath}`);
    return '';
  }
  return readFileSync(absolutePath, 'utf8');
}

function isSafetyNegated(line) {
  return /\b(no|not|none|blocked|must not|do not|without|out of scope|forbid|prohibit|never|future-only|placeholder|placeholders only|missing|absent|unrun|not run|does not|did not|remains blocked|not approved|approval missing|not execute|not enabled|not created|not claimed|not performed|skip|skipped|unavailable|probe only|import-only|local-only|blocked\/skipped|needs_runtime_review|package_runtime_blocked)\b/i.test(
    line,
  );
}

function scanUnsafeLines(relativePath, text) {
  const unsafePatterns = [
    /fixture generation:\s*`?(yes|true|executed|ran|created|enabled)`?/i,
    /generated artifacts?:\s*`?(yes|true|created|present|enabled)`?/i,
    /tool execution:\s*`?(enabled|yes|true|executed|ran|unlocked)`?/i,
    /worker execution:\s*`?(enabled|yes|true|executed|ran|unlocked)`?/i,
    /(provider|model) (call|calls):\s*`?(enabled|yes|true|executed|ran)`?/i,
    /render\/export:\s*`?(enabled|yes|true|executed|ran|unlocked)`?/i,
    /media processing:\s*`?(enabled|yes|true|executed|ran)`?/i,
    /browser capture:\s*`?(enabled|yes|true|executed|ran)`?/i,
    /Docker\/Cloud Run.*:\s*`?(enabled|yes|true|executed|ran)`?/i,
    /Google Cloud.*:\s*`?(enabled|yes|true|used|fetched|called)`?/i,
    /Secret Manager.*:\s*`?(enabled|yes|true|used|fetched|called)`?/i,
    /Supabase environment touched:\s*`?(local|staging|remote|production|yes|true)`?/i,
    /SQL executed:\s*`?(local|staging|remote|production|yes|true)`?/i,
    /Migration deployed:\s*`?(yes|true|staging|production)`?/i,
    /public artifact(?:s)?:\s*`?(created|yes|true|present|allowed|enabled)`?/i,
    /signed URL(?:s)?(?: source(?:-| )of(?:-| )truth)?:\s*`?(created|allowed|enabled|yes|true)`?/i,
    /runtime unlock:\s*`?(enabled|yes|true|unlocked)`?/i,
    /production capability enabled:\s*`?(?!none; AI Tools creative graphics resvg runtime review only|none; AI Tools creative graphics package runtime enablement only|none; Track A private preview composition plan only|none; Track A creative graphics handoff review only|none; controlled local creative graphics fixture execution only|none; AI Tools creative graphics execution approval gate packet only|none; AI Tools creative graphics controlled execution plan only|none; AI Tools creative graphics static fixture gate review only|none; AI Tools creative graphics generated\/local fixture candidate pack only|none; AI Tools creative graphics dry-run fixture pack only|none; AI Tools creative graphics manifest contract only|none; AI Tools creative graphics repo audit only)/i,
    /postgres(?:ql)?:\/\/[^@\s]+@/i,
    /eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}/,
    /service[_-]?role[_-]?key\s*[:=]\s*[A-Za-z0-9_-]{12,}/i,
    /X-Goog-Signature|X-Amz-Signature|sig=|signature=/i,
    /https?:\/\/(?!github\.com\/yuzastudio6-cyber\/Reedkt\/pull\/|github\.com\/yuzastudio6-cyber\/Reedkt\/actions\/)/i,
    /gs:\/\/|storage\.googleapis\.com/i,
    /\bsupabase\s+(start|status|link|db|migration|functions|projects)\b/i,
    /\bgcloud\b/i,
    /\bpsql\b/i,
    /\bdocker\s+(run|build|compose|pull|push|exec)\b/i,
  ];

  for (const [index, line] of text.split('\n').entries()) {
    if (isSafetyNegated(line)) {
      continue;
    }
    for (const pattern of unsafePatterns) {
      if (pattern.test(line)) {
        failures.push(`Unsafe claim in ${relativePath}:${index + 1}: ${pattern}`);
      }
    }
  }
}

const docsText = requiredDocs.map((file) => readFile(file)).join('\n');
const trackersText = requiredTrackers.map((file) => readFile(file)).join('\n');
const runnerText = readFile(runnerPath);
const packageJsonText = readFile('package.json');
const packageLockText = readFile('package-lock.json');
const foundationRunnerText = readFile('scripts/validation/run-foundation-validation.mjs');
const workflowText = readFile('.github/workflows/foundation-validation.yml');
const combinedText = `${docsText}\n${trackersText}`;

for (const file of [...requiredDocs, ...requiredTrackers]) {
  const text = readFile(file);
  if (!text.includes('GD-8') && !text.includes('package_runtime_probe')) {
    failures.push(`File does not reference GD-8/package runtime status: ${file}`);
  }
  scanUnsafeLines(file, text);
}

let packageJson;
try {
  packageJson = JSON.parse(packageJsonText);
} catch (error) {
  failures.push(`package.json is not valid JSON: ${error.message}`);
  packageJson = {};
}

const dependencies = packageJson.dependencies ?? {};
for (const packageName of packageNames) {
  if (!dependencies[packageName]) {
    failures.push(`Missing GD-8 package dependency: ${packageName}`);
  }
  if (!packageLockText.includes(`"node_modules/${packageName}"`) && !packageLockText.includes(`"node_modules/${packageName.split('/').join('/')}"`)) {
    failures.push(`package-lock.json missing node_modules entry for: ${packageName}`);
  }
}

if (dependencies['@remotion/renderer'] || packageLockText.includes('"node_modules/@remotion/renderer"')) {
  failures.push('@remotion/renderer must not be added by GD-8.');
}

if (!packageJsonText.includes('ai-tools:creative-graphics:package-runtime:diagnostics')) {
  failures.push('Missing GD-8 package runtime diagnostic script.');
}

if (!foundationRunnerText.includes('ai-tools:creative-graphics:package-runtime:diagnostics')) {
  failures.push('Foundation validation runner does not include GD-8 diagnostic.');
}

if (!workflowText.includes('codex/rp-gd-7-ai-tools-creative-graphics-controlled-local-fixture-execution')) {
  failures.push('Foundation Validation workflow does not cover the GD-7 PR base branch.');
}

for (const toolId of toolIds) {
  if (!combinedText.includes(toolId)) {
    failures.push(`Missing GD-8 tool reference: ${toolId}`);
  }
}

for (const packageName of packageNames) {
  if (!docsText.includes(packageName)) {
    failures.push(`Missing GD-8 package reference in docs: ${packageName}`);
  }
  if (!runnerText.includes(packageName)) {
    failures.push(`Probe script missing package probe entry: ${packageName}`);
  }
}

for (const term of requiredTerms) {
  if (!combinedText.includes(term)) {
    failures.push(`Missing required GD-8 term: ${term}`);
  }
}

if (!docsText.includes('@resvg/resvg-js') || !docsText.includes('ERR_DLOPEN_FAILED')) {
  failures.push('GD-8 docs must record the @resvg/resvg-js native import blocker.');
}

for (const forbiddenRunnerPattern of [
  /child_process/,
  /\bfetch\s*\(/,
  /https?:\/\//,
  /\bsupabase\s+(start|status|link|db|migration|functions|projects)\b/i,
  /\bgcloud\b/i,
  /\bpsql\b/i,
  /\bdocker\s+(run|build|compose|pull|push|exec)\b/i,
]) {
  if (forbiddenRunnerPattern.test(runnerText)) {
    failures.push(`Probe script contains forbidden pattern: ${forbiddenRunnerPattern}`);
  }
}

const summary = {
  status: failures.length === 0 ? 'passed' : 'failed',
  packageRuntimeStatus: 'package_runtime_probe_mostly_passed_with_native_blocker',
  packagesChecked: packageNames.length,
  toolsChecked: toolIds.length,
  packagesPassed: 12,
  packagesBlocked: 1,
  packageRuntimeBlocked: ['@resvg/resvg-js'],
  supabaseUpdateRequired: 'docs/status only',
  supabaseUpdateStatus: 'docs_only',
  supabaseEnvironmentTouched: 'none',
  sqlExecuted: 'none',
  migrationDeployed: 'no',
  productionCapabilityEnabled: 'none; AI Tools creative graphics package runtime enablement only',
  nextRecommendedPrompt: 'Prompt GD-8A - Package Runtime Fixes',
  failures,
};

console.log(JSON.stringify(summary, null, 2));

if (failures.length > 0) {
  process.exit(1);
}
