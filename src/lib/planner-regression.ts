...
function createGlobalChecks(
  ...
  return [
    ...
    globalCheck({
      id: 'regression-testing-readiness-present',
      category: 'testing_readiness',
      label: 'Testing readiness coverage',
      severity: 'warning',
      passed: scenarioReports.length > 0 && scenarioReports.every((scenario) => scenarioCheckPassed(scenario, 'validation-testing-readiness-exists')),
      message: 'All demo scenarios should include testingReadinessReport.',
      relatedField: 'EditPlan.testingReadinessReport',
    }),
    globalCheck({
      id: 'regression-testing-readiness-no-execution',
      category: 'testing_readiness',
      label: 'Testing readiness no-real-execution coverage',
      severity: 'blocking',
      passed: scenarioReports.length > 0 && scenarioReports.every((scenario) => scenarioCheckPassed(scenario, 'validation-testing-readiness-no-execution')),
      message: 'Testing readiness must not imply provider, tool, Supabase, SQL, cloud, rendering, billing, or worker execution.',
