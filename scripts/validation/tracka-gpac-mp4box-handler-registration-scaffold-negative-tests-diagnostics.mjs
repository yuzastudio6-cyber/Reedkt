...
const lane = 'TRACKA-GPAC-MP4BOX-HANDLER-REGISTRATION-SCAFFOLD-NEGATIVE-TESTS-1'
const packetDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-handler-registration-scaffold-negative-tests'
const priorDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-disabled-handler-registration-scaffold'
const decisionText = 'tracka_gpac_mp4box_handler_registration_scaffold_negative_tests_passed_ready_for_guarded_handler_implementation_review'
const executionText = 'completed_handler_registration_scaffold_negative_tests_no_route_or_worker_execution'
const priorDecision = 'tracka_gpac_mp4box_disabled_handler_registration_scaffold_passed_ready_for_handler_registration_scaffold_negative_tests'
const priorMergeSha = '2978ac0ad4926b32e063336c9e154406f578e821'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-GUARDED-HANDLER-IMPLEMENTATION-REVIEW-1'

const packetFiles = [
  `${packetDir}/gpac-mp4box-handler-registration-scaffold-negative-tests-decision.json`,
  `${packetDir}/gpac-mp4box-handler-registration-scaffold-negative-tests-decision.md`,
...
const priorFiles = [
  `${priorDir}/gpac-mp4box-disabled-handler-registration-scaffold-decision.json`,
...
const implementationReviewDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-guarded-handler-implementation-review'
const implementationReviewFiles = [
  `${implementationReviewDir}/gpac-mp4box-guarded-handler-implementation-review-decision.json`,
  `${implementationReviewDir}/gpac-mp4box-guarded-handler-implementation-review-decision.md`,
  `${implementationReviewDir}/handler-implementation-review-matrix.json`,
  `${implementationReviewDir}/handler-implementation-review-matrix.md`,
  `${implementationReviewDir}/readiness-report.json`,
  `${implementationReviewDir}/source-of-truth-audit.json`,
  `${implementationReviewDir}/source-of-truth-audit.md`,
  `${implementationReviewDir}/validation-results.md`,
...
if (packageJson.scripts?.['tracka:gpac-mp4box-handler-registration-scaffold-negative-tests:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-handler-registration-scaffold-negative-tests-diagnostics.mjs') fail('missing diagnostics script')
if (packageJson.scripts?.['smoke:tracka-gpac-mp4box-handler-registration-scaffold-negative-tests'] !== 'tsx server/smoke/tracka-gpac-mp4box-handler-registration-scaffold-negative-tests-smoke.ts') fail('missing smoke script')

const decision = json(`${packetDir}/gpac-mp4box-handler-registration-scaffold-negative-tests-decision.json`)
...
if (decision.prior?.disabledHandlerRegistrationScaffold !== priorDecision) fail('prior decision drift')
if (decision.prior?.disabledHandlerRegistrationScaffoldMergeSha !== priorMergeSha) fail('prior merge drift')
...
if (readiness.readyForGuardedHandlerImplementationReview !== true) fail('next readiness drift')
...
const smokeText = read('server/smoke/tracka-gpac-mp4box-handler-registration-scaffold-negative-tests-smoke.ts')
for (const text of ['all_rejected_input_cases_block', 'all_runtime_attempt_cases_block', 'all_delivery_attempt_cases_block', 'guarded_handler_registration_plan_drift_blocks', 'operator_confirmation_drift_blocks']) {
...
for (const file of changedFiles) {
  ...
  if (/^server\//.test(file) && file !== 'server/smoke/tracka-gpac-mp4box-handler-registration-scaffold-negative-tests-smoke.ts') fail(`unexpected server path ${file}`)
}
...
console.log('Handler-registration scaffold negative tests passed')
