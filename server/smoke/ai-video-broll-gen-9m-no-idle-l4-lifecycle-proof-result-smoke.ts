...
import {
  AI_VIDEO_BROLL_GEN_9M_NO_IDLE_L4_LIFECYCLE_PROOF_RESULT,
  AI_VIDEO_BROLL_GEN_9N_NO_IDLE_L4_PROOF_EXECUTE_US_CENTRAL1_C_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9m-no-idle-l4-lifecycle-proof-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9m-no-idle-l4-lifecycle-proof-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-9n-no-idle-l4-proof-execute-us-central1-c.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-9m-no-idle-l4-lifecycle-proof-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-9m-no-idle-l4-lifecycle-proof-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9m-no-idle-l4-lifecycle-proof-result'
const DECISION = 'ai_video_broll_gen_9m_no_idle_l4_lifecycle_proof_blocked_resource_pool_exhausted_cleanup_verified'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll9mResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9m-no-idle-l4-lifecycle-proof-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `9M result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9N prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9M_NO_IDLE_L4_LIFECYCLE_PROOF_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9m_no_idle_l4_lifecycle_proof_result')
assert.equal(result.lifecycleStatus, 'blocked')
assert.equal(result.lifecycleBlocker, 'us_central1_a_l4_resource_pool_exhausted')
assert.equal(result.gcpErrorCode, 'ZONE_RESOURCE_POOL_EXHAUSTED_WITH_DETAILS')
assert.equal(result.targetZone, 'us-central1-a')
assert.equal(result.nextRetryZone, 'us-central1-c')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9N_NO_IDLE_L4_PROOF_EXECUTE_US_CENTRAL1_C_PROMPT)
  ...
  JSON.stringify(
    {
      ...
      nextRetryZone: result.nextRetryZone,
