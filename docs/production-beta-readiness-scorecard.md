...
- Result: the probe covers all 21 AI graphics tools. It reports `externalAgentRouteCallableNowTools=21` and keeps the legacy route-call count `externalAgentRouteExecutableNowTools=21`, with six CPU/static controlled tools, seven browser-runtime controlled tools, and eight GPU/model tools callable in explicit local/mock route mode through the controlled on-demand adapter. Real runtime execution remains limited to the 13 CPU/static plus browser-runtime tools. `realRuntimeExecutableNowTools=13`, `gpuModelRuntimeAdmissionBlockedTools=0`, `gpuModelRuntimeAdmissionEvaluatedFailClosedTools=8`, `gpuModelRuntimeUnblockPlanExposedTools=8`, `gpuModelRuntimeProofRequiredTools=8`, `gpuModelToolsReadyForExecutionAfterCurrentEvidence=0`, `modelWeightManifestRequiredTools=5`, `gpuRuntimeShouldStartNowTools=0`, `workerDispatchPerformedTools=0`, `publicArtifactCreatedTools=0`, and `signedUrlCreatedTools=0`.
