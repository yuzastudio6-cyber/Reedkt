...
const requiredPhrases = [
  'sound_music_audio_tool_study_passed_docs_only',
  'complete_for_SOUND_MUSIC_AUDIO_owner_study',
...
const allowedConfirmations = new Set([
  'REEDITPRO_CONFIRM_TOOL_STUDY_0_SOUND_MUSIC_AUDIO',
...
const forbiddenPatterns = [
  ...
  ['demucs_runtime_ready', /\bDemucs\s+(?:runtime|execution)\s*(?::|=)?\s*(?:ready|enabled|allowed|approved|true)\b/i],
...
let audit
try {
  audit = JSON.parse(readFileSync('docs/tool-studies/sound-music-audio-source-of-truth-audit.json', 'utf8'))
} catch (error) {
  failures.push(`source_audit_json_invalid:${error.message}`)
}

if (audit) {
  if (audit.decision !== 'sound_music_audio_tool_study_passed_docs_only') {
    failures.push(`source_audit_decision:${audit.decision}`)
  }
  if (audit.status !== 'complete_for_SOUND_MUSIC_AUDIO_owner_study') {
  ...
  if (audit.runtimeReadiness?.demucsRuntimeBlocked !== true) failures.push('source_audit_demucs_not_blocked')
...
if (packageJson.scripts?.['tool-study:sound-music-audio:diagnostics'] !== 'node scripts/validation/tool-study-sound-music-audio-diagnostics.mjs') {
