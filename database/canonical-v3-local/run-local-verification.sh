...
npx --no-install tsx \
  "${REPOSITORY_ROOT}/server/smoke/edit-reference-canonical-v3-exact-edit-target-source-smoke.ts"
"${PSQL_BIN}" "${DATABASE_URL}" -X -q -v ON_ERROR_STOP=1 \
...
npx --no-install tsx \
  "${REPOSITORY_ROOT}/server/smoke/motion-studio-private-local-command-repository-smoke.ts"
