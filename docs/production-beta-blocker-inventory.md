...
| Critical | Local/staging Supabase/RLS not executed | Access policies are not proven. | Cross-workspace data exposure. | Repair arm64 Supabase CLI and `psql`, then run scoped local SQL; staging SQL requires later human approval. | Prompt 20C, then Prompt 20B | No. | Yes |
| Critical | Remote/staging migration not validated | Schema chain may not apply cleanly. | Runtime data corruption or missing tables. | Validate migration chain in approved local environment first, then staging only with approval. | Prompt 20C, Prompt 20B, then staging validation | No. | Yes |
...
| Medium | Supabase CLI/local SQL blocker | Local SQL validation blocked. | Slower RLS iteration and no RLS execution evidence. | Install compatible arm64 Supabase CLI or approved container path, provide `psql` or approved local SQL executor, then add the first executable local SQL candidate. | Prompt 20C, then Prompt 20B | Yes for static planning only. | Yes before production beta |

## Decision

Prompt 20A adds local-only `supabase/config.toml`, hardens preflight/runner behavior, and confirms Docker is reachable on this host. Local SQL/RLS execution remains blocked by wrong-architecture Supabase CLI error `-86`, missing `psql`, no verified local DB URL, and no local executable SQL candidates. Production beta still cannot proceed because local/staging Supabase/RLS has not executed. The next safe milestone is Prompt 20C - Local Supabase Environment Manual Setup, followed by Prompt 20B - Local RLS First Executable Smoke Test once preflight reports `canRunLocalSql=true`.
