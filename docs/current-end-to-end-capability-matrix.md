...
| Source upload | `private_executable` for the small internal test path; large-media transport and finalization control plane are `contract_ready` with private lifecycle execution evidence | Authenticated upload intent, private source artifact, checksums, source-order lineage, project-scoped authority revision/checksum, cross-project non-invalidation, and same-project change invalidation. Separate incremental upload batches now derive collision-resistant source-sequence identities from their upload-plan/source identity instead of reusing a batch-local ordinal, and finalized backend media-asset identity is preserved into planning rather than replaced by a mock identity. Local and GCS source/reference probing reopen bytes through the storage adapter, bind GCS to the verified generation/ETag, stage through random scope-hashed create-only `0700`/`0600` private attempts, enforce exact size plus SHA-256, and remove only the exact owned attempt identity before finalization returns. Source-level policy admits up to 1 TiB source/250 GiB reference video, selects create-only resumable GCS transport above 16 MiB, resumes from exact committed offsets, and avoids browser whole-file hashing/auth leakage. Resumable sources fail closed on inline finalization and use checksum-protected private enqueue/poll authority with domain idempotency, one active lease, bounded retries, fresh-instance recovery, expired-lease reclamation, and a canonical final-read; browser code never invokes the internal worker route. Before lease claim, the full-stage worker must prove space for one source copy plus 8 GiB/10% headroom; insufficient capacity reads zero bytes and consumes zero attempts. The local media foundation executable-smoke-verifies a high-quality bounded 1080p Rec.709 proxy while retaining the original render master, and declared HDR/wide-gamut sources are explicitly withheld from that SDR path. | No large real object was uploaded or hashed/probed by the lifecycle smoke. The current worker still performs a full hash traversal plus private probe staging per attempt; distributed dispatch, durable byte progress, live resumable CORS/IAM/session behavior, representative worker disk/I/O capacity, and an executed color-managed HDR transform are unverified. Real-user storage, GCS orphan lifecycle, malware scanning, parser/subprocess isolation, privacy operations, and signed delivery remain gated. |
| Upload-to-planning authority handoff | `private_executable` for authenticated local/private backend scope | Named-edit creation/save idempotently initializes server-owned Exact Edit Preferences. Before canonical save, the browser strictly reads and synchronizes explicit current-edit values, then rebuilds canonical components with the server baseline/revision. The strict handoff verifies finalized upload lineage and exact source order before deriving source-preparation and frame-confirmation evidence server-side, records that evidence in the exact authority, and returns the exact source/planning binding used by publication. Exact duration-preserving ordered source sequences with one full-duration caption or two through seven ordered, non-overlapping captions can compile into the bounded source-sequence graph; a distinct two-source/two-caption fixture passes canonical execution through final QA/replay. Confirmed keep/preserve source-order planning now derives exact segments from approved source ranges, and short captions report readability risk without extending the timeline. | The normal two-source/rich named-edit scenario now preserves exact source duration and one-to-one segments, but still stops before presentation/approval because it requires transitions, SFX/ducking, color/audio, and unsupported segment-operation work. Real-user Supabase/storage, deployed identity/tenancy, and production operations remain blocked. |
...
| Canonical private job execution | `private_executable` for one immutable job and one exact ordered two-source/two-caption composition | Authenticated job-only route derives the approved work item, output, tool, operation, lease, dispatch, private artifact, QA, reconciliation, and replay from server authority. Before approval, a content-addressed plan-time manifest reconciles planner declarations and work items with stable identities and proof hashes from the server catalog. Exact multi-source trim, one checksum-bound libass artifact per cue, source-bound timed Remotion composition, and dependency-bound final-QA jobs pass through the adapter. | Product, beta, and production flags remain false. The executed proof contains two sources and two captions; the two-through-eight source and two-through-seven caption contracts are not maximum-count runtime evidence. A plan-time identity manifest does not replace package, lease, dispatch, artifact, QA, or deployment evidence. |
...
| Private render and review journey | `private_executable` for bounded internal fixtures including exact ordered two-source/two-caption composition; narrow named-edit browser review and exact bounded revision continuation remain connected | The canonical graph applies approved trims and exact caption frame ranges, renders the private H.264/AAC MP4, independently probes that exact dependency, proves lease/artifact lineage, reconciles and replays safely, and supports authenticated private download. The focused caption-track lifecycle passes through that private download boundary. The Remotion image must carry the aggregate SHA-256 of its current reviewed source tree before activation. Its canonical-private Docker tag, build context, and authority root are isolated from Motion Studio runtime work while image-change checks remain fail-closed. The 16-phase canonical aggregate, 22-phase full internal regression, and all 11 named-edit Chromium scenarios passed on 2026-07-14. | The legacy upload-to-render route remains disabled. Rich plan semantics, structural preference-changing revisions, deployed real-user storage/workers, public delivery, and production promotion are not connected. |
...
`npm run qa:canonical-private-pipeline` is the authoritative bounded canonical audit command. It verifies exact source, Edit Preferences, Edit Brief, 4K estimate/export policy, browser-safe publication/approval/package/preparation/review/recovery clients, real local backend authority, the bounded multi-source execution lifecycle, active named-edit Playwright states, and the versioned proven-tool identity report. Its 2026-07-14 verification completed all 16 phases with exit code 0, including 11/11 Chromium scenarios and the exact 50-identity catalog. `npm run qa:internal-pipeline` runs that canonical sequence plus auth/runtime, project/edit tenancy, approved tool-manifest, private Playwright-capture, and a signed-in two-source upload/rich-plan fail-closed regression; it completed all 22 phases with exit code 0 on the same date. Both commands stop on the first failure and emit a machine-readable step report; a zero exit code means every included assertion passed, including assertions that unavailable or production-only behavior stays blocked.

The historical private-upload smoke remains a fail-closed regression for the retired caller-authored execution route; it is no longer the aggregate pipeline verdict. The authenticated local/private canonical path preserves unique incremental source identity and finalized media identity, synchronizes current Exact Edit Preferences into server authority, derives verified source/frame evidence, and can present and approve an exactly representable source-and-caption plan through frontend-safe coordinators. The named-edit browser can request or recover one exact private execution handoff from the approved snapshot without starting execution, then explicitly request backend-owned private preparation from that exact package. The preparation coordinator reloads immutable authority, derives the dependency-ready work graph server-side, and returns only bounded progress/blocker counts or private-review readiness; it does not accept or expose jobs, tools, commands, paths, credentials, providers, or prices. The browser can now integrity-check the exact private review bytes, record acceptance or a structured revision, refresh into the persisted decision state, reopen immutable history, and submit an exactly representable replacement candidate. The revision coordinator reloads and injects the exact decision authority server-side, presents only the next plan version with a fresh estimate, and requires a separate fresh approval without starting execution. The exact compiler supports bounded duration-preserving source sequences, and a dedicated two-source fixture completes snapshot, synthetic reservation, lease, one-use dispatch, private composition, final QA, reconciliation, replay, downstream verification, and private download. The normal two-source/rich named-edit scenario remains visibly blocked rather than silently simplifying the plan. Rich/structural revision compilation and deployed real-user infrastructure remain required.
...
| Authenticated private upload-to-review pipeline | narrow exact plan presentation/approval/handoff/preparation/playback/decision/history/revision-plan continuation connected; exact ordered multi-source backend lifecycle proven; rich compilation remains blocked | The authenticated backend persists a checksum-protected tenant-scoped planning handoff and content-addressed publication candidate, then presents one server-owned canonical plan only when the plan is exactly representable by the bounded source-and-caption graph. Exact one-source and duration-preserving sequence candidates can be compiled; a two-source backend fixture passes the complete private canonical lifecycle. The named-edit browser binds the visible plan version/hash and estimate identity/hash/maximum to a frontend-safe approval coordinator, requests a bounded private execution-handoff receipt, explicitly requests backend-owned preparation, integrity-checks the exact private MP4, records acceptance or structured revision, reopens immutable history, and carries one exact revision into plan v2 plus fresh estimate/approval. Approval and handoff creation start no execution; raw package/jobs/tools/artifact/revision authority remain backend-only. | Normal rich multi-source work-item compilation, structural preference-changing revisions, deployed real-user operations, external beta, public delivery, and production remain blocked. |
...
> ReeditPro has deterministic planning and canonical approval authority plus 50 exact private tool lifecycles. Its bounded compiler can publish one exact source or a duration-preserving ordered source sequence with one full-duration caption or two through seven ordered, non-overlapping captions; a two-source/two-caption fixture passes the complete canonical private lifecycle through final QA, reconciliation, replay, and private download. A named-edit browser can request a bounded private handoff without starting execution, ask the backend to advance only that exact package through private preparation and review assembly, integrity-check the private MP4, record acceptance or structured revision, reopen immutable history, and present one exactly representable replacement plan with a fresh estimate and approval. Confirmed preserve-source-order planning no longer fabricates duration or extends captions past the timeline. The normal rich multi-source editor plan and rich/structural revisions remain approval-blocked because transitions, audio/SFX/ducking, color, unsupported segment operations, or changed locked preferences are not silently discarded. Deployed real-user infrastructure, external beta, public delivery, live billing, Supabase/RLS tenancy, distributed workers, and production promotion remain gated.
...
5. **Completed for canonical bounded private review and exact ordered two-source/two-caption composition:** authenticated package orchestration advances dependency-ready jobs, persists exact blockers, reuses completed job evidence, and records content-addressed monotonic package-progress checkpoints behind an atomic latest pointer. The focused six-job caption-track graph reaches snapshot validation, approved trims, two caption artifacts, final composition, downstream final-artifact QA, reconciliation, replay, and authenticated private download while preserving exact source order/ranges and audio. The prior five-job single-caption assembly/revision baseline remains evidence, but its entire aggregate branch has not been rerun after this caption-track change.
...
15. **Backend bridge and recovery complete through exact bounded revision-plan continuation; named-edit planning, bounded sequence presentation eligibility, exact approval, approved-snapshot handoff, private preparation, playback, decision, history, and plan-v2 fresh approval are connected:** bind the browser journey to exact preference synchronization, authenticated planning handoff, content-addressed publication candidate, server-owned plan-presentation coordinator, exact approval coordinator, bounded execution-package request coordinator, bounded private-preparation coordinator, exact private-review media/decision coordinators, revision-plan presentation coordinator, and journey recovery without reviving caller-authored authority. The browser saves rich-plan components but requests presentation only for the proven source-and-caption graph, which includes exact duration-preserving ordered sequences; the normal two-source/rich scenario is still visibly gated at `publication_request_required` and creates no snapshot or reservation. For an exactly representable plan, approval requires the exact presented project/edit/version/plan hash/estimate identity/hash/visible maximum, derives revision and idempotency authority server-side, and atomically creates or replays one immutable snapshot, synthetic private-test reservation, and ready/blocked jobs. Approval starts no package or execution. Separate authenticated actions request the package, advance only its server-derived dependency-ready graph, assemble the review, integrity-check exact no-store MP4 bytes, persist acceptance or a structured revision, and reopen hash-bound history. For an exact revision, the browser sends no raw revision authority; the backend reloads the saved decision, preserves prior snapshot/review and locked preferences, presents the next plan version with a fresh estimate, and still requires a separate approval. None returns raw jobs, tools, artifact identity, commands, paths, credentials, providers, prices, billing, wallet, public-delivery, deployment, or production authority. Rich/structural revision compilation remains a separate milestone.
