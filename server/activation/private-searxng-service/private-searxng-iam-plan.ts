...
export function buildPrivateSearxngIamPlan(activeAccount = 'aiediting@reeditpro.com'): PrivateSearxngIamPlanEntry[] {
  const runInvokerMember = activeAccount.endsWith('.gserviceaccount.com')
  ...
  return [
    {
      ...
      member: runInvokerMember,
      description: 'Minimal authenticated Cloud Run invoker grant for the approved active Phase 49F principal. Never grant allUsers.',
      commandString: `gcloud run services add-iam-policy-binding ${privateSearxngServiceConfig.serviceName} --project ${privateSearxngServiceConfig.projectId} --region ${privateSearxngServiceConfig.region} --member="${runInvokerMember}" --role="roles/run.invoker"`,
      reportOnly: true,
    },
