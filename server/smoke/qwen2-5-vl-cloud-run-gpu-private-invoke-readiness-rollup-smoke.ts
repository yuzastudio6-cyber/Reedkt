...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 81)
assert.equal(
...
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeExecutionPreflightRequired, true)
