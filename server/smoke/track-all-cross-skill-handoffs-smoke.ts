...
const graph = createTrackGraphV2({
  ...
  cameraMotionRef: ref('camera_motion_graph_v1', hashSkillValue(cameraMotion)),
  targets: [
  ...
  tracks: [
    ...
    track('surface_001', 'surface-target', 'phone_screen', surfaceBoxes, masks[2]!, 1, {
      planarGeometryRef: ref('planar_track_graph_v1', hashSkillValue(planar)),
    }),
  ],
...
function track(
  ...
  return {
    ...
    maskSequenceRef: ref('track_mask_sequence_v1', hashSkillValue(maskSequenceValue)),
    anchorGraphRef: ref('track_anchor_graph_v1', hashSkillValue(anchorGraph)),
