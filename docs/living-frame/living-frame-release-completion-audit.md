...
| Narrative and animation-aware illustration | `verified_private_runtime` for a real modern illustration source, alpha preparation, component decomposition, and one timed 2.5D composite; `verified_source_contract` for canonical controlled generation | mini-skill identities, component asset intent, synthesis routing, work admission, controlled-illustration qualification, selected-scene private conditioning, `server/smoke/fixtures/assets/living-frame-musashi-animation-aware-illustration-v1.png`, `server/smoke/living-frame-animation-aware-illustration-private-alpha-internal-test-smoke.ts`, and `server/smoke/living-frame-animation-aware-illustration-private-composite-internal-test-smoke.ts` | A real animation-aware illustration workflow is exercised using a modern cinematic anime/ink character fixture created for separability, preserved as illustrative rather than archival evidence, normalized to the canonical 1024-square isolated-component frame, passed through actual private alpha processing, decomposed into a stable character base plus pivot-centered hair and trailing-robe layers, and rendered in a real 120-frame 16:9 deep-multiplane scene with differentiated articulated motion, ink-background parallax, foreground slash motion, Focus Handoff/restoration, captions, and narration-protected sound. Exact ComfyUI generation remains gated by the five released model weights and GPU execution. Automatic arbitrary-character decomposition, hidden-area reconstruction, arm/sword articulation, and skeletal deformation are not claimed. |
