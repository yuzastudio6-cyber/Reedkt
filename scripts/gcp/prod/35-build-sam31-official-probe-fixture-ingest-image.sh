...
# Builds the source-bound, control-plane-only official SAM 3.1 probe ingest
# image. It neither fetches the asset nor starts the job.
...
readonly CONFIG='scripts/gcp/prod/cloudbuild-sam31-official-probe-fixture-ingest.yaml'
readonly CONFIRMATION='build-weeditpro-sam31-official-probe-fixture-ingest-v1'
...
[[ "${WEEDITPRO_CONFIRM_SAM31_PROBE_FIXTURE_INGEST_IMAGE_BUILD:-}" \
...
printf '{"operation":"weeditpro_sam31_probe_fixture_ingest_image_build_v1",'
...
printf '"immutableImage":"%s@%s",' \
  ...
  "${digest}"
printf '"assetFetched":false,"mediaProcessed":false,'
