...
After the local accepted evidence snapshot passes, operators must run `npm run beta:readiness:deployed-evidence-input-manifest` before any deployed evidence collector. The manifest binds the operator-supplied deployed source SHA, 14 accepted tool IDs, libass container image, staging-only platform packet, launch owner approvals, and separate idempotency keys into one no-network readiness check.
