...
| Selective deterministic motion | `verified_private_runtime` for the bounded scalar renderer; `verified_source_contract` for role/activation divergence detection and environmental-particle admission | canonical motion compiler, independent sample-digest verification, Remotion sampler, actual pixel-displacement measurement, selected-scene selective-motion reconciliation, selected-scene environmental-particle admission | The current canonical compiler broadcasts a scene-level `rotate` verb to every component. The read-only reconciliation proves that static anchors, stable subjects, and environmental effects receive mechanical rotation even when only explicitly activated mechanical components should rotate. The environmental admission binds the exact selected component, MasterTiming phases, confirmed frame, procedural geometry, existing PixiJS identity, and required particle primitive. Materialization remains blocked because the selected-scene contract has no typed environmental-effect profile, the current PixiJS operation produces one opaque 640×360 static PNG, and the current Remotion Living Frame overlay accepts one static PNG. The canonical owners must add the typed profile, bounded time-sampled PixiJS operation, Remotion adapter, work/cost/asset binding, primitive QA, and private review. |
