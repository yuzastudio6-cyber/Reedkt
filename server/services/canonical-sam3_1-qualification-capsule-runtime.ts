...
export const CANONICAL_SAM3_1_GCS_QUALIFICATION_CAPSULE_READ_PORT_VERSION =
  'canonical-sam3_1-gcs-qualification-capsule-read-port-v1' as const
...
/**
 * Exact-generation, streaming GCS reread for the immutable qualification
 * capsule. The final metadata check runs only after every byte is consumed,
 * so the canonical tar verifier cannot accept a cross-generation read.
 */
export function createCanonicalSam31GcsQualificationCapsuleReadPort(
  input: { readonly storage?: Storage } = {},
): CanonicalSam31PrivateBuildCapsuleReadPort & {
  readonly schemaVersion:
    typeof CANONICAL_SAM3_1_GCS_QUALIFICATION_CAPSULE_READ_PORT_VERSION
} {
  ...
  return Object.freeze({
    ...
    async readExact(untrusted: CanonicalSam31PrivateCapsuleCoordinate) {
      ...
      if (!Number.isSafeInteger(generation) || generation <= 0) {
        throw new Error('SAM 3.1 capsule generation is not safely addressable.')
      }
...
async function* exactBody(input: {
  ...
  for await (const chunk of input.stream) {
    if (!(chunk instanceof Uint8Array) || chunk.byteLength === 0) {
      throw new Error('SAM 3.1 capsule stream returned an invalid chunk.')
    }
    ...
    ) throw new Error('SAM 3.1 capsule stream exceeded its exact bound.')
    yield chunk
  }
  if (observedBytes !== input.coordinate.byteLength) {
    throw new Error('SAM 3.1 capsule stream ended at the wrong length.')
  }
