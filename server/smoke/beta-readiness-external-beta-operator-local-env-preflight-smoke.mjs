...
assert.equal(blank.operatorInputs.pending, 47)
assert.equal(blank.operatorInputs.humanActionablePending, 47)
assert.equal(blank.operatorInputs.autoFillablePending, 0)
assert.equal(blank.ownerApprovalIntake.counts.pending, 29)
assert.equal(blank.deployedEvidenceInputManifest.pendingRequiredInputs, 47)
...
assert.equal(blankProgress.progress.requiredInputs, 62)
assert.equal(blankProgress.progress.presentOrAutofilledInputs, 15)
assert.equal(blankProgress.progress.pendingInputs, 47)
assert.equal(blankProgress.progress.humanActionablePending, 47)
assert.equal(blankProgress.progress.percentComplete, 24)
...
assert.equal(complete.envFile.betaInputKeysLoaded, 47)
...
assert.equal(completeProgress.progress.presentOrAutofilledInputs, 62)
...
assert.equal(bootstrappedPreflight.operatorInputs.pending, 47)
