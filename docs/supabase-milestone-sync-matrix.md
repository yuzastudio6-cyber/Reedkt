...
| `approved_for_staging_validation_when_gates_pass` | Conditional human approval exists; execution gates remain required before staging validation. |
| `conditional_approval_recorded` | User/owner chat authorization is recorded but does not execute staging or approve production. |
...
| 23A | Human approval decision completion | `staging approval packet only` | `local_evidence_recorded` | `approved_for_staging_validation_when_gates_pass` | `blocked` | Conditional user/owner chat authorization recorded. Staging sync is not applied and execution gates remain incomplete. | Prompt 24D evidence review and Prompt 26 only after gates pass. |
...
| 25 | Staging Supabase/RLS dry-run command packet | `docs/status only` | `local_evidence_recorded` | `blocked_missing_evidence` / `conditional_approval_recorded` | `blocked` | Future-only dry-run command packet prepared; no command, SQL, migration, or Supabase environment update applied. | Prompt 24D redacted evidence review before Prompt 26. |
| 25A | GCP Secret Manager reference contract | `docs/status only` | `local_evidence_recorded` | `blocked_missing_evidence` / `conditional_approval_recorded` / `access_not_verified` | `blocked` | Secret Manager Supabase reference names, metadata-only discovery expectations, and placeholder policy prepared; no Google Cloud API, Secret Manager metadata fetch, Secret Manager value fetch, command, SQL, migration, or Supabase environment update applied. | Prompt 24D redacted evidence review before Prompt 26. |
