...
assert.equal(manifest.policy.intentionalBlanketBlocksAllowed, false, 'blockers must not act as intentional blanket freezes')
assert.equal(manifest.policy.safeForwardProgressRequired, true, 'manifest must require safe forward progress lanes')
assert.equal(manifest.policy.nextSafeActionRequiredForBlockers, true, 'manifest must require next safe actions for blockers')
