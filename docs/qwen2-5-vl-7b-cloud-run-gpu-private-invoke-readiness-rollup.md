...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_execution_preflight_passed_execution_attempt_required`.
...
| Controlled persisted worker dispatch runtime execution attempt | blocked controlled persisted worker dispatch runtime execution attempt required | Runtime execution preflight passed, but the approved-fixture execution attempt has not run or been reviewed. | Run the controlled approved-fixture execution attempt while keeping generated assets, public artifacts, signed URLs, beta, and production blocked. |
...
The next action is controlled persisted worker dispatch runtime execution attempt for approved fixture scope only. That future prompt must remain fail-closed and must not create generated assets, public artifacts, signed URLs, beta, production, or claim `generated_local_fixture_passed`.
