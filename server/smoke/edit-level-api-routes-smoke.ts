...
assert.ok(summary.productionReadiness.blockers.length > 0)
...
assert.ok((await callRoute<string>('project.editLevel.toolRouting.summary', { level: 'premium' }))?.data?.includes('planning depth'))
assert.ok((await callRoute<string>('project.editLevel.qwenRouting.summary', { level: 'ultra_premium' }))?.data?.includes('visual understanding'))
