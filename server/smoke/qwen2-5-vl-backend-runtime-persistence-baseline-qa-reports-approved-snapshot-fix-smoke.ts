...
for (const phrase of [
  'alter table if exists public.qa_reports',
...
check(!/insert\s+into\s+public\.qa_reports/i.test(sql), 'Migration must not insert QA reports.')
check(!/insert\s+into\s+public\.approved_plan_snapshots/i.test(sql), 'Migration must not insert approved snapshots.')
check(!/update\s+public\.qa_reports\s+set\s+approved_plan_snapshot_id/i.test(sql), 'Migration must not backfill QA report approved snapshots.')

const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-baseline-qa-reports-approved-snapshot-fix.md')
for (const phrase of [
  DECISION,
  ...
  '`qa_reports.approved_plan_snapshot_id uuid`',
...
assert.equal(
  fix.upstreamBackendRuntimePersistenceLocalHarnessValidationRetry10ResultDecision,
...
assert.equal(fix.repairedMigration.failedStatementCategory, 'qa_reports_approved_plan_snapshot_id_missing')
assert.deepEqual(fix.repairedMigration.compatibilityColumns, ['public.qa_reports.approved_plan_snapshot_id'])
assert.deepEqual(fix.repairedMigration.compatibilityBackfills, [])
assert.deepEqual(fix.repairedMigration.constraintsUnblocked, ['qa_reports_approved_plan_snapshot_id_fkey'])
...
assert.deepEqual(
  ...
  [],
  `Forbidden values in Qwen QA reports fix data: ${forbiddenDataFindings.join('; ')}`,
)
...
console.log(JSON.stringify({
  ...
  constraintsUnblocked: fix.repairedMigration.constraintsUnblocked,
  indexesUnblocked: fix.repairedMigration.indexesUnblocked,
  qaReportsApprovedSnapshotBackfillSkipped:
    fix.runtimeFlags.qaReportsApprovedSnapshotBackfillSkipped,
