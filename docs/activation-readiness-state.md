...
Phase 37B adds the PaddleOCR exact asset selection and guarded private download/upload workflow after Phase 37A. The selected safe-zone v1 assets are `PP-OCRv5_mobile_det_infer.tar`, `PP-OCRv5_mobile_rec_infer.tar`, and `ppocrv5_dict.txt` under the private `paddle3.0.0-mobile-safe-zone-v1` prefix. Default commands remain non-mutating, and the selected assets are not verified in private staging GCS unless the guarded download and upload confirmations are used. OCR runtime, real media OCR, real video OCR, caption/render integration, production, external beta, broad media, arbitrary media, providers, public output, Revideo, FILM, slow motion, and final delivery remain blocked.
...
| OCR exact assets | Phase 37B guarded workflow added | `PP-OCRv5_mobile_det_infer.tar`, `PP-OCRv5_mobile_rec_infer.tar`, and `ppocrv5_dict.txt` are selected for `paddle3.0.0-mobile-safe-zone-v1`. Default plan/report/smoke modes do not download or upload. Phase 37C remains blocked until checksums and private GCS verification exist. |
...
- next activation work: guarded Phase 37B OCR asset download/upload review, then Phase 37C generated OCR runtime verification only after checksums and private GCS evidence pass. Arbitrary media, OCR runtime, real-video OCR, production, beta, RNNoise active routing, Demucs download/runtime, providers, public output, Revideo, FILM, and slow motion remain blocked
