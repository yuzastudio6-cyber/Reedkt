...
This recovery bridge remains read-only and never executes its returned action. Separate reviewed local/private clients now cover canonical planning-handoff submission, server-owned candidate persistence and plan presentation, exact plan/estimate approval, exact approved-snapshot package request, backend-owned private preparation, integrity-verified current-review playback, exact acceptance or structured revision, and immutable history reopening. Every mutation reloads canonical authority on the backend and returns a bounded receipt; the browser never supplies jobs, tools, artifact IDs, paths, credentials, providers, prices, credit actions, or release authority. A recorded revision refreshes the journey to `revision_requested` and explicitly requires a fresh plan, estimate, approval, and review. Direct browser consumption of that revision handoff into the replacement-plan publication/approval cycle remains the next separate slice. Public delivery remains blocked. See `docs/canonical-plan-approval-coordinator.md` and `docs/canonical-private-review-decision.md`.
