...
const baseRef = 'origin/codex/rp-ai-graphics-draft-package-proof-merge-order-qa-review'
const expectedDecision = 'ai_graphics_draft_package_proof_draft_ready_approval_passed_with_warnings'
const allowedDecisions = new Set([
  expectedDecision,
  'ai_graphics_draft_package_proof_draft_ready_approval_passed',
  'blocked_pending_ai_graphics_pr425_draft_ready_approval',
  'blocked_pending_ai_graphics_pr433_draft_ready_approval',
  'blocked_pending_ai_graphics_pr441_draft_ready_approval',
  'blocked_pending_ai_graphics_draft_ready_scope_review',
  'blocked_pending_ai_graphics_source_pr_state_review',
])

const requiredDocs = [
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-source-lockfile.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-matrix.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr425-draft-ready-approval.md',
  ...
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-stack-order.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-boundary-review.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-blocked-use-register.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-decision.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-next-lane-recommendation.md',
  'docs/prompt-ai-graphics-draft-package-proof-draft-ready-approval-results.md',
  'docs/implementation-prompts/prompt-ai-graphics-draft-package-proof-draft-ready-approval.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-source-lockfile.json',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-matrix.json',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-stack-order.json',
]

const expectedSources = ['PR #556', 'PR #554', 'PR #552', 'PR #550', 'PR #548', 'PR #543', 'PR #536', 'PR #416', 'PR #425', 'PR #433', 'PR #441', 'PR #542', 'PR #544']
...
const requiredPrFields = [
  ...
  'tools',
  'stackOrderPosition',
  'draftReadyApprovalStatus',
  'canBeMarkedReadyInFutureExecution',
  'canBeMarkedReadyNow',
  'readyForMergeLater',
...
const requiredTrueBooleans = [
  'draftReadyApprovalCompleted',
  'mergeOrderQaAccepted',
  'allSourcePrsReviewed',
  'pr425DraftReadyApprovedForFutureExecution',
  'stackOrderAccepted',
  'readyForDraftReadyExecution',
]
...
const forbiddenPatterns = [
  ['draft_ready_now', /\b(canBeMarkedReadyNow|draftMarkedReadyNow|draftReadyTransitionNow)\b\s*[:=]\s*`?true`?/i],
...
let stackOrder = {}
try {
  sourceLockfile = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-source-lockfile.json')
  matrix = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-approval-matrix.json')
  stackOrder = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-draft-ready-stack-order.json')
} catch (error) {
...
if (stackOrder.firstDraftReadyTarget !== 425) failures.push(`first_draft_ready_target_unexpected:${stackOrder.firstDraftReadyTarget}`)
...
for (const expectedPr of expectedOrder) {
  ...
  if (row.canBeMarkedReadyNow !== false) failures.push(`source_pr_can_be_marked_ready_now_not_false:${expectedPr}`)
  if (row.readyForMergeLater !== false) failures.push(`source_pr_merge_later_not_false:${expectedPr}`)
  ...
  if (row.stackOrderPosition !== expectedOrder.indexOf(expectedPr) + 1) failures.push(`source_pr_stack_position_mismatch:${expectedPr}`)
  ...
  if (expectedPr === 425) {
    if (row.canBeMarkedReadyInFutureExecution !== true) failures.push('pr425_future_mark_ready_not_true')
    if (row.draftReadyApprovalStatus !== 'approved_with_warnings_for_future_execution') failures.push('pr425_status_unexpected')
    if (row.blockedReason !== '') failures.push('pr425_blocker_not_empty')
  } else {
    if (row.canBeMarkedReadyInFutureExecution !== false) failures.push(`source_pr_future_mark_ready_not_false:${expectedPr}`)
    if (!row.draftReadyApprovalStatus.startsWith('deferred_pending_')) failures.push(`source_pr_status_not_deferred:${expectedPr}`)
    if (!row.blockedReason.startsWith('deferred_until_')) failures.push(`source_pr_blocker_not_deferred:${expectedPr}`)
  }
...
for (const source of ['pr556', 'pr554', 'pr552', 'pr550', 'pr548', 'pr543', 'pr536', 'pr425', 'pr433', 'pr441']) {
...
try {
  ...
  const expectedScript = 'node scripts/validation/ai-graphics-draft-package-proof-draft-ready-approval-diagnostics.mjs'
  if (packageAfter.scripts?.['ai-graphics:draft-package-proof-draft-ready-approval:diagnostics'] !== expectedScript) failures.push('missing_package_script')
} catch (error) {
...
if (failures.length) {
  console.error('AI graphics draft package proof draft-ready approval diagnostics failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('AI graphics draft package proof draft-ready approval diagnostics passed.')
