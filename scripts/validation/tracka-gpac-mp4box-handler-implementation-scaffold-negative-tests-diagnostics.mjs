...
const lane = 'TRACKA-GPAC-MP4BOX-HANDLER-IMPLEMENTATION-SCAFFOLD-NEGATIVE-TESTS-1'
const packetDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-handler-implementation-scaffold-negative-tests'
const priorDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-disabled-handler-implementation-scaffold'
const decisionText = 'tracka_gpac_mp4box_handler_implementation_scaffold_negative_tests_passed_ready_for_guarded_handler_implementation_review'
const executionText = 'completed_handler_implementation_scaffold_negative_tests_no_route_or_worker_execution'
const priorDecision = 'tracka_gpac_mp4box_disabled_handler_implementation_scaffold_passed_ready_for_handler_implementation_scaffold_negative_tests'
const priorMergeSha = '56953419511f582d6fd318e81322649952e34213'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-GUARDED-HANDLER-IMPLEMENTATION-REVIEW-2'

const packetFiles = [
  `${packetDir}/gpac-mp4box-handler-implementation-scaffold-negative-tests-decision.json`,
  `${packetDir}/gpac-mp4box-handler-implementation-scaffold-negative-tests-decision.md`,
...
const priorFiles = [
  `${priorDir}/gpac-mp4box-disabled-handler-implementation-scaffold-decision.json`,
...
if (packageJson.scripts?.['tracka:gpac-mp4box-handler-implementation-scaffold-negative-tests:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-handler-implementation-scaffold-negative-tests-diagnostics.mjs') fail('missing diagnostics script')
if (packageJson.scripts?.['smoke:tracka-gpac-mp4box-handler-implementation-scaffold-negative-tests'] !== 'tsx server/smoke/tracka-gpac-mp4box-handler-implementation-scaffold-negative-tests-smoke.ts') fail('missing smoke script')

const decision = json(`${packetDir}/gpac-mp4box-handler-implementation-scaffold-negative-tests-decision.json`)
...
if (decision.prior?.disabledHandlerImplementationScaffold !== priorDecision) fail('prior decision drift')
if (decision.prior?.disabledHandlerImplementationScaffoldMergeSha !== priorMergeSha) fail('prior merge drift')
...
const smokeText = read('server/smoke/tracka-gpac-mp4box-handler-implementation-scaffold-negative-tests-smoke.ts')
for (const text of ['all_rejected_input_cases_block', 'all_runtime_attempt_cases_block', 'all_delivery_attempt_cases_block', 'guarded_handler_implementation_plan_drift_blocks', 'operator_confirmation_drift_blocks']) {
...
for (const file of changedFiles) {
  ...
  if (/^server\//.test(file) && file !== 'server/smoke/tracka-gpac-mp4box-handler-implementation-scaffold-negative-tests-smoke.ts') fail(`unexpected server path ${file}`)
}
...
console.log('Handler-implementation scaffold negative tests passed')
