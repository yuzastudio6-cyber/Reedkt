...
const runScriptCommand =
  'tsx server/cli/ai-graphics-external-beta-api-route-mount-implementation-review.ts'
const diagnosticScriptName =
  'ai-graphics:external-beta-api-route-mount-implementation-review:diagnostics'
const diagnosticScriptCommand =
  'node scripts/validation/ai-graphics-external-beta-api-route-mount-implementation-review-diagnostics.mjs'

const requiredFiles = [
  'server/routes/ai-graphics-external-beta-tool-call-routes.ts',
  'server/tool-registry/ai-graphics-external-beta-api-route-mount-implementation-review.ts',
  'server/cli/ai-graphics-external-beta-api-route-mount-implementation-review.ts',
  'scripts/validation/ai-graphics-external-beta-api-route-mount-implementation-review-diagnostics.mjs',
  'docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-implementation-review.json',
  'docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-implementation-review.md',
...
const trueKeys = [
  'externalBetaApiRouteMountImplementationReviewPrepared',
  'sourceRouteMountReadinessAccepted',
  'sourceRouteBoundServiceRoleQueueSmokeOperatorPreflightAcceptedWithProvidedEvidence',
  'sourceControlledRouteImplementationReviewed',
  'appMountStillDeferred',
  'apiRouteMountImplementationReadyWithProvidedEvidence',
  'all21ToolsCovered',
...
function verifySourceReadiness() {
  const source = json('docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-readiness.json')
  requireEqual(source.decision, sourceDecision, 'source_readiness_decision')
  requireEqual(source.status, sourceStatus, 'source_readiness_status')
  ...
  requireTruthy(source.booleans?.apiRouteMountReadyWithProvidedEvidence, 'source_route_mount_ready_boolean')
  requireTruthy(
...
function verifyDoc(doc, markdown) {
  ...
  requireEqual(
    doc.routeImplementationCandidate?.routeFile,
    'server/routes/ai-graphics-external-beta-tool-call-routes.ts',
    'doc_route_file',
  )
  requireEqual(
    doc.routeImplementationCandidate?.routePath,
    '/api/ai-graphics/external-beta/tool-call',
    'doc_route_path',
  )
  requireFalse(doc.routeImplementationCandidate?.mountedInAppNow, 'doc_mounted_false')
  requireTruthy(doc.routeImplementationCandidate?.disabledRuntimeHandlerOnly, 'doc_disabled_handler')
  requireTruthy(doc.implementationPolicy?.appMountDeferred, 'doc_app_mount_deferred')
  requireTruthy(doc.implementationPolicy?.runtimeHandlerDisabled, 'doc_runtime_handler_disabled')
  requireTruthy(doc.implementationPolicy?.queueWriteDeferred, 'doc_queue_write_deferred')
  requireTruthy(doc.implementationPolicy?.toolExecutionDeferred, 'doc_tool_deferred')
  requireTruthy(doc.implementationPolicy?.onDemandGpuOnly, 'doc_on_demand_gpu')
  for (const action of [
    'app route mount',
    'API route execution',
    'live queue write',
...
function verifySourceFiles() {
  ...
  const source = read('server/tool-registry/ai-graphics-external-beta-api-route-mount-implementation-review.ts')
  const cli = read('server/cli/ai-graphics-external-beta-api-route-mount-implementation-review.ts')
  ...
  requireIncludes(route, 'evaluateAiGraphicsToolCallPlan', 'route_plan_evaluator_binding')
  requireIncludes(route, 'buildAiGraphicsExternalBetaToolCallBlockedDetails', 'route_blocked_detail_builder')
  ...
  requireIncludes(route, 'requestAcceptedForPlanningMetadata', 'route_detail_planning_acceptance')
  requireIncludes(route, 'planEvaluationDecision', 'route_detail_plan_decision')
  requireIncludes(route, 'requestedToolsAcceptedForPlanning', 'route_detail_accepted_tools')
  requireIncludes(route, 'requestedToolsEliminated', 'route_detail_eliminated_tools')
  requireIncludes(route, 'selectedPlanningTools', 'route_detail_selected_planning_tools')
  requireIncludes(route, 'missingProofBeforeExecution', 'route_detail_missing_proof')
  requireIncludes(route, 'missingExecutionGates', 'route_detail_missing_execution_gates')
  ...
  requireIncludes(cli, '--route-mount-readiness-packet', 'cli_source_packet_flag')
  requireIncludes(index, "export * from './ai-graphics-external-beta-api-route-mount-implementation-review'", 'index_export')
  requireIncludes(scorecard, decision, 'scorecard_decision')
  requireIncludes(scorecard, 'apiRouteMountImplementationReadyToolsWithProvidedEvidence=21', 'scorecard_impl_count')
  requireIncludes(
    markdown,
...
function verifyRuntimeReport(report) {
  ...
  requireFalse(report.routeImplementationCandidate?.mountedInAppNow, 'runtime_mounted_false')
  requireTruthy(report.routeImplementationCandidate?.disabledRuntimeHandlerOnly, 'runtime_disabled_handler')
...
function verifyStaleRouteMountOperatorPreflightRejected() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ai-graphics-route-impl-stale-mount-'))
  const stalePacketPath = path.join(tempDir, 'stale-route-mount-readiness.json')
  try {
    const source = json('docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-readiness.json')
    ...
    const report = parseJson(runNpm(runScriptName, [
      ...
      stalePacketPath,
    ]), 'stale_route_mount_implementation_review_cli')

    requireEqual(report.status, 'route_mount_readiness_rejected', 'stale_status')
    requireFalse(report.sourceRouteMountReadinessAccepted, 'stale_source_mount_readiness_rejected')
    requireFalse(
    ...
    requireEqual(
      report.apiRouteMountImplementationReadyToolsWithProvidedEvidence,
      0,
      'stale_impl_ready_zero',
    )
    requireFalse(
      report.booleans?.apiRouteMountImplementationReadyWithProvidedEvidence,
      'stale_impl_ready_false',
    )
...
const doc = json('docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-implementation-review.json')
const markdown = read('docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-implementation-review.md')
...
const runtimeReport = parseJson(runNpm(runScriptName, [
  '--route-mount-readiness-packet',
  'docs/tool-intelligence/ai-graphics/external-beta-api-route-mount-readiness.json',
]), 'route_mount_implementation_review_cli')
verifyRuntimeReport(runtimeReport)
verifyStaleRouteMountOperatorPreflightRejected()
