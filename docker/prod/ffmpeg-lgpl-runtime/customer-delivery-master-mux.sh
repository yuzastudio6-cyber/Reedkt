...
TAB=$(printf '\t')
MAGIC=REEDITPRO_FFMPEG_CUSTOMER_DELIVERY_MUX_V1
MAX_CHUNKS=124
MAX_BLOCK_BYTES=65536
ACTIVE_WORK=/tmp/reeditpro-customer-delivery-mux-active

fail() {
  printf '%s\n' "customer-delivery mux failed: $*" >&2
  exit 1
}
...
write_program_audio() {
  ...
  until [ -p "$work/audio.out" ] \
    ...
    [ ! -f "$work/audio.failed" ] || fail 'mux already failed'
    attempt=$((attempt + 1))
  ...
  count_pid=$!
  tee "$work/audio.out" "$work/audio.count" \
    | sha256sum > "$work/audio.observed-sha256"
...
if [ "$#" -eq 1 ] \
  && [ "$1" = 'customer-delivery-master-audio-writer-v1' ]; then
  write_program_audio
  exit 0
fi

[ "$#" -eq 1 ] && [ "$1" = 'customer-delivery-master-mux-v1' ] \
...
IFS="$TAB" read -r kind width height fps total_frames chunk_count \
  audio_bytes audio_sha extra || fail 'missing timeline authority'
[ "$kind" = timeline ] && [ -z "${extra:-}" ] \
  || fail 'invalid timeline authority'
for value in "$width" "$height" "$fps" "$total_frames" \
  "$chunk_count" "$audio_bytes"; do
...
[ "$fps" -eq 30 ] && [ "$total_frames" -ge 2700 ] \
...
while [ "$index" -le "$chunk_count" ]; do
  ...
  [ "$parsed_index" -eq "$index" ] \
    && [ "$start" -eq "$expected_start" ] \
    && [ "$end" -eq $((start + duration)) ] \
    && [ "$duration" -ge 1350 ] && [ "$duration" -le 5400 ] \
    && [ "$bytes" -ge 1024 ] && [ "$bytes" -le 3221225472 ] \
    || fail 'chunk authority is not exact and contiguous'
  printf '%s\n' "$bytes" \
    > "$(printf '%s/video-%03d.bytes' "$work" "$index")"
  printf '%s\n' "$sha" \
    > "$(printf '%s/video-%03d.expected' "$work" "$index")"
  output_fifo=$(printf '%s/video-%03d.out' "$work" "$index")
  mkfifo "$output_fifo"
  chunk_duration_seconds=$(awk -v frames="$duration" -v rate="$fps" \
    'BEGIN { printf "%.9f", frames / rate }')
  printf "file '%s'\nduration %s\n" \
    "$output_fifo" "$chunk_duration_seconds" >> "$work/concat.txt"
  expected_start=$end
  index=$((index + 1))
done
[ "$expected_start" -eq "$total_frames" ] \
  || fail 'chunks do not cover the timeline'
...
mkfifo "$audio_out"
duration_seconds=$(awk -v frames="$total_frames" -v rate="$fps" \
  'BEGIN { printf "%.9f", frames / rate }')

# A fragmented MP4 with an empty front-loaded movie box is required here
# because the bounded runtime streams stdout directly into create-only private
# persistence. It provides immediate-playback metadata before media data while
# avoiding any full-program video re-encode or whole-output tmpfs spool.
$FFMPEG -hide_banner -loglevel error -nostdin \
  -f concat -safe 0 -i "$work/concat.txt" -i "$audio_out" \
  -map 0:v:0 -map 1:a:0 -c:v copy \
  -c:a aac -b:a 192k -ar 48000 -ac 2 \
  -af "atrim=duration=${duration_seconds},asetpts=PTS-STARTPTS,aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo" \
  -threads:a 1 -sn -dn -t "$duration_seconds" \
  -avoid_negative_ts make_zero -fflags +bitexact -flags:a +bitexact \
  ...
  case "$kind:$target" in
    block:video)
      ...
      if [ "$current_video" -eq 0 ]; then
        ...
        mkfifo "$video_in"
        (tee "$video_out" < "$video_in" \
          | sha256sum > "$(printf '%s/video-%03d.sha' "$work" "$current_video")") &
        video_pid=$!
        exec 3>"$video_in"
      fi
      [ "$parsed_index" -eq "$current_video" ] \
        || fail 'video block changed chunk'
      dd bs="$length" count=1 iflag=fullblock status=none > "$work/block"
      [ "$(stat -c '%s' "$work/block")" -eq "$length" ] \
        || fail 'short video block'
      ...
      ;;
    close:video)
      valid_uint "$parsed_index" \
        && [ "$parsed_index" -eq "$current_video" ] \
        && [ "$length" = 0 ] || fail 'invalid video close'
      expected=$(cat "$(printf '%s/video-%03d.bytes' "$work" "$current_video")")
      [ "$video_observed" -eq "$expected" ] \
        || fail 'video byte length changed'
      exec 3>&-
      wait "$video_pid" || fail 'video checksum stream failed'
      actual=$(sed -n 's/  -$//p' \
        "$(printf '%s/video-%03d.sha' "$work" "$current_video")")
      expected_sha=$(cat \
        "$(printf '%s/video-%03d.expected' "$work" "$current_video")")
      ...
      [ "$parsed_index" = 0 ] && [ "$length" = 0 ] \
        || fail 'invalid protocol end'
      break
      ;;
...
IFS="$TAB" read -r observed_audio_bytes observed_audio_sha extra \
  < "$work/audio.done" || fail 'program audio proof is missing'
[ -z "${extra:-}" ] \
  && [ "$observed_audio_bytes" = "$audio_bytes" ] \
  && [ "$observed_audio_sha" = "$audio_sha" ] \
  || fail 'program audio proof changed'
wait "$ffmpeg_pid" || fail 'H.264 stream-copy/AAC customer mux failed'
ffmpeg_pid=
