...
| Professional long-form customer-delivery review | `private_executable` for one bounded two-chunk synthetic 4K package and its browser transport | The separate immutable nine-job delivery graph completes H.264 transcodes, independent probes, one stream-copy/AAC mux, full decoded-video and decoded-audio/sync QA, explicit authenticated acceptance, private-download reconciliation, restart-safe replay, and attempt-level internal cost while reusing the original approved 4K estimate/reservation. Actual Express routes deny missing or cross-tenant identity and expose only a strict browser-safe receipt. The frontend-safe client reads bounded authenticated pre-decision ranges, records accept or revision with deterministic idempotency, and reads accepted private-download ranges while verifying `206`, exact range/length, no-store, MP4, artifact hash, and review/decision authority headers. | The smoke deletes the private fixture, so there is no retained review URL or user-viewable video. Named-edit discovery, UI/player/MediaSource consumption, durable whole-program watch evidence, representative long footage, distributed storage/workers, deployed Google sign-in, GCS/Supabase, public delivery, customer settlement, and production remain blocked. Export never creates a second estimate, charge, or credit prompt. |
...
second estimate or charge. Six of 255 jobs are complete and the remaining 249
children are capability-blocked. They include the other 123 renders and QA
pairs, cross-chunk color, finalization, and final QA. The exercised inputs are
