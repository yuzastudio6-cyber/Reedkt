...
15. **Backend bridge and recovery complete; frontend consumption remains:** bind the browser journey to the authenticated canonical planning handoff, content-addressed publication-request candidate, and canonical journey recovery route without reviving legacy caller-authored authority. Submit/inspect, latest recovery, internal publish-from-candidate, plan-to-snapshot recovery, and exact-next-action state now pass local/private canonical evidence; no frontend call site is added by this backend-only task.
