...
for (const required of [
  ...
  '`broll_no_idle_l4_iap_wheelhouse_transfer_prompt_required_before_dependency_or_inference`',
...
assert.equal(rollup.recommendedNextPrompt, BROLL_9O_NEXT_PROMPT)
...
assert.equal(
  ...
  'bounded_no_idle_l4_iap_wheelhouse_transfer_prompt_required_before_dependency_or_inference',
)
assert.equal(
...
assert.equal(broll?.nextAction, BROLL_9O_NEXT_PROMPT)
