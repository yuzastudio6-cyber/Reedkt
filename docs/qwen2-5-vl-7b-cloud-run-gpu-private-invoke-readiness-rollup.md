...
- backend runtime persistence generation requests approved snapshot baseline fix: blocked, compatibility column required
...
| Backend runtime persistence generated asset versions version baseline fix | blocked backend runtime persistence generated asset versions version baseline fix required | Retry 8 isolated the next active baseline blocker to `generated_asset_versions.version` before `idx_generated_asset_versions_asset_version`; the older active baseline table exposes `version_number`. | Guard `generated_asset_versions.version` before `idx_generated_asset_versions_asset_version` without inventing generated asset versions, storage objects, jobs, worker rows, provider outputs, credit records, or Qwen runtime records. |
