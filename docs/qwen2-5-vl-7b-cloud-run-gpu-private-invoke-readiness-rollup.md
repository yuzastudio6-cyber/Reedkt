...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_real_dispatch_approval_plan_recorded_decision_required`.
...
- controlled persisted worker dispatch runtime real-dispatch approval decision: blocked, approval decision required
...
| Controlled persisted worker dispatch runtime real-dispatch approval decision | blocked controlled persisted worker dispatch runtime real-dispatch approval decision required | The real-dispatch approval plan is recorded, but no explicit approval decision exists for real persisted Qwen worker dispatch. | Record a real-dispatch approval decision before any real persisted Qwen worker dispatch attempt can run. |
...
The next action is the controlled persisted worker dispatch runtime real-dispatch approval decision. That future prompt must keep generated assets, public artifacts, signed URLs, beta, production, and `generated_local_fixture_passed` blocked now.
