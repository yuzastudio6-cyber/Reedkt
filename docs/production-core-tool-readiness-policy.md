...
When the local synthetic QA proof passes, `npm run beta:tools:libass-synthetic-burnin-qa-evidence-preflight` and `npm run beta:tools:libass-synthetic-burnin-qa-evidence` are the staging handoff. They record only the accepted `libass` evidence packet through the authenticated/idempotent beta-readiness evidence route and fail closed if readback does not include `libass`.
