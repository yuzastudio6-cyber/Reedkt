...
interface CanonicalRouteControlledResult {
  toolId: string
  capabilityId: string
  statusCode: number
  ok: boolean
...
function buildRuntimeEnv(browserRuntimeExecutionEnabled: boolean) {
  return loadRuntimeEnv({
    ...
    [AI_GRAPHICS_EXTERNAL_BETA_TOOL_CALL_ROUTE_CPU_STATIC_CONTROLLED_EXECUTION_FLAG]:
      'true',
    [AI_GRAPHICS_EXTERNAL_BETA_TOOL_CALL_ROUTE_BROWSER_RUNTIME_CONTROLLED_EXECUTION_FLAG]:
      browserRuntimeExecutionEnabled ? 'true' : 'false',
...
async function withServer<T>(
  browserRuntimeExecutionEnabled: boolean,
  callback: (baseUrl: string) => Promise<T>,
) {
  const app = createReeditProApiApp(buildRuntimeEnv(browserRuntimeExecutionEnabled))
...
async function runControlledCase(
  baseUrl: string,
  request: AiGraphicsExternalBetaToolCallRequest,
): Promise<CanonicalRouteControlledResult> {
...
function makeMarkdown(report: ReturnType<typeof buildReport>): string {
  const rows = [
    ...report.cpuStaticResults.map((item) => ({ ...item, group: 'cpu_static' })),
    ...report.browserRuntimeResults.map((item) => ({ ...item, group: 'browser_runtime' })),
  ]
    .map((item) => (
      `| \`${item.toolId}\` | \`${item.group}\` | \`${item.capabilityId}\` | \`${item.statusCode}\` | \`${item.controlledAdapterExecutedNow}\` | \`${item.outputKind}\` | \`${item.outputSha256}\` |`
    ))
    .join('\n')

  return `# AI Graphics External Beta Canonical Tool-Call Browser Runtime Controlled Execution Smoke

Decision: \`${report.decision}\`

Status: \`${report.status}\`

This smoke proves the canonical external-beta tool-call route can execute the seven proven browser/runtime tools when \`${report.browserRuntimeRouteFlag}\` is explicitly enabled. The same run keeps the six proven CPU/static tools executable through their canonical flag, bringing controlled canonical route execution to 13 of 21 tools. The eight GPU/model tools remain blocked and GPU runtime stays off.
...
function buildReport(
  ...
  cpuStaticResults: CanonicalRouteControlledResult[],
  browserRuntimeResults: CanonicalRouteControlledResult[],
  blockedResults: CanonicalRouteBlockedResult[],
) {
  return {
    schemaVersion:
      '2026-07-02.ai-graphics.external-beta-tool-call-route-browser-runtime-controlled-execution-smoke',
    decision,
    status,
    routePath: AI_GRAPHICS_EXTERNAL_BETA_TOOL_CALL_ROUTE_PATH,
    cpuStaticRouteFlag:
      AI_GRAPHICS_EXTERNAL_BETA_TOOL_CALL_ROUTE_CPU_STATIC_CONTROLLED_EXECUTION_FLAG,
    browserRuntimeRouteFlag:
      AI_GRAPHICS_EXTERNAL_BETA_TOOL_CALL_ROUTE_BROWSER_RUNTIME_CONTROLLED_EXECUTION_FLAG,
    disabledBrowserRuntimeCanonicalRouteStatus: disabledStatus,
    cpuStaticResults,
    browserRuntimeResults,
    blockedResults,
    counts: {
      totalAiGraphicsTools: 21,
      controlledCanonicalRouteExecutedTools:
        cpuStaticResults.length + browserRuntimeResults.length,
      cpuStaticControlledCanonicalRouteExecutedTools: cpuStaticResults.length,
      browserRuntimeControlledCanonicalRouteExecutedTools: browserRuntimeResults.length,
      localPackageExecutionPerformedTools:
        cpuStaticResults.filter((item) => item.localPackageExecutionPerformed).length +
        browserRuntimeResults.filter((item) => item.localPackageExecutionPerformed).length,
      controlledAdapterExecutedTools:
        cpuStaticResults.filter((item) => item.controlledAdapterExecutedNow).length +
        browserRuntimeResults.filter((item) => item.controlledAdapterExecutedNow).length,
      gpuModelBlockedTools: blockedResults.length,
      all21ExecutableNowTools: 0,
      workerDispatchPerformedTools:
        cpuStaticResults.filter((item) => item.workerDispatchPerformed).length +
        browserRuntimeResults.filter((item) => item.workerDispatchPerformed).length,
      gpuRuntimeShouldStartNowTools:
        cpuStaticResults.filter((item) => item.gpuRuntimeShouldStartNow).length +
        browserRuntimeResults.filter((item) => item.gpuRuntimeShouldStartNow).length,
      publicArtifactCreatedTools:
        cpuStaticResults.filter((item) => item.publicArtifactCreated).length +
        browserRuntimeResults.filter((item) => item.publicArtifactCreated).length,
      signedUrlCreatedTools:
        cpuStaticResults.filter((item) => item.signedUrlCreated).length +
        browserRuntimeResults.filter((item) => item.signedUrlCreated).length,
    },
    booleans: {
      ...
      thirteenControlledToolsExecutableViaCanonicalRouteNow:
        cpuStaticResults.length === 6 && browserRuntimeResults.length === 7,
      thirteenControlledToolsExecutedViaCanonicalRouteNow: [
        ...cpuStaticResults,
        ...browserRuntimeResults,
      ].every((item) => item.controlledAdapterExecutedNow),
      sevenBrowserRuntimeToolsExecutableViaCanonicalRouteNow:
        browserRuntimeResults.length === 7,
      sevenBrowserRuntimeToolsExecutedViaCanonicalRouteNow:
        browserRuntimeResults.every((item) => item.controlledAdapterExecutedNow),
      sixCpuStaticToolsStillExecutableViaCanonicalRouteNow:
        cpuStaticResults.length === 6,
      localControlledPackageExecutionPerformed: true,
      privateOutputMetadataReturned: [...cpuStaticResults, ...browserRuntimeResults]
        .every((item) => item.outputSha256),
      gpuModelToolsRemainBlocked: blockedResults.length === 8,
...
async function main() {
  ...
  const cpuStaticCases = cases
    ...
    .map((item) => ({ request: withoutBlockedDetailsPayload(item.request, 'canonical-cpu-static-execution') }))
  const browserRuntimeCases = cases
    .filter((item) => browserRuntimeToolIds.has(item.request.toolId))
    .map((item) => ({ request: withoutBlockedDetailsPayload(item.request, 'canonical-browser-runtime-execution') }))
  const blockedCases = cases.filter(
    (item) => !cpuStaticToolIds.has(item.request.toolId) && !browserRuntimeToolIds.has(item.request.toolId),
  )

  assert(cpuStaticCases.length === 6, `Expected six CPU/static cases, got ${cpuStaticCases.length}`)
  assert(browserRuntimeCases.length === 7, `Expected seven browser/runtime cases, got ${browserRuntimeCases.length}`)
  assert(blockedCases.length === 8, `Expected eight GPU/model blocked cases, got ${blockedCases.length}`)

  const disabledStatus = await withServer(false, async (baseUrl) => {
    const { response } = await postToolCall(baseUrl, browserRuntimeCases[0].request)
    return response.status
  })

  const { cpuStaticResults, browserRuntimeResults, blockedResults } = await withServer(true, async (baseUrl) => {
    const cpuStatic: CanonicalRouteControlledResult[] = []
    const browserRuntime: CanonicalRouteControlledResult[] = []
    const blocked: CanonicalRouteBlockedResult[] = []
    for (const item of cpuStaticCases) {
      cpuStatic.push(await runControlledCase(baseUrl, item.request))
    }
    for (const item of browserRuntimeCases) {
      browserRuntime.push(await runControlledCase(baseUrl, item.request))
    }
    for (const item of blockedCases) {
      blocked.push(await runBlockedCase(baseUrl, item.request))
    }
    return { cpuStaticResults: cpuStatic, browserRuntimeResults: browserRuntime, blockedResults: blocked }
  })

  assert(disabledStatus === 409, `disabled canonical browser route should return 409, got ${disabledStatus}`)
  for (const result of cpuStaticResults) {
    ...
    assert(result.localPackageExecutionPerformed === true, `${result.toolId} local package execution was not recorded`)
  ...
  for (const result of browserRuntimeResults) {
    assert(result.statusCode === 200, `${result.toolId} did not return 200`)
    assert(result.ok === true, `${result.toolId} did not return ok envelope`)
    assert(
      result.routeStatus ===
    ...
    assert(result.controlledAdapterExecutedNow === true, `${result.toolId} adapter did not execute`)
    assert(result.localPackageExecutionPerformed === true, `${result.toolId} local browser runtime was not recorded`)
    assert(Boolean(result.outputKind), `${result.toolId} missing output kind`)
    assert(Boolean(result.outputSha256), `${result.toolId} missing output hash`)
    assert(result.publicArtifactCreated === false, `${result.toolId} created public artifact`)
    assert(result.signedUrlCreated === false, `${result.toolId} created signed URL`)
  ...
  const report = buildReport(disabledStatus, cpuStaticResults, browserRuntimeResults, blockedResults)
