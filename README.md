...
The consolidated local E2E MVP branch makes the first full demo loop testable without production services. `/projects/new` creates a browser-local project from selected file metadata, `/editor?projectId=...` loads that project into the chat-native AI editor, source order and approval gates are preserved in `localStorage`, and approval runs mock credit reservation, queue, worker lease, dispatch, completion events, and preview-ready placeholder status. See `docs/end-to-end-video-editor-readiness.md`. Real uploads, real rendering/export, provider calls, Stripe, deployed Cloud Run, and production Supabase validation remain separate production milestones.
