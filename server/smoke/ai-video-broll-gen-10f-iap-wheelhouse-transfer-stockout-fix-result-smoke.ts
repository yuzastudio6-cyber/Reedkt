...
import {
  AI_VIDEO_BROLL_GEN_10F_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_10G_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST4_A_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-10g-no-idle-l4-iap-wheelhouse-transfer-proof-us-west4-a.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
  'ai_video_broll_gen_10f_iap_wheelhouse_transfer_stockout_fix_ready_for_us_west4_a_transfer_proof_no_vm_no_inference'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll10fStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10f-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  '`us-east1-d`',
  '`us-west4-a`',
  '| `us-west4-a` | `us-west4` | `UP` | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | selected |',
  'Select `us-west4-a` as the next bounded no-idle IAP wheelhouse transfer proof target.',
  ...
  assert.equal(doc.includes(required), true, `10F stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `10G prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10F_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10f_iap_wheelhouse_transfer_stockout_fix_result')
...
assert.equal(result.selectedCapacityStrategy, 'bounded_cross_region_no_idle_transfer_proof_retry')
assert.equal(result.selectedRetryZone, 'us-west4-a')
...
assert.equal(
  result.readOnlyChecks.candidates.some(
    (candidate) =>
      ...
      candidate.selected &&
      candidate.disposition === 'selected_untried_cross_region_no_idle_transfer_proof' &&
...
assert.equal(result.futureRetryPreflightRequired.includes('us_west4_a_zone_status'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10G_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST4_A_PROMPT)
