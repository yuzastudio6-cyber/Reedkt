...
The next action is controlled persisted worker dispatch runtime smoke planning for the implemented fail-closed runtime. That future prompt must remain fail-closed and must not deploy migrations, invoke Cloud Run, run Qwen inference, create generated assets, public artifacts, signed URLs, beta, production, or claim `generated_local_fixture_passed`.
