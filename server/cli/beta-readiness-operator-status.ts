...
function buildNextActions(
  ...
  actions.push('Use npm run beta:readiness:owner-approval-packet plus docs/beta-readiness/owner-approval-packet-current-gates/2026-06-28-owner-approval-packet-current-gates.md as the current non-secret source for platform and launch owner approval inputs; the packet does not grant approval by itself. Generate the exact local fill-in template with npm run beta:readiness:owner-approval-env-template, review pending operator input status with npm run beta:readiness:external-beta-operator-input-template -- --status, and export only safe non-secret constants/idempotency keys with npm run beta:readiness:external-beta-operator-autofill-env before adding human-owned values.')
