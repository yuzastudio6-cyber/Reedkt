...
- approved-fixture inference service deploy: blocked
...
| Approved-fixture inference service deploy | blocked approved-fixture inference service deploy required | Service and caller source are ready for new image builds. | GPU service image built and deployed with gated fixture inference source, CPU caller image built/job updated, and post-deploy fail-closed verification. |
...
- `approvedFixtureInferenceServiceDeployed=false`
...
The next action is a gated service/caller deploy. That future prompt must build and deploy the updated source, verify the persistent default fail-closed configuration, and must not run the fixture inference request yet.
