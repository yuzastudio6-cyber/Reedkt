...
const BROLL_9Y_US_EAST4_C_TRANSFER_PROMPT =
...
for (const required of [
  'external_agent_tool_execution_readiness_qwen_ready_for_explicit_gate_broll_9x_us_east4_c_selected_future_prompt_required',
  ...
  '9X no-VM strategy selecting `us-east4-c`',
  ...
  '`broll_no_idle_l4_iap_wheelhouse_transfer_us_east4_c_selected_future_prompt_required`',
  ...
  BROLL_9Y_US_EAST4_C_TRANSFER_PROMPT,
...
assert.equal(rollup.recommendedNextPrompt, BROLL_9Y_US_EAST4_C_TRANSFER_PROMPT)
...
assert.equal(
  ...
  'bounded_no_idle_l4_iap_wheelhouse_transfer_us_east4_c_selected_future_prompt_required_before_vm_create',
)
assert.equal(
...
assert.equal(broll?.nextAction, BROLL_9Y_US_EAST4_C_TRANSFER_PROMPT)
...
assert.equal(broll?.noIdleLifecycleGate?.targetRegion, 'us-east4')
assert.equal(broll?.noIdleLifecycleGate?.targetZone, 'us-east4-c')
