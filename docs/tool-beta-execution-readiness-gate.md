...
The deployed probe adapter is the tested seam for that future staging transport. It lets real Supabase/readback/monitoring checks plug into the evidence verifier without changing the gate logic, and keeps a single failed observation from producing a platform evidence packet.
