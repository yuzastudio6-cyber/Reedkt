...
function invokeControlledProductToolExecutionBoundary(envelope) {
  const stopReasons = []
  ...
  for (const descriptor of envelope.toolDescriptors ?? []) {
    ...
    if (descriptor.productToolBoundaryInvoked !== true) stopReasons.push('product_tool_boundary_not_invoked')
  ...
  return {
    ...
    controlledProductToolExecutionBoundaryInvoked: true,
...
const proofPlan = parseJsonBlock(
  proofPlanDoc,
...
assert(source.ownerReview.controlledProductToolExecutionProofMayProceedNext === true, 'source does not allow next proof')
assert(source.ownerReview.ownerEvidenceMustRecordWhatHappened === true, 'source missing what-happened rule')
assert(source.soundCpuTools.readyForControlledProductToolExecutionProofNoRealUserMedia === 15, 'source proof readiness count mismatch')
...
assert(proofPlan.requiredBeforeNextProof.whatHappenedEvidenceRequired === true, 'proof plan missing what-happened rule')
assert(prompt.requiredSourceDecision === sourceDecision, 'prompt source decision mismatch')
assert(prompt.proofScope.runControlledProductToolExecutionBoundary === true, 'prompt does not allow controlled boundary')
...
const toolDescriptors = tools.map((toolId, index) => ({
  toolId,
  descriptorId: `phase119-product-tool-boundary-${String(index + 1).padStart(2, '0')}`,
  descriptorKind:
    index % 4 === 0
  ...
  productToolBoundaryInvoked: true,
  mediaFileOpen: false,
...
const invocations = jobTypes.map((jobType, index) => {
  const envelope = {
    approvedPlanSnapshotId: 'phase119-synthetic-approved-plan-snapshot',
    workspaceId: 'phase119-synthetic-workspace',
    projectId: 'phase119-synthetic-project',
    jobId: `phase119-synthetic-job-${index + 1}`,
    toolCallRequestId: `phase119-product-tool-call-${index + 1}`,
    idempotencyKey: `phase119-controlled-product-tool-execution-proof-${jobType}`,
  ...
  return {
    ...
    result: invokeControlledProductToolExecutionBoundary(envelope),
  }
})
...
const allInvocationsPassed = invocations.every(
  ({ result }) =>
    result.ok &&
    result.controlledProductToolExecutionBoundaryInvoked &&
...
const proof = {
  ...
  proofKind: 'controlled_product_tool_execution_boundary_no_real_user_media',
  sourceDecisionVerified: true,
  controlledProductToolExecutionBoundaryInvoked: allInvocationsPassed,
