...
Paid-production blockers must not be used to stop internal break/fix testing. Internal testing is allowed to proceed when its own sign-in, dry-run, safety, cost, mock/test-credit, approved-snapshot, idempotency, and private-artifact guardrails pass. Paid-production blockers still close paid users, live billing, production wallet mutation, public artifact delivery, and production launch.
