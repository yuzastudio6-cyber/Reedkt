...
import {
  AI_VIDEO_BROLL_GEN_9T_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_9U_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_C_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9t-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9t-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-9u-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-c.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-9t-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-9t-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9t-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
...
function scanForbiddenValues(value: unknown, prefix = 'broll9tStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9t-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  'A capacity reservation is not selected now because it would be a mutating, potentially billable strategy',
  ...
  assert.equal(doc.includes(required), true, `9T stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9U prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9T_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9t_iap_wheelhouse_transfer_stockout_fix_result')
assert.deepEqual(result.priorTransferProofZones, ['us-central1-c', 'us-central1-c', 'us-west1-a', 'us-west1-b'])
assert.equal(result.selectedRetryZone, 'us-west1-c')
assert.equal(result.selectedRetryRegion, 'us-west1')
assert.equal(result.sameRegionFinalAlternateSelected, true)
...
assert.equal(result.readOnlyChecks.privateCacheReadinessPassed, true)
...
assert.equal(result.selectionRationale.includes('us_west1_b_transfer_proof_stocked_out_before_vm_creation'), true)
assert.equal(
  result.selectionRationale.includes('us_west1_c_is_remaining_untried_same_region_transfer_proof_zone'),
  true,
)
...
assert.equal(result.futureRetryPreflightRequired.includes('us_west1_c_zone_status'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9U_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_C_PROMPT)
  ...
  JSON.stringify(
    {
      ...
      sameRegionFinalAlternateSelected: result.sameRegionFinalAlternateSelected,
