...
assert.equal(report.productionBillingPersistence, 'backend_required', 'durable billing persistence must remain backend-required')
...
assert.ok(report.blockers.some((blocker) => blocker.blockerId === 'production_billing_persistence_missing'), 'durable billing persistence must be required')
