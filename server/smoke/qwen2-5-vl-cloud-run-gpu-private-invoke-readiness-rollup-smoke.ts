...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt result review: blocked, result review required',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt result review | blocked controlled persisted worker dispatch runtime real-dispatch approved fixture private invoke attempt result review required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 122)
assert.equal(
...
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptResultReviewRequired, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptResultReviewRecorded, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptResultReviewAccepted, false)
...
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeAttemptResultReview, true)
assert.equal(
