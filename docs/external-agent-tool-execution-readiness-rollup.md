...
Decision: `external_agent_tool_execution_readiness_qwen_ready_broll_cloud_side_cache_staging_runner_ready_execute_required`.
...
| `ai_video_broll_generation_wan` | controlled L4 private proof, Wan/Wan2.1 selected, private cache/proof-runner/fast cache readiness, B-roll quota verification result, 10ZB dependency/install wrapper evidence, 11A model-import plan evidence, 11B runner evidence, 11C Storage Transfer naming-test evidence, 11D cache-staging strategy evidence, and 11E no-GPU cloud-side cache staging runner evidence | callable for confirmed no-GPU private cache staging, but not ready for the 11B GPU import proof until the 11E runner succeeds and creates the private ready marker; Wan inference remains blocked | The canonical B-roll wrapper can now expose both cache preparation and 11B import/load commands. Historical 10ZB proved the selected zone can create the no-public-IP G2/L4 VM, use IAP SSH, fetch the private GCS wheelhouse by exact object paths, validate the cache marker and manifest, install dependencies offline, import `torch`, `diffusers`, and `transformers`, then delete the prompt-scoped VM and verify cleanup. 11B adds the narrower no-inference import/load runner because the historical tabletop runner goes past import into frame generation. 11C proved that a private GCS-hosted URL-list through HTTPS fails with HTTP 403 for Storage Transfer. 11D rejected repeat local upload, public URL-list, signed URL-list, GPU transfer host, and runtime auto-download, then selected a no-GPU cloud-side cache staging runner as the next strategy. 11E implements that runner and wires `external-agent-tool-prepare-broll-wan-cache` to delegate to it with explicit confirmation. Wan inference, prompt encoding, denoising, VAE decode, frame/video creation, generated assets, Supabase, SQL, signed URLs, credits, beta, and production remain blocked. | `AI-VIDEO-BROLL-GEN-11E-EXECUTE-CLOUD-SIDE-CACHE-STAGING: run no-GPU Wan private cache staging runner with explicit confirmation, no inference/no generated video` |
...
- B-roll proof path has quota for one L4 VM, the 9K no-idle prompt, cleanup-verified proof/stockout evidence through 10ZA, a passed 10ZB fixed-delivery dependency/install runner, the completed `AI-VIDEO-BROLL-GEN-11A-MODEL-IMPORT-PLAN: plan Wan model import proof after payload/install readiness, no inference`, the 11B model import/load runner, the 11C Storage Transfer naming-test result, the 11D cache-staging strategy fix, and the 11E no-GPU cloud-side cache staging runner implementation. 11E evidence is `docs/ai-video-broll-gen-11e-cloud-side-cache-staging-runner.md`, `src/backend/mock/mock-ai-video-broll-gen-11e-cloud-side-cache-staging-runner.ts`, `server/cli/ai-video-broll-gen-11e-cloud-side-cache-staging-runner.ts`, and `server/smoke/ai-video-broll-gen-11e-cloud-side-cache-staging-runner-smoke.ts`. The completed 11D prompt is `AI-VIDEO-BROLL-GEN-11D-CACHE-STAGING-STRATEGY-FIX: choose approved Wan private cache staging strategy after local upload stall and private URL-list 403, no GPU/no inference`. The current Wan blocker is `broll_private_gcs_model_cache_staging_runner_ready_not_yet_executed`. Agents can call `npm run external-agent-tool-prepare-broll-wan-cache` with `REEDITPRO_CONFIRM_EXTERNAL_AGENT_BROLL_WAN_CACHE_FILL=true`; that wrapper now delegates to `npm run ai-video-broll-gen-11e:cloud-side-cache-staging-runner -- --execute` with `REEDITPRO_CONFIRM_BROLL_11E_CLOUD_SIDE_CACHE_STAGING=true`. 11E may create only a prompt-scoped no-GPU Cloud Run Job and private GCS support/cache objects for model cache staging, then must delete the job/support payloads and verify cleanup. The 11B runner remains ready to use a prompt-scoped no-public-IP L4 VM, exact private GCS object downloads, offline wheel install, dependency import readiness, local-files-only `WanPipeline` import/load, and mandatory cleanup verification after the private model cache is staged. Model inference, prompt encoding, denoising, frame generation, generated video, generated assets, Supabase, SQL, signed URLs, credits, beta, and production remain blocked.
...
For B-roll execution preparation, agents may run `npm run external-agent-tool-execute-broll-wan` without `--execute` to see the fail-closed static guard. The current executable B-roll prep step is `npm run external-agent-tool-prepare-broll-wan-cache -- --execute --json` with `REEDITPRO_CONFIRM_EXTERNAL_AGENT_BROLL_WAN_CACHE_FILL=true`; that wrapper delegates to `npm run ai-video-broll-gen-11e:cloud-side-cache-staging-runner -- --execute` with `REEDITPRO_CONFIRM_BROLL_11E_CLOUD_SIDE_CACHE_STAGING=true`. The 11E runner may stage private GCS model-cache files using a prompt-scoped no-GPU Cloud Run Job, must delete the job/support files, and must verify cleanup. Only after the private ready marker exists may agents use `REEDITPRO_CONFIRM_EXTERNAL_AGENT_BROLL_WAN_PROOF=true` with `npm run external-agent-tool-execute-broll-wan -- --execute --json` to repeat live quota preflight and stat-only private cache readiness before delegating to `npm run ai-video-broll-gen-11b:l4-model-import-runner -- --execute`. That 11B delegated runner may create only the prompt-scoped no-public-IP L4 proof VM, must delete it before completion, and must verify cleanup. It may import `WanPipeline` and load the approved local-files-only Diffusers cache only after the explicit 11B confirmation and private cache checks pass. It must not run Wan inference, prompt encoding, denoising, VAE decode, frame creation, video encoding, FFmpeg, generated video, generated assets, touch Supabase, execute SQL, create signed URLs, mutate credits, or unlock beta/production. The current safe B-roll next prompt is `AI-VIDEO-BROLL-GEN-11E-EXECUTE-CLOUD-SIDE-CACHE-STAGING: run no-GPU Wan private cache staging runner with explicit confirmation, no inference/no generated video`; the 11B proof must wait until the no-GPU cloud-side private model-cache staging runner succeeds.
