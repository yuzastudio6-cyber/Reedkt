...
Use `worker_runtime_jobs_sound_cpu_bounded_external_beta_state_change_scope_fix_completed_with_warnings_ready_for_bounded_external_beta_state_change_execution` as source evidence. The scope fix is required because the bounded external beta scorecard change must include the beta readiness type boundary.
