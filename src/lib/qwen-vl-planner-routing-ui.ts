...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'controlled_persisted_worker_dispatch_runtime_real_dispatch_execution_approval_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58CC-CONTROLLED-PERSISTED-WORKER-DISPATCH-RUNTIME-REAL-DISPATCH-EXECUTION-APPROVAL: approve first real persisted Qwen worker dispatch execution plan, no Cloud Run invocation/no inference/no generated assets/no beta'
}
  ...
  return {
    ...
    privateInvokeClient: {
      ...
      boundaryNotes: [
        ...
        'The next runtime gate is the controlled persisted worker dispatch runtime real-dispatch execution approval; it must keep Cloud Run invocation, inference, generated assets, published artifacts, signed URLs, beta, and production blocked now.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
