...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt approval: blocked, attempt approval required',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt approval | blocked controlled persisted worker dispatch runtime real-dispatch approved fixture private invoke attempt approval required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 120)
assert.equal(
...
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptApprovalRequired, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptApprovalRecorded, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptApprovalAccepted, false)
...
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeAttemptApproval, true)
assert.equal(
