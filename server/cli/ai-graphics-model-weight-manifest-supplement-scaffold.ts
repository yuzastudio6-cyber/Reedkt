...
import {
  ...
  type AiGraphicsModelWeightManifestSupplementScaffoldPacket,
} from '../tool-registry/ai-graphics-model-weight-manifest-supplement-scaffold'
...
if (!outputDirectory) {
  console.log(JSON.stringify({
    ...
    reason: 'Missing required --out-dir for local-only model manifest supplement scaffold output.',
    localOnly: true,
...
const packet = buildAiGraphicsModelWeightManifestSupplementScaffoldPacket()
const writtenFiles: string[] = []
const skippedFiles: string[] = []

function checklistMarkdown(packet: AiGraphicsModelWeightManifestSupplementScaffoldPacket): string {
  const lines = [
    '# AI Graphics Model-Weight Manifest Supplement Authoring Checklist',
    '',
    'Local-only checklist for private model-weight manifest review supplements. Do not commit these files.',
    '',
    '## Required Fields',
    '',
    ...packet.authoringChecklist[0].requiredSupplementFields.map((field) => `- \`${field}\``),
    '',
  ...
  for (const item of packet.authoringChecklist) {
    ...
    lines.push(`- Local supplement path: \`${item.localOnlySupplementPath}\``)
    lines.push(`- Accepted private evidence refs: \`${item.acceptedPrivateEvidenceRefNamespaces.join('`, `')}\``)
    lines.push(`- Committed supplement approved: ${item.committedManifestSupplementApproved}`)
