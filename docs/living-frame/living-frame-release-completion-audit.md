...
| Narrative and animation-aware illustration | `verified_private_runtime` for three real generated illustration treatments, alpha preparation, adaptive flat/shallow/deep rendering, one fixture-specific component decomposition, and timed selective motion; `verified_source_contract` for canonical controlled generation | mini-skill identities, component asset intent, synthesis routing, work admission, controlled-illustration qualification, selected-scene private conditioning, the Musashi cinematic-anime fixture, the fictional astronomer flat-editorial fixture, the locomotive paper-collage fixture, `server/smoke/living-frame-animation-aware-illustration-private-alpha-internal-test-smoke.ts`, `server/smoke/living-frame-animation-aware-illustration-private-composite-internal-test-smoke.ts`, and `server/smoke/living-frame-style-depth-breadth-private-render-internal-test-smoke.ts` | The private runtime now uses three real animation-aware illustration treatments rather than a universal style. The existing modern cinematic anime/ink Musashi fixture remains the deep-multiplane articulated proof. Two additional built-in image-generation fixtures are content-addressed, converted through the unchanged chroma-key soft-matte/despill helper, decoded as RGBA, and rendered through the actual Remotion canvas: a fictional astronomer remains intentionally flat with zero measured settled centroid drift, while a paper-collage locomotive uses restrained shallow 2.5D with about twice as much foreground displacement as far-plane displacement plus independently measured paper-smoke rise. All three remain illustrative rather than archival evidence. Exact ComfyUI generation remains gated by the five released model weights and GPU execution. Automatic arbitrary-character decomposition, hidden-area reconstruction, arm/sword articulation, and skeletal deformation are not claimed. |
...
| Component decomposition and rigging | `verified_private_runtime` for one fixture-specific appendage cutout rig; `verified_source_contract` for the general component architecture | component asset intent, geometry, rig, background-plate reconstruction, artifact reconciliation, and `server/smoke/living-frame-animation-aware-illustration-private-composite-internal-test-smoke.ts` | The real Musashi fixture is decoded, split into a stable base plus pivot-centered hair and trailing-robe RGBA components, byte-revalidated, independently animated, and measured in the final render. Hidden-area reconstruction is unnecessary for these transparent-edge appendages. Automatic arbitrary-character decomposition, occluded-part reconstruction, skeletal deformation, and broader character fixtures remain open; no claim is made that all stills can be safely decomposed. |
...
- real 120-frame animation-aware illustration composition in a confirmed
  ...
  pivot-centered hair and trailing-robe RGBA components; differentiated 2.5D
  depth and articulated appendage motion; foreground slash reveal/settle;
  ...
  retained content-addressed private review frames spanning both sides of the
  appendage motion arc. This closes one bounded component-decomposition and
  selective-part-motion fixture, not automatic arbitrary-character
  decomposition, hidden-area reconstruction, arm/sword articulation, or
...
- real style-adaptive flat and shallow illustration rendering using two
  ...
  scene, approximately 32.5 pixels of far-plane displacement versus 63.3
  pixels of foreground displacement for the shallow scene, and approximately
  27.6 pixels of paper-smoke rise. Caption priority, H.264 media profile,
  create-only private persistence, reopen verification, and three retained
  review frames also pass. Together with the Musashi scene this proves actual
  flat → shallow-2.5D → deep-multiplane style adaptation without claiming
  dimensional or true-3D support;
