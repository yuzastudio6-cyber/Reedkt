...
export const EDIT_LEVEL_QA_GATE_DEFINITIONS: EditLevelQAGateDefinition[] = [
  ...
  gate('qwen_response_validation', 'Planning response validation', 'qwen', 'Checks future planning response shape and policy compliance without calling the planning service.', 'runtime_disabled'),
  gate('visual_intelligence_confidence', 'Visual understanding confidence', 'visual', 'Checks future visual confidence policy without calling a model.', 'provider_required'),
...
export function createEditLevelQAGateRoute(input: {
  ...
  return {
    ...
    purpose: hideInternalToolNamesInCopy(definition.purpose),
    ...
    technicalNotes: [
      ...
      'No AI planning, visual understanding, technical assembly, asset service, planner, edit-plan, media backend, render, Supabase, file-byte, external-fetch, or credit operation runs.',
    ],
    ...sideEffectFlags,
    sideEffectFlags,
  }
...
export function createEditLevelQAGateBoundarySummary(): string[] {
  return [
    ...
    'No AI planning, visual review call, technical assembly, asset service, plan generation, media processing, render, storage, file-byte read, external fetch, or credit operation runs.',
...
export function createEditLevelQAFallbackPolicy(level: ReEditProCanonicalEditLevel): string[] {
  ...
  if (level === 'premium') {
    return [
      'Premium keeps stronger creative QA warnings visible when visual, transcript, audio, or planning context is unavailable.',
  ...
  return [
    ...
    'Ultra Premium may degrade to Premium-safe QA policy when future visual, transcript, audio, render, revision, or credit gates are unavailable.',
  ]
}
...
function readinessSummaryForLevel(level: ReEditProCanonicalEditLevel): string {
  if (level === 'normal') return 'Ready for local planning with baseline QA gates.'
...
function warningsForLevel(level: ReEditProCanonicalEditLevel): string[] {
  if (level === 'normal') {
    return [
      'Missing asset, marker clarification, and marker conflict checks are warning-only in the local Normal package.',
  ...
  if (level === 'premium') {
    return [
      'Premium exposes warnings for source context, transcript, visual confidence, and future render/credit gates.',
      'Recommended planning and media confidence gates remain local policy only.',
    ]
  }
...
const normalGateSpecs: GateSpec[] = [
  ...
  route('qwen_response_validation', 'optional', 'runtime_disabled', 'Future planning response validation is optional policy only for Normal.', 'Use deterministic local validation.'),
  route('visual_intelligence_confidence', 'not_used', 'provider_required', 'Deep visual confidence QA is not required for Normal.', 'Use source metadata and user markers.'),
...
const premiumGateSpecs: GateSpec[] = [
  ...
  route('qwen_response_validation', 'recommended', 'runtime_disabled', 'Premium validates future planning response policy as local rules.', 'Use deterministic fallback while planning runtime is disabled.'),
  route('visual_intelligence_confidence', 'warning_only', 'provider_required', 'Premium shows visual-confidence gaps as warnings.', 'Use source markers and key-moment fallback.'),
...
const ultraGateSpecs: GateSpec[] = [
  ...
  route('qwen_response_validation', 'required', 'runtime_disabled', 'Ultra Premium requires future planning response validation policy.', 'Degrade to deterministic Premium-safe validation while planning runtime is disabled.', true),
  route('visual_intelligence_confidence', 'required', 'provider_required', 'Ultra Premium requires future visual confidence.', 'Degrade to Premium-safe visual context until visual understanding exists.', true),
