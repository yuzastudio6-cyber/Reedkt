...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'backend_runtime_persistence_current_edit_session_baseline_fix_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58Y-BACKEND-RUNTIME-PERSISTENCE-BASELINE-CURRENT-EDIT-SESSION-FIX: fix ReEditPro local baseline current_edit_session_id prerequisite for Qwen harness validation, no deploy/no cloud/no assets/no beta'
}
  ...
  return {
    ...
    privateInvokeClient: {
      ...
      boundaryNotes: [
        ...
        'The next runtime gate must fix the current_edit_session_id local baseline prerequisite; it must not touch Supabase cloud, staging, production, live data, Cloud Run, Qwen inference, worker dispatch, generated assets, public artifacts, signed URLs, beta, or production.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
