...
const lane = 'TRACKA-GPAC-MP4BOX-GUARDED-HANDLER-REGISTRATION-PLAN-1'
const packetDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-guarded-handler-registration-plan'
const priorDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-disabled-handler-registration-contract-negative-tests'
const decisionText = 'tracka_gpac_mp4box_guarded_handler_registration_plan_passed_ready_for_disabled_handler_registration_scaffold'
const executionText = 'completed_docs_only_guarded_handler_registration_plan_no_runtime_execution'
const priorDecision = 'tracka_gpac_mp4box_disabled_handler_registration_contract_negative_tests_passed_ready_for_guarded_handler_registration_plan'
const priorMergeSha = '7600ec197854d64784289fa9d2ee6ba654772486'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-DISABLED-HANDLER-REGISTRATION-SCAFFOLD-1'

const packetFiles = [
  `${packetDir}/gpac-mp4box-guarded-handler-registration-plan-decision.json`,
  `${packetDir}/gpac-mp4box-guarded-handler-registration-plan-decision.md`,
  `${packetDir}/handler-registration-guard-matrix.json`,
  `${packetDir}/handler-registration-guard-matrix.md`,
...
const priorFiles = [
  `${priorDir}/gpac-mp4box-disabled-handler-registration-contract-negative-tests-decision.json`,
...
const requiredText = [
  ...
  priorMergeSha,
  nextPrompt,
  'ready_for_disabled_handler_registration_scaffold_only',
  'disabled_handler_registration_scaffold_only',
...
if (packageJson.scripts?.['tracka:gpac-mp4box-guarded-handler-registration-plan:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-guarded-handler-registration-plan-diagnostics.mjs') fail('missing package diagnostics script')
...
const decision = json(`${packetDir}/gpac-mp4box-guarded-handler-registration-plan-decision.json`)
...
if (decision.prior?.disabledHandlerRegistrationContractNegativeTests !== priorDecision) fail('prior decision drift')
if (decision.prior?.disabledHandlerRegistrationContractNegativeTestsMergeSha !== priorMergeSha) fail('prior merge drift')
if (decision.handlerRegistrationPlan?.nextAllowedImplementation !== nextPrompt) fail('next implementation drift')
if (decision.handlerRegistrationPlan?.handlerRegistrationReadiness !== 'ready_for_disabled_handler_registration_scaffold_only') fail('handler readiness drift')
if (decision.handlerRegistrationPlan?.productRuntime !== 'blocked') fail('product runtime drift')
...
const guardMatrix = json(`${packetDir}/handler-registration-guard-matrix.json`)
if (guardMatrix.nextAllowedImplementation !== nextPrompt) fail('guard matrix next prompt drift')
if (guardMatrix.handlerRegistrationReadiness !== 'ready_for_disabled_handler_registration_scaffold_only') fail('guard matrix readiness drift')
for (const requiredGuard of requiredFutureGuards) {
...
if (readiness.guardedHandlerRegistrationPlanPassed !== true) fail('readiness plan drift')
if (readiness.readyForDisabledHandlerRegistrationScaffold !== true) fail('readiness scaffold drift')
...
const prior = json(`${priorDir}/gpac-mp4box-disabled-handler-registration-contract-negative-tests-decision.json`)
...
console.log('Guarded handler registration plan is docs-only and routes only to a disabled scaffold')
