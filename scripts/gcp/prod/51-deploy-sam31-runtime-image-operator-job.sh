...
# Deploys the private scale-from-zero image-build operator permanently unarmed.
# Exact authority/submission refs are supplied only as execution overrides.
...
readonly CONFIRMATION='deploy-weeditpro-sam31-runtime-image-operator-v1'
...
[[ "${WEEDITPRO_CONFIRM_SAM31_RUNTIME_IMAGE_OPERATOR_DEPLOY:-}" \
...
build_id="${WEEDITPRO_SAM31_RUNTIME_IMAGE_OPERATOR_BUILD_ID:-}"
[[ "${build_id}" =~ ^[a-f0-9-]{36}$ ]] || fail 'exact Cloud Build ID is missing'
image_tag="sam31-runtime-image-operator-${source_commit:0:16}"
...
' <<<"${project_policy}" >/dev/null || fail 'Cloud Build operator IAM changed'
for binding in \
  ...
  'reeditpro-production-reeditpro-image-build-inputs:roles/storage.objectViewer'; do
  bucket="${binding%%:*}"
  role="${binding#*:}"
  ...
  jq -e --arg role "${role}" \
    ...
    || fail 'runtime image operator bucket IAM changed'
done
...
gcloud run jobs deploy "${JOB}" \
  ...
  --set-env-vars='WEEDITPRO_SAM31_RUNTIME_IMAGE_OPERATOR_ACTION=disabled' \
  --labels='app=weeditpro,operation=sam31-runtime-image-operator,scale=zero' \
  --quiet
...
if jq -e 'any(.bindings[]?.members[]?;
  ...
  fail 'SAM 3.1 runtime image operator has a public IAM principal'
fi
...
printf '{"operation":"weeditpro_sam31_runtime_image_operator_deploy_v1",'
...
printf '"sam31RuntimeImageBuildStarted":false,"gpuOrModelRuntimeStarted":false,'
