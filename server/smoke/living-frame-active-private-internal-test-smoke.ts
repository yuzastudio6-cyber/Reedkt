...
const runDefinitions = [
  ...
  run(
    ...
    'server/smoke/living-frame-representative-private-source-binding-smoke.ts',
    'source_contract_regression',
    'passed_source_only',
    0,
    (receipt) => {
      assert.equal(receipt.compiledBindingCount, 3)
      assert.equal(receipt.exactPrivateUploadAuthoritySchemasConsumed, true)
      assert.equal(receipt.canonicalSourceAnalysisRefConsumedOpaque, true)
      assert.equal(receipt.rawStorageIdentitySerialized, false)
