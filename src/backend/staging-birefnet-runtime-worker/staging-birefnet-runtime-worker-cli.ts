...
async function main(): Promise<void> {
  ...
  try {
    ...
    artifacts.push(await uploadFile(storage, env.generatedAssetsBucket, `${env.artifactPrefix}/fixture/synthetic-input.png`, fixturePath, 'image/png', env.runId))
    const maskArtifact = await uploadFile(storage, env.generatedAssetsBucket, `${env.artifactPrefix}/mask/mask.png`, maskPath, 'image/png', env.runId)
    const cutoutArtifact = await uploadFile(storage, env.generatedAssetsBucket, `${env.artifactPrefix}/mask/cutout.png`, cutoutPath, 'image/png', env.runId)
    ...
    const reportObject = `${env.artifactPrefix}/reports/phase33c-report.json`
    const report = {
...
function readRuntimeEnv(): RuntimeEnv {
  ...
  if (process.env.REEDITPRO_BIREFNET_RUNTIME_MODE !== 'phase33c_generated_image') throw new Error('Only phase33c_generated_image mode is allowed.')
  ...
  const runId = process.env.REEDITPRO_PHASE33C_RUN_ID ?? `phase33c-${new Date().toISOString().replace(/[^0-9A-Za-z]/g, '').slice(0, 14)}`
  if (!/^phase33c-[0-9A-Za-z]+$/.test(runId)) throw new Error(`Unsafe Phase 33C run id: ${runId}`)
  ...
  return {
    ...
    generatedAssetsBucket: 'reeditpro-staging-reeditpro-generated-assets',
    qaBucket: 'reeditpro-staging-reeditpro-qa-artifacts',
    artifactPrefix: `activation-mask-runtime/phase33c/${runId}`,
  }
}
...
async function scanCustomCode(runtimePath: string): Promise<{
  ...
  for (const file of customCodeFiles) {
    ...
    if (!isExecuted && /\brequests\b/.test(body)) warnings.push(`${file} contains network helper code and is intentionally never imported in Phase 33C.`)
  }
...
async function uploadFile(
  ...
  assertPhase33COutput(bucketName, objectName, runId)
...
function buildMaskQa(input: {
  ...
  const blockers: string[] = []
  const warnings = ['mask_temporal_stability is not_applicable because Phase 33C uses one generated image only.']
  ...
  const gates = [
    {
      ...
      summary: nonEmpty && notFullFrame ? 'Generated fixture mask has foreground/background separation.' : 'Mask lacks foreground/background separation.',
    },
    {
    ...
    },
    {
      ...
      summary: 'Temporal stability is not claimed for single-frame generated fixture.',
    },
  ]
...
function assertPhase33COutput(bucketName: string, objectName: string, runId: string): void {
  const prefix = `activation-mask-runtime/phase33c/${runId}/`
  ...
  if (!allowedBuckets.includes(bucketName) || !objectName.startsWith(prefix)) {
    throw new Error(`Blocked Phase 33C output object: gs://${bucketName}/${objectName}`)
  }
}
