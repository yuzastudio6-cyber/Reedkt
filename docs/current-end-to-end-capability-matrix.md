...
| Authenticated private upload-to-review pipeline | one-publication persisted backend handoff plus bounded two-pass execution fixture | The authenticated backend persists a checksum-protected, tenant-scoped upload/planning handoff, revalidates it at canonical publication, and freezes its exact binding into plan/snapshot lineage. The canonical plan binding durably limits each handoff to one full publication request/idempotency key while allowing exact restart-safe replay. All authenticated canonical HTTP fixtures and the full initial/review/revision lifecycle publish through that path; the former caller-authority HTTP route fails closed. The fixture reaches first review, accepted second review, and restart-safe current/superseded history download. Browser consumption and deployed real-user operations remain blocked. |
