...
| Canonical private work graph | `private_executable` run-to-blocked orchestration | Authenticated package route derives topological order, executes only dependency-ready jobs, persists exact per-job outcomes, and reuses completed jobs across continuation runs. Snapshot and dependency-bound source-trim plan validation now execute. | The orchestrator fixture does not yet schedule caption generation, final composition, downstream final QA, and terminal private review as one complete graph. |
| Private render and review journey | blocked at terminal review assembly | The adapter verifies source-trim JSON plus caption PNG, renders/persists/downloads the exact private H.264/AAC MP4, then executes an independent downstream ffprobe job against that exact final dependency and replays both jobs without duplicate execution. | The legacy upload-to-render route is intentionally disabled. Complete graph scheduling, terminal private-review authority, and journey-level recovery are not yet connected. |
...
5. **Completed for canonical run-to-blocked scheduling:** authenticated package orchestration advances dependency-ready jobs, persists exact blockers, and reuses completed job evidence across continuation runs. Bounded final composition and downstream final-artifact QA now work through the single-job adapter; complete graph scheduling and private-review assembly are next.
