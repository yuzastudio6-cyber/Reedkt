...
const rawPromptSnapshot = buildPricingSnapshot({
  sourceKind: 'external_provider',
  provider: 'mock-provider',
  model: 'mock-model',
  pricingUnits: { rawPrompt: 'make this edit from raw chat text' },
})
assert.equal(rawPromptSnapshot.ok, false)
...
assert.equal(validatePricingSnapshotHasNoSecrets({
  ...providerCost.pricingSnapshot,
  pricingUnits: { sourceReference: 'https://storage.googleapis.com/bucket/object?X-Goog-Signature=abc' },
}).ok, false)
...
assert.equal(toolCostPricingSnapshotSchema.safeParse({
  ...providerCost.pricingSnapshot,
  pricingUnits: { promptText: 'copy the user prompt into the pricing snapshot' },
}).success, false)
...
assert.equal(
  packageJson.scripts?.['smoke:tool-cost-metering'],
  'tsx server/smoke/professional-tool-cost-metering-smoke.ts',
)
