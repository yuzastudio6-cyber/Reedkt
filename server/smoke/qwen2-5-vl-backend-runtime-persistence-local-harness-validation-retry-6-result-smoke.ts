...
function assertFalseFlags(flags: JsonRecord) {
  for (const key of [
    'backendRuntimePersistenceLocalHarnessValidationRetry6Passed',
...
const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-local-harness-validation-retry-6-result.md')
for (const phrase of [
  DECISION,
  '`55430`',
  '`55431`',
...
const result = QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_LOCAL_HARNESS_VALIDATION_RETRY_6_RESULT
assert.equal(result.decision, DECISION)
assert.equal(
  result.upstreamBackendRuntimePersistenceBaselineCreditApprovalSnapshotsFixDecision,
...
assert.equal(
  result.attempt.previouslyFailedBaselineForeignKeyColumnPassed,
  'credit_reservations.approved_plan_snapshot_id',
)
assert.deepEqual(result.attempt.siblingApprovedSnapshotCompatibilityColumnsPassed, [
  'credit_ledger_entries.approved_plan_snapshot_id',
  'approval_records.approved_snapshot_id',
])
assert.equal(
...
assert.equal(result.attempt.failedStatementCategory, 'idx_credit_estimates_project_plan')
assert.equal(result.attempt.missingBaselineColumn, 'credit_estimates.edit_plan_version_id')
assert.equal(
...
assert.equal(result.validationOutcome.status, 'blocked_credit_estimates_plan_version_baseline_column')
assert.equal(result.validationOutcome.creditApprovalSnapshotsBaselineFixVerified, true)
assert.equal(result.validationOutcome.creditEstimatesPlanVersionBaselineFixRequired, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry6ResultRecorded, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry6Attempted, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceCreditApprovalSnapshotsBaselineFixVerified, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceCreditEstimatesPlanVersionBaselineFixRequired, true)

const forbiddenDataFindings = scanValues({ result })
assert.deepEqual(
  forbiddenDataFindings,
  [],
  `Forbidden values in Qwen validation retry 6 data: ${forbiddenDataFindings.join('; ')}`,
)

console.log(JSON.stringify({
  ok: true,
  decision: DECISION,
  localHarnessValidationRetry6Attempted: result.validationOutcome.localHarnessValidationRetry6Attempted,
  localHarnessValidationRetry6Passed: result.validationOutcome.localHarnessValidationRetry6Passed,
  creditApprovalSnapshotsBaselineFixVerified: result.validationOutcome.creditApprovalSnapshotsBaselineFixVerified,
