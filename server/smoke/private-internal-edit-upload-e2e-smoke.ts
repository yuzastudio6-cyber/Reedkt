...
try {
  ...
  if (privateInternalTestRunResponse.status === 503) {
    ...
    assert.equal(
      ...
      'canonical_real_user_upload_binding_revision_approval_execution_and_recovery',
    )
    console.log(JSON.stringify({
      ok: true,
      status: 'blocked_by_canonical_real_user_upload_binding_revision_approval_execution_and_recovery',
      checks: [
      ...
      skippedReason: 'The legacy private internal execution route is intentionally disabled. The bounded canonical fixture reaches and records private review and publishes replacement plan v2; real-user upload binding, revision approval/execution, and recovery must use future canonical authority.',
      nextRequiredGate: 'canonical_real_user_upload_binding_revision_approval_execution_and_recovery',
    }))
  } else {
