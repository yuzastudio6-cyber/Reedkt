...
const runId = 'worker-10-local-claim-lease-noop'
const outputRelativeDir = `.local-artifacts/worker-runtime/worker-10/${runId}`
...
const runnerPath = 'scripts/validation/worker-runtime-job-claim-lease-noop-execution.mjs'
...
const requiredSourceDocs = [
  {
    path: 'docs/worker-runtime/worker-9-approval-decision-record.md',
    markers: ['approved_with_warnings_for_worker_10', 'futureControlledJobClaimLeaseNoopApproved: `true`'],
  },
  {
    path: 'docs/worker-runtime/worker-10-allowed-blocked-scope.md',
    markers: ['WORKER-10 - Controlled Job Claim/Lease No-Op Execution', 'Real job claim', 'Worker lease mutation'],
  },
  {
    path: 'docs/worker-runtime/worker-8-controlled-noop-qa-review.md',
    markers: ['worker_runtime_controlled_noop_qa_passed_with_warnings'],
  },
  {
    path: 'docs/worker-runtime/worker-7-readiness-decision.md',
    markers: ['worker_runtime_controlled_noop_passed_with_warnings'],
  },
  {
...
function fail(message) {
  console.error(`WORKER-10 controlled claim/lease no-op execution failed: ${message}`)
  process.exit(1)
}
...
function assertSafeString(value, label) {
  ...
  if (/\/private\/tmp\/[^\s`]+\/\.local-artifacts/i.test(value)) fail(`${label} contains an absolute local artifact path`)
...
function assertNoUnsafeRunnerImports() {
  ...
  for (const importPath of dynamicImports) fail(`runner uses dynamic import ${importPath}`)

  const executableText = runnerText.replace(/(['"`])(?:\\.|(?!\1)[\s\S])*\1/g, '')
  for (const [pattern, label] of [[/\bfetch\s*\(/, 'fetch'], [/\bXMLHttpRequest\b/, 'XMLHttpRequest'], [/\bWebSocket\b/, 'WebSocket']]) {
...
const fixtureResults = fixtureData.fixtures.map((fixture) => {
  ...
  return {
    ...
    jobClaimNoopValidated: true,
    leaseNoopValidated: true,
    queueNoopValidated: true,
    realJobClaimMade: false,
    realLeaseMutationPerformed: false,
    queueExecutionPerformed: false,
    warning: 'controlled_claim_lease_noop_only_live_runtime_remains_blocked',
  }
})
...
const claimLeaseReport = {
  ...sharedStatus,
  command: 'npm run --silent worker:runtime-job-claim-lease-noop:execute',
  sourceFixtureFile: fixturePath,
  sourceEvidence: {
    worker9Decision: 'approved_with_warnings_for_worker_10',
    futureControlledJobClaimLeaseNoopApproved: true,
    worker8Qa: 'worker_runtime_controlled_noop_qa_passed_with_warnings',
    worker7Result: 'worker_runtime_controlled_noop_passed_with_warnings',
    worker2FixtureContracts: 'worker_runtime_dry_run_fixture_contract_tests_passed_with_warnings',
  },
  fixtures: fixtureResults,
}

const jobClaimSummary = {
  ...sharedStatus,
  realJobClaimMade: false,
  serviceRoleMutationPerformed: false,
  claimRows: fixtureResults.map((fixture) => ({
    fixtureId: fixture.fixtureId,
    workerJobRef: fixture.workerJobRef,
    idempotencyKeyRef: fixture.idempotencyKeyRef,
    claimNoopRef: `<CLAIM_NOOP_${fixture.fixtureId.toUpperCase()}>`,
...
const leaseSummary = {
  ...sharedStatus,
  realLeaseMutationPerformed: false,
  leaseRows: fixtureResults.map((fixture) => ({
    fixtureId: fixture.fixtureId,
    workerJobRef: fixture.workerJobRef,
    idempotencyKeyRef: fixture.idempotencyKeyRef,
    leaseNoopRef: `<LEASE_NOOP_${fixture.fixtureId.toUpperCase()}>`,
...
const qaEvidence = {
  ...
  jobClaimNoopValidationResult: 'passed_with_warnings',
  leaseNoopValidationResult: 'passed_with_warnings',
  queueNoopValidationResult: 'passed_with_warnings',
  noWorkerRuntimeImportProof: true,
  noRealJobClaimProof: true,
...
const observabilityEvidence = {
  ...sharedStatus,
  localExecutionScope: 'controlled_claim_lease_noop_fixture_validation_only',
  commandsRun: ['npm run --silent worker:runtime-job-claim-lease-noop:execute'],
...
const writtenFiles = [
  writeJson('claim-lease-noop-report.json', claimLeaseReport),
  writeJson('job-claim-noop-summary.json', jobClaimSummary),
  writeJson('lease-noop-summary.json', leaseSummary),
...
console.log(JSON.stringify({
  ...
  readinessDecision: sharedStatus.readinessDecision,
  runId,
  outputDirectory: outputRelativeDir,
