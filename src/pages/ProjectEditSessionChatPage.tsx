...
import { ProjectEditReferenceBriefWorkspace } from '../components/projects/brief/ProjectEditReferenceBriefWorkspace'
...
import { readConnectedPreferenceApplicationContext } from '../lib/edit-reference-downstream-context'
import {
  recoverConnectedPreferenceApplicationForProjectEditSession,
  recoverMissingProjectEditSessionFromPreferenceApplication,
...
export function ProjectEditSessionChatPage() {
  ...
  const loadBundleWithPreferenceRecovery = useCallback(async () => {
    if (!editSessionId) return undefined
    let nextBundle = await loadProjectEditSessionChatBundleForUI({
      projectId,
      editSessionId,
      client: apiClient,
    })
    if (!nextBundle.bundle) {
      const shellRecovery = await recoverMissingProjectEditSessionFromPreferenceApplication({
        projectEditSessionClient: apiClient,
        projectId,
        editSessionId,
      })
      if (shellRecovery.recovered) {
        nextBundle = await loadProjectEditSessionChatBundleForUI({
          projectId,
          editSessionId,
          client: apiClient,
        })
    ...
    if (nextBundle.bundle) {
      const recovery = await recoverConnectedPreferenceApplicationForProjectEditSession({
        projectEditSessionClient: apiClient,
        session: nextBundle.bundle.session,
      })
  ...
  async function refreshBundle(nextStatus?: string) {
    ...
    const nextBundle = await loadBundleWithPreferenceRecovery()
  ...
  useEffect(() => {
    ...
    loadBundleWithPreferenceRecovery().then((nextBundle) => {
      if (cancelled) return
      if (!nextBundle) return
      setBundleModel(nextBundle)
  ...
  }, [editSessionId, loadBundleWithPreferenceRecovery])
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const nextStatus = result.referenceCommand?.handled
        ? result.referenceCommand.confirmationRequired
          ? 'Edit Reference change needs your explicit confirmation.'
          : result.referenceCommand.mutated
            ? 'Canonical Edit Reference application updated and downstream context refreshed.'
            : 'Edit Reference command handled without changing the canonical application.'
  ...
  const connectedPreferenceApplication = readConnectedPreferenceApplicationContext(bundleModel?.bundle?.session)
    ...
    >
      ...
      >
        ...
        {header && routeSection === 'brief' ? (
          <ProjectEditReferenceBriefWorkspace
            editSessionId={editSessionId ?? ''}
            editSessionTitle={header.title}
            preferenceApplicationContext={connectedPreferenceApplication}
            projectId={projectId}
          />
        ) : null}
        ...
        {header && context && routeSection !== 'brief' ? (
          <div className="project-edit-session-chat-page__layout">
            ...
            <ProjectEditSessionContextPanel
              ...
              context={context}
              editSessionId={editSessionId ?? ''}
              key={connectedPreferenceApplication?.applicationId ?? 'no-connected-edit-reference'}
