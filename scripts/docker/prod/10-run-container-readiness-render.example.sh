...
if [[ "${REEDITPRO_RENDER_WORKER_IMAGE}" == "manual-not-set" ]]; then
  echo "REEDITPRO_RENDER_WORKER_IMAGE must be set." >&2
  exit 1
fi

if [[ ! "${REEDITPRO_RENDER_WORKER_IMAGE}" =~ ^[^[:space:]@]+@sha256:[0-9a-f]{64}$ ]]; then
  echo "REEDITPRO_RENDER_WORKER_IMAGE must be an immutable name@sha256:digest reference." >&2
  exit 1
fi
...
cmd=(docker run --rm --network none --read-only --cap-drop=ALL --security-opt=no-new-privileges --user=65532:65532
  ...
  --env REEDITPRO_CONTAINER_IMAGE_ROLE=render_worker
  --env "REEDITPRO_CONTAINER_IMAGE_REFERENCE=${REEDITPRO_RENDER_WORKER_IMAGE}"
  ...
  "${REEDITPRO_RENDER_WORKER_IMAGE}" node dist-server/container-readiness-receipt.js)
echo "Render container readiness command:"
printf ' %q' "${cmd[@]}"
echo
"${cmd[@]}"
