...
export const TRACK_ALL_CAPABILITY_MANIFEST = createSkillCapabilityManifest({
  ...
  toolRoutes: [
    route({ routeKey: 'source_truth', operationIds: ['tool.ffprobe.inspect_approved_media.v1'], supportedJobTypes: ['track_all.plan_assignment'], requiredArtifactTypes: ['source_media_artifact_v1'], priority: 1, description: 'Inspect exact source facts.' }),
    ...
    route({ routeKey: 'ffmpeg_treatments', operationIds: ['tool.ffmpeg.execute_approved_media_recipe.v1'], supportedJobTypes: ['track_all.apply_privacy_redaction'], requiredArtifactTypes: ['tracked_redaction_plan_v1'], priority: 5, description: 'Apply deterministic private redaction.' }),
    route({ routeKey: 'remotion_preview', operationIds: ['tool.remotion.render_approved_composition.v1'], supportedJobTypes: ['track_all.integrate_preview'], requiredArtifactTypes: ['track_graph_v2'], priority: 6, description: 'Render an approved private integration preview.' }),
  ],
  fallbackRoutes: [
