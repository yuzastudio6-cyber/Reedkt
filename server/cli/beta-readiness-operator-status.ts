...
function buildNextActions(
  ...
  actions.push('Use npm run beta:readiness:owner-approval-packet plus docs/beta-readiness/owner-approval-packet-current-gates/2026-06-28-owner-approval-packet-current-gates.md as the current non-secret source for platform and launch owner approval inputs; the packet does not grant approval by itself. Generate the exact local fill-in template with npm run beta:readiness:owner-approval-env-template, review pending operator input status with npm run beta:readiness:external-beta-operator-input-template -- --status, export only safe non-secret constants/idempotency keys with npm run beta:readiness:external-beta-operator-autofill-env, review the 45 human-owned inputs with npm run beta:readiness:external-beta-operator-human-input-checklist, then validate the completed local ignored env file with npm run beta:readiness:external-beta-operator-local-env-preflight before any deployed collector.')
  actions.push('Before rerunning deployed evidence manifests or collectors with owner-provided values, run npm run beta:readiness:owner-approval-intake-status for a value-free owner input progress review, then run npm run beta:readiness:owner-approval-intake-preflight to require approval/attestation booleans to be explicitly true, reject secret-like owner notes, reject real-user-media or paid-production scope flags, and avoid echoing evidence note values.')
  ...
  actions.push('After API deployment preflight and owner approval intake status/preflight pass, set REEDITPRO_BETA_DEPLOYED_EVIDENCE_SOURCE_SHA to the current deployed staging source SHA, then run npm run beta:readiness:deployed-evidence-input-manifest to verify the historical local evidence bundle, current 16 bounded accepted-proven / 0 product-ready Track B boundary, libass container image, platform approvals, and launch approvals before any deployed evidence call.')
