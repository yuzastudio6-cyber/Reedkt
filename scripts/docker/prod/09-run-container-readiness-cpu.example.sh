...
cmd=(docker run --rm --env REEDITPRO_READINESS_MODE=container_runtime --env REEDITPRO_CONTAINER_IMAGE_ROLE=cpu_worker "${REEDITPRO_CPU_WORKER_IMAGE}" npm run prod:readiness:summary -- --mode=static_only)
