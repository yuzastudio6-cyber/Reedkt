...
function invokeControlledProductToolCallBoundary(envelope) {
  const stopReasons = []
  ...
  if (envelope.externalAgentAdapterMode !== 'disabled_synthetic_boundary') {
    stopReasons.push('external_agent_adapter_must_be_disabled')
  }
  ...
  for (const descriptor of envelope.toolDescriptors ?? []) {
    ...
    if (descriptor.productToolCallBoundaryInvoked !== true) {
      stopReasons.push('product_tool_call_boundary_not_invoked')
    }
  ...
  return {
    ...
    controlledProductToolCallBoundaryInvoked: true,
...
assert(source.ownerReview.controlledProductToolCallExecutionProofMayProceedNext === true, 'source does not allow proof')
assert(source.ownerReview.acceptedExpectedInvocationCount === jobTypes.length, 'source invocation count mismatch')
assert(source.soundCpuTools.readyForControlledProductToolCallExecutionProofNoRealUserMedia === tools.length, 'source proof count mismatch')
assert(source.soundCpuTools.readyForProductToolCallExecutionToday === 0, 'source product execution widened')
assert(acceptance.acceptedForNextProofOnly.whatHappenedEvidenceMustBeRecorded === true, 'acceptance missing what-happened rule')
...
assert(prompt.proofScope.runControlledProductToolCallExecutionProofCommand === true, 'prompt does not allow proof command')
...
const toolDescriptors = tools.map((toolId, index) => ({
  toolId,
  descriptorId: `phase122-product-tool-call-${String(index + 1).padStart(2, '0')}`,
  descriptorKind:
    index % 4 === 0
...
const invocations = jobTypes.map((jobType, index) => {
  const envelope = {
    approvedPlanSnapshotId: 'phase122-synthetic-approved-plan-snapshot',
    workspaceId: 'phase122-synthetic-workspace',
    projectId: 'phase122-synthetic-project',
    jobId: `phase122-synthetic-job-${index + 1}`,
    toolCallRequestId: `phase122-product-tool-call-${index + 1}`,
    idempotencyKey: `phase122-controlled-product-tool-call-execution-proof-${jobType}`,
    externalAgentAdapterMode: 'disabled_synthetic_boundary',
  ...
  return {
    whatHappened: {
      ...
      realExternalAgentUsed: envelope.realExternalAgentUsed,
      realUserMediaUsed: envelope.realUserMediaUsed,
      toolDescriptorCount: envelope.toolDescriptors.length,
      boundaryOnly: true,
    },
    result: invokeControlledProductToolCallBoundary(envelope),
  }
})
...
const allInvocationsPassed = invocations.every(
  ({ result }) =>
    result.ok &&
    result.controlledProductToolCallBoundaryInvoked &&
    result.toolCountCovered === tools.length &&
    result.runtimeFlagsAllFalse &&
    result.realExternalAgentUsed === false &&
...
const proof = {
  ...
  proofKind: 'controlled_product_tool_call_execution_boundary_no_real_user_media',
  sourceDecisionVerified: true,
  controlledProductToolCallBoundaryInvoked: allInvocationsPassed,
  syntheticOrNoMediaInputAccepted: allInvocationsPassed,
  recordWhatHappened: true,
  proofCommandRunCount: 1,
  invocationCount: invocations.length,
  totalSyntheticProductToolCallBoundaryInvocationsObserved: invocations.length,
  ...
  realExternalAgentUsed: false,
  realUserMediaUsed: false,
  whatHappened: invocations.map(({ whatHappened, result }) => ({
    ...whatHappened,
    accepted: result.ok,
