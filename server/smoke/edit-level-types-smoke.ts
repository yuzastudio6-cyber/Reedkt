...
import { evaluateEditLevelProductionReadiness } from '../../src/lib/edit-level-production-readiness'
...
for (const profile of EDIT_LEVEL_PROFILES) {
  ...
  assert.ok(profile.productionReadiness.blockers.length > 0)
}

const completeProductionReadiness = evaluateEditLevelProductionReadiness({
  profileContractApproved: true,
  repositoryPersistenceApproved: true,
  apiRouteApproved: true,
  clientRuntimeWiringApproved: true,
  plannerWiringApproved: true,
  providerRoutingApproved: true,
  mediaWorkerApproved: true,
  renderExportApproved: true,
  creditReservationApproved: true,
  supabasePersistenceApproved: true,
  deploymentApproved: true,
  qaApproved: true,
})
assert.equal(completeProductionReadiness.productionReady, true)
assert.equal(completeProductionReadiness.blockers.length, 0)
