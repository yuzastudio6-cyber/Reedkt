...
-- Generated for QWEN2_5_VL_STACK_TOOL_58O planning.

-- Purpose: draft Qwen2.5-VL backend runtime persistence constraints on top of
-- the existing ReEditPro runtime foundations. This draft reuses approved plan
...
begin
  ...
  then
    raise exception 'QWEN2_5_VL_DRAFT_REQUIRES_REEDITPRO_RUNTIME_BASELINE';
  end if;
end $$;
...
'Draft Qwen2.5-VL job guard. Qwen jobs must be media_analysis jobs with approved snapshot, credit reservation, idempotency, private storage, checksum, structured finding, and edit intent refs. Payloads must not store raw prompts, signed URLs, public URLs, service URLs, tokens, secrets, raw model output, beta claims, or production claims.';
...
'Draft Qwen2.5-VL event guard. Qwen event payloads are sanitized summaries only.';
...
'Draft Qwen2.5-VL runtime config guard. Qwen config rows are non-secret L4 scale-to-zero metadata only.';
...
'Draft Qwen2.5-VL lease guard. Qwen leases require workspace, project, and job refs and sanitized metadata.';
...
'Draft Qwen2.5-VL transport guard. Runtime messages store only sanitized private invoke summaries.';
...
'Draft Qwen2.5-VL claim-attempt guard. Claim metadata stays sanitized.';

-- Draft extension: Qwen needs a tool runtime check entry, but the active
-- RP-E2E check currently limits tool_name to deterministic editing tools.
-- This draft expands the check constraint rather than creating a parallel table.
...
'Job orchestration records. Draft Qwen2.5-VL constraints require approved snapshot refs, credit reservation, idempotency, private source refs, and sanitized payload summaries before future dispatch.';
comment on table public.backend_runtime_messages is
'Runtime transport envelopes. Draft Qwen2.5-VL constraints require sanitized summaries only and no service URL, token, secret, signed URL, public URL, or raw output payload values.';
comment on table public.tool_runtime_checks is
'Worker tool readiness checks. Draft Qwen2.5-VL support adds qwen_vl as metadata-only readiness evidence, not model execution.';
