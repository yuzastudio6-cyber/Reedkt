...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'backend_runtime_persistence_active_migration_deploy_approval_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58BD-BACKEND-RUNTIME-PERSISTENCE-ACTIVE-MIGRATION-DEPLOY-APPROVAL: approve deployment target for validated Qwen persistence migration, no deploy/no cloud/no assets/no beta'
}
...
export function getQwenVlPlannerRoutingUiData(): QwenVlPlannerRoutingUiData {
  return {
    ...
    privateInvokeClient: {
      ...
      currentStatus: 'backend_runtime_persistence_active_migration_deploy_approval_required',
      currentBlocker:
        'Controlled private invoke auth works, the dedicated Direct VPC private route subnet is configured, the no-model CPU-only caller source is defined, the controlled caller job is deployed, one caller contract smoke observed the expected fail-closed response with inference disabled, runtime readiness review is recorded, the first approved-fixture inference smoke plan is defined, gated fixture inference service source is deployed, structured-output retry/result review passed, private runtime and approved worker integration reviews are accepted, fail-closed backend dispatch and controlled dispatch dry-run are recorded, and backend runtime persistence has advanced through local harness validation, active migration creation, and active migration validation. The active migration deploy plan is now recorded, so user-facing readiness remains blocked until deployment target approval, actual migration deployment, and later runtime approval.',
      boundaryNotes: [
        ...
        'The next runtime gate is deployment approval for the locally validated Qwen persistence migration; it must not deploy migrations, touch Supabase cloud, staging, production, live data, Cloud Run, Qwen inference, worker dispatch, generated assets, public artifacts, signed URLs, beta, or production.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
