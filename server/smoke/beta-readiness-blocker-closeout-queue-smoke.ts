...
assert.equal(report.batches[0]?.nextCommands[2], 'REEDITPRO_BETA_OPERATOR_ENV_FILE=.env.reeditpro-beta-operator.local npm run beta:readiness:external-beta-operator-value-progress -- --markdown')
assert.equal(report.batches[0]?.nextCommands[3], 'npm run beta:readiness:external-beta-operator-autofill-env')
assert.equal(report.batches[0]?.nextCommands[4], 'npm run beta:readiness:external-beta-operator-autofill-local-env')
assert.equal(report.batches[0]?.nextCommands[5], 'npm run beta:readiness:external-beta-operator-human-input-checklist')
assert.equal(report.batches[0]?.nextCommands[6], 'REEDITPRO_BETA_OPERATOR_ENV_FILE=.env.reeditpro-beta-operator.local npm run beta:readiness:external-beta-operator-local-env-preflight')
assert.equal(report.batches[0]?.nextCommands[7], 'npm run beta:readiness:external-beta-operator-input-template')
assert.equal(report.batches[0]?.nextCommands[8], 'npm run beta:readiness:owner-approval-intake-status')
assert.equal(report.batches[0]?.nextCommands[9], 'REEDITPRO_BETA_OWNER_APPROVAL_ENV_FILE=.env.reeditpro-beta-operator.local npm run beta:readiness:owner-approval-intake-preflight')
assert.equal(report.batches[0]?.nextCommands[10], 'npm run beta:readiness:deployed-evidence-input-manifest -- --status')
assert.equal(report.batches[0]?.nextCommands[11], 'npm run beta:readiness:deployed-evidence-input-manifest')
assert.equal(
...
assert.equal(
  report.batches[0]?.blockedUntil[0]?.includes('redacted markdown progress'),
  true,
)
assert.equal(
  report.batches[0]?.blockedUntil[0]?.includes('REEDITPRO_BETA_OPERATOR_ENV_FILE=.env.reeditpro-beta-operator.local'),
  true,
)
assert.equal(
  report.batches[0]?.blockedUntil[0]?.includes('local-env preflight'),
  true,
)
assert.equal(
...
assert.equal(report.batches[1]?.nextCommands.includes('npm run beta:readiness:external-beta-sequence-preflight'), true)
...
assert.equal(report.batches[2]?.nextCommands[2], 'npm run beta:tools:core-real-check-preview:hydrated -- --local-defaults')
...
assert.equal(report.batches.some((batch) => batch.nextCommands.some((command) => command.includes('external-beta-sequence-preflight'))), true)
