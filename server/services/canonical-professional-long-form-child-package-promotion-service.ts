  ...
  return {
    async promote(input: {
      ...
      if (!isExplicitLocalInternalTestRuntime(context.env)) {
        throw new ApiError(
          'TOOL_NOT_READY',
          'Professional long-form child package promotion remains private/local until distributed transaction and service-identity evidence exists.',
          503,
          {
            requiredGate:
      ...
      if (!ownerUserId) {
        throw new ApiError(
          'AUTH_REQUIRED',
          'Professional long-form child package promotion requires authenticated authority.',
          401,
        )
      }
      ...
      if (
        ...
        throw new ApiError(
          ...
          'Professional long-form child package promotion requires the exact funded approved reservation.',
          409,
        )
      }
      const postApproval =
