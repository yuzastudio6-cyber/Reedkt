...
import {
  CAPTION_CANONICAL_TRACK_ALL_RESUME_ADMISSION_VERSION,
  type CaptionCanonicalTrackAllResumeAdmissionReceipt,
} from '../../src/types/caption-canonical-track-all-resume'
import type {
...
import { parseCaptionCanonicalSpecialistSupportResumeRecord } from
  './caption-canonical-specialist-resume-read'
...
const receiptSchema:
z.ZodType<CaptionCanonicalTrackAllResumeAdmissionReceipt> = z.object({
...
export function runCaptionCanonicalTrackAllResumeAdmission(input: {
  canonicalResumeRecord: unknown
  canonicalTrackAllEvidenceRecord: unknown
}): OrchestraSkillJobResult {
  assertClosedContractTree(
    input, 'Caption canonical Track All resume admission input')
  const resumeRecord = parseCaptionCanonicalSpecialistSupportResumeRecord(
    input.canonicalResumeRecord)
  const evidenceRecord = parseCaptionCanonicalTrackAllEvidenceRecord(
    input.canonicalTrackAllEvidenceRecord)
  if (resumeRecord.authenticatedOwnerProjection.ownerKey !== 'track_all'
    ...
    || !exactRef({
      ...
      contentHash: resumeRecord.selectedSupportRequest.requestDigestSha256,
    ...
    throw new Error('Caption canonical Track All resume records are crossed.')
  }
  const replayedResult = runCaptionsSpecialistJob({
    ...
    canonicalTrackAllEvidenceRecord: evidenceRecord,
  })
  if (replayedResult.resultDigestSha256
    ...
    throw new Error(
      'Caption canonical Track All replay changed its persisted result.')
  }
  return structuredClone(replayedResult)
}

export function parseCaptionCanonicalTrackAllResumeAdmissionReceipt(
  value: unknown,
): CaptionCanonicalTrackAllResumeAdmissionReceipt {
  assertClosedContractTree(
    value, 'Caption canonical Track All resume admission receipt')
  const receipt = receiptSchema.parse(value)
  if (receipt.receiptDigestSha256 !== calculateSkillContractDigest(
    receipt as unknown as Record<string, unknown>,
    'receiptDigestSha256')) {
    throw new Error('Caption canonical Track All resume receipt is stale.')
  }
...
const receiptWithoutDigest: Omit<
  ...
  'receiptDigestSha256'
> = {
  schemaVersion: CAPTION_CANONICAL_TRACK_ALL_RESUME_ADMISSION_VERSION,
  receiptId: 'captions.canonical.track-all.resume-admission',
...
export const CAPTION_CANONICAL_TRACK_ALL_RESUME_ADMISSION_RECEIPT =
