...
const standardRate = assertCanonicalCurrentGoogleCloudGpuRateAuthority(
  l4Standard,
  publishedAt,
)
const fallbackRate = assertCanonicalCurrentGoogleCloudGpuRateAuthority(
  l4Fallback,
  publishedAt,
)
...
assert.deepEqual(primaryRead, standardRate)
assert.notEqual(primaryRead, standardRate)
const fallbackRead = await repository.rereadApprovedCurrentRate({
  rateAuthorityRef: fallback.rateAuthorityRef,
  routeId: 'l4_heavy_fallback',
  at: readAt,
})
assert.deepEqual(fallbackRead, fallbackRate)
...
const hostile = Object.defineProperty(
  { authority: standardRate, publishedAt },
  'authority',
  {
    enumerable: true,
    get() {
...
console.log(JSON.stringify({
  ...
  checks: 27,
  standardAndFallbackL4RateAuthoritiesPersistedCreateOnly: true,
