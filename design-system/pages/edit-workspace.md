...
- Edit Brief is a full-main-space `?view=brief` workspace inside the same named edit. It hides Chat messages, the floating composer, and the preview rail while active.
...
- Edit Brief uses one continuous source-preview, professional-timeline, and collapsed-constraints editing desk. Marker entry is a playhead-anchored plain-language popup; detailed metadata remains progressive disclosure.
- Expanded Brief constraints use a flat, sectioned hierarchy with an explicit Optional/Draft/Ready status, restrained planning-impact notice, and a semantic read-only lock after approval.
...
- The exact parent project and edit name remain visible in the workspace header. Chat controls move to a second row below 1320px; the Brief keeps a compact single-row header from 721px through 1180px and stacks only at compact mobile width.
...
- Guarded local browser review confirms the upload gate and wide Chat preview rail at 1280px, the full-width Brief editing desk at 1024px, and no horizontal overflow or console errors in either state.
- The Brief timeline interaction review proves click-to-seek, double-click-to-direct, prompt focus, Escape/focus return, Ctrl/Command + Enter canonical save, and a 64px medium-width Brief header without adding a second marker or planning authority.
...
- Exact bounded revision-continuation evidence proves an accessible preparation state, plan version `prior + 1`, a fresh visible estimate, a separately enabled approval action, locked Current Edit Preferences, no browser-supplied revision authority/private paths, and no automatic execution after approval.
