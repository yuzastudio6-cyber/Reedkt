...
| Component decomposition and rigging | `verified_private_runtime` for one fixture-specific character-action cutout rig, one fixture-specific mechanical-object rig, one fixed Blender armature/IK/skinning component runtime, one digest-bound real-illustration textured Blender runtime, exact selected-scene private persistence, independent component QA, and private Remotion review; `verified_source_contract` for general component geometry, Head Intelligence Rigging Direction, Rigging v2, Rig Action v1, deep re-signed-artifact validation, and fixed Blender/OpenToonz adapter candidates | component asset intent, geometry, component rig, Rigging Direction v1, Rigging v2, Rig Action v1, fixed-adapter candidate v1, background-plate reconstruction, artifact reconciliation, `docs/living-frame/living-frame-rigging-director-and-adapter-candidates.md`, `docs/living-frame/living-frame-blender-fixed-adapter-private-internal-test.md`, `docs/living-frame/living-frame-blender-fixed-textured-adapter-private-internal-test.md`, `docs/living-frame/living-frame-blender-selected-scene-private-persistence-internal-test.md`, `docs/living-frame/living-frame-blender-selected-scene-private-review-internal-test.md`, `server/smoke/living-frame-rigging-v2-smoke.ts`, `server/smoke/living-frame-blender-fixed-adapter-private-internal-test-smoke.ts`, `server/smoke/living-frame-blender-fixed-textured-adapter-private-internal-test-smoke.ts`, `server/smoke/living-frame-blender-selected-scene-private-review-internal-test-smoke.ts`, `server/smoke/living-frame-animation-aware-illustration-private-composite-internal-test-smoke.ts`, and `server/smoke/living-frame-style-depth-breadth-private-render-internal-test-smoke.ts` | The real Musashi fixture is decoded, split into a stable base plus pivot-centered sword-arm, hair, and trailing-robe RGBA components, byte-revalidated, independently animated, and measured in the final render. The paper-collage locomotive supplies a second cross-style proof with three isolated wheels, bounded undercarriage reconstruction, exact-hub rotation, and one static connecting rod. The provider-neutral Head Intelligence contract chooses narrative purpose, focal action, motion arc, capabilities, risk, and route without giving workers raw chat or executable code. Rigging v2 validates parts, bones, joints, constraints, IK, meshes, skin/rigidity maps, mechanical linkages, secondary motion, output passes, QA, fallbacks, and final-canvas restraint. Rig Action v1 separately binds exact control/bone/mechanical keyframes to the MasterTiming-derived range so an external tool never invents the pose or clock. Native mechanics route to Remotion; flat mesh deformation may route to the OpenToonz Plastic evaluation candidate; armature/IK/skinning/2.5D may route to the Blender headless candidate. The fixed reviewed Blender adapter now executes one real signed/notarized native ARM64 Blender 4.5.11 fixture: 256 vertices, 450 triangles, two bones, bounded joints, IK, 60 exact 1920×1080 frames, transparent RGBA, mask, depth, meaningful pose change, and exact final-pose restoration. Its additive v2 texture request also executes a real 1024×1536 RGBA illustration through the same 60-frame path, preserving 102 quantized color buckets, producing 684,293 changed pixels at the action pose, and returning to an exact source pose. The sampled flat preview measures about 3.6 seconds and the full render about 34.5 seconds, with peak resident memory measured for both. The selected-scene continuation persists all 180 outputs create-only, independently rereads and verifies alpha/mask/depth plus articulated motion and exact source-pose restoration, executes four real bounded Remotion compositions, packages the exact 60 selected frames, and persists/rereads one private review MP4. The textured test is not yet bound into that selected-scene approval/persistence/review chain. The operation remains unregistered and has no canonical dispatch, manifest, cost, billing, QA/private-review approval, public, or production authority. Blender zero-network/offline non-root worker evidence, exact texture approval lineage, canonical reconciliation/work admission, broader fixtures, and OpenToonz qualification remain open; the inspected unsigned x86_64 OpenToonz 1.8.0 macOS package is fail-closed on the ARM64 host. |
...
fail-closed pending a supported controlled runtime. The aggregate
`server/smoke/living-frame-private-internal-end-to-end-audit-smoke.ts` must
remain `passed_with_explicit_blocked_model_runtimes` until those three gates are
