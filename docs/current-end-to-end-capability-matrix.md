...
this snapshot, every child is capability-blocked from lease, exact tool/cost
binding, and dispatch, and no long media, object storage, distributed database,
...
The exact-code v18 full run on 2026-07-18 supersedes the older aggregate verdicts
for the current branch. It started at `2026-07-18T07:34:57.913Z`, finished at
`2026-07-18T08:04:51.495Z`, and passed `37/37` stages in `1,793,582 ms`.
The 30-minute/six-hour long-form planning-contract stage passed 26 adversarial
checks in `356 ms`, the approved-snapshot binding bridge passed 22 checks in
`622 ms`, and the canonical post-approval/promotion authority passed 32 checks
in `4,997 ms`. The database-neutral distributed package-state port passed in
`561 ms`, its locked server-only RPC adapter passed in `524 ms`, the
single-host package transaction passed in `18,755 ms`, and the timeout-aware
receiver passed in `2,186 ms`. Three-source composition passed in `570,188 ms`,
professional color in `665,900 ms`, and the final stage completed the exact
eight-source, 27-job, 16-second `3840x2160` private-review journey in
`394,619 ms`. Exactly 50 canonical E2E and 50 job-adapter identities remained
...
creation, child lease, exact tool/cost binding, dispatch, long-form media
...
The v18 aggregate also reverified the private Docker execution transport after
