...
import {
  AI_VIDEO_BROLL_GEN_9U_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_C_RESULT,
  AI_VIDEO_BROLL_GEN_9V_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9u-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-c-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9u-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-c-result.md'
const PROMPT_PATH = 'docs/implementation-prompts/prompt-ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix.md'
const SPEC_PATH =
...
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9u-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-c-result'
const DECISION =
...
function scanForbiddenValues(value: unknown, prefix = 'broll9uResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9u-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-c-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `9U result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9V stockout fix prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9U_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_C_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9u_no_idle_l4_iap_wheelhouse_transfer_proof_us_west1_c_result')
...
assert.equal(result.targetZone, 'us-west1-c')
...
assert.equal(result.cleanupState.globalGpusAllRegionsQuotaUsageAfterCleanup, 0)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9V_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT)
