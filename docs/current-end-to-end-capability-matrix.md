...
| Canonical private job execution | `private_executable` for one immutable job | Authenticated job-only route derives the approved work item, output, tool, operation, lease, dispatch, private artifact, QA, reconciliation, and replay from server authority. The exact source-trim + caption + Remotion final job now passes through this adapter. | Product, beta, and production flags remain false. |
| Canonical private work graph | `private_executable` run-to-blocked orchestration | Authenticated package route derives topological order, executes only dependency-ready jobs, persists exact per-job outcomes, and reuses completed jobs across continuation runs. Snapshot and dependency-bound source-trim plan validation now execute. | The orchestrator fixture does not yet schedule caption generation, final composition, independent downstream final QA, and terminal private review as one complete graph. |
| Private render and review journey | blocked at the canonical multi-job boundary | The adapter now verifies source-trim authority JSON plus caption PNG, applies exact approved frames to the approved source MP4, preserves source audio, renders H.264/AAC, runs independent FFprobe QA, persists/reconciles privately, downloads exact bytes, and replays without rerendering. | The legacy upload-to-render route is intentionally disabled. Complete graph scheduling, terminal private-review assembly, and journey-level recovery are not yet connected. |
...
| Job-only canonical adapter | 15 runner classes implemented; 4 adapter paths executed | Snapshot authority, dependency-bound source trim, ECharts, and the exact Remotion final-composition job prove server-derived adapter execution. All 50 tool identities retain independent canonical coordinator proof; adapter-wide per-tool route proof remains future coverage. |
...
5. **Completed for canonical run-to-blocked scheduling:** authenticated package orchestration advances dependency-ready jobs, persists exact blockers, and reuses completed job evidence across continuation runs. The bounded final-composition job now works through the single-job adapter; complete caption/render/final-QA graph scheduling and private-review assembly are next.
