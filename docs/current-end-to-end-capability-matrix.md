...
| Authenticated private upload-to-review pipeline | discoverable one-publication persisted backend handoff plus bounded two-pass execution fixture | The authenticated backend persists a checksum-protected, tenant-scoped upload/planning handoff, serializes preparation, and maintains a checksum-protected latest pointer per edit session. It revalidates the exact handoff at canonical publication and freezes its binding into plan/snapshot lineage. The canonical plan binding durably limits each handoff to one full publication request/idempotency key while allowing exact restart-safe replay; authenticated path-free inspection recovers the latest or a known unpublished/published handoff without mutation authority. All authenticated canonical HTTP fixtures and the full initial/review/revision lifecycle publish through that path; the former caller-authority HTTP route fails closed. The fixture reaches first review, accepted second review, and restart-safe current/superseded history download. Browser consumption and deployed real-user operations remain blocked. |
