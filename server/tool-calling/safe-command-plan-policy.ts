...
export const SAFE_COMMAND_INTENT_POLICIES = [
  ...
  policy({
    commandIntentId: 'rembg_background_remove_plan',
    toolId: 'rembg',
    ...
    parameters: [
      ...
      enumParam('alphaMatteMode', 'Alpha representation.', ['straight', 'premultiplied']),
      boolParam('includeAlphaPreview', 'Include an alpha preview expectation.'),
    ],
    defaults: {
  ...
  policy({
    commandIntentId: 'kornia_mask_refine_plan',
    toolId: 'kornia',
    ...
    parameters: [
      ...
      numberParam('featherRadiusPixels', 'Maximum bounded feather radius.', 0, 32),
      boolParam('preserveContactObjects', 'Preserve approved contact-object mask evidence.'),
    ],
    defaults: {
      edgeRefinementPreset: 'balanced',
      featherRadiusPixels: 2,
      preserveContactObjects: true,
    },
    resourceLimits: resourceLimits({ timeoutMs: 120_000, maxFrames: 1200, maxInputBytes: 4_000_000_000, gpuAllowed: true }),
  }),
  policy({
    commandIntentId: 'sharp_enhance_asset_plan',
    toolId: 'sharp',
    supportedOperationIds: ['video.enhance'],
    workerType: 'render_worker',
    parameters: [
      numberParam('scaleFactor', 'Bounded image scale factor.', 1, 4),
      enumParam('enhancementPreset', 'Deterministic enhancement preset.', ['balanced', 'detail_preserve']),
      boolParam('limitArtifacts', 'Limit sharpening and resize artifacts.'),
    ],
    defaults: {
      scaleFactor: 2,
    ...
    resourceLimits: resourceLimits({ timeoutMs: 120_000, maxFrames: 240, maxInputBytes: 4_000_000_000 }),
  }),
  policy({
    commandIntentId: 'ffmpeg_slow_motion_plan',
    toolId: 'ffmpeg',
    supportedOperationIds: ['video.slow_motion'],
    workerType: 'render_worker',
    parameters: [
      numberParam('interpolationFactor', 'Slow-motion factor.', 2, 8),
      enumParam('motionMethod', 'Approved deterministic FFmpeg motion method.', ['minterpolate', 'frame_duplication']),
      boolParam('includeArtifactQa', 'Include artifact QA expectation.'),
    ],
    defaults: {
      interpolationFactor: 2,
      motionMethod: 'minterpolate',
      includeArtifactQa: true,
    },
    resourceLimits: resourceLimits({ timeoutMs: 240_000, maxFrames: 1200, maxInputBytes: 4_000_000_000 }),
  }),
  policy({
