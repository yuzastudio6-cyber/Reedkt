...
for (const file of [
  ...
  'server/smoke/qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-approved-fixture-inference-attempt-result-smoke.ts',
  ...
  'server/smoke/qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-persisted-job-lease-bridge-plan-smoke.ts',
  ...
  'server/smoke/qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-approved-fixture-private-invoke-attempt-approval-smoke.ts',
...
assert.equal(
  ...
  'approved fixture inference attempt result package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'persisted job lease bridge plan package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'persisted job lease bridge implementation package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'persisted job lease bridge result review package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
...
assert.equal(
  ...
  'approved fixture private invoke execution approval package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'approved fixture private invoke preflight package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'approved fixture private invoke attempt approval package script mismatch',
)
assert.equal(
...
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement approval package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement implementation package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement preflight package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution plan package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution approval package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution preflight package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution attempt result package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution attempt result review package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport readiness plan package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport readiness approval package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport preflight plan package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport preflight approval package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport preflight package script mismatch',
)
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch transport attempt approval package script mismatch',
)
assert.equal(
  packageJson.scripts?.['smoke:qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-transport-attempt-result'],
  'tsx server/smoke/qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-transport-attempt-result-smoke.ts',
  'controlled persisted worker dispatch runtime real-dispatch transport attempt result package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
    'smoke:qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-transport-attempt-result-review'
  ],
...
assert.equal(
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture inference approval package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture inference preflight package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture inference attempt approval package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt result package script mismatch',
)
assert.equal(
  packageJson.scripts?.[
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt result review package script mismatch',
)
...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke execution approval: ready, execution approval recorded',
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke preflight: ready, preflight recorded',
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt approval: ready, attempt approval recorded',
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt: ready, fail-closed attempt recorded',
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt result review: ready, result review accepted',
  'controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference plan: ready, plan recorded',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution preflight | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution attempt | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport dependency enablement execution attempt result review | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport readiness plan | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport readiness approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport preflight plan | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport preflight approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch transport preflight | ready',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture inference approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture inference preflight | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture inference attempt approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture inference attempt | ready',
  'Controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge | ready',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke execution approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke preflight | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt approval | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private invoke attempt result review | ready',
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference plan | ready',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference attempt | blocked controlled persisted worker dispatch runtime real-dispatch approved fixture private inference retry attempt approval required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementApprovalDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_DEPENDENCY_ENABLEMENT_APPROVAL.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementImplementationDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementPreflightDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_DEPENDENCY_ENABLEMENT_PREFLIGHT.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionPlanDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionApprovalDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionPreflightDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionAttemptResultDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionAttemptResultReviewDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportReadinessPlanDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_READINESS_PLAN.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportReadinessApprovalDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_READINESS_APPROVAL.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightPlanDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_PREFLIGHT_PLAN.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightApprovalDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_PREFLIGHT_APPROVAL.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_PREFLIGHT.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptApprovalDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_ATTEMPT_APPROVAL.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultDecision,
...
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_APPROVED_FIXTURE_INFERENCE_APPROVAL.decision,
)
assert.equal(
  rollup.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightDecision,
...
assert.equal(
  ...
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_retry_gate_recorded_attempt_approval_required',
)
...
assert.deepEqual(gateIds, [
  ...
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_implementation',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_preflight',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_execution_plan',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_execution_approval',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_execution_preflight',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_execution_attempt',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_dependency_enablement_execution_attempt_result_review',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_readiness_plan',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_readiness_approval',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_preflight_plan',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_preflight_approval',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_preflight',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_attempt_approval',
  ...
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_inference_preflight',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_inference_attempt_approval',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_inference_attempt',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_persisted_job_lease_bridge',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_persisted_job_lease_bridge_implementation',
  ...
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_invoke_preflight',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_invoke_attempt_approval',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_invoke_attempt',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_invoke_attempt_result_review',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_plan',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_approval',
  'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_preflight',
...
assert.equal(rollup.runtimeFlags.approvedSnapshotTransportScopePlanned, true)
assert.equal(rollup.runtimeFlags.serviceUrlAudienceResolutionPlanned, true)
assert.equal(rollup.runtimeFlags.identityTokenAuthHeaderPlanned, true)
assert.equal(rollup.runtimeFlags.privateRequestSendPlanned, true)
assert.equal(rollup.runtimeFlags.responseClassificationPersistencePlanned, true)
assert.equal(rollup.runtimeFlags.qaAuditCostCreditReadinessPlanned, true)
assert.equal(rollup.runtimeFlags.billingCreditNoSpendBoundaryPlanned, true)
assert.equal(rollup.runtimeFlags.retryTimeoutCleanupRollbackPlanned, true)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightPlanRequired,
  false,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightPlanRecorded,
  true,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightApprovalRequired,
  false,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightApprovalRecorded,
  true,
)
assert.equal(
  rollup.runtimeFlags
...
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightRequired,
  false,
)
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightRecorded, true)
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportPreflightPassed, true)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptApprovalRequired,
  false,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptApprovalRecorded,
  true,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptApprovedForFutureBoundedAttempt,
  true,
)
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptRequired, false)
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptRecorded, true)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptPassedFailClosed,
  true,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultReviewRequired,
  false,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultReviewRecorded,
  true,
)
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultReviewAccepted,
  true,
)
assert.equal(
...
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded,
  true,
)
assert.equal(
  rollup.runtimeFlags
...
assert.equal(
  rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired,
  false,
)
assert.equal(
  rollup.runtimeFlags
...
assert.equal(rollup.runtimeFlags.persistedWorkerDispatchRefsPreflightVerified, true)
assert.equal(rollup.runtimeFlags.privateSourceOfTruthRefsPreflightVerified, true)
assert.equal(rollup.runtimeFlags.qwenRequestEnvelopePreflightVerified, true)
assert.equal(rollup.runtimeFlags.privateInvokeTransportDependenciesPreflightVerified, true)
assert.equal(rollup.runtimeFlags.qwenRuntimeInferenceBoundaryPreflightVerified, true)
assert.equal(rollup.runtimeFlags.responseSchemaAndResultHandlingPreflightVerified, true)
assert.equal(rollup.runtimeFlags.qaAuditCostAndCreditNoSpendPreflightVerified, true)
assert.equal(rollup.runtimeFlags.cleanupRetryAndBetaLockPreflightVerified, true)
assert.equal(rollup.runtimeFlags.approvedSnapshotAndFixtureScopeAttemptApproved, true)
assert.equal(rollup.runtimeFlags.persistedWorkerDispatchRefsAttemptApproved, true)
assert.equal(rollup.runtimeFlags.privateSourceOfTruthRefsAttemptApproved, true)
assert.equal(rollup.runtimeFlags.qwenRequestEnvelopeAttemptApproved, true)
assert.equal(rollup.runtimeFlags.privateInvokeTransportDependenciesAttemptApproved, true)
assert.equal(rollup.runtimeFlags.qwenRuntimeInferenceBoundaryAttemptApproved, true)
assert.equal(rollup.runtimeFlags.responseSchemaAndResultHandlingAttemptApproved, true)
assert.equal(rollup.runtimeFlags.qaAuditCostAndCreditNoSpendAttemptApproved, true)
assert.equal(rollup.runtimeFlags.cleanupRetryAndBetaLockAttemptApproved, true)
assert.equal(
...
assert.equal(
  rollup.runtimeFlags.transportReachabilityAcceptedForFutureInferencePlan,
  true,
)
assert.equal(
  rollup.runtimeFlags.failClosedTransportAttemptEvidenceAccepted,
  true,
)
assert.equal(rollup.runtimeFlags.cloudRunJobExecutionCreated, true)
assert.equal(rollup.runtimeFlags.serviceUrlValueStored, false)
assert.equal(rollup.runtimeFlags.audienceValueStored, false)
assert.equal(rollup.runtimeFlags.identityTokenPrinted, false)
assert.equal(rollup.runtimeFlags.identityTokenValueStored, false)
assert.equal(rollup.runtimeFlags.authHeaderValueStored, false)
assert.equal(rollup.runtimeFlags.approvedSnapshotTransportPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.serviceUrlResolutionPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.audienceResolutionPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.identityTokenDependencyPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.authHeaderRedactionPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.privateRequestEnvelopePreflightPlanned, true)
assert.equal(rollup.runtimeFlags.timeoutRetryIdempotencyPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.responseClassificationPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.persistenceQaAuditCostCreditPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.cleanupRollbackBetaProductionLockPreflightPlanned, true)
assert.equal(rollup.runtimeFlags.approvedSnapshotTransportPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.serviceUrlResolutionPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.audienceResolutionPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.identityTokenDependencyPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.authHeaderRedactionPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.privateRequestEnvelopePreflightAccepted, true)
assert.equal(rollup.runtimeFlags.timeoutRetryIdempotencyPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.responseClassificationPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.persistenceQaAuditCostCreditPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.cleanupRollbackBetaProductionLockPreflightAccepted, true)
assert.equal(rollup.runtimeFlags.approvedSnapshotTransportAttemptApproved, true)
assert.equal(rollup.runtimeFlags.serviceUrlResolutionAttemptApproved, true)
assert.equal(rollup.runtimeFlags.audienceResolutionAttemptApproved, true)
assert.equal(rollup.runtimeFlags.identityTokenDependencyAttemptApproved, true)
assert.equal(rollup.runtimeFlags.authHeaderRedactionAttemptApproved, true)
assert.equal(rollup.runtimeFlags.boundedPrivateRequestEnvelopeAttemptApproved, true)
assert.equal(rollup.runtimeFlags.inferenceDisabledContractAttemptApproved, true)
assert.equal(rollup.runtimeFlags.responseClassificationAttemptApproved, true)
assert.equal(rollup.runtimeFlags.persistenceWithoutGeneratedAssetsAttemptApproved, true)
assert.equal(rollup.runtimeFlags.cleanupRollbackBetaProductionLockAttemptApproved, true)
...
assert.equal(rollup.runtimeFlags.qaAuditCostAndCreditNoSpendAttemptApproved, true)
assert.equal(rollup.runtimeFlags.cleanupRetryAndBetaLockAttemptApproved, true)
assert.equal(rollup.runtimeFlags.approvedFixtureInferenceAttemptResultRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixtureInferenceAttemptExecuted, false)
assert.equal(rollup.runtimeFlags.approvedFixtureInferenceAttemptBlockedByPersistedJobLeaseBridge, true)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeRequired, false)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgePlanRecorded, true)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeImplementationRequired, false)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeImplemented, true)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeResultReviewRequired, false)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeResultReviewRecorded, true)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeResultReviewAccepted, true)
...
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeExecutionApprovalRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeExecutionApprovalRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeExecutionApprovalAccepted, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokePreflightRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokePreflightRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokePreflightPassed, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptApprovalRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptApprovalRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptApprovalAccepted, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptPassedFailClosed, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptResultReviewRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptResultReviewRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptResultReviewAccepted, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferencePlanRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferencePlanRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceApprovalRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceApprovalRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceApprovalAccepted, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferencePreflightRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferencePreflightRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferencePreflightPassed, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceAttemptApprovalRequired, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceAttemptApprovalRecorded, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceAttemptApprovedForFutureBoundedAttempt, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceAttemptRequired, false)
...
assert.equal(rollup.runtimeFlags.privateSourceOfTruthRefsPreflightVerified, true)
assert.equal(rollup.runtimeFlags.qwenPrivateInferenceRequestEnvelopePreflightVerified, true)
assert.equal(rollup.runtimeFlags.privateInvokeTransportAndCredentialHandlingPreflightVerified, true)
assert.equal(rollup.runtimeFlags.modelRuntimeBoundaryPreflightVerified, true)
assert.equal(rollup.runtimeFlags.metadataOnlyResponseContractPreflightVerified, true)
assert.equal(rollup.runtimeFlags.qaAuditCostCreditNoSpendPreflightVerified, true)
assert.equal(rollup.runtimeFlags.cleanupRetryRollbackReviewPreflightVerified, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferencePreflightExecuted, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceAttemptExecuted, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInferenceAcceptedForPersistedDispatch, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptLocalOutputTargetUrlDetected, true)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptTargetUrlValueStoredInRepo, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptLogPayloadUrlValueDetected, false)
assert.equal(rollup.runtimeFlags.approvedFixturePrivateInvokeAttemptLogPayloadTokenValueDetected, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeExecutionPlanning, true)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeExecutionApproval, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokePreflight, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeAttemptApproval, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeAttempt, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInvokeAttemptResultReview, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInferencePlan, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInferenceApproval, false)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInferencePreflight, false)
assert.equal(rollup.runtimeFlags.approvedSnapshotAndFixtureScopePrivateInferenceAttemptApproved, true)
assert.equal(rollup.runtimeFlags.persistedJobLeaseAndIdempotencyRefsAttemptApproved, true)
assert.equal(rollup.runtimeFlags.privateSourceOfTruthRefsPrivateInferenceAttemptApproved, true)
assert.equal(rollup.runtimeFlags.qwenPrivateInferenceRequestEnvelopeAttemptApproved, true)
assert.equal(rollup.runtimeFlags.privateInvokeTransportAndCredentialHandlingAttemptApproved, true)
assert.equal(rollup.runtimeFlags.modelRuntimeBoundaryAttemptApproved, true)
assert.equal(rollup.runtimeFlags.metadataOnlyResponseContractAttemptApproved, true)
assert.equal(rollup.runtimeFlags.qaAuditCostCreditNoSpendAttemptApproved, true)
assert.equal(rollup.runtimeFlags.cleanupRetryRollbackReviewAttemptApproved, true)
assert.equal(rollup.runtimeFlags.readyForApprovedFixturePrivateInferenceAttemptApproval, false)
...
assert.equal(rollup.runtimeFlags.privateInvokeTransportReadinessAccepted, true)
assert.equal(rollup.runtimeFlags.runtimeCredentialValueHandlingReadinessAccepted, true)
assert.equal(rollup.runtimeFlags.qwenRuntimeInferenceBoundaryReadinessAccepted, true)
assert.equal(rollup.runtimeFlags.responseSchemaAndResultHandlingReadinessAccepted, true)
assert.equal(rollup.runtimeFlags.qaAuditCostAndCreditNoSpendReadinessAccepted, true)
assert.equal(rollup.runtimeFlags.cleanupRetryAndBetaLockReadinessAccepted, true)
assert.equal(rollup.runtimeFlags.cpuCallerInferencePathReusedForPersistedDispatchAttempt, false)
assert.equal(rollup.runtimeFlags.approvedSnapshotAndFixtureScopeAccepted, true)
assert.equal(rollup.runtimeFlags.persistedWorkerDispatchRefsAccepted, true)
assert.equal(rollup.runtimeFlags.privateSourceOfTruthRefsAccepted, true)
assert.equal(rollup.runtimeFlags.qwenRequestEnvelopeAccepted, true)
assert.equal(rollup.runtimeFlags.privateInvokeTransportDependenciesAccepted, true)
assert.equal(rollup.runtimeFlags.qwenRuntimeInferenceBoundaryAccepted, true)
assert.equal(rollup.runtimeFlags.responseSchemaAndResultHandlingAccepted, true)
assert.equal(rollup.runtimeFlags.qaAuditCostAndCreditNoSpendAccepted, true)
assert.equal(rollup.runtimeFlags.cleanupRetryAndBetaLockAccepted, true)
assert.equal(rollup.runtimeFlags.selectedGpuL4Accepted, true)
assert.equal(rollup.runtimeFlags.scaleToZeroCostPostureAccepted, true)
assert.equal(rollup.runtimeFlags.minInstancesZeroAccepted, true)
assert.equal(rollup.runtimeFlags.initialMaxInstancesOneAccepted, true)
assert.equal(rollup.runtimeFlags.cpuFallbackDisabledAccepted, true)
...
const forbiddenDataFindings = scanValues({
  ...
  approvedFixtureInferenceApproval:
  ...
  controlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionAttemptResult:
  ...
  controlledPersistedWorkerDispatchRuntimeRealDispatchTransportDependencyEnablementExecutionAttemptResultReview:
  ...
  controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixturePrivateInvokeAttemptResultReview:
