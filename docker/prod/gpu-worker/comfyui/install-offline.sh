...
if [ "$#" -ne 0 ]; then
  echo "Canonical ComfyUI installer does not accept arguments." >&2
  exit 64
fi
...
CUSTOM_NODE_ROOT="${TARGET_ROOT}/custom_nodes"
...
require_regular_file() {
  if [ ! -f "$1" ] || [ -L "$1" ]; then
    echo "Required fixed ComfyUI build input is missing or unsafe." >&2
    exit 65
  fi
}
...
verify_archive() {
  ...
  if [ "${actual_bytes}" != "${expected_bytes}" ]; then
    echo "ComfyUI source archive byte length does not match." >&2
    exit 66
  fi
...
require_regular_file "${PACKAGE_INPUT_ROOT}/runner.py"
...
if [ "${wheel_entry_count}" != "35" ] \
  ...
  echo "Offline ComfyUI wheelhouse closure does not match." >&2
  exit 68
fi
...
cp "${PACKAGE_INPUT_ROOT}/runner.py" "${TARGET_ROOT}/runner.py"
cp "${LAYOUT_VERIFIER}" "${TARGET_ROOT}/verify-installed-layout.sh"
...
echo "Canonical ComfyUI offline package installed."
