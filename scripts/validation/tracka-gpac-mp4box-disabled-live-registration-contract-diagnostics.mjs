...
const priorDecision = 'tracka_gpac_mp4box_guarded_live_registration_review_passed_ready_for_disabled_live_registration_contract'
const priorMergeSha = 'd0aaa82eac128b71fc37be63a5c2194b2990f432'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-LIVE-REGISTRATION-CONTRACT-NEGATIVE-TESTS-1'

const packetFiles = [
  `${packetDir}/gpac-mp4box-disabled-live-registration-contract-decision.json`,
  `${packetDir}/gpac-mp4box-disabled-live-registration-contract-decision.md`,
  `${packetDir}/live-registration-contract.json`,
  `${packetDir}/live-registration-contract.md`,
...
const requiredText = [
  ...
  'render.gpacMp4box.disabledLiveRegistrationContract',
  'backend_service_role_only',
  'disabled_metadata_contract_only',
...
const forbiddenDocPatterns = [
  ...
  /"liveHandlerRegistered"\s*:\s*true/i,
  /"liveHandlerEnabled"\s*:\s*true/i,
  /"featureFlagDefault"\s*:\s*true/i,
...
if (packageJson.scripts?.['tracka:gpac-mp4box-disabled-live-registration-contract:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-disabled-live-registration-contract-diagnostics.mjs') fail('missing package diagnostics script')
if (packageJson.scripts?.['smoke:tracka-gpac-mp4box-disabled-live-registration-contract'] !== 'tsx server/smoke/tracka-gpac-mp4box-disabled-live-registration-contract-smoke.ts') fail('missing smoke script')
...
const decision = json(`${packetDir}/gpac-mp4box-disabled-live-registration-contract-decision.json`)
...
if (decision.prior?.guardedLiveRegistrationReview !== priorDecision) fail('prior decision drift')
if (decision.prior?.guardedLiveRegistrationReviewMergeSha !== priorMergeSha) fail('prior merge SHA drift')
if (decision.contract?.contractId !== 'liveRegistration.gpacMp4box.disabled') fail('contract id drift')
if (decision.contract?.routeId !== 'render.gpacMp4box.disabledLiveRegistrationContract') fail('route id drift')
if (decision.contract?.routeOwner !== 'backend_service_role_only') fail('route owner drift')
if (decision.contract?.routeRegistrationMode !== 'disabled_metadata_contract_only') fail('registration mode drift')
if (decision.contract?.registrationStatus !== 'disabled_live_registration_contract_registered_no_handler') fail('registration status drift')
for (const key of ['liveHandlerRegistered', 'liveHandlerEnabled', 'featureFlagDefault', 'runtimeExecutionApproved']) {
  if (decision.contract?.[key] !== false) fail(`contract ${key} drift`)
}
for (const guard of requiredGuards) {
  if (!decision.requiredGuards?.includes(guard)) fail(`missing guard ${guard}`)
}
...
const contractDoc = json(`${packetDir}/live-registration-contract.json`)
if (contractDoc.routeRegistrationMode !== 'disabled_metadata_contract_only') fail('contract doc registration mode drift')
for (const key of ['liveHandlerRegistered', 'liveHandlerEnabled', 'featureFlagDefault', 'runtimeExecutionApproved']) {
  if (contractDoc[key] !== false) fail(`contract doc ${key} drift`)
}
for (const guard of requiredGuards) {
  if (!contractDoc.requiredGuards?.includes(guard)) fail(`contract doc missing guard ${guard}`)
}

const readiness = json(`${packetDir}/readiness-report.json`)
if (readiness.disabledLiveRegistrationContractPassed !== true) fail('readiness contract pass drift')
if (readiness.readyForLiveRegistrationContractNegativeTests !== true) fail('readiness next gate drift')
...
const prior = json(`${priorDir}/gpac-mp4box-guarded-live-registration-review-decision.json`)
if (prior.decision !== priorDecision) fail('prior guarded live registration decision drift')

const contractSource = read('src/backend/contracts/gpac-mp4box-disabled-live-registration-contracts.ts')
for (const text of [
  ...
  'summarizeGpacMp4boxDisabledLiveRegistrationBoundary',
  'disabled_metadata_contract_only',
  ...
  if (!contractSource.includes(text)) fail(`contract source missing ${text}`)
}
for (const pattern of forbiddenSourcePatterns) {
  if (pattern.test(contractSource)) fail(`forbidden executable pattern ${pattern} in contract source`)
}
if (!read('src/backend/contracts/index.ts').includes("export * from './gpac-mp4box-disabled-live-registration-contracts'")) fail('missing contract export')

const smokeText = read('server/smoke/tracka-gpac-mp4box-disabled-live-registration-contract-smoke.ts')
for (const text of [
...
console.log('Disabled live-registration contract validates with no executable HTTP handler')
