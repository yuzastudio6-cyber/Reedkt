  ...
  return visualAssetPlan.map((asset) => {
    ...
    return {
      ...
      qaChecks: [
        ...
        `Mask strategy: ${depthItem.maskStrategy.replaceAll('_', ' ')}; backend execution gated.`,
      ],
    }
  })
}
  ...
  return segmentEditPlans.map((segment) => {
    ...
    return {
      ...
      workerNotes: [
        ...
        'Mask generation, tracking, object detection, and rendering remain backend-gated.',
      ],
    }
  })
}
  ...
  return segmentEditPlans.map((segment) => {
    ...
    return {
      ...
      workerNotes: [
        ...segment.workerNotes,
        `Tool strategy items linked: ${toolStrategyItemIds.join(', ')}.`,
        'Future tool workers must use the approved snapshot; tool execution remains backend-gated.',
      ],
    }
  })
}
    ...
    return {
      ...
      colorGradePlan: {
        ...
        notes: [
          ...
          'Color pipeline planned at project/clip/asset level; color processing remains backend-gated.',
        ],
      },
      workerNotes: [
        ...segment.workerNotes,
        `Color pipeline operations linked: ${colorPipelineOperationIds.slice(0, 5).join(', ')}.`,
        'Future color workers must use the approved snapshot; FFmpeg/OpenColorIO/OpenCV/Sharp processing remains backend-gated.',
      ],
    }
  })
}
  ...
  return segmentEditPlans.map((segment) => {
    ...
    return {
      ...
      workerNotes: [
        ...
        'Future audio workers must use the approved snapshot; FFmpeg/AudioFlux/Signalsmith Stretch/Essentia/librosa/Rubber Band/whisper.cpp processing remains backend-gated.',
      ],
    }
  })
}
  ...
  return segmentEditPlans.map((segment) => {
    ...
    return {
      ...
      workerNotes: [
        ...
        'Future chart workers must use approved source-aware data; D3/ECharts/Vega-Lite execution remains backend-gated.',
      ],
    }
  })
}
...
export function createMockEditPlan(input: PlannerInput): EditPlan {
  ...
  const professionalSkillPlan = createProfessionalSkillPlan({
    plannerInput: finalAnalysisInput,
  })
