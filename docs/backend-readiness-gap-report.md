...
| Worker/job queue runtime missing | Partially fixed | The canonical private package queue is checksum-protected and restart-readable. Its server-selected claim plus dispatch-outbox insert and its accepted-worker queue completion plus terminal outbox receipt each share a cooperative cross-process same-host write-ahead commit with real process-exit recovery and exact replay. The completion receipt binds private artifact/QA/reconciliation/downstream and attempt-level internal-cost evidence while excluding customer price/credits/service fee/wallet/billing authority. The distributed database transaction, live Cloud Tasks/Cloud Run dispatch and completion, worker execution, and production operations remain open. |
