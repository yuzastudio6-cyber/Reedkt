...
function successfulSupplyChainBuild(
  ...
  const artifacts = structuredClone(value.artifacts) as {
  ...
  return {
    ...
    results: {
      artifactManifest:
        `gs://${buildAdmission.evidenceBucket}/${buildAdmission.evidencePrefix}/artifacts-${id}.json`,
      numArtifacts: '3',
    },
  }
}
...
function createEvidenceObjects(input: {
  ...
  const port: VisualIntelligencePrivateObjectReadPort = {
    async readExact(request) {
      ...
      if (request.objectName === manifestName && !request.generation) {
        return {
