...
| --- | --- | --- | --- | --- |
| `qwen2_5_vl_7b_instruct` | controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference retry plan | auth verified, runtime blocked | read-only gcloud token refresh, Cloud Run service describe, and private caller job describe now pass; bounded private inference retry planning is recorded and the retry gate recheck remains required before any invocation, model load, or inference | `QWEN2_5_VL_STACK_TOOL_58DS-PRIVATE-INFERENCE-RETRY-GATE: verify bounded approved-fixture private inference retry gate after auth refresh, no inference/no mutation` |
