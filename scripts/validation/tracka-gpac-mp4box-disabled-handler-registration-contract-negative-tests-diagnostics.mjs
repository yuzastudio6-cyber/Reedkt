...
const priorDecision = 'tracka_gpac_mp4box_disabled_handler_registration_contract_passed_ready_for_handler_registration_contract_negative_tests'
const priorMergeSha = '9b05e74ecc8fc5a4c9097e8930a6bb7536d369d4'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-GUARDED-HANDLER-REGISTRATION-PLAN-1'

const packetFiles = [
  `${packetDir}/gpac-mp4box-disabled-handler-registration-contract-negative-tests-decision.json`,
  `${packetDir}/gpac-mp4box-disabled-handler-registration-contract-negative-tests-decision.md`,
...
if (packageJson.scripts?.['tracka:gpac-mp4box-disabled-handler-registration-contract-negative-tests:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-disabled-handler-registration-contract-negative-tests-diagnostics.mjs') fail('missing diagnostics script')
if (packageJson.scripts?.['smoke:tracka-gpac-mp4box-disabled-handler-registration-contract-negative-tests'] !== 'tsx server/smoke/tracka-gpac-mp4box-disabled-handler-registration-contract-negative-tests-smoke.ts') fail('missing smoke script')

const decision = json(`${packetDir}/gpac-mp4box-disabled-handler-registration-contract-negative-tests-decision.json`)
...
if (decision.prior?.disabledHandlerRegistrationContract !== priorDecision) fail('prior decision drift')
if (decision.prior?.disabledHandlerRegistrationContractMergeSha !== priorMergeSha) fail('prior merge drift')
...
if (readiness.readyForGuardedHandlerRegistrationPlan !== true) fail('next readiness drift')
...
for (const pattern of [/Product-ready(?: end-to-end)? local OSS tools:\s*`?[1-9]/i, /readyForExecutableHttpHandler"\s*:\s*true/i, /readyForRouteExecution"\s*:\s*true/i, /"routeExecution"\s*:\s*true/i, /"workerExecution"\s*:\s*true/i, /"gpacMp4boxExecution"\s*:\s*true/i]) {
  if (pattern.test(docsCorpus)) fail(`forbidden doc claim ${pattern}`)
}

const smokeText = read('server/smoke/tracka-gpac-mp4box-disabled-handler-registration-contract-negative-tests-smoke.ts')
for (const text of ['all_rejected_input_cases_block', 'all_runtime_attempt_cases_block', 'all_delivery_attempt_cases_block', 'operator_confirmation_drift_blocks']) {
...
for (const file of changedFiles) {
  ...
  if (/package-lock\.json|^docker\/|^supabase\/|^src\/|^dist|^node_modules\/|\\.dockerignore$|\\.(mp4|mov|mkv|webm|srt|png|jpg|wav|mp3)$/i.test(file)) fail(`forbidden changed path ${file}`)
  if (/^server\//.test(file) && file !== 'server/smoke/tracka-gpac-mp4box-disabled-handler-registration-contract-negative-tests-smoke.ts') fail(`unexpected server path ${file}`)
}
...
console.log('Disabled handler-registration contract negative tests passed')
