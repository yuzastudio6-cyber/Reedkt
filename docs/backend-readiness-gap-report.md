...
| Worker/job queue runtime missing | Partially fixed | The canonical private package queue is checksum-protected and restart-readable. Its server-selected claim plus outbox insert, accepted-worker completion, and accepted-worker pre-commit failure each use cooperative cross-process same-host write-ahead commits with real process-exit recovery and exact replay. Completion and failure are mutually exclusive; failure derives bounded retry, exhaustion, or user review and rejects post-commit retry. Terminal receipts bind attempt-level internal-cost evidence while excluding customer price/credits/service fee/wallet/billing authority. The distributed database transaction, live Cloud Tasks/Cloud Run dispatch and terminal callbacks, worker execution, and production operations remain open. |
