...
test.describe('Project Edit Brief Marker Chat live Qwen beta', () => {
  ...
  test('routes Marker Chat through backend Qwen 3.7 without exposing secrets or starting execution', async ({ page }) => {
    ...
    await expect(page.getByTestId('project-edit-brief-marker-chat-runtime')).toContainText('Qwen beta route configured')
    await expect(page.getByTestId('project-edit-brief-marker-chat-readiness')).toContainText('Qwen beta ready', { timeout: 45_000 })
    ...
    await expect(page.getByTestId('project-edit-brief-marker-chat-status')).toContainText('Qwen 3.7 understood this marker', { timeout: 60_000 })
