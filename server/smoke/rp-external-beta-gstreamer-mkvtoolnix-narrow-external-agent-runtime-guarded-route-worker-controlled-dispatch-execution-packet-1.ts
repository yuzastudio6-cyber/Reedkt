...
const confirmationEnv =
  'REEDITPRO_CONFIRM_GSTREAMER_MKVTOOLNIX_NARROW_ROUTE_WORKER_CONTROLLED_DISPATCH_EXECUTION'
...
const sourcePlanningMergeSha = 'c4228c3939f30fae3646d7f477207d64d566309c'
const sourcePlanningDecision = 'completed_gstreamer_mkvtoolnix_narrow_registered_noop_source_route_worker_controlled_dispatch_planning'
const sourcePlanningReadiness = 'ready_for_guarded_registered_noop_source_route_worker_controlled_dispatch_execution_packet'
const nextMilestone =
...
type ControlledDispatchEnvelope = {
  executionPacketMode: 'registered_noop_source_route_worker_controlled_dispatch_contract_only'
  dispatchMode: 'confirmation_gated_controlled_dispatch_contract_without_tool_or_media_execution'
  dispatchId: string
    ...
    JSON.stringify(
      {
        ...
        execution: 'blocked_confirmation_absent_no_controlled_dispatch_execution_packet',
        blocker,
...
function buildEnvelope(input: GstreamerMkvtoolnixNarrowRegisteredNoopSourceInput): ControlledDispatchEnvelope {
  ...
  const dispatchId = 'controlled-dispatch-contract-gstreamer-mkvtoolnix-narrow-1'
  const dispatchIdempotencyKey = [
    'gstreamer-mkvtoolnix',
    'narrow-route-worker-controlled-dispatch-execution-packet-1',
    validated.sanitizedSource.routeSourceId,
    validated.sanitizedSource.sourceIdempotencyKey,
    dispatchId,
  ].join(':')
...
function validateEnvelope(envelope: Record<string, unknown>): string[] {
  ...
  if (envelope.executionPacketMode !== 'registered_noop_source_route_worker_controlled_dispatch_contract_only') {
    failures.push('invalid_execution_packet_mode')
  }
  if (envelope.dispatchMode !== 'confirmation_gated_controlled_dispatch_contract_without_tool_or_media_execution') {
    failures.push('invalid_dispatch_mode')
  }
...
const outputDir = path.join(
  '/tmp/reeditpro-rp-external-beta-gstreamer-mkvtoolnix-narrow-route-worker-controlled-dispatch-execution-packet-1',
  runId,
)
const envelopePath = path.join(outputDir, 'controlled-dispatch-execution-packet-envelope.json')
const reportPath = path.join(outputDir, 'controlled-dispatch-execution-packet-report.json')
const manifestPath = path.join(outputDir, 'controlled-dispatch-execution-packet-manifest.json')
...
if (positiveFailures.length > 0) failClosed('blocked_controlled_dispatch_execution_packet_positive_validation_failed', { positiveFailures })
...
const negativeCases = [
  ...
  { name: 'reject_docker_ffmpeg', patch: { dockerExecution: true, ffmpegFfprobeExecution: true } },
...
writeJson(envelopePath, {
  packet,
  runId,
  sourcePlanningMergeSha,
  sourcePlanningDecision,
  sourcePlanningReadiness,
...
writeJson(reportPath, {
  ...
  execution,
  sourceChain: {
    controlledDispatchPlanningMergeSha: sourcePlanningMergeSha,
    controlledDispatchPlanningDecision: sourcePlanningDecision,
    controlledDispatchPlanningReadiness: sourcePlanningReadiness,
  },
  confirmation: {
  ...
  commandMatrix: [
    ...
    {
      command: 'local controlled dispatch contract envelope validation',
      status: 'passed',
    },
    {
      command: 'negative fail-closed controlled dispatch blocker matrix',
      status: 'passed',
    },
  ],
  acceptedEnvelope: {
