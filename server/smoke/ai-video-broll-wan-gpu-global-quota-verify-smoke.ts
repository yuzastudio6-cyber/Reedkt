...
assert.equal(
  spec.nextActionIfQuotaCleared.includes(
    '10S-NO-GPU-IAP-SSH-CANARY-BOUNDED-RUNNER-EXECUTE',
  ),
  true,
)
assert.equal(spec.allowedReadOnlyCommands.length, 7)
assert.equal(resultSpec.decision, 'ai_video_broll_wan_gpu_global_quota_verified_10s_fixed_canary_execute_required')
...
assert.equal(resultSpec.noIdleLifecycleGate.readyForBroll10sFixedCanaryExecutePrompt, true)
...
assert.equal(resultSpec.readyForBroll10sFixedCanaryExecutePrompt, true)
...
assert.equal(live.readyForBroll10sFixedCanaryExecutePrompt, live.quotaSufficientForOneL4Vm)
