...
| Zone | Region | Status | `g2-standard-4` visible | `nvidia_l4` visible | Regional L4 quota | Regional CPU quota | Regional SSD quota | 10F disposition |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| `us-east1-d` | `us-east1` | previously `UP` | previously true | previously true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | rejected_recent_stockout |
| `us-west4-a` | `us-west4` | `UP` | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | selected |
| `us-west4-c` | `us-west4` | visible | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | available_cross_zone_fallback |
| `northamerica-northeast1-b` | `northamerica-northeast1` | visible | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | available_later_cross_region_fallback |
| `northamerica-northeast2-a` | `northamerica-northeast2` | visible | true | true | limit `1`, usage `0` | limit `100`, usage `0` | limit `500`, usage `0` | available_later_cross_region_fallback |
...
- The full `us-east1` transfer-proof zone set now has cleanup-verified stockouts before any VM existed.
- `us-west4-a` is a visible, untried L4 zone with `g2-standard-4`, `nvidia_l4`, zone status `UP`, and unused regional L4 quota.
- `us-west4` was already part of the earlier candidate inventory and remains the nearest untried US-region capacity option in the checked set.
...
- A delayed retry is not selected because an untried no-idle L4 transfer target remains visible and quota-compatible.
...
- `us-west4-a` zone status;
- `g2-standard-4` visibility in `us-west4-a`;
- `nvidia_l4` visibility in `us-west4-a`;
...
## Future 10G Guardrails

The next execution prompt may attempt exactly one no-public-IP `g2-standard-4` VM with one NVIDIA L4 in `us-west4-a` only after repeated preflight passes.
