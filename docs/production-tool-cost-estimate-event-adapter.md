...
## Conditional Private Playwright Evidence

RP-TOOLEXEC-02 uses the same event adapter only after the fixed-template Playwright runner produces and validates real private PNG bytes. Its event is an `infrastructure_runtime` event for Playwright, is idempotently bound to the manifest fingerprint, operation instance, approved source-spec hash, and output hash, and always remains nonbillable with no service fee, wallet mutation, or settlement. Planning-only browser capture entries emit no actual-work event. See `private-playwright-capture-policy.md` and `smoke:private-playwright-capture`.
