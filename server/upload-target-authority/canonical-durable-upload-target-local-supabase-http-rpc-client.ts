...
import {
  CANONICAL_DURABLE_UPLOAD_TARGET_RPC_REGISTRY,
  type CanonicalDurableUploadTargetRpcClient,
  type CanonicalDurableUploadTargetRpcClientResult,
} from './canonical-durable-upload-target-state-rpc-adapter'

export const CANONICAL_DURABLE_UPLOAD_TARGET_LOCAL_HTTP_CLIENT_VERSION =
  'canonical-durable-upload-target-local-http-client-v1' as const

const allowedFunctions = new Set<string>(
  Object.values(CANONICAL_DURABLE_UPLOAD_TARGET_RPC_REGISTRY.functions),
)
const MAX_RESPONSE_BYTES = 2 * 1024 * 1024
const LOCAL_HTTP_TIMEOUT_MS = 30_000

export interface CanonicalDurableUploadTargetLocalHttpClient
  extends CanonicalDurableUploadTargetRpcClient {
  readonly schemaVersion: typeof CANONICAL_DURABLE_UPLOAD_TARGET_LOCAL_HTTP_CLIENT_VERSION
  ...
  readonly rawUploadCredentialAcceptedOrReturned: false
  readonly productionAuthority: false
}

export function createCanonicalDurableUploadTargetLocalHttpClient(input: {
  readonly endpointOrigin: string
  readonly anonKey: string
  readonly authenticatedAccessToken: string
  readonly localInternalSigningSecret: string
}): CanonicalDurableUploadTargetLocalHttpClient {
  ...
  return Object.freeze({
    schemaVersion: CANONICAL_DURABLE_UPLOAD_TARGET_LOCAL_HTTP_CLIENT_VERSION,
    ...
    async rpc(
      functionName: string,
      parameters: Parameters<CanonicalDurableUploadTargetRpcClient['rpc']>[1],
      ...
      if (
        ...
        parameters.p_contract_version !== 'canonical-durable-upload-target-authority-port-v1'
...
export function assertCanonicalDurableUploadTargetLocalHttpClientIsNotProduction(
  client: CanonicalDurableUploadTargetLocalHttpClient,
): never {
  void client
...
function invalid(reason: string): never {
  throw new ApiError(
    ...
    'The canonical upload-target local Postgres proof client is unavailable or unsafe.',
    503,
    {
      reason,
      endpointClass: 'canonical_loopback_only',
      remoteMutationAttempted: false,
      rawUploadCredentialAcceptedOrReturned: false,
