...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_retry_gate_recorded_attempt_approval_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58DT-PRIVATE-INFERENCE-RETRY-ATTEMPT-APPROVAL: approve one bounded approved-fixture private inference retry after gate verification, no inference/no mutation'
}
...
export function getQwenVlPlannerRoutingUiData(): QwenVlPlannerRoutingUiData {
  return {
    ...
    privateInvokeClient: {
      ...
      currentBlocker:
        'Controlled private invoke auth works, the dedicated Direct VPC private route subnet is configured, the no-model CPU-only caller source is defined, the controlled caller job is deployed, one caller contract smoke observed the expected fail-closed response with inference disabled, runtime readiness review is recorded, structured-output retry/result review passed, private runtime and approved worker integration reviews are accepted, fail-closed backend dispatch and controlled dispatch dry-run are recorded, backend runtime persistence has advanced through local harness validation, active migration creation, active migration validation, deploy planning, target-class deploy approval, deploy-execution metadata preflight, read-only migration-history reconciliation, local history adoption to remote version 20260628000100, adopted active-migration local validation, remote satisfaction/no-deploy review, persisted worker dispatch readiness review, controlled persisted worker dispatch smoke planning, controlled persisted worker dispatch smoke execution, controlled persisted worker dispatch smoke result review, controlled persisted worker dispatch runtime planning, controlled persisted worker dispatch runtime implementation, controlled persisted worker dispatch runtime smoke planning, controlled persisted worker dispatch runtime smoke execution, controlled persisted worker dispatch runtime smoke result review, controlled persisted worker dispatch runtime approval planning, controlled persisted worker dispatch runtime approval decision, controlled persisted worker dispatch runtime execution plan, controlled persisted worker dispatch runtime execution approval, controlled persisted worker dispatch runtime execution preflight, controlled approved-fixture runtime execution attempt result, controlled attempt result review, controlled real-dispatch approval planning, controlled real-dispatch approval decision, controlled real-dispatch execution plan, controlled real-dispatch execution approval, controlled real-dispatch execution preflight, controlled real-dispatch execution attempt result, controlled real-dispatch attempt result review, controlled real-dispatch transport dependency enablement plan, controlled real-dispatch transport dependency enablement approval, controlled real-dispatch transport dependency enablement implementation, controlled real-dispatch transport dependency enablement preflight, controlled real-dispatch transport dependency enablement execution plan, controlled real-dispatch transport dependency enablement execution approval, controlled real-dispatch transport dependency enablement execution preflight, controlled real-dispatch transport dependency enablement execution attempt result, controlled real-dispatch transport dependency enablement execution attempt result review, controlled real-dispatch transport readiness plan, controlled real-dispatch transport readiness approval, controlled real-dispatch transport preflight plan, controlled real-dispatch transport preflight approval, controlled real-dispatch transport preflight, controlled real-dispatch transport attempt approval, a fail-closed controlled real-dispatch transport attempt result, the transport attempt result review, the controlled approved-fixture inference plan, approval, static preflight, attempt approval, blocked 58DB attempt result, the 58DC persisted job/lease bridge plan, the 58DD fail-closed persisted job/lease bridge implementation, the 58DE bridge result review, the 58DF private invoke readiness review, the 58DG approved-fixture private invoke execution plan, the 58DH approval, the 58DI static preflight, the 58DJ attempt approval, the 58DK fail-closed approved-fixture private invoke attempt result, the 58DL attempt result review, the 58DM private inference plan, the 58DN private inference approval, the 58DO private inference preflight, the 58DP private inference attempt approval, the 58DQ private inference attempt result, the 58DQ auth-refresh result, the 58DR retry plan, and the 58DS retry gate are recorded. Read-only auth refresh, Cloud Run service describe, and caller job describe now pass in this shell. User-facing readiness is now blocked on bounded approved-fixture private inference retry attempt approval before any Cloud Run invocation, job execution, identity token fetch, private request, model load, inference, generated assets, signed URLs, beta, or production.',
      boundaryNotes: [
        ...
        'The retry gate is recorded; the next runtime step is explicit bounded approved-fixture private inference retry attempt approval. Generated assets, published artifacts, signed URLs, beta, and production remain blocked now.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
