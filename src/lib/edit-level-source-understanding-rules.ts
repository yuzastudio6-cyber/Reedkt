...
export const EDIT_LEVEL_SOURCE_UNDERSTANDING_LAYER_DEFINITIONS: EditLevelSourceUnderstandingLayerDefinition[] = [
  ...
  layer('visual_intelligence_segments', 'Visual understanding segments', 'future visual understanding route', 'Visual/video specialist context for key moments, scenes, visible text, objects, and layout.', 'provider_required'),
  layer('visual_intelligence_marker_windows', 'Visual marker windows', 'future visual marker route', 'Visual context around Edit Brief markers and user timestamp questions.', 'provider_required'),
  ...
  layer('qwen3_reasoning_context', 'Main planning context', 'future planning prompt context', 'Future prompt-context package for the main reasoning brain.', 'runtime_disabled'),
]
...
export function createEditLevelSourceUnderstandingLayerRoute(input: {
  ...
  return {
    ...
    displayName: hideInternalToolNamesInCopy(definition.displayName),
    requiredness: input.requiredness,
    status: input.status ?? definition.defaultStatus,
    sourceTool: definition.sourceTool,
    purpose: hideInternalToolNamesInCopy(input.purpose),
    fallback: hideInternalToolNamesInCopy(input.fallback),
    userFacingSummary: hideInternalToolNamesInCopy(input.userFacingSummary),
    technicalNotes: [
...
export function createEditLevelQwenContextPolicy(
  ...
  if (level === 'normal') {
    return {
      ...
      userFacingSummary: 'Future planning context stays compact: source summary, marker note, export settings, and targeted context only.',
      mockOnly: true,
    }
  }
  ...
  if (level === 'premium') {
    return {
      ...
      userFacingSummary: 'Future planning context includes source summary, transcript/visual/audio key moments, marker context, Preference DNA, and QA warnings.',
      mockOnly: true,
    }
  }
  ...
  return {
    ...
    userFacingSummary: 'Future planning context uses scene-level source understanding, transcript, visual, audio, graphic context, Preference DNA, strict QA warnings, and planning priority policy.',
    mockOnly: true,
  }
}
...
export function createEditLevelSourceUnderstandingBoundarySummary(): string[] {
  return [
    'Source understanding routing only; no activities execute.',
    'No AI planning, visual review call, technical assembly, asset service, transcript, media extraction, audio analysis, graphic preparation, private processing, render, progress, storage, file-byte, external-fetch, or credit operation runs.',
    'No unavailable activity is claimed to have run; fallbacks and degraded notices are explicit.',
  ]
}
...
function createNormalSourceUnderstandingRoutes(): EditLevelSourceUnderstandingLayerRoute[] {
  ...
  return [
    ...
    route(level, 'visual_intelligence_segments', 'targeted', 'provider_required', 'Normal requests visual context only when a marker or user ambiguity requires it.', 'Use visible-context fallback or ask for clarification.', 'Targeted visual context only when needed.'),
    route(level, 'visual_intelligence_marker_windows', 'targeted', 'provider_required', 'Normal uses marker-window visual context only for visual questions or vague requests.', 'Use marker note and source summary without claiming visual analysis ran.', 'Targeted marker windows.'),
    ...
    route(level, 'qwen3_reasoning_context', 'required', 'runtime_disabled', 'Normal future planning context stays compact and targeted.', 'Use deterministic compact summary; no planning service call.', 'Compact future planning context.'),
  ]
}
...
function createPremiumSourceUnderstandingRoutes(): EditLevelSourceUnderstandingLayerRoute[] {
  ...
  return [
    ...
    route(level, 'visual_intelligence_segments', 'recommended', 'provider_required', 'Premium routes key visual moments where available.', 'Use key-moment local fallback and clarify visual ambiguity.', 'Key visual moments recommended.'),
    route(level, 'visual_intelligence_marker_windows', 'recommended', 'provider_required', 'Premium routes visual summaries around marker windows.', 'Use marker note, source summary, and local key moments.', 'Marker window visual context recommended.'),
    ...
    route(level, 'qwen3_reasoning_context', 'required', 'runtime_disabled', 'Premium future planning context includes source, transcript, visual, audio, marker, Preference DNA, and QA warnings.', 'Use deterministic enhanced summary; no planning service call.', 'Enhanced future planning context.'),
  ]
}
...
function createUltraPremiumSourceUnderstandingRoutes(): EditLevelSourceUnderstandingLayerRoute[] {
  ...
  return [
    ...
    route(level, 'visual_intelligence_segments', 'required', 'provider_required', 'Ultra Premium routes scene-level visual analysis when future access exists.', 'Degrade to Premium-safe visual context and show a degraded capability notice.', 'Scene-level visual analysis.'),
    route(level, 'visual_intelligence_marker_windows', 'required', 'provider_required', 'Ultra Premium routes scene-level marker-window context for precise creative and source-truth decisions.', 'Use Premium-safe marker-window fallback and strict clarification.', 'Scene-level marker windows.'),
    ...
    route(level, 'qwen3_reasoning_context', 'required', 'runtime_disabled', 'Ultra Premium future planning context includes scene-level source summary, transcript, visual, audio, graphic context, Preference DNA, strict QA warnings, and priority policy.', 'Use deterministic studio summary; no planning service call.', 'Studio future planning context.'),
  ]
}
...
function createPackage(
  ...
  return {
    ...
    warnings: [
      ...
      'No AI planning, visual understanding, technical assembly, asset service, transcript, media extraction, audio, graphic, backend, render, progress, Supabase, file-byte, external-fetch, or credit operation is executed.',
...
function fallbackPolicyForLevel(level: ReEditProCanonicalEditLevel): string[] {
  if (level === 'normal') {
    return [
      ...
      'Do not claim visual understanding, transcript, media extraction, or audio analysis ran.',
    ]
  }
  ...
  if (level === 'premium') {
    return [
      ...
      'Use source summary and marker notes when visual or transcript context is unavailable.',
      'Do not claim unavailable visual/audio/media backend steps ran.',
    ]
  }
...
function warningsForLevel(level: ReEditProCanonicalEditLevel): string[] {
  ...
  if (level === 'premium') return ['Premium source package, transcript, visual, audio, and media layers are recommended policy only until backend and AI asset gates exist.']
