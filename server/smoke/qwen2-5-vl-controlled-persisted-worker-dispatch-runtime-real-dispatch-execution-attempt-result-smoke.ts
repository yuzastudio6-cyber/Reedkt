...
const DECISION =
  'qwen2_5_vl_controlled_persisted_worker_dispatch_runtime_real_dispatch_execution_attempt_recorded_result_review_required'
const NEXT_PROMPT =
...
function assertFalseRuntimeFlags(flags: JsonRecord) {
  for (const key of [
    'controlledPersistedWorkerDispatchRuntimeRealDispatchExecutionAttemptRequired',
...
assert.equal(
  packageJson.scripts?.[
    'smoke:qwen2-5-vl-controlled-persisted-worker-dispatch-runtime-real-dispatch-execution-attempt-result'
  ],
...
assert.equal(
  result.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchExecutionPreflightDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_EXECUTION_PREFLIGHT.decision,
)
assert.equal(
  result.realDispatchExecutionAttemptSummary
    .controlledPersistedWorkerDispatchRuntimeRealDispatchExecutionPreflightPassed,
  true,
)
assert.equal(
  result.realDispatchExecutionAttemptSummary
...
assert.equal(result.realDispatchExecutionAttemptSummary.approvedFixtureOnly, true)
assert.equal(result.realDispatchExecutionAttemptSummary.generatedLocalFixturePassedClaimed, false)
assert.deepEqual(result.executedAttemptChecks.map((checkResult) => checkResult.id), [
  'realDispatchDefaultAttempt',
  'realDispatchAdapterPreviewAttempt',
...
assert.equal(result.runtimeFlags.realDispatchDefaultAttemptBlocked, true)
assert.equal(result.runtimeFlags.realDispatchAdapterPreviewBlocked, true)
assert.equal(result.runtimeFlags.realDispatchTransportPreviewBlocked, true)
...
assert.deepEqual(
  ...
  [],
  `Forbidden values in controlled persisted real-dispatch execution attempt result: ${forbiddenDataFindings.join('; ')}`,
)
