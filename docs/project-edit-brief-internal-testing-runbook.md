...
8. Confirm Marker Chat shows visual context available or fallback-used without sending raw frames to Qwen 3.7 prompts.
...
Visual Context beta testing must preserve: no full-video upload, no raw frame persistence, no backend file-byte read, no media tools/workers, no render/export, no credits, no Supabase CLI, no migrations, no DeepSeek, and no Qwen 3.7 visual processing.
