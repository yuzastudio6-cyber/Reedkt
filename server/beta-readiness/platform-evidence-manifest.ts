  ...
  return [
    {
      ...
      sourceFiles: [
        ...
        'server/beta-readiness/production-tool-execution-readiness-evidence-store.ts',
    ...
    {
      id: 'production_readiness_evidence_backend_only_deployment',
      label: 'production readiness evidence packets are backend-only in deployment',
      status: 'deployed_evidence_required',
      localProofCommands: ['smoke:production-tool-execution-readiness-evidence-preflight'],
      sourceFiles: [
        'supabase/migrations/20260702221112_production_tool_execution_readiness_evidence_packets.sql',
        'server/beta-readiness/production-tool-execution-readiness-evidence-store.ts',
        'server/routes/beta-readiness-routes.ts',
      ],
      localEvidence: [
        'Migration source exists for public.production_tool_execution_readiness_evidence_packets with workspace-scoped idempotency and backend-only grants.',
        'Production readiness evidence store source fails closed when the production evidence table is missing or unavailable.',
      ],
      remainingEvidence: [
        'Apply the production readiness evidence packet migration in staging or production.',
        'Verify backend service-role readback for production_tool_execution_readiness_evidence_packets before accepting paid-production readiness evidence.',
      ],
      nextSafeAction: 'Run production readiness evidence preflight and deployed service-role readback for the production readiness evidence packet table.',
      clearsPlatformGate: false,
      requiresDeployedEvidence: true,
      requiresOwnerApproval: false,
    },
