...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge implementation: blocked, implementation required',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge implementation | blocked controlled persisted worker dispatch runtime real-dispatch persisted job lease bridge implementation required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 114)
assert.equal(
...
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeImplementationRequired, true)
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeImplemented, false)
