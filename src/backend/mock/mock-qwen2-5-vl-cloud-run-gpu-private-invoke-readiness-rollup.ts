...
export const QWEN2_5_VL_CLOUD_RUN_GPU_PRIVATE_INVOKE_READINESS_ROLLUP = {
  ...
  decision:
    'qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_plan_recorded_runtime_implementation_required',
  ...
  readinessGates: [
    ...
    },
    {
      ...
      status: 'blocked_controlled_persisted_worker_dispatch_runtime_implementation_required',
      evidence: [
        'Controlled persisted worker dispatch runtime plan is recorded.',
        'The next gate must implement the planned backend-only runtime fail-closed before any real dispatch attempt.',
      ],
      missingEvidence: [
        'Implement the controlled persisted Qwen worker dispatch runtime with approved snapshot intake, idempotency, transactional lease/claim semantics, Qwen adapter/envelope composition, private invoke transport still disabled, sanitized events, QA/audit/cost/credit placeholders, and cleanup gates.',
  ...
  blockedUntil: [
    'controlled_persisted_worker_dispatch_runtime_implementation_required',
