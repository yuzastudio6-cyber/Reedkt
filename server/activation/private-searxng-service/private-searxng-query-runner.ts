...
export async function runPrivateSearxngControlledQuery(input: {
  serviceUrl: string
  identityToken: string
}): Promise<{ response: PrivateSearxngQueryResponse; queriedAt: string; blockers: string[]; warnings: string[] }> {
  ...
  const response = await fetch(url, {
    headers: {
      Authorization: `Bearer ${input.identityToken.trim()}`,
      'User-Agent': 'ReeditPro-Phase49F-Private-SearXNG-Validation/1.0',
    },
    signal: AbortSignal.timeout(30_000),
  })
  if (!response.ok) {
    throw new Error(`Private SearXNG controlled query failed with HTTP ${response.status}.`)
  }
  ...
  if ((json.results?.length ?? 0) > privateSearxngServiceConfig.maxResults) {
    warnings.push(`Private SearXNG returned ${json.results.length} results; Phase 49F will normalize only the first ${privateSearxngServiceConfig.maxResults}.`)
  }
