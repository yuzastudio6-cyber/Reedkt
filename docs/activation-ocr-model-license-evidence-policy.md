...
Phase 37B selects only `PP-OCRv5_mobile_det_infer.tar`, `PP-OCRv5_mobile_rec_infer.tar`, and `ppocrv5_dict.txt` for `paddle3.0.0-mobile-safe-zone-v1`. It records official PaddleOCR docs/repository evidence, Apache-2.0 project evidence, exact source URLs, computed checksums, and private GCS upload verification. Project/package license evidence is not enough to permit production legal approval, runtime auto-download, OCR inference, public output, or unpinned model assets.

If asset-specific evidence cannot be tied back to official PaddleOCR/Paddle model sources during any future replacement execution, the report must block and Phase 37C readiness must be reset until reviewed evidence passes again.
