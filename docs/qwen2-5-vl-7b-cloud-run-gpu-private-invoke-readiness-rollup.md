...
| Controlled persisted worker dispatch runtime real-dispatch transport dependency enablement approval | blocked controlled persisted worker dispatch runtime real-dispatch transport dependency enablement approval required | The transport dependency enablement plan is recorded and requires explicit approval before any backend lease, adapter, transport, response handling, QA/audit/cost/credit, or cleanup dependency can be enabled. | Approve controlled dependency enablement before any real runtime dependency can be enabled. |
...
The next action is controlled persisted worker dispatch runtime real-dispatch transport dependency enablement approval. That future prompt must keep generated assets, public artifacts, signed URLs, beta, production, and `generated_local_fixture_passed` blocked now.
