...
- Result: all 21 AI graphics tools and all 12 product-facing capabilities are visible to the external agent as planning/select/rank/eliminate candidates, with 21 callable-scope candidates, one request-admission candidate, and 21 route-mount-ready tools accepted as evidence. The gate now understands the CPU/static saved queue-write smoke proof and saved worker claim/dispatch smoke proof packets, but the checked-in packet still has `cpuStaticNonProductionServiceRoleQueueWriteSmokeProofAcceptedWithProvidedEvidenceTools=0`, `cpuStaticWorkerClaimAndDispatchSmokeProofAcceptedWithProvidedEvidenceTools=0`, and `toolExecutionDryRunProofRequiredTools=0`. The default route-mount flag remains off with `apiRouteMountedNow=false` and `apiRouteMountedNowTools=0`; the gate still returns `executionAllowedNow=false` and exits `2` in `--require-go` mode.
...
- Result: the probe covers all 21 AI graphics tools. It reports `externalAgentRouteExecutableNowTools=21`, with six CPU/static controlled tools, seven browser-runtime controlled tools, and eight GPU/model tools callable in explicit local/mock route mode through the controlled on-demand adapter. The GPU/model tools still expose future runtime-boundary blockers, but the route no longer reports them as admission-blocked. `gpuModelRuntimeAdmissionBlockedTools=0`, `gpuModelRuntimeAdmissionEvaluatedFailClosedTools=8`, `gpuModelToolsReadyForExecutionAfterCurrentEvidence=8`, `modelWeightManifestRequiredTools=5`, `gpuRuntimeShouldStartNowTools=0`, `workerDispatchPerformedTools=0`, `publicArtifactCreatedTools=0`, and `signedUrlCreatedTools=0`.
- Runtime/beta/production: this probe does not execute tools, dispatch Workers, call providers/models, start browser/WebGL/canvas or GPU runtime, download model weights, mutate Supabase/GCS, create signed URLs, create public artifacts, unlock external beta traffic, or unlock production. `externalAgentCanExecuteSomeToolsNow=true`, `agentCanExecuteControlledCpuStaticAndBrowserRuntimeToolsNow=true`, `agentCanExecuteAll21ToolsNow=true`, and `agentCanExecuteGpuModelToolsNow=true`, while `gpuRuntimeShouldStartNow=false`, `runtimeReadyNow=false`, `externalBetaReadyNow=false`, and `productionReadyNow=false`.
...
- Result: the real Express app returns `200` for all 21 AI graphics tool calls through the canonical route. The six CPU/static tools and seven browser-runtime tools execute their controlled adapters and return private metadata plus SHA-256 hashes. The eight GPU/model tools invoke the controlled on-demand adapter without starting local GPU/model runtime by default. `controlledRouteHttp200Tools=21`, `controlledRouteAdapterInvokedTools=21`, `controlledRouteAdapterExecutedTools=13`, `cpuStaticControlledRouteExecutedTools=6`, `browserRuntimeControlledRouteExecutedTools=7`, `gpuModelControlledRouteInvokedTools=8`, `localPackageExecutionPerformedTools=13`, `localGpuModelRuntimeExecutionPerformedTools=0`, `gpuRuntimeShouldStartNowTools=0`, `workerDispatchPerformedTools=0`, `providerRuntimePerformedTools=0`, `publicArtifactCreatedTools=0`, and `signedUrlCreatedTools=0`.
- Runtime/beta/production: this unlocks controlled route execution readiness for all 21 tools only. It does not dispatch Workers, call providers/models, mutate Supabase/GCS, create signed URLs, create public artifacts, download model weights, unlock paid production, or mark runtime/beta/production ready. `agentCanExecuteToolsNow=true`, `agentCanExecuteAll21ToolsNow=true`, `agentCanExecuteGpuModelToolsNow=true`, `gpuRuntimeOnDemandOnly=true`, and `noIdleGpuRuntimeApproved=true`, while `gpuRuntimeShouldStartNow=false`, `runtimeReadyNow=false`, `externalBetaReadyNow=false`, and `productionReadyNow=false`.
