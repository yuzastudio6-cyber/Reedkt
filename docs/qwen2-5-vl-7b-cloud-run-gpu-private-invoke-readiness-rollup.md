...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_blocked_direct_vpc_route_config_required`.

This packet rolls up the current Qwen2.5-VL 7B ReeditPro stack-tool state. It confirms that the registry, production readiness metadata, private-invoke mock route, frontend-safe client, chat-native UI surfacing, guarded read-only Cloud Run auth/IAM reverify, controlled private invoke smoke plan, non-key impersonation token-path runner, narrow authz bindings, routing fix, internal caller harness plan, internal caller deploy preflight, and internal route approval are in place. The latest controlled smoke minted an identity token and sent one authenticated contract request, but the response was HTTP `404` instead of the expected fail-closed contract JSON. The routing fix records the more precise blocker: the service ingress is `internal-and-cloud-load-balancing`, so a direct local generated-host request is not valid. The internal caller harness plan selects a CPU-only Cloud Run Job with Direct VPC egress as the preferred no-idle-GPU future path. The deploy preflight found that the preferred route is not ready because Private Google Access is disabled on the inspected default subnet, and the route approval conditionally accepts Direct VPC egress with Private Google Access for a future configuration prompt only.
...
- controlled private invoke smoke execution: blocked until Direct VPC private route configuration
...
| Controlled private invoke smoke execution | blocked Direct VPC route config required | Read-only auth/IAM reverify passed, narrow TokenCreator and Run Invoker bindings were applied, the smoke runner reviewed cost posture, minted an identity token without printing or storing it, and sent one authenticated contract request. The response was HTTP `404` with no expected JSON contract, classified as `private_invoke_response_unexpected`. The routing fix records ingress `internal-and-cloud-load-balancing`. The internal caller harness plan selects a CPU-only Cloud Run Job with Direct VPC egress as the preferred no-idle-GPU future path. The deploy preflight records Private Google Access disabled on the inspected default subnet, and the internal route approval accepts Direct VPC plus Private Google Access only for a future config prompt. | Configured Direct VPC private route or approved equivalent private path, CPU-only caller source/image, deployed controlled caller harness, plus a successful controlled contract response with inference disabled. |
...
The next action is configuring the approved Direct VPC private route or an approved equivalent private path for the controlled private invoke smoke. That future prompt must not relax public ingress. It must preserve the same single-request/no-retry/no-token-printing/no-service-URL-storage/no-inference/no-assets/no-credits/no-beta/no-production posture.
