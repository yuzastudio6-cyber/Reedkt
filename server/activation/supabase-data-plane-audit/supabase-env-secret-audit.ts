...
export function buildSupabaseEnvSecretAudit(source: NodeJS.ProcessEnv = process.env): SupabaseEnvSecretAudit {
  ...
  if (!hasUrl || !hasServiceRole) warnings.push('Remote activity audit cannot run unless SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are available to the backend execution environment.')
  ...
  return {
    ...
    remoteAuditCredentialReady: hasUrl && hasServiceRole,
    remoteAuditCredentialBlocker: hasUrl && hasServiceRole ? undefined : 'Missing backend read-only audit credentials: SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.',
    blockers,
    warnings,
  }
}
