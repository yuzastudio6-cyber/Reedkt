import { runVlmStructuredOutputRecovery } from '../activation/vlm-structured-output'
...
if (!execute) {
  console.log([
    'Phase 39C-Q-SO VLM structured-output enforcement CLI',
  ...
  const result = await runVlmStructuredOutputRecovery({
    execute: true,
    keepTemp,
    runId,
    safeArtifactDir,
  ...
  if (json) console.log(JSON.stringify(result, null, 2))
    ...
    console.log([
      'Phase 39C-Q-SO VLM structured-output enforcement completed with recorded status.',
      `Run: ${result.runId}`,
