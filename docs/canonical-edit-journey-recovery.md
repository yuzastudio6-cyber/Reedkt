...
The presentation adapter projects only user-facing status, bounded preparation counts, plan/estimate summaries when applicable, the persisted review decision when applicable, exact private-review media/decision authority for the dedicated client, and the next safe explanation. The review client consumes that authority without rendering routes, hashes, record identifiers, filesystem/storage details, provider/tool names, credentials, signed URLs, or execution authority. Returned `nextAction` metadata is validated; only separately reviewed, stage-specific clients may perform the corresponding action.
...
This recovery bridge remains read-only and never executes its returned action. Separate reviewed local/private clients now cover canonical planning-handoff submission, server-owned candidate persistence and plan presentation, exact plan/estimate approval, exact approved-snapshot package request, backend-owned private preparation, integrity-verified current-review playback, exact acceptance or structured revision, immutable history reopening, and exact bounded revision continuation into replacement-plan presentation and fresh approval. Every mutation reloads canonical authority on the backend and returns a bounded receipt; the browser never supplies jobs, tools, artifact IDs, paths, credentials, providers, prices, credit actions, or release authority. A recorded revision refreshes the journey to `revision_requested`, returns the revision direction to Chat, clears stale approval/review surfaces, and explicitly requires a fresh plan, estimate, approval, and review. For an exactly representable source-and-caption revision, the backend reloads and injects revision authority, preserves the prior snapshot/review and locked preferences, and presents only plan version `prior + 1`; structural preference changes and rich-plan revision compilation remain gated. Public delivery remains blocked. See `docs/canonical-plan-approval-coordinator.md`, `docs/canonical-private-review-decision.md`, and `docs/canonical-revision-plan-browser-continuation.md`.
