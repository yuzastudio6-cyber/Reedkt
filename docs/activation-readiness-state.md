...
- next activation work: Phase 37F Track B caption/render runtime hook planning only, using Phase 37E metadata contracts. Arbitrary media, broad OCR, production, beta, RNNoise active routing, Demucs download/runtime, providers, public output, Revideo, FILM, slow motion, raw frame upload, overlay upload, Cloud Run, Docker push, GPU jobs, render execution, OCR runtime execution, and Track A remain blocked
