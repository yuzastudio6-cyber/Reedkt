  ...
  return createCanonicalGpuWorkerOperationRuntimePort({
    ...
    async execute(input) {
      if (
        ...
        throw notReady(
          'canonical_gpu_worker_sam2_subprocess_request_invalid',
        )
      }
...
async function runFixedSubprocess(input: {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      ['-I', '-B', CANONICAL_GPU_WORKER_SAM2_RUNNER],
      {
        cwd: '/tmp',
    ...
    child.once('error', () => {
      ...
      settled = true
      reject(notReady(
        'canonical_gpu_worker_sam2_fixed_subprocess_unavailable',
      ))
    })
    child.once('close', (exitCode) => {
      ...
      if (captureExceeded) {
        reject(notReady(
          'canonical_gpu_worker_sam2_subprocess_capture_exceeded',
        ))
        return
      }
