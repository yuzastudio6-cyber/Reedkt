...
| Source upload | `private_executable` for the internal test path | Authenticated upload intent, private source artifact, checksums, source-order lineage, project-scoped authority revision/checksum, cross-project non-invalidation, and same-project change invalidation. Local and GCS source/reference probing reopen bytes through the storage adapter, bind GCS to the verified generation/ETag, stage through random scope-hashed create-only `0700`/`0600` private attempts, enforce exact size plus SHA-256, and remove only the exact owned attempt identity before finalization returns. A bounded inspect-first local reconciler covers stale private attempts only with stopped request serving, exclusive-root authority, hard limits, revalidation, and aggregate-only evidence. Integrity contradictions fail the intent with no ready authority; operational failures remain retryable. | The reconciler is single-process/local only, not shared-storage or production retention. Real-user storage, deployed bucket policy, GCS orphan lifecycle, malware scanning, parser/subprocess isolation, privacy operations, and signed delivery remain gated. |
...
> ReeditPro has deterministic planning and canonical approval authority plus 50 exact private tool lifecycles. Its authenticated local/private backend can bind finalized uploads and current planning inputs into exact plan-publication authority, compile an explicitly described grouped controlled-tool node into atomic canonical jobs, freeze every required tool through the server proof catalog before approval, then execute a bounded original review and revision pass through immutable plans, synthetic reservation reconciliation, independent final QA, accepted second review, and restart-safe current/superseded history download. Browser consumption, deployed real-user infrastructure, external beta, public delivery, live billing, Supabase/RLS tenancy, distributed workers, and production promotion remain gated.
...
15. **Backend bridge and recovery complete through private-review acceptance; named-edit planning handoff and bounded plan presentation now connected:** bind the browser journey to the authenticated canonical planning handoff, content-addressed publication-request candidate, server-owned plan-presentation coordinator, and canonical journey recovery route without reviving legacy caller-authored authority. The browser saves exact rich-plan components but requests presentation only for the proven private source-and-caption graph; missing execution representation remains visibly gated. The plan-presentation route persists the candidate, reloads it server-side, derives internal idempotency from the immutable candidate hash, and invokes the protected publisher without returning or accepting an internal token. Concurrent exact presentations converge on one presented plan; no approval, snapshot, credit reservation, job, tool, provider, render, or delivery authority is created. Submit/inspect, latest recovery, internal publish-from-candidate, plan-to-snapshot recovery, package execution, monotonic in-progress/blocked checkpoint recovery, restart-safe required-work completion, exact review-assembly readiness, first-review readiness, revision request, replacement-review readiness, accepted-review recovery, current/superseded private-history reopening, and exact-next-action state pass local/private canonical evidence. The named-edit chat strictly parses the bounded responses and never renders backend routes, hashes, private paths, credentials, or tool identities. Canonical approval, package request, execution advancement, review assembly/decision, revision submission, and history opening remain separate frontend mutation milestones.
