...
export function createEditLevelSourceUnderstandingSummaryModel(
  ...
  return {
    ...
    highlights: [
      ...
      'No source-understanding tools execute from this selection.',
    ],
    mockOnly: true,
  }
}
...
function visualPolicyForLevel(level: ReEditProCanonicalEditLevel): string {
  if (level === 'normal') return 'Targeted visual understanding only when a marker or ambiguity requires it.'
