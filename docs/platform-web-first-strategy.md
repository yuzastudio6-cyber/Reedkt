...
Phase 44A sets ReeditPro's product boundary: web is the first launch path, desktop is a future path, and shared packages support both without weakening the server runtime.
...
Phase 44A prepares the repo so desktop can be added later without forcing a framework decision now. It creates `apps/desktop` as a future boundary only.
