...
Prompt 23S may use only `docs_only`, `local_evidence_recorded`, `pending_human_approval`, `ready_for_staging_dry_run`, `blocked`, and `not_needed` in repo docs. It must not claim `applied_to_staging`, `validated_in_staging`, `approved_for_production`, or `applied_to_production`.
