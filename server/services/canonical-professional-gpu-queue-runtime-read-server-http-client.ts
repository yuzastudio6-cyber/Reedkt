...
import {
  CANONICAL_PROFESSIONAL_GPU_QUEUE_RUNTIME_READ_RPC_REGISTRY,
  type CanonicalProfessionalGpuQueueRuntimeReadRpcClient,
  type CanonicalProfessionalGpuQueueRuntimeReadRpcClientResult,
} from './canonical-professional-gpu-queue-runtime-read-port'

const allowedFunctions = new Set<string>(
  Object.values(CANONICAL_PROFESSIONAL_GPU_QUEUE_RUNTIME_READ_RPC_REGISTRY),
)
const maximumResponseBytes = 16 * 1024 * 1024

export function createCanonicalProfessionalGpuQueueRuntimeReadServerHttpClient(
  input: {
    readonly endpointOrigin: string
    readonly serviceRoleKey: string
  },
): CanonicalProfessionalGpuQueueRuntimeReadRpcClient {
  ...
  return Object.freeze({
    ...
    ): Promise<CanonicalProfessionalGpuQueueRuntimeReadRpcClientResult> {
...
function invalid(reason: string): never {
  throw new ApiError(
    ...
    'The server-only professional GPU queue runtime reread is unavailable.',
    503,
    {
      reason,
