...
| Private render and review journey | `private_executable` for bounded internal fixtures including exact ordered two-source composition; narrow named-edit browser review and exact bounded revision continuation are connected | The canonical graph applies approved trims and captions, renders the exact private H.264/AAC MP4, independently probes that exact dependency, proves lease/artifact lineage, reconciles and replays safely, and supports authenticated private download. The named-edit browser can integrity-check the exact no-store MP4, record exact acceptance or a structured revision, reopen immutable history, and present an exactly representable replacement plan with a fresh estimate and approval. The Remotion image must carry the aggregate SHA-256 of its current reviewed source tree before activation. | The legacy upload-to-render route remains disabled. Rich plan semantics, structural preference-changing revisions, deployed real-user storage/workers, public delivery, and production promotion are not connected. |
