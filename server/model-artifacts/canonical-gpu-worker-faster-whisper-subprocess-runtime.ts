...
export const CANONICAL_GPU_WORKER_FASTER_WHISPER_PYTHON =
  '/opt/reeditpro/gpu-operations/faster-whisper/venv/bin/python' as const
export const CANONICAL_GPU_WORKER_FASTER_WHISPER_RUNNER =
  '/opt/reeditpro/gpu-operations/faster-whisper/runner.py' as const

const MAXIMUM_CAPTURE_BYTES = 128 * 1_024

export function createCanonicalGpuWorkerFasterWhisperSubprocessRuntimePort():
CanonicalGpuWorkerOperationRuntimePort {
  return createCanonicalGpuWorkerOperationRuntimePort({
    ...
    async execute(input) {
      if (
        ...
        throw notReady(
          'canonical_gpu_worker_subprocess_request_invalid',
        )
      }
...
async function runFixedSubprocess(input: {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('error', () => {
      ...
      settled = true
      reject(notReady(
        'canonical_gpu_worker_fixed_subprocess_unavailable',
      ))
    })
    child.once('close', (exitCode) => {
      ...
      if (captureExceeded) {
        reject(notReady(
          'canonical_gpu_worker_subprocess_capture_exceeded',
        ))
        return
      }
...
function notReady(code: string): ApiError {
  return new ApiError('TOOL_NOT_READY', code, 409, {
    requiredGate:
      'canonical_gpu_worker_fixed_subprocess_runtime',
  })
}
