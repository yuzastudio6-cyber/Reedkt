...
This packet records the current-source local no-write evidence bundle for `codex/sound-music-audio-1abc-checkpoint` at `e8821759a10a43a60795accb596b3b83c15f9dfb`.
...
- The hydrated core readiness lane accepted 15 tools with `.reeditpro-tool-readiness-python` and `.reeditpro-tool-readiness-bin` available.
...
- A first current-source local bundle rerun correctly reported only 15 accepted tools while the ignored Signalsmith readiness binary was absent. After rerunning the bounded source-build proof and putting `.reeditpro-tool-readiness-bin` on `PATH`, the bundle accepted all 15 core tools plus libass.
...
- Libass synthetic burn-in QA passed through that same approved image with synthetic-only input video/captions, safe ASS style, temp-root cleanup, and no private or user media. Output checksum was `301ee872212d00c243c8d220a0552157bdebc7766238cc178353089de6b912a8`, size was `40771` bytes, and duration was `1` second.
...
1. Run `npm run beta:tools:core-real-check-evidence-preflight`, then `npm run beta:tools:core-real-check-evidence` against deployed staging with source SHA `e8821759a10a43a60795accb596b3b83c15f9dfb`.
