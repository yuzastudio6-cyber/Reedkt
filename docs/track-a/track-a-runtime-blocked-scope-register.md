...
| GPAC/MP4Box install source | blocked_gpac_mp4box_package_source_unavailable | `TRACKA-NATIVE-CONTAINER-RENDER-TOOLS-INSTALL-PROOF-3` records `blocked_no_safe_resolved_identity_install_source_available` and does not add Dockerfile package declarations, third-party repositories, package dependencies, or runtime execution. |
| VapourSynth install source | blocked_core_vapoursynth_package_source_unavailable | `TRACKA-NATIVE-CONTAINER-RENDER-TOOLS-INSTALL-PROOF-3` records the core package-source blocker and keeps plugin policy separately blocked as `blocked_vapoursynth_native_plugin_policy_not_satisfied`. |
| Revideo install source | evaluation_only_non_core_owner_approval_required_before_install_source | Revideo remains evaluation-only/non-core and owner-gated before any install-source proof. |
