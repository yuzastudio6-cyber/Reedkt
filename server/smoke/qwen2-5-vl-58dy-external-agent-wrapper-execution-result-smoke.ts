...
import { QWEN2_5_VL_58DY_EXTERNAL_AGENT_WRAPPER_EXECUTION_RESULT } from '../../src/backend/mock/mock-qwen2-5-vl-58dy-external-agent-wrapper-execution-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/qwen2-5-vl-7b-58dy-external-agent-wrapper-execution-result.md'
const SPEC_PATH = 'src/backend/mock/mock-qwen2-5-vl-58dy-external-agent-wrapper-execution-result.ts'
const SMOKE_PATH = 'server/smoke/qwen2-5-vl-58dy-external-agent-wrapper-execution-result-smoke.ts'
const WRAPPER_PATH = 'server/cli/external-agent-tool-execute-qwen.ts'
const PACKAGE_SCRIPT = 'smoke:qwen2-5-vl-58dy-external-agent-wrapper-execution-result'
const DECISION = 'qwen2_5_vl_58dy_external_agent_wrapper_execution_passed_result_review_required'
const NEXT_PROMPT =
...
function scanForbiddenValues(value: unknown, prefix = 'qwen58dyWrapperResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/qwen2-5-vl-58dy-external-agent-wrapper-execution-result-smoke.ts',
...
for (const required of [
  ...
  'the wrapper re-runs `npm run external-agent-tool-next-command`',
...
const result = QWEN2_5_VL_58DY_EXTERNAL_AGENT_WRAPPER_EXECUTION_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'qwen2_5_vl_58dy_external_agent_wrapper_execution_result')
...
assert.equal(result.reviewedRun.runId, 'qwen58dw-20260701T215300')
assert.equal(result.reviewedRun.executionName, 'reeditpro-qwen2-5-vl-private-caller-x64s2')
