...
The guarded runner now exists:

`REEDITPRO_CONFIRM_TRACKA_GPAC_MP4BOX_GUARDED_RUNTIME_DISPATCH=true npm run tracka:gpac-mp4box-guarded-runtime-dispatch-confirmed-execution-1`
...
If the confirmation gate is absent, record `blocked_pending_gpac_mp4box_guarded_runtime_dispatch_confirmation`.

If the confirmation gate is present but committed source still declares the route handler disabled, record `blocked_gpac_mp4box_route_handler_not_enabled_for_confirmed_dispatch` and do not run route, worker, or tool execution.
