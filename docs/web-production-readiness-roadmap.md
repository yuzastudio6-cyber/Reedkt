...
| 44B web app structure migration | Establish `apps/web` as canonical while preserving the current root Vite app. | Transitional web structure is documented without desktop runtime or launch claims. |
| 44C web production shell (current) | Build the professional shell for dashboard, projects, editor workspace, jobs, artifacts, readiness, and compute route visibility. | Users can navigate a coherent web product surface with execution controls disabled/mock-safe. |
