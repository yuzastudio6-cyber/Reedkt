...
| Character/object/environment/style consistency | `verified_source_contract`; `controlled_fixture_runtime` for the optional AuraFace CPU boundary | continuity pack sheets, controlled generic IP-Adapter binding, `server/smoke/living-frame-auraface-artifact-requirements-smoke.ts`, `server/smoke/living-frame-auraface-offline-runner-protocol-smoke.ts`, `server/smoke/living-frame-auraface-continuity-measurement-smoke.ts`, and `server/smoke/living-frame-auraface-cpu-runtime-smoke.ts` | The aggregate now proves the exact two-artifact requirements, fixed offline request/private embedding/no-face-review protocol, measurement-only cosine contract, one separately metered controlled CPU attempt, and fail-closed no-identity/no-threshold authority. The real 277,617,978-byte ONNX pair, signed/scanned non-root image, atomic read-only mount, detector/landmark/embedding inference, resource/cost evidence, privacy/fairness/threshold owner review, project-calibrated continuity thresholds, and canonical private review remain open. |
...
CPU-boundary cases, but its real two-model atomic-mount inference and
privacy/fairness/threshold review remain an additional internal gate. The aggregate
