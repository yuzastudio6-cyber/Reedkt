...
const baseRef = 'origin/codex/rp-worker-ai-graphics-metadata-job-payload-owner-review-after-dry-run'
const expectedDecision = 'worker_ai_graphics_metadata_job_payload_dry_run_gate_status_ready_with_warnings'
const expectedScript = 'node scripts/validation/worker-ai-graphics-metadata-job-payload-dry-run-gate-status-diagnostics.mjs'
const packageScriptName = 'worker:ai-graphics-metadata-job-payload-dry-run-gate-status:diagnostics'
...
const allowedDecisions = new Set([
  ...
  'blocked_pending_worker_ai_graphics_job_payload_dry_run_gate_status_fixes',
  'blocked_pending_worker_ai_graphics_dry_run_gate_status_source_mismatch',
  'blocked_pending_worker_ai_graphics_pass_claim_status_review',
...
const requiredDocs = [
  ...
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-matrix.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-scoped-pass-claim.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-generic-claim-rejection.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-valid-case.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-blocked-case.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-invalid-case.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-static-executor.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-plan-snapshot.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-scoped-manifest.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-private-artifact.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-claim-lease.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-queue.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-no-execution.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-observability-audit.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-fail-closed.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-worker-intake.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-blocked-use-register.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-decision.md',
  'docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-next-lane-recommendation.md',
  'docs/prompt-worker-ai-graphics-metadata-job-payload-dry-run-gate-status-packet-results.md',
  'docs/implementation-prompts/prompt-worker-ai-graphics-metadata-job-payload-dry-run-gate-status-packet.md',
]
...
const requiredTrueBooleans = [
  'jobPayloadDryRunGateStatusReady',
  'jobPayloadDryRunGateStatusReadyWithWarnings',
  'workerAiGraphicsMetadataJobPayloadDryRunPassedAccepted',
  'ownerReviewAfterDryRunAccepted',
  'validDryRunCaseGateStatusReady',
  'blockedDryRunCaseGateStatusReady',
  'invalidDryRunCaseGateStatusReady',
  'staticDryRunExecutorGateStatusReady',
  'planSnapshotDryRunGateStatusReady',
  'scopedManifestDryRunGateStatusReady',
  'privateArtifactDryRunGateStatusReady',
  'claimLeaseDryRunGateStatusReady',
  'queueDryRunGateStatusReady',
  'noExecutionDryRunGateStatusReady',
  'observabilityAuditDryRunGateStatusReady',
  'failClosedDryRunGateStatusReady',
  'workerIntakeDryRunGateStatusReady',
]
...
const requiredTokens = [
  ...
  'PR #398',
  'PR #164',
  'WORKER_AI_GRAPHICS_METADATA_JOB_PAYLOAD_DRY_RUN_GATE_STATUS_QA_REVIEW',
  'no write',
  'docs_only',
...
const decisionDoc = read('docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-decision.md')
const matrixDoc = read('docs/worker-runtime/ai-graphics-job-payload-dry-run-gate-status-matrix.md')
...
for (const token of requiredTokens) {
  if (!evidenceText.includes(token)) failures.push(`required_token_missing:${token}`)
}
...
const unsafeClaimText = docsText
  ...
  .filter(
    (line) =>
      !/\b(?:No|no|not|blocked|unapproved|does not|do not|must not|remains|remain|false|status-only|metadata-only|static|separately gated|without|warning|warnings|defer|deferred|none|placeholder|required|source evidence|source chain|context only|policy context|not source truth|rejected|pending|later)\b/i.test(
        line,
      ),
  )
  .join('\n')
...
const basePackageJsonText = git(['show', `${baseRef}:package.json`])
let basePackageJson = null
try {
  basePackageJson = JSON.parse(basePackageJsonText)
} catch (error) {
  failures.push(`base_package_json_parse_failed:${error.message}`)
}
...
for (const file of new Set(changedFiles)) {
  if (/(^|\/)(\.local-artifacts|media|render|browser|canvas|webgl|public-artifacts?|dry-run-output|schema-validation-output|local-fixture-output)\//i.test(file)) {
    failures.push(`generated_output_path_tracked:${file}`)
  }
}

if (failures.length > 0) {
  console.error('WORKER_AI_GRAPHICS_METADATA_JOB_PAYLOAD_DRY_RUN_GATE_STATUS diagnostics failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('WORKER_AI_GRAPHICS_METADATA_JOB_PAYLOAD_DRY_RUN_GATE_STATUS diagnostics passed.')
