...
export const EDIT_LEVEL_SOURCE_UNDERSTANDING_LAYER_DEFINITIONS: EditLevelSourceUnderstandingLayerDefinition[] = [
  ...
  layer('qwen25vl_visual_segments', 'Visual understanding segments', 'future visual understanding route', 'Visual/video specialist context for key moments, scenes, visible text, objects, and layout.', 'provider_required'),
  layer('qwen25vl_marker_windows', 'Visual marker windows', 'future visual marker route', 'Visual context around Edit Brief markers and user timestamp questions.', 'provider_required'),
...
function createNormalSourceUnderstandingRoutes(): EditLevelSourceUnderstandingLayerRoute[] {
  ...
  return [
    ...
    route(level, 'qwen25vl_visual_segments', 'targeted', 'provider_required', 'Normal requests visual context only when a marker or user ambiguity requires it.', 'Use visible-context fallback or ask for clarification.', 'Targeted visual context only when needed.'),
...
function createPremiumSourceUnderstandingRoutes(): EditLevelSourceUnderstandingLayerRoute[] {
  ...
  return [
    ...
    route(level, 'qwen25vl_visual_segments', 'recommended', 'provider_required', 'Premium routes key visual moments where available.', 'Use key-moment local fallback and clarify visual ambiguity.', 'Key visual moments recommended.'),
    route(level, 'qwen25vl_marker_windows', 'recommended', 'provider_required', 'Premium routes visual summaries around marker windows.', 'Use marker note, source summary, and local key moments.', 'Marker window visual context recommended.'),
...
function createUltraPremiumSourceUnderstandingRoutes(): EditLevelSourceUnderstandingLayerRoute[] {
  ...
  return [
    ...
    route(level, 'qwen25vl_visual_segments', 'required', 'provider_required', 'Ultra Premium routes scene-level visual analysis when future access exists.', 'Degrade to Premium-safe visual context and show a degraded capability notice.', 'Scene-level visual analysis.'),
