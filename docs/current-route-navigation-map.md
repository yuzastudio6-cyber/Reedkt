...
- Applying any of the six user-editable current-edit fields requires a fresh plan and estimate when a draft exists. Cleanup changes rerun Footage Prep, and target-platform changes reconfirm the output frame. Edit Level is not exposed during private internal testing; the compatibility value remains internal.
