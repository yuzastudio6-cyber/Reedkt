...
run_gcloud run jobs deploy reeditpro-sound-audio-metadata-worker \
  --project="${GCP_PROJECT_ID}" \
  --region="${GCP_REGION}" \
  --image="$(artifact_image reeditpro-sound-audio-metadata-worker)" \
  ...
  --parallelism=1 \
  --max-retries=0 \
  --set-env-vars="REEDITPRO_ENV=${REEDITPRO_ENV},WORKER_GROUP=cpu_analysis_worker,SOUND_CPU_WORKER_NAME=sound-audio-metadata-worker,REEDITPRO_SOUND_CPU_RUNTIME_ENABLED=0,REEDITPRO_WORKER_EXECUTION_ENABLED=0,REEDITPRO_MEDIA_PROCESSING_ENABLED=0,REEDITPRO_SUPABASE_MUTATION_ENABLED=0,REEDITPRO_ARTIFACT_WRITE_ENABLED=0"
