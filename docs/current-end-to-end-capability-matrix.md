...
| Authenticated private upload-to-review pipeline | narrow exact plan presentation/approval/handoff/preparation connected; exact ordered multi-source backend lifecycle proven; rich compilation remains blocked | The authenticated backend persists a checksum-protected tenant-scoped planning handoff and content-addressed publication candidate, then presents one server-owned canonical plan only when the plan is exactly representable by the bounded source-and-caption graph. Exact one-source and duration-preserving sequence candidates can be compiled; a two-source backend fixture passes the complete private canonical lifecycle. The named-edit browser binds the visible plan version/hash and estimate identity/hash/maximum to a frontend-safe approval coordinator, requests a bounded private execution-handoff receipt, and can explicitly request backend-owned preparation from the exact package. Approval and handoff creation start no execution. Preparation returns bounded progress or private-review readiness while raw package/jobs/tools remain backend-only. | Normal rich multi-source work-item compilation, browser playback/review decisions, revision submission, deployed real-user operations, external beta, and production remain blocked. |
...
> ReeditPro has deterministic planning and canonical approval authority plus 50 exact private tool lifecycles. Its bounded compiler can publish one exact source or a duration-preserving ordered source sequence with one approved caption; a two-source fixture passes the complete canonical private lifecycle through final QA, reconciliation, replay, and private download. A named-edit browser can request a bounded private handoff without starting execution, then explicitly ask the backend to advance only that exact package through private preparation and review assembly. The normal rich multi-source editor plan remains approval-blocked because its captions, transitions, audio/SFX/ducking, color, provider, and segment operations are not silently discarded. Browser playback/review decisions, deployed real-user infrastructure, external beta, public delivery, live billing, Supabase/RLS tenancy, distributed workers, and production promotion remain gated.
