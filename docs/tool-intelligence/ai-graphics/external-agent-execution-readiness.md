...
| `real_esrgan` | `gpu_model` | `real_esrgan` | true | `install_target_prepared_runtime_blocked_pending_cuda_private_inputs` | `blocked_with_reason` | true | false | true | `realEsrganModelLocalPath` | `outputDirectory, nativeCudaRuntime, sourceImageLocalPath, modelWeightManifestEvidence` | `outputDirectory, nativeCudaRuntime, sourceImageLocalPath, realEsrganModelLocalPath, modelWeightManifestEvidence` | `npm run --silent ai-graphics:external-agent-gpu-model-local-dev-runtime-execution-harness -- --attempt-local-runtime --tool real_esrgan --output-dir .local-artifacts/ai-graphics/gpu-model-local-dev-runtime/<private-run> --result-out .local-artifacts/ai-graphics/gpu-model-local-dev-runtime/<private-run>/harness-result.json --source-image <private-approved-frame.png> --real-esrgan-model <private-real-esrgan-model.pth>` | `approved native CUDA host; outputDirectory; nativeCudaRuntime; sourceImageLocalPath; realEsrganModelLocalPath; modelWeightManifestEvidence; reviewed private proof refs; adapter skip reason: real_esrgan_model_missing` |
