...
## 26F. Function Search Path Hardening Migration Plan

Prompt 26F - Function Search Path Hardening Migration Plan.

- Purpose: create a function `search_path` hardening migration plan for the nine connected-advisor functions without creating active migrations or changing function definitions.
- Deliverables after Prompt 26F: `docs/supabase-function-search-path-migration-plan.md`, `docs/supabase-function-search-path-signature-preservation-contract.md`, `docs/supabase-function-search-path-schema-qualification-checklist.md`, `docs/supabase-function-search-path-future-test-matrix.md`, `docs/supabase-function-search-path-rollback-cleanup-plan.md`, `docs/supabase-function-search-path-staging-evidence-requirements.md`, `docs/prompt-26f-validation-results.md`, `docs/implementation-prompts/prompt-26f-function-search-path-hardening-migration-plan.md`, `scripts/validation/supabase-function-search-path-migration-plan-diagnostics.mjs`, package script wiring, foundation validation runner wiring, workflow coverage for the Prompt 26E-3 base branch, and tracker updates.
- Implementation status after Prompt 26F: validation prepared / function search_path hardening migration plan only. Function search path plan status is `function_search_path_migration_plan_created`.
- Implements: signature preservation requirements, schema qualification checklist, future local/staging behavior test matrix, rollback/cleanup requirements, staging evidence requirements, and stricter draft-only Markdown SQL labels for function search-path remediation planning.
- Must not implement: active migration files, executable SQL files, Supabase lifecycle/status commands, SQL execution, migrations, advisor remediation, function/grant/policy/index changes, Google Cloud or Secret Manager calls, secret fetching, provider/tool/worker/render/storage/credit/Stripe/telemetry execution, human approval grant, staging approval, production approval, deployment, or beta unlock.
- Main files/concepts: mutable function `search_path` advisors, fixed search path, schema-qualified function bodies, signature preservation, `SECURITY DEFINER`/`SECURITY INVOKER` preservation, future local candidate gates, and source-of-truth trackers.
- What remains blocked: advisor findings are unresolved; function bodies/signatures are not captured as executable evidence; accepted redacted evidence and Secret Manager reference evidence remain incomplete; Prompt 23 remains `pending_human_approval` on this base; staging SQL, production readiness, and beta remain blocked.
- Acceptance criteria: Prompt 26F docs exist, diagnostics pass, all nine functions are covered, draft SQL remains Markdown-only, active migrations are unchanged, no staging/production update is claimed, and no Supabase/Google Cloud/Secret Manager/SQL/runtime execution capability is enabled.
- Next prompt recommendation: Prompt 26F-1 - Function Search Path Local Migration Candidate, or Prompt 26G - SECURITY DEFINER Exposure Migration Plan. Prompt 23A and Prompt 24D remain required before staging execution.

## 26G. SECURITY DEFINER Exposure Migration Plan

Prompt 26G - SECURITY DEFINER Exposure Migration Plan.

- Purpose: create a SECURITY DEFINER exposure migration plan for seven connected-advisor functions without creating active migrations, changing grants, or altering function definitions.
- Deliverables after Prompt 26G: `docs/supabase-security-definer-exposure-migration-plan.md`, `docs/supabase-security-definer-function-classification.md`, `docs/supabase-security-definer-grant-review-contract.md`, `docs/supabase-security-definer-invoker-decision-contract.md`, `docs/supabase-security-definer-future-test-matrix.md`, `docs/supabase-security-definer-rollback-cleanup-plan.md`, `docs/supabase-security-definer-staging-evidence-requirements.md`, `docs/prompt-26g-validation-results.md`, `docs/implementation-prompts/prompt-26g-security-definer-exposure-migration-plan.md`, `scripts/validation/supabase-security-definer-migration-plan-diagnostics.mjs`, package script wiring, foundation validation runner wiring, workflow coverage for the Prompt 26F base branch, and tracker updates.
- Implementation status after Prompt 26G: validation prepared / SECURITY DEFINER exposure migration plan only. SECURITY DEFINER plan status is `security_definer_exposure_migration_plan_created`.
- Implements: conservative function classification, grant review requirements, SECURITY INVOKER decision gates, future local/staging test matrix, rollback/cleanup requirements, staging evidence requirements, and stricter draft-only Markdown SQL labels for grant/security-mode remediation planning.
- Must not implement: active migration files, executable SQL files, Supabase lifecycle/status commands, SQL execution, migrations, advisor remediation, function/grant/policy/index changes, Google Cloud or Secret Manager calls, secret fetching, provider/tool/worker/render/storage/credit/Stripe/telemetry execution, human approval grant, staging approval, production approval, deployment, or beta unlock.
- Main files/concepts: SECURITY DEFINER exposure advisors, `anon` execute grants, `authenticated` execute grants, RLS helper dependencies, trigger helper dependencies, SECURITY INVOKER conversion gates, future local candidate gates, and source-of-truth trackers.
- What remains blocked: advisor findings are unresolved; function bodies/grants/policy dependencies are not captured as executable evidence; accepted redacted evidence and Secret Manager reference evidence remain incomplete; Prompt 23 remains `pending_human_approval` on this base; staging SQL, production readiness, and beta remain blocked.
- Acceptance criteria: Prompt 26G docs exist, diagnostics pass, all seven functions are covered, draft SQL remains Markdown-only, active migrations are unchanged, no staging/production update is claimed, and no Supabase/Google Cloud/Secret Manager/SQL/runtime execution capability is enabled.
- Next prompt recommendation: Prompt 26G-1 - SECURITY DEFINER Local Migration Candidate, or Prompt 26H - FK Index Hardening Migration Plan. Prompt 23A and Prompt 24D remain required before staging execution.

## 26H. FK Index Hardening Migration Plan

Prompt 26H - FK Index Hardening Migration Plan.

- Purpose: create an FK index hardening migration plan for fifteen connected-advisor unindexed FK findings without creating active migrations, indexes, or SQL execution.
- Deliverables after Prompt 26H: `docs/supabase-fk-index-migration-plan.md`, `docs/supabase-fk-index-priority-matrix.md`, `docs/supabase-fk-index-duplicate-review-contract.md`, `docs/supabase-fk-index-naming-contract.md`, `docs/supabase-fk-index-write-amplification-risk.md`, `docs/supabase-fk-index-future-test-matrix.md`, `docs/supabase-fk-index-rollback-cleanup-plan.md`, `docs/supabase-fk-index-staging-evidence-requirements.md`, `docs/prompt-26h-validation-results.md`, `docs/implementation-prompts/prompt-26h-fk-index-hardening-migration-plan.md`, `scripts/validation/supabase-fk-index-migration-plan-diagnostics.mjs`, package script wiring, foundation validation runner wiring, workflow coverage for the Prompt 26G base branch, and tracker updates.
- Implementation status after Prompt 26H: validation prepared / FK index hardening migration plan only. FK index plan status is `fk_index_migration_plan_created`.
- Implements: priority matrix, duplicate/overlap review contract, index naming contract, write-amplification risk matrix, future local/staging test matrix, rollback/cleanup requirements, staging evidence requirements, and stricter draft-only Markdown SQL labels for FK index planning.
- Must not implement: active migration files, executable SQL files, Supabase lifecycle/status commands, SQL execution, migrations, advisor remediation, policy/index/function/grant changes, Google Cloud or Secret Manager calls, secret fetching, provider/tool/worker/render/storage/credit/Stripe/telemetry execution, human approval grant, staging approval, production approval, deployment, or beta unlock.
- Main files/concepts: unindexed FK advisor findings, duplicate-index review, additive index names, write-amplification risk, advisor recheck evidence, rollback requirements, and source-of-truth trackers.
- What remains blocked: advisor findings are unresolved; duplicate-index review and write-amplification review are not executed; accepted redacted evidence and Secret Manager reference evidence remain incomplete; Prompt 23 remains `pending_human_approval` on this base; staging SQL, production readiness, and beta remain blocked.
- Acceptance criteria: Prompt 26H docs exist, diagnostics pass, all fifteen FK findings are covered, draft SQL remains Markdown-only, active migrations are unchanged, no staging/production update is claimed, and no Supabase/Google Cloud/Secret Manager/SQL/runtime execution capability is enabled.
- Next prompt recommendation: Prompt 26H-1 - FK Index Local Migration Candidate, or Prompt GD-0 - AI Tools / Graphic Design Stack Repo Audit. Prompt 23A and Prompt 24D remain required before staging execution.
