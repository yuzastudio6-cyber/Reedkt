...
const nextPrompt =
  'WORKER_RUNTIME_JOBS-SOUND-CPU-PHASE125-LIMITED-EXTERNAL-AGENT-PRODUCT-TOOL-CALL-EXECUTION-PROOF-OWNER-REVIEW-NO-REAL-USER-MEDIA'
...
const docs = {
  ...
  runner:
    'scripts/validation/worker-runtime-jobs-sound-cpu-phase125-limited-external-agent-product-tool-call-execution-proof-no-real-user-media-runner.mjs',
  result:
  ...
  blockers:
    'docs/worker-runtime-jobs-sound-cpu-phase125-limited-external-agent-product-tool-call-execution-blocker-register-no-real-user-media.md',
  policy:
...
const parsed = {
  ...
  blockers: parseJsonBlock(
    docs.blockers,
    'worker-runtime-jobs-sound-cpu-phase125-limited-external-agent-product-tool-call-execution-blocker-register-no-real-user-media',
  ),
...
assert(read(docs.runner).includes('invokeLimitedExternalAgentProductToolCallBoundary'), 'runner boundary missing')
assert(read(docs.runner).includes('proofOutputWrittenToDisk: false'), 'runner output policy missing')
assert(
  read(docs.runner).includes("externalAgentAdapterMode: 'limited_synthetic_no_real_agent'"),
  'runner limited agent boundary missing',
)

assert(parsed.source.decision === sourceDecision, 'source decision mismatch')
assert(parsed.source.sourceVerification.sourcePr === 2099, 'source PR mismatch')
assert(
  parsed.source.sourceVerification.sourceMergeCommit === '8f653e42e426b7082e9f94f2e64bd4384256c8a5',
  'source merge mismatch',
)
assert(parsed.source.ownerReview.limitedExternalAgentProductToolCallExecutionProofMayProceedNext === true, 'source proof next missing')
assert(parsed.source.ownerReview.acceptedExpectedInvocationCount === 4, 'source invocation count mismatch')
assert(parsed.source.ownerReview.missingWhatHappenedEvidenceBlocksReadiness === true, 'source evidence blocker missing')
assert(parsed.source.soundCpuTools.readyForLimitedExternalAgentProductToolCallExecutionProofNoRealUserMedia === 15, 'source proof count mismatch')
...
assert(parsed.sourceAcceptance.acceptedForNextProofOnly.missingWhatHappenedEvidenceMustBlockReadiness === true, 'source acceptance evidence blocker missing')
assert(parsed.sourceHandoff.requiredEvidenceToCarryForward.whatHappenedEvidenceRequired === true, 'source handoff evidence rule missing')
assert(parsed.sourceHandoff.requiredEvidenceToCarryForward.missingWhatHappenedEvidenceBlocksReadiness === true, 'source handoff blocker rule missing')
assert(parsed.sourceHandoff.requiredEvidenceToCarryForward.expectedInvocationCount === 4, 'source handoff invocation mismatch')
...
assert(parsed.prompt.proofScope.runLimitedExternalAgentProductToolCallExecutionProofCommand === true, 'prompt proof command missing')
...
assert(parsed.prompt.proofScope.missingWhatHappenedEvidenceBlocksReadiness === true, 'prompt evidence blocker missing')
assert(parsed.prompt.proofScope.allowedToolCount === 15, 'prompt tool count mismatch')
assert(parsed.prompt.proofScope.expectedInvocationCount === 4, 'prompt invocation mismatch')
assert(parsed.prompt.proofScope.allowRealExternalAgentExecution === false, 'prompt real agent widened')
...
assert(parsed.result.sourceVerification.sourcePr === 2101, 'result source PR mismatch')
assert(
  parsed.result.sourceVerification.sourceMergeCommit === 'aeecde77bc9b1b9876fd989810eac4cd61bd87d3',
...
assert(parsed.result.proofResult.invocationCount === 4, 'proof invocation mismatch')
assert(
  parsed.result.proofResult.totalSyntheticLimitedExternalAgentProductToolCallBoundaryInvocationsObserved === 4,
  'proof total invocation mismatch',
)
assert(parsed.result.proofResult.toolCountCovered === 15, 'proof tool count mismatch')
assert(parsed.result.proofResult.whatHappenedEvidenceRecorded === true, 'proof what-happened evidence missing')
assert(parsed.result.proofResult.missingWhatHappenedEvidenceBlocksReadiness === true, 'proof evidence blocker missing')
assert(parsed.result.proofResult.realExternalAgentUsed === false, 'proof real agent widened')
...
assert(parsed.result.soundCpuTools.limitedExternalAgentProductToolCallExecutionProofPassed === 15, 'tool proof count mismatch')
assert(parsed.result.soundCpuTools.readyForLimitedExternalAgentProductToolCallExecutionProofOwnerReview === 15, 'tool owner review count mismatch')
assert(parsed.result.soundCpuTools.readyForLimitedExternalAgentProductToolCallExecutionToday === 0, 'tool limited execution widened')
...
assert(parsed.output.whatHappened.length === 4, 'what-happened count mismatch')
for (const [index, row] of parsed.output.whatHappened.entries()) {
  assert(
    row.invocationId === `phase125-limited-external-agent-product-tool-call-${index + 1}`,
    'what-happened invocation mismatch',
  )
  assert(row.toolDescriptorCount === 15, 'what-happened tool count mismatch')
  assert(row.accepted === true, 'what-happened row not accepted')
  assert(row.whatHappened.includes('accepted_synthetic_limited_external_agent'), 'what-happened detail missing')
}
...
assert(
  parsed.coverage.coverageKind === 'limited_external_agent_product_tool_call_execution_boundary_no_real_user_media',
  'coverage kind mismatch',
)
assert(parsed.coverage.whatHappenedEvidenceRecorded === true, 'coverage evidence missing')
assert(parsed.coverage.missingWhatHappenedEvidenceBlocksReadiness === true, 'coverage blocker missing')

assert(
  parsed.blockers.resolvedForThisGate.some(
    (row) => row.blockerId === 'limited_external_agent_product_tool_call_execution_proof_pending',
  ),
  'proof blocker resolution missing',
)
assert(
  parsed.blockers.remainingBlockers.some(
    (row) => row.blockerId === 'limited_external_agent_product_tool_call_execution_proof_owner_review_pending',
  ),
  'owner review blocker missing',
)
assert(parsed.blockers.readyForLimitedExternalAgentProductToolCallExecutionProofOwnerReview === true, 'owner review readiness missing')
assert(parsed.blockers.readyForLimitedExternalAgentProductToolCallExecutionToday === false, 'limited execution today widened')
assert(parsed.blockers.readyForProductToolCallExecutionToday === false, 'product today widened')
assert(parsed.blockers.soundCpuToolsReadyForLimitedExternalAgentProductToolCallExecutionProofOwnerReview === 15, 'blocker tool count mismatch')

assert(parsed.policy.allowedClaims.limitedExternalAgentProductToolCallExecutionProofPassedClaimed === true, 'policy proof claim missing')
assert(parsed.policy.allowedClaims.limitedExternalAgentProductToolCallExecutionProofOwnerReviewMayProceedClaimed === true, 'policy owner review missing')
...
assert(parsed.policy.nextGateMayRunLimitedExternalAgentProductToolCallExecutionProofOwnerReview === true, 'policy next owner review missing')
assert(parsed.policy.nextGateMayRunLimitedExternalAgentProductToolCallExecutionWithRealAgents === false, 'policy product widened')
...
assert(parsed.next.reviewScope.reviewLimitedExternalAgentProductToolCallExecutionProofOnly === true, 'next review scope missing')
assert(parsed.next.reviewScope.requireWhatHappenedEvidence === true, 'next evidence rule missing')
assert(parsed.next.reviewScope.missingWhatHappenedEvidenceBlocksReadiness === true, 'next blocker rule missing')
assert(parsed.next.reviewScope.mayReconcileLimitedExternalAgentProductToolCallExecutionReadinessNext === true, 'next reconciliation missing')
assert(parsed.next.reviewScope.allowedToolCount === 15, 'next tool count mismatch')
assert(parsed.next.reviewScope.expectedInvocationCount === 4, 'next invocation mismatch')
assert(parsed.next.reviewScope.allowLimitedExternalAgentProductToolCallExecutionToday === false, 'next limited execution widened')
...
assert(parsed.next.reviewScope.allowRouteExecutionToday === false, 'next route widened')
  ...
  JSON.stringify(
    {
      ...
      limitedExternalAgentProductToolCallExecutionProofPassed: 15,
      readyForLimitedExternalAgentProductToolCallExecutionProofOwnerReview: 15,
