...
const AUTHORIZATION_ID = 'MS-012C2-ACCOUNT-SUBSCRIPTION-EXT-006' as const
const PACKET_RELATIVE_PATH =
  'tasks/motion-studio/MS-012C/c2-account-subscription-live-authorization-request-ext-006.json' as const
const AUTHORIZATION_RECORD_RELATIVE_PATH =
  'tasks/motion-studio/MS-012C/c2-account-subscription-live-authorization-record-ext-006.json' as const
const PRIVATE_RUN_ROOT_RELATIVE_PATH =
  '.reeditpro-local-storage-ms012c2-account-subscription-ext-006' as const
const CONSUMPTION_RECORD_FILENAME = 'authority-consumed.json' as const
const EVIDENCE_FILENAME = 'private-account-subscription-evidence.json' as const
const FAILURE_FILENAME = 'terminal-failure.json' as const
const PROVIDER_HOSTNAME = 'api.elevenlabs.io' as const
const PROVIDER_PATH = '/v1/user/subscription' as const
...
const packetSchema = z.object({
  schemaVersion: z.literal('motion-studio.speech-account-subscription-live-authorization.v1'),
  ...
  preservedTerminalHistory: z.array(reviewedFileSchema).min(4).max(16),
...
const authorizationRecordSchema = z.object({
  schemaVersion: z.literal(
    'motion-studio.speech-account-subscription-live-authorization-record.v1',
  ),
...
export interface MotionStudioSpeechAccountSubscriptionLiveOperatorResultV1 {
  schemaVersion: 'motion-studio.speech-account-subscription-live-operator-result.v1'
  state: 'private_account_subscription_evidence_ready'
...
export interface MotionStudioSpeechAccountSubscriptionFixtureInput {
...
export const MOTION_STUDIO_SPEECH_ACCOUNT_SUBSCRIPTION_LIVE_OPERATOR_PATHS =
  Object.freeze({
...
export async function executeMotionStudioSpeechAccountSubscriptionLiveOperator(input: {
  repositoryRoot: string
  now: string
}): Promise<MotionStudioSpeechAccountSubscriptionLiveOperatorResultV1> {
...
export async function executeMotionStudioSpeechAccountSubscriptionFixtureOperator(
  input: MotionStudioSpeechAccountSubscriptionFixtureInput,
): Promise<MotionStudioSpeechAccountSubscriptionLiveOperatorResultV1> {
...
async function executeOperator(input: ({
  ...
  repositoryRoot: string
  now: string
} | ({ mode: 'fixture' } & MotionStudioSpeechAccountSubscriptionFixtureInput))): Promise<
  MotionStudioSpeechAccountSubscriptionLiveOperatorResultV1
> {
  const now = exactIso(input.now, 'account-subscription operator time')
  const repositoryRoot = await realpath(input.repositoryRoot)
  const packetBytes = await readRegularBoundedFile(
    inside(repositoryRoot, PACKET_RELATIVE_PATH),
    MAXIMUM_PACKET_BYTES,
    'account-subscription authority packet',
  )
  const packetDigest = sha256Bytes(packetBytes)
  const packet = packetSchema.parse(parseJson(packetBytes, 'account-subscription authority packet'))
  ...
  const authorizationBytes = await readRegularBoundedFile(
    ...
    'account-subscription authorization record',
  )
  const authorization = authorizationRecordSchema.parse(
    parseJson(authorizationBytes, 'account-subscription authorization record'),
  )
  if (authorization.authorityPacketDigest !== packetDigest) {
    blocked('Speech account-subscription authorization does not bind the exact packet.')
  }
  ...
  if (nowMs < recordedAt || nowMs >= expiresAt || expiresAt - recordedAt > 86_400_000) {
    blocked('Speech account-subscription authority is outside its exact execution window.')
  }
  if (
    authorization.maximumInternalProductionCostMicros !==
      packet.costPolicy.maximumInternalProductionCostMicros
  ) {
    blocked('Speech account-subscription cost authority changed after packet review.')
  }
  ...
  if (catalogEvidence.evidenceDigest !== packet.acceptedVoiceDecision.catalogEvidenceDigest) {
    blocked('Speech account-subscription lane lost its accepted catalog binding.')
  }
  ...
  await writePrivateJsonCreateOnly(consumptionPath, {
    schemaVersion: 'motion-studio.speech-account-subscription-authority-consumption.v1',
    ...
    consumedAt: now,
    immutable: true,
  }, 'Speech account-subscription authority is already consumed.')
  ...
  try {
    ...
    const response = input.mode === 'live'
      ? await requestSubscription({ apiKey, selectedAddress })
      : fixtureResponse(input)
    progress.lastPhase = 'provider_response_completed'
    const subscription = parseExactSubscription(response)
    const completedAt = now
    const evidenceBase = {
      schemaVersion: 'motion-studio.speech-account-subscription-private-evidence.v1' as const,
      state: 'private_account_subscription_evidence_ready' as const,
    ...
    await writePrivateJsonCreateOnly(
      evidencePath,
      evidence,
      'Speech account-subscription evidence already exists.',
    )
    ...
    await writePrivateJsonCreateOnly(failurePath, {
      schemaVersion: 'motion-studio.speech-account-subscription-terminal-failure.v1',
      ...
      failedAt: now,
      immutable: true,
    }, 'Speech account-subscription terminal failure already exists.')
    blocked('Speech account-subscription lane stopped terminally without exposing provider material.')
  }
}
...
function validateApiKey(value: string): string {
  ...
  if (normalized.length < 16 || normalized.length > 4_096 || /\s/.test(normalized)) {
    blocked('Speech account-subscription credential is malformed.')
  }
  return normalized
}
  ...
  return new Promise((resolveRequest, rejectRequest) => {
    ...
    const finishError = () => {
      ...
      rejectRequest(new ApiError(
        ...
        'Speech account-subscription provider request failed.',
        409,
      ))
    }
...
function fixtureResponse(
  ...
  if (
    ...
    blocked('Speech account-subscription fixture response violates the exact response boundary.')
  }
  return {
...
function parseExactSubscription(response: SubscriptionResponse): {
  status: string
  tier: string
  ...
  if (response.statusCode !== 200 || response.contentType !== 'application/json') {
    blocked('Speech account-subscription response is not an accepted subscription record.')
  }
  const value = parseJson(response.bytes, 'account subscription')
  if (!isRecord(value)) blocked('Speech account-subscription record is malformed.')
  const status = boundedString(value.status, 'subscription status')
  const tier = boundedString(value.tier, 'subscription tier')
  ...
  if (quotaConsumed > quotaLimit) {
    blocked('Speech account-subscription counters are inconsistent.')
  }
  ...
  const base = {
    ...
    quotaSufficientWithoutPurchase:
      quotaRemaining >= EXPECTED_SPOKEN_TEXT_CHARACTER_COUNT,
  }
...
function selectOnePublicIpv4(addresses: readonly string[]): string {
  ...
  if (unique.length === 0 || unique.some((address) => !isPublicIpv4(address))) {
    blocked('Speech account-subscription DNS results are empty or unsafe.')
  }
  return unique[0]!
}
...
async function verifyAcceptedVoiceDecision(repositoryRoot: string, packet: Packet): Promise<void> {
  ...
  if (sha256Bytes(decisionBytes) !== packet.acceptedVoiceDecision.fileSha256) {
    blocked('Speech account-subscription accepted voice decision changed after review.')
  }
  ...
  ) blocked('Speech account-subscription accepted voice decision lost its exact binding.')
}
...
async function verifyReviewedFiles(repositoryRoot: string, files: readonly {
  ...
  for (const file of files) {
    if (seen.has(file.relativePath)) blocked('Speech account-subscription packet repeats a reviewed file.')
    seen.add(file.relativePath)
    const bytes = await readRegularBoundedFile(
      inside(repositoryRoot, file.relativePath),
      MAXIMUM_REVIEWED_FILE_BYTES,
      'reviewed account-subscription file',
    )
    if (bytes.byteLength !== file.byteLength || sha256Bytes(bytes) !== file.sha256) {
      blocked('Speech account-subscription reviewed file identity changed.')
    }
  }
}
...
async function assertPrivateDirectory(path: string, repositoryRoot: string): Promise<void> {
  ...
  ) blocked('Speech account-subscription private run root is not owner-private.')
}
...
async function writePrivateJsonCreateOnly(path: string, value: unknown, conflict: string): Promise<void> {
  try {
    ...
    blocked('Speech account-subscription private evidence could not be persisted.')
  }
  const metadata = await lstat(path)
  if (
    !metadata.isFile() || metadata.isSymbolicLink() || (metadata.mode & 0o777) !== 0o600 ||
    (typeof process.getuid === 'function' && metadata.uid !== process.getuid())
  ) blocked('Speech account-subscription evidence is not owner-private.')
}

function inside(repositoryRoot: string, relativePath: string): string {
  if (relativePath.startsWith('/') || relativePath.includes('..') || relativePath.includes('\\')) {
    blocked('Speech account-subscription path is unsafe.')
  }
  const target = resolve(repositoryRoot, relativePath)
  if (!target.startsWith(`${repositoryRoot}${sep}`)) blocked('Speech account-subscription path escaped its repository.')
  return target
}
...
function classifyFailure(error: unknown, phase: string): string {
  ...
  if (phase === 'provider_request_started') return 'subscription_read_failed'
  if (phase === 'provider_response_completed') return 'subscription_response_validation_failed'
