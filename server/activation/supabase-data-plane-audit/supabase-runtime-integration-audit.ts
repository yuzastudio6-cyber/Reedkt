...
export function buildSupabaseRuntimeIntegrationAudit(
  ...
  const backendCredentialReady = env.hasSupabaseAdmin || options.secretManagerCredentialReady === true
  ...
  const whyLowOrNoActivity = [
    ...
    backendCredentialReady
      ? 'Backend Supabase credentials are available for the audit path, but most activation phases still write GCS/docs rather than Supabase rows.'
  ...
  if (env.mockOnly && !options.secretManagerCredentialReady) warnings.push('Current runtime env summary indicates mock-only behavior unless service-role Supabase env or Secret Manager access is configured.')
  if (env.mockOnly && options.secretManagerCredentialReady) warnings.push('Process env is still mock-only, but Phase 51A verified backend-only Supabase audit credentials through Secret Manager.')
  ...
  return {
    ...
    serverPublicClient,
    runtimeEnvSummary: {
      supabaseUrlConfigured: Boolean(env.supabaseUrl) || options.secretManagerCredentialReady === true,
      supabaseAnonKeyConfigured: Boolean(env.supabaseAnonKey),
      supabaseServiceRoleConfigured: backendCredentialReady,
      mockOnlyLikely: env.mockOnly && !options.secretManagerCredentialReady,
    },
