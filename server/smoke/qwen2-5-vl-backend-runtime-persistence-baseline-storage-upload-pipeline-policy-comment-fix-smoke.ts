...
function assertFalseFlags(flags: JsonRecord) {
  for (const key of [
    'storageUploadPipelinePolicySemanticsChanged',
...
const migration = read('supabase/migrations/202605200001_storage_upload_pipeline_readiness.sql')
for (const phrase of [
  ...
  'Skipping storage upload pipeline storage.objects policy comments because the local migration role does not own the Supabase platform table.',
]) {
...
check(
  ...
  'storage upload pipeline storage.objects policy comments should exist exactly twice inside the guarded block.',
)
check(
  migration.indexOf('do $$', migration.indexOf('create policy "reeditpro_project_editors_update_workspace_source_and_thumbnails"')) <
    migration.indexOf('comment on policy "reeditpro_project_members_read_workspace_project_objects" on storage.objects is') &&
    migration.indexOf('comment on policy "reeditpro_project_editors_upload_workspace_source_and_thumbnails" on storage.objects is') <
      migration.lastIndexOf('exception'),
  'storage upload pipeline storage.objects policy comments must be guarded before the insufficient_privilege handler.',
)
...
const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-baseline-storage-upload-pipeline-policy-comment-fix.md')
for (const phrase of [
  DECISION,
...
const fix = QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_BASELINE_STORAGE_UPLOAD_PIPELINE_POLICY_COMMENT_FIX
assert.equal(fix.decision, DECISION)
assert.equal(
  fix.upstreamBackendRuntimePersistenceLocalHarnessValidationRetry14ResultDecision,
  QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_LOCAL_HARNESS_VALIDATION_RETRY_14_RESULT.decision,
)
assert.equal(fix.repairedMigration.path, 'supabase/migrations/202605200001_storage_upload_pipeline_readiness.sql')
assert.equal(fix.repairedMigration.failedStatementCategory, 'storage_upload_pipeline_policy_comment_ownership')
assert.equal(
...
assert.equal(fix.runtimeFlags.baselineStorageUploadPipelinePolicyCommentFixRecorded, true)
assert.equal(fix.runtimeFlags.activeBaselineStorageUploadPipelineReadinessMigrationEdited, true)
assert.equal(fix.runtimeFlags.storageUploadPipelinePolicyCommentInsufficientPrivilegeGuarded, true)
assert.equal(fix.runtimeFlags.storageUploadPipelinePolicyCommentsSkippedWhenNotOwner, true)

const forbiddenDataFindings = scanValues({ fix })
assert.deepEqual(
  forbiddenDataFindings,
  [],
  `Forbidden values in storage upload pipeline policy comment fix data: ${forbiddenDataFindings.join('; ')}`,
)
...
console.log(JSON.stringify({
  ...
  storageUploadPipelinePolicyCommentInsufficientPrivilegeGuarded:
    fix.runtimeFlags.storageUploadPipelinePolicyCommentInsufficientPrivilegeGuarded,
  storageUploadPipelinePolicySemanticsChanged: fix.runtimeFlags.storageUploadPipelinePolicySemanticsChanged,
