...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'backend_runtime_persistence_local_harness_config_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58R-BACKEND-RUNTIME-PERSISTENCE-LOCAL-HARNESS-CONFIG-CREATE: create safe local Supabase config for Qwen persistence validation, no SQL/no deploy/no cloud/no assets/no beta'
}
  ...
  return {
    ...
    privateInvokeClient: {
      ...
      boundaryNotes: [
        ...
        'The next runtime gate may create a safe repo-local Supabase config for persistence validation; it must not run SQL, start Supabase, start Docker, deploy migrations, rerun inference, enable beta, enable production, create public artifacts, create signed URLs, or create generated assets.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
