...
assert.equal(ui.privateInvokeClient.currentStatus, 'structured_fixture_output_fix_required')
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 14)
assert.equal(
