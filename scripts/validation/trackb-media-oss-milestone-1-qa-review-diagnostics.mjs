...
sameSet(status.acceptedProvenBounded?.map((tool) => tool.id), ['ffmpeg', 'ffprobe', 'sharp_libvips', 'duckdb', 'polars_nodejs_polars', 'exiftool', 'mediainfo', 'tesseract', 'imagemagick'], 'status_accepted_after_qa')
sameSet(status.blockedNotInstalledProven, ['opencv', 'pyav', 'pyscenedetect', 'paddleocr', 'paddlepaddle', 'opencolorio', 'openimageio'], 'status_blocked_after_qa')
...
if (owner?.acceptedProvenBoundedCount !== 9 || owner?.blockedNotInstalledProvenCount !== 7 || owner?.endToEndProductReadyToolCount !== 0) {
