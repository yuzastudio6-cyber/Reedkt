...
assert.equal(privateSearxngServiceConfig.serviceAccountEmail, 'reeditpro-private-search-sa@reeditpro.iam.gserviceaccount.com')
assert.equal(privateSearxngServiceConfig.provider, 'searxng')
assert.equal(privateSearxngServiceConfig.cpuOnly, true)
assert.equal(privateSearxngServiceConfig.gpuAllowed, false)
assert.equal(privateSearxngServiceConfig.modelWeightsAllowed, false)
assert.equal(privateSearxngServiceConfig.maxResults <= 5, true)
assert.equal(privateSearxngServiceConfig.publicUnauthenticatedAccessAllowed, false)
assert.equal(privateSearxngServiceConfig.paidProvidersAllowed, false)
assert.equal(privateSearxngServiceConfig.publicSearxngInstanceAllowed, false)
assert.equal(privateSearxngServiceConfig.browserCaptureAllowed, false)
assert.equal(privateSearxngServiceConfig.readabilityExtractionAllowed, false)
assert.equal(privateSearxngServiceConfig.productionReadyAllowed, false)
assert.equal(privateSearxngServiceConfig.externalBetaAllowed, false)
assert.equal(privateSearxngServiceConfig.broadMediaAllowed, false)
assert.equal(privateSearxngServiceConfig.externalBetaAllowed, defaultActivationLaunchPermissions.externalBetaAllowed)
const launchGatePass = createActivationLaunchPermissions({
  phaseQaPassed: true,
...
assert.equal(launchGatePass.externalBetaAllowed, true, 'activation external beta permission should be able to pass with complete evidence')
assert.equal(launchGatePass.paidProductionAllowed, true, 'activation paid production permission should be able to pass with complete evidence')
