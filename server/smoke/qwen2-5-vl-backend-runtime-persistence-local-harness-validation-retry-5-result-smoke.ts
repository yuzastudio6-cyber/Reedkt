...
function assertFalseFlags(flags: JsonRecord) {
  for (const key of [
    'backendRuntimePersistenceLocalHarnessValidationRetry5Passed',
...
const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-local-harness-validation-retry-5-result.md')
for (const phrase of [
  DECISION,
  '`55430`',
  '`55431`',
...
const result = QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_LOCAL_HARNESS_VALIDATION_RETRY_5_RESULT
assert.equal(result.decision, DECISION)
assert.equal(
  result.upstreamBackendRuntimePersistenceBaselineEditPlanSegmentsVersionFixDecision,
...
assert.equal(result.attempt.failedStatementCategory, 'credit_reservations_approved_plan_snapshot_id_fkey')
assert.equal(
...
assert.equal(result.validationOutcome.status, 'blocked_credit_approval_snapshots_baseline_column')
assert.equal(result.validationOutcome.editPlanSegmentsVersionBaselineFixVerified, true)
assert.equal(result.validationOutcome.creditApprovalSnapshotsBaselineFixRequired, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry5ResultRecorded, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry5Attempted, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceEditPlanSegmentsVersionBaselineFixVerified, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceCreditApprovalSnapshotsBaselineFixRequired, true)

const forbiddenDataFindings = scanValues({ result })
assert.deepEqual(
  forbiddenDataFindings,
  [],
  `Forbidden values in Qwen validation retry 5 data: ${forbiddenDataFindings.join('; ')}`,
)

console.log(JSON.stringify({
  ok: true,
  decision: DECISION,
  localHarnessValidationRetry5Attempted: result.validationOutcome.localHarnessValidationRetry5Attempted,
  localHarnessValidationRetry5Passed: result.validationOutcome.localHarnessValidationRetry5Passed,
  editPlanSegmentsVersionBaselineFixVerified: result.validationOutcome.editPlanSegmentsVersionBaselineFixVerified,
