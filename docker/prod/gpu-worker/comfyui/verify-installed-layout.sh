...
if [ "$#" -ne 0 ]; then
  echo "Canonical ComfyUI layout verifier does not accept arguments." >&2
  exit 64
fi
...
fail() {
  echo "Canonical ComfyUI installed layout is invalid." >&2
  exit "$1"
}
...
require_regular_file "${TARGET_ROOT}/runner.py"
