...
| Critical | Local/staging Supabase/RLS not executed | Access policies are not proven. | Cross-workspace data exposure. | Repair local Supabase CLI/config/Docker/psql path, then run scoped local SQL; staging SQL requires later human approval. | Prompt 20A | No. | Yes |
| Critical | Remote/staging migration not validated | Schema chain may not apply cleanly. | Runtime data corruption or missing tables. | Validate migration chain in approved local environment first, then staging only with approval. | Prompt 20A, then staging validation | No. | Yes |
...
| Medium | Supabase CLI/config/local SQL blocker | Local SQL validation blocked. | Slower RLS iteration and no RLS execution evidence. | Install compatible arm64 Supabase CLI or approved container path, add safe local config, make Docker daemon/local runtime available, and provide `psql` or approved local SQL executor. | Prompt 20A | Yes for static planning only. | Yes before production beta |

## Decision

Prompt 20 adds local-only safety preflight and dry-run/list RLS runner tooling, but local SQL/RLS execution remains blocked by missing `supabase/config.toml`, wrong-architecture Supabase CLI error `-86`, unavailable Docker daemon, missing `psql`, no verified local DB URL, and no local executable SQL candidates. Production beta still cannot proceed because local/staging Supabase/RLS has not executed. The next safe milestone is Prompt 20A - Local Supabase Toolchain Repair.
