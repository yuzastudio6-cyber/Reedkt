...
export function buildGcpStagingCommandPlans(
  ...
  const plans: GcpStagingCommandPlan[] = [
    ...
    commandPlan('create-secret-placeholders', 'secret_placeholders', resourceMap.secretPlaceholders.map((secret) => `gcloud secrets create "${secret.name}" --project="${config.projectId}" --replication-policy=automatic`).join('\n'), ['GCP_PROJECT_ID', 'REEDITPRO_CONFIRM_STAGING_GCP_SETUP=true'], true, true),
