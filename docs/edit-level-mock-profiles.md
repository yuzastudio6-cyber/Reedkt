...
Every profile is `mockOnly: true` and `betaReady: true`. `productionReady` is computed from the Edit Level production readiness evidence gates; it defaults to `false` while profile contracts, persistence, route, planner, provider, worker, render/export, credit, Supabase, deployment, and QA approvals are missing.
