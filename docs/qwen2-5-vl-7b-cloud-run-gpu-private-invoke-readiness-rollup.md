...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_execution_approval_recorded_preflight_required`.
...
- controlled persisted worker dispatch runtime execution preflight: blocked, execution preflight required
...
| Controlled persisted worker dispatch runtime execution preflight | blocked controlled persisted worker dispatch runtime execution preflight required | Runtime execution approval is recorded, but the preflight has not verified approved snapshot, credit, private refs, idempotency, lease, private invoke, QA, audit, cost, credit, and rollback inputs. | Verify the preflight while keeping Cloud Run invocation, Qwen inference, generated assets, beta, and production blocked. |
