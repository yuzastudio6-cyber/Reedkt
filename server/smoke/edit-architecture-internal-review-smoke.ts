...
const steps: SmokeStep[] = [
  ...
  {
    id: 'canonical_private_review_client',
    script: 'smoke:canonical-private-review-client',
  ...
  {
    id: 'professional_long_form_customer_delivery_mp4_fragment_index',
    script: 'smoke:professional-long-form-customer-delivery-mp4-fragment-index',
    requiredFor: 'bounded streaming ISO-BMFF initialization/track/fragment authority, exact time-to-byte recovery windows, split-header handling, and no retained media payload bytes',
  },
  {
    id: 'professional_long_form_customer_delivery_media_source',
    script: 'smoke:professional-long-form-customer-delivery-media-source',
    requiredFor: 'bounded authenticated range appends, rolling MediaSource buffering, exact evicted backward-seek recovery, fail-closed review identity and codec checks, and seek-aware client playback coverage without durable-server authority',
  },
  {
    ...
    requiredFor: 'actual Chromium H.264/AAC fragmented-MP4 decode, full 36-second playback, rolling eviction, exact authenticated backward-seek recovery, and resumed decode without a whole-file browser Blob, plus exact frame-zero and completed-coverage watch-client requests',
  },
  {
