...
| Component decomposition and rigging | `verified_private_runtime` for one fixture-specific character-action cutout rig and one fixture-specific mechanical-object rig; `verified_source_contract` for general component geometry, Head Intelligence Rigging Direction, Rigging v2, deep re-signed-artifact validation, and fixed Blender/OpenToonz adapter candidates | component asset intent, geometry, component rig, Rigging Direction v1, Rigging v2, fixed-adapter candidate v1, background-plate reconstruction, artifact reconciliation, `docs/living-frame/living-frame-rigging-director-and-adapter-candidates.md`, `server/smoke/living-frame-rigging-v2-smoke.ts`, `server/smoke/living-frame-animation-aware-illustration-private-composite-internal-test-smoke.ts`, and `server/smoke/living-frame-style-depth-breadth-private-render-internal-test-smoke.ts` | The real Musashi fixture is decoded, split into a stable base plus pivot-centered sword-arm, hair, and trailing-robe RGBA components, byte-revalidated, independently animated, and measured in the final render. Selected shoulder/robe pixels receive deterministic nearest-opaque-border fill while the selected blade/open-space region clears to transparency. The paper-collage locomotive supplies a second cross-style proof: three drive wheels are assigned to non-overlapping nearest-hub circular alpha components, exposed undercarriage pixels receive bounded deterministic reconstruction, each wheel rotates around its exact hub, and one static connecting-rod overlay remains above the wheels. The new provider-neutral Head Intelligence contract chooses narrative purpose, focal action, motion arc, capabilities, risk, and route without giving workers raw chat or executable code. Rigging v2 validates parts, bones, joints, constraints, IK, meshes, skin/rigidity maps, mechanical linkages, secondary motion, output passes, QA, fallbacks, and final-canvas restraint. Native mechanics route to Remotion; flat mesh deformation may route to the OpenToonz Plastic evaluation candidate; armature/IK/skinning/2.5D may route to the Blender headless evaluation candidate. The fixed adapter request embeds only typed verified plans and server-derived adapter identity. Neither external operation is registered or executable yet. Pinned/scanned runtimes, fixed-adapter source evidence, real internal benchmarks, transparent component output, private persistence/QA/review, canonical work admission, and broader arbitrary-character/object fixtures remain open. |
...
11. Advanced rigging runtime qualification. The Head Intelligence Rigging
    Direction, Rigging v2 relational compiler/verifier, native mechanical
    route, OpenToonz Plastic route candidate, Blender headless route candidate,
    and fixed non-executable adapter materialization are source-contract
    complete. Private internal use still requires pinned/scanned runtime
    images, reviewed fixed-adapter source, real flat-2D and advanced-2.5D scene
    benchmarks, transparent component/depth/mask outputs, private
    persistence/reread, rig QA, resource/cost evidence, canonical work
    admission, and fallback/private-review evidence.
...
for the two GPU runtime paths, plus pinned fixed-adapter Blender/OpenToonz
internal runtime and rig-output qualification. The aggregate
