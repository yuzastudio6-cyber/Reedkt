...
import {
  AI_VIDEO_BROLL_GEN_10B_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_10C_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_C_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-10c-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-c.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
...
function scanForbiddenValues(value: unknown, prefix = 'broll10bStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  '`us-east1-b`',
  '`us-east1-c`',
  '| `us-east1-c` | `us-east1` | `UP` | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | selected |',
  '| `us-east1-d` | `us-east1` | `UP` | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | available fallback |',
  'Select `us-east1-c` as the next bounded no-idle IAP wheelhouse transfer proof target.',
  ...
  assert.equal(doc.includes(required), true, `10B stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `10C prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10B_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10b_iap_wheelhouse_transfer_stockout_fix_result')
...
assert.equal(result.selectedRetryZone, 'us-east1-c')
...
assert.equal(result.futureRetryPreflightRequired.includes('us_east1_c_zone_status'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10C_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_C_PROMPT)
