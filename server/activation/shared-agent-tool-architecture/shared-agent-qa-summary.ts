...
export function buildSharedAgentToolArchitectureQaSummary(packageScripts?: Record<string, string>, options: { supabaseMilestoneSyncPassed?: boolean; executionMode?: boolean } = {}): SharedAgentToolArchitectureQaSummary {
  const syncPassed = Boolean(options.supabaseMilestoneSyncPassed)
  const executionMode = Boolean(options.executionMode)
  const gates: SharedAgentToolArchitectureQaGate[] = [
    ...
    gate('capability_manifest_schema', required(toolCapabilityManifestSchema.requiredFields, ['manifestVersion', 'toolId', 'owner', 'status', 'internalTestingReady', 'internalBetaCandidateReady', 'productionReady', 'readinessEvidence', 'supabaseMilestoneRefs', 'lastValidatedPhase']) && (toolCapabilityManifestSchema.examples?.length ?? 0) >= 6, 'Capability manifest schema includes required readiness/ownership fields, Supabase refs, and examples.'),
    ...
    gate('supabase_milestone_sync', syncPassed || !executionMode, syncPassed ? 'Phase 52A Supabase milestone sync wrote and read back through the Phase 51D contract.' : 'Supabase milestone sync is required for confirmed execution and remains pending in static report mode.'),
  ...
  return {
    ...
    warnings: [
      ...
      ...(!syncPassed ? ['Static report mode does not write Supabase milestone rows; confirmed execution must pass the sync gate.'] : []),
    ],
  }
}
