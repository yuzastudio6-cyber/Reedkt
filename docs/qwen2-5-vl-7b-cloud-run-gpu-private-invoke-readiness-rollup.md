...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_smoke_result_review_accepted_runtime_plan_required`.
...
| Controlled persisted worker dispatch runtime plan | blocked controlled persisted worker dispatch runtime plan required | Smoke result review is accepted, but real runtime preconditions, backend-only lease/claim mutation ownership, private invoke envelope handling, source-of-truth refs, idempotency, QA/audit/cost evidence, and rollback/fail-closed behavior are not yet planned for a real dispatch gate. | Plan the controlled persisted worker dispatch runtime path before any real worker dispatch readiness claim. |
