...
} from '../cli/production-wallet-lifecycle-evidence-collector'
...
const dryRun = await runProductionWalletLifecycleEvidenceCollectorFromEnv(walletLifecycleEnv(), async () => {
  dryRunFetchCalled = true
  throw new Error('fetch must not run in dry-run mode')
})
assert.equal(dryRun.ok, true, 'complete wallet lifecycle evidence should pass dry-run mode')
assert.equal(dryRun.mode, 'dry_run', 'collector should default to dry-run mode')
assert.equal(dryRun.readyForWalletLifecycleEvidence, true, 'wallet lifecycle evidence should be locally ready')
assert.equal(dryRun.recordConfirmationRequired, true, 'collector should require explicit record confirmation')
assert.equal(Object.values(dryRun.walletLifecycle.checks).every(Boolean), true, 'all wallet lifecycle checks should pass')
assert.equal(dryRunFetchCalled, false, 'dry-run must not call backend fetch')

const blocked = await runProductionWalletLifecycleEvidenceCollectorFromEnv({
  ...walletLifecycleEnv(),
  REEDITPRO_PRODUCTION_WALLET_REFUND_VERIFIED: 'false',
}, async () => {
  throw new Error('fetch must not run with incomplete wallet lifecycle evidence')
})
assert.equal(blocked.ok, false, 'missing refund evidence should block wallet lifecycle readiness')
assert.equal(blocked.walletLifecycle.ready, false, 'wallet lifecycle slice should be blocked')
assert.equal(blocked.walletLifecycle.blockers.includes('refundVerified is not verified.'), true)

await assert.rejects(
  () => runProductionWalletLifecycleEvidenceCollectorFromEnv({
    ...walletLifecycleEnv(),
    REEDITPRO_PRODUCTION_WALLET_NOTES: 'Wallet review accidentally includes service_role_key',
  }, async () => {
    throw new Error('fetch must not run with secret-like wallet notes')
  }),
  /secret-like|WALLET_NOTES/,
  'collector should reject secret-like wallet evidence fields before backend calls',
)

const blockedAuditCalls: Array<{ url: string; method: string; headers: Record<string, string>; body?: Record<string, unknown> }> = []
const blockedAuditRecord = await runProductionWalletLifecycleEvidenceCollectorFromEnv({
  ...walletLifecycleEnv(),
  ...
  REEDITPRO_PRODUCTION_READINESS_BEARER_TOKEN: 'production-wallet-lifecycle-bearer-token',
  REEDITPRO_PRODUCTION_READINESS_IDEMPOTENCY_KEY: 'production-wallet-lifecycle-blocked-audit-smoke',
...
const recorded = await runProductionWalletLifecycleEvidenceCollectorFromEnv(completeEnv(), fakeFetch(calls))
...
assert.equal(recorded.walletLifecycle.ready, true, 'wallet lifecycle slice should remain ready')
assert.equal(calls.length, 2, 'confirmed collector should POST evidence and GET readback')
assert.equal(calls[0]?.method, 'POST')
assert.equal(calls[1]?.method, 'GET')
assert.equal(calls[0]?.headers['idempotency-key'], 'production-wallet-lifecycle-evidence-smoke')
assert.equal(JSON.stringify(recorded).includes('production-wallet-lifecycle-bearer-token'), false, 'collector result must not expose bearer token')

console.log(JSON.stringify({
  ok: true,
  dryRunMode: dryRun.mode,
  recordedMode: recorded.mode,
  checks: recorded.walletLifecycle.checks,
  ...
  return async (url, init) => {
    ...
    if (init.method === 'POST' && url.includes('/v1/beta-readiness/production-tool-execution-readiness/evidence')) {
      return jsonResponse(201, {
        ...
        warnings: ['Production wallet lifecycle route smoke response.'],
      })
    }
...
function walletLifecycleEnv(): ProductionWalletLifecycleEvidenceCollectorEnv {
  const yes = 'true'
  return {
    REEDITPRO_PRODUCTION_READINESS_SOURCE_ID: 'production-wallet-lifecycle-evidence-smoke',
    REEDITPRO_PRODUCTION_READINESS_SOURCE_SHA: 'ab3561f411f5e4cb4ebc5efcc02ebb152ea3e624',
    REEDITPRO_PRODUCTION_READINESS_WORKSPACE_ID: 'workspace-production-wallet-lifecycle-smoke',
    REEDITPRO_PRODUCTION_READINESS_PROJECT_ID: 'project-production-wallet-lifecycle-smoke',
    REEDITPRO_PRODUCTION_READINESS_CONFIRM_EVIDENCE_REVIEW: yes,
    REEDITPRO_PRODUCTION_EVIDENCE_REVIEWED_BY: 'production-wallet-lifecycle-reviewer',
    REEDITPRO_PRODUCTION_EVIDENCE_REVIEWED_AT: '2026-07-03T00:00:00.000Z',
    REEDITPRO_PRODUCTION_WALLET_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:wallet',
    ...
    REEDITPRO_PRODUCTION_WALLET_NOTES: 'Wallet lifecycle evidence reviewed for reservation, spend, release, refund, settlement RPC, service-role-only access, idempotent replay, and no silent charge.',
  }
}

function completeEnv(): ProductionWalletLifecycleEvidenceCollectorEnv {
  const yes = 'true'
  return {
    ...walletLifecycleEnv(),
    REEDITPRO_PRODUCTION_WALLET_LIFECYCLE_CONFIRM_RECORD_EVIDENCE: yes,
    REEDITPRO_PRODUCTION_READINESS_API_BASE_URL: 'https://api.production.reeditpro.example',
    REEDITPRO_PRODUCTION_READINESS_BEARER_TOKEN: 'production-wallet-lifecycle-bearer-token',
    REEDITPRO_PRODUCTION_READINESS_IDEMPOTENCY_KEY: 'production-wallet-lifecycle-evidence-smoke',
    REEDITPRO_PRODUCTION_SUPABASE_ENVIRONMENT: 'production',
    REEDITPRO_PRODUCTION_SUPABASE_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:supabase',
    ...
    REEDITPRO_PRODUCTION_TOOL_COST_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:tool-cost-ledger',
    ...
    REEDITPRO_PRODUCTION_STRIPE_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:stripe',
    ...
    REEDITPRO_PRODUCTION_OBSERVABILITY_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:observability',
    ...
    REEDITPRO_PRODUCTION_OPERATIONS_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:operations',
    ...
    REEDITPRO_PRODUCTION_TOOLS_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:tools',
    REEDITPRO_PRODUCTION_TOOLS_SOURCE_ID: 'production-wallet-lifecycle-evidence-smoke:tools',
    REEDITPRO_PRODUCTION_TOOLS_SOURCE_SHA: 'ab3561f411f5e4cb4ebc5efcc02ebb152ea3e624',
    ...
    REEDITPRO_PRODUCTION_HARD_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:hard-safety',
    ...
    REEDITPRO_PRODUCTION_OWNER_EVIDENCE_ARTIFACT_ID: 'prod-wallet-lifecycle-artifact:owner-signoff',
