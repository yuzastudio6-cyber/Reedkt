...
export const CANONICAL_GOOGLE_VISUAL_CALIBRATION_OPERATION_ID =
  'provider.google.generate_visual_calibration_candidate.v1' as const
export const CANONICAL_GOOGLE_VISUAL_CALIBRATION_BOUNDARY_PROFILE_ID =
  'google_gemini_omni_flash_visual_calibration_provider_boundary' as const
export const CANONICAL_GOOGLE_VISUAL_CALIBRATION_ROUTE_ID =
  'gemini_omni_flash' as const
export const CANONICAL_GOOGLE_VISUAL_CALIBRATION_MODEL_ID =
  'gemini-omni-flash-preview' as const
export const CANONICAL_GOOGLE_GEMINI_SECRET_REFERENCE_ENV_KEY =
  'GOOGLE_SECRET_GEMINI_API_KEY_NAME' as const
export const CANONICAL_GOOGLE_GEMINI_EXPECTED_SECRET_ID =
  'reeditpro-prod-gemini-api-key' as const
...
export function assertCanonicalProviderWorkAuthorizationAny(input: {
  ...
  const version = input.value && typeof input.value === 'object'
    ...
    : undefined
  if (version === CANONICAL_PROVIDER_WORK_AUTHORIZATION_V4_VERSION) {
    return assertCanonicalProviderWorkAuthorizationV4(input)
  }
...
/**
 * Forward-only single-output visual-calibration authority. V1-V3 registry,
 * authorization, dispatch, cost, and receipt hashes remain historical. This
 * V4 operation freezes one primary Gemini Omni Flash route while its preview
 * alias, provider transport, account, rate, cloud, and release gates remain
 * deliberately unqualified.
 */
export const CANONICAL_PROVIDER_OPERATION_REGISTRY_V4_VERSION =
  'canonical-provider-operation-registry-v4' as const
export const CANONICAL_PROVIDER_WORK_AUTHORIZATION_V4_VERSION =
  'canonical-provider-work-authorization-v4' as const

const canonicalVisualCalibrationContextSchema = z.object({
...
export const canonicalProviderOperationProfileV4Schema = z.object({
  schemaVersion: z.literal(CANONICAL_PROVIDER_OPERATION_REGISTRY_V4_VERSION),
  operationId: z.literal(CANONICAL_GOOGLE_VISUAL_CALIBRATION_OPERATION_ID),
  intent: z.literal('visual_calibration_candidate'),
  providerBoundaryProfileId: z.literal(
  ...
  immutableProviderRevision: z.null(),
  expectedWorkItemType: z.literal('generate_visual_calibration_candidate'),
  expectedWorkerClass: z.literal('provider_worker'),
  expectedOutput: z.object({
    role: z.literal('provider_visual_calibration_video_mp4'),
    artifactType: z.literal('provider_visual_calibration_video_mp4'),
    assetRole: z.literal('generated'),
    contentType: z.literal('video/mp4'),
    maximumByteLength: z.literal(67_108_864),
    privateCreateOnlyRequired: z.literal(true),
  ...
  generationBounds: z.object({
    ...
    maximumPixelCount: z.literal(921_600),
    frameRate: z.literal(24),
  }).strict(),
  requestPolicy: z.object({
    maximumSecretPayloadReads: z.literal(1),
    maximumPrivateInputUploads: z.literal(0),
    maximumGenerationSubmissions: z.literal(1),
    maximumStatusReads: z.literal(12),
    maximumResultReads: z.literal(1),
    maximumBinaryDownloads: z.literal(1),
    maximumCancellations: z.literal(0),
    maximumNetworkRequests: z.literal(15),
    maximumAddressConnectionAttemptsPerRequest: z.literal(1),
    maximumRetries: z.literal(0),
    maximumFallbacks: z.literal(0),
    maximumRedirects: z.literal(0),
    maximumRequestBodyBytes: z.literal(131_072),
    maximumCapturedResponseBytes: z.literal(67_174_400),
    maximumElapsedMilliseconds: z.literal(900_000),
    addressFallbackAllowed: z.literal(false),
    proxyOrPacAllowed: z.literal(false),
  ...
  secretLocator: z.object({
    ...
    expectedSecretId: z.literal(CANONICAL_GOOGLE_GEMINI_EXPECTED_SECRET_ID),
    payloadMayAppearInPackageQueueOrEvidence: z.literal(false),
    metadataReadAuthorizedByProfile: z.literal(false),
    payloadReadAuthorizedByProfile: z.literal(false),
  }).strict(),
  costPolicy: z.object({
    currency: z.literal('USD'),
    providerBillingUnit: z.literal('input_and_video_output_tokens'),
    exactAttemptRateCardRequired: z.literal(true),
    providerUsageReconciliationRequired: z.literal(true),
    failedAndUnknownAttemptCostRetained: z.literal(true),
    providerAndInfrastructureCostSeparated: z.literal(true),
  ...
  readiness: z.object({
    operationIdentityFrozen: z.literal(true),
    primaryRouteOnly: z.literal(true),
    asyncLifecycleRequestAccountingFrozen: z.literal(true),
    privateInjectedAuthorizationAllowed: z.literal(true),
    previewAliasIsNotImmutableRevision: z.literal(true),
    immutableProviderRevisionQualified: z.literal(false),
    exactProductionRateAuthorityRequired: z.literal(true),
    providerTransportActivated: z.literal(false),
    cloudRuntimeQualified: z.literal(false),
...
export type CanonicalProviderOperationProfileV4 = z.infer<
  typeof canonicalProviderOperationProfileV4Schema
>

const canonicalProviderWorkAuthorizationV4CoreSchema = z.object({
  schemaVersion: z.literal(CANONICAL_PROVIDER_WORK_AUTHORIZATION_V4_VERSION),
  source: z.literal('canonical_approved_package_provider_work_authority'),
  authorityClass: z.literal('private_injected_nonprovider_test'),
  ownerUserId: identity,
  workspaceId: identity,
  ...
  providerModelId: z.literal(CANONICAL_GOOGLE_VISUAL_CALIBRATION_MODEL_ID),
  providerExecutionMode: z.literal('primary'),
  visualCalibrationContext: canonicalVisualCalibrationContextSchema,
  ...
  costPolicyHash: sha256,
  providerRateAuthority: z.object({
    evidenceClass: z.literal('official_public_pricing_snapshot_unreleased'),
    snapshotId: identity,
    snapshotDigest: sha256,
    sourceCode: z.literal('google_gemini_api_pricing_2026_07_21'),
    billingUnit: z.literal('input_and_video_output_tokens'),
  ...
  boundaries: z.object({
    ...
    rawCredentialAllowed: z.literal(false),
    rawPromptOrRequestBodyPersistedInQueue: z.literal(false),
    providerUrlPersisted: z.literal(false),
...
export const canonicalProviderWorkAuthorizationV4Schema =
  canonicalProviderWorkAuthorizationV4CoreSchema.extend({
    authorityHash: sha256,
  }).strict().superRefine((value, context) => {
    if (
      value.expectedOutputId !== value.expectedOutputIds[0] ||
      ...
      value.visualCalibrationContextDigest !==
        sha256AuthorityValue(value.visualCalibrationContext) ||
      value.maximumAuthorizedTotalInternalCostMicros !==
        value.maximumAuthorizedProviderCostMicros +
          value.maximumAuthorizedInfrastructureCostMicros ||
      Date.parse(value.expiresAt) <= Date.parse(value.authorizedAt) ||
      Date.parse(value.providerRateAuthority.expiresAt) <=
        Date.parse(value.authorizedAt) ||
      Date.parse(value.providerRateAuthority.capturedAt) >
        Date.parse(value.authorizedAt)
    ) {
      context.addIssue({
        code: 'custom',
        message: 'Canonical provider-work V4 authorization policy is inconsistent.',
      })
    }
  })

export type CanonicalProviderWorkAuthorizationV4 = z.infer<
  typeof canonicalProviderWorkAuthorizationV4Schema
>

export interface CreateCanonicalProviderWorkAuthorizationV4Input {
  ownerUserId: string
  executionPackage: CanonicalApprovedEditExecutionPackage
  queueDefinition: CanonicalPrivatePackageWorkQueueDefinition
  jobId: string
  expectedOutputId: string
  visualCalibrationContext: z.input<typeof canonicalVisualCalibrationContextSchema>
  sourceRequestId: string
  sourceRequestDigest: string
  providerRequestPayloadDigest: string
  projectDataPolicyDigest: string
  providerAccountPolicyDigest: string
  idempotencyKey: string
  providerRateAuthority: CanonicalProviderWorkAuthorizationV4['providerRateAuthority']
  maximumAuthorizedProviderCostMicros: number
  maximumAuthorizedInfrastructureCostMicros: number
  authorizedAt: string
  expiresAt: string
...
export function createCanonicalProviderOperationRegistryV4(): readonly [
  CanonicalProviderOperationProfileV4,
] {
  const withoutHash = {
    schemaVersion: CANONICAL_PROVIDER_OPERATION_REGISTRY_V4_VERSION,
    operationId: CANONICAL_GOOGLE_VISUAL_CALIBRATION_OPERATION_ID,
    intent: 'visual_calibration_candidate' as const,
    providerBoundaryProfileId:
      CANONICAL_GOOGLE_VISUAL_CALIBRATION_BOUNDARY_PROFILE_ID,
    providerRouteId: CANONICAL_GOOGLE_VISUAL_CALIBRATION_ROUTE_ID,
    providerModelId: CANONICAL_GOOGLE_VISUAL_CALIBRATION_MODEL_ID,
    immutableProviderRevision: null,
    expectedWorkItemType: 'generate_visual_calibration_candidate' as const,
    expectedWorkerClass: 'provider_worker' as const,
    expectedOutput: {
      role: 'provider_visual_calibration_video_mp4' as const,
      artifactType: 'provider_visual_calibration_video_mp4' as const,
      assetRole: 'generated' as const,
      contentType: 'video/mp4' as const,
      maximumByteLength: 67_108_864 as const,
      privateCreateOnlyRequired: true as const,
    ...
    generationBounds: {
      minimumDurationSeconds: 3 as const,
      maximumDurationSeconds: 10 as const,
      maximumPixelCount: 921_600 as const,
      frameRate: 24 as const,
    },
    requestPolicy: {
      maximumSecretPayloadReads: 1 as const,
      maximumPrivateInputUploads: 0 as const,
      maximumGenerationSubmissions: 1 as const,
      maximumStatusReads: 12 as const,
      maximumResultReads: 1 as const,
      maximumBinaryDownloads: 1 as const,
      maximumCancellations: 0 as const,
      maximumNetworkRequests: 15 as const,
      maximumAddressConnectionAttemptsPerRequest: 1 as const,
      maximumRetries: 0 as const,
      maximumFallbacks: 0 as const,
      maximumRedirects: 0 as const,
      maximumRequestBodyBytes: 131_072 as const,
      maximumCapturedResponseBytes: 67_174_400 as const,
      maximumElapsedMilliseconds: 900_000 as const,
      addressFallbackAllowed: false as const,
      proxyOrPacAllowed: false as const,
    ...
    secretLocator: {
      ...
      expectedSecretId: CANONICAL_GOOGLE_GEMINI_EXPECTED_SECRET_ID,
      payloadMayAppearInPackageQueueOrEvidence: false as const,
      metadataReadAuthorizedByProfile: false as const,
      payloadReadAuthorizedByProfile: false as const,
    },
    costPolicy: {
      currency: 'USD' as const,
      providerBillingUnit: 'input_and_video_output_tokens' as const,
      exactAttemptRateCardRequired: true as const,
      providerUsageReconciliationRequired: true as const,
      failedAndUnknownAttemptCostRetained: true as const,
      providerAndInfrastructureCostSeparated: true as const,
    ...
    readiness: {
      ...
      asyncLifecycleRequestAccountingFrozen: true as const,
      privateInjectedAuthorizationAllowed: true as const,
      previewAliasIsNotImmutableRevision: true as const,
      immutableProviderRevisionQualified: false as const,
      exactProductionRateAuthorityRequired: true as const,
      providerTransportActivated: false as const,
      cloudRuntimeQualified: false as const,
  ...
  return Object.freeze([
    Object.freeze(canonicalProviderOperationProfileV4Schema.parse({
      ...withoutHash,
      profileHash: sha256AuthorityValue(withoutHash),
    })),
  ]) as readonly [CanonicalProviderOperationProfileV4]
}

export function resolveCanonicalProviderOperationV4(
  operationId: string,
): CanonicalProviderOperationProfileV4 {
  const profile = createCanonicalProviderOperationRegistryV4().find((candidate) =>
    candidate.operationId === operationId)
  if (!profile) {
    throw new Error(`Canonical provider operation V4 ${operationId} is not registered.`)
  }
  return profile
}

export function createCanonicalProviderWorkAuthorizationV4(
  input: CreateCanonicalProviderWorkAuthorizationV4Input,
): CanonicalProviderWorkAuthorizationV4 {
  ...
  const productionAuthorityRef =
    executionPackage.componentRefs.motionStudioStorytellingProductionAuthority
  if (
    !packageJob || !workItem || !expectedOutput ||
    !styleAuthorityRef || !productionAuthorityRef ||
    styleAuthorityRef.sha256 !==
      input.visualCalibrationContext.storytellingStyleAuthorityRefDigest ||
    productionAuthorityRef.sha256 !==
      input.visualCalibrationContext.storytellingProductionAuthorityRefDigest ||
    executionPackage.workspaceId !== queueDefinition.identity.workspaceId ||
    executionPackage.projectId !== queueDefinition.identity.projectId ||
    executionPackage.editSessionId !== queueDefinition.identity.editSessionId ||
    executionPackage.packageRecordId !== queueDefinition.identity.packageRecordId ||
    ...
    expectedOutput.contentType !== profile.expectedOutput.contentType ||
    !expectedOutput.required ||
    !executionPackage.approvedProviderRoutes.includes(profile.providerRouteId)
  ) {
    throw new Error(
      'Canonical visual-calibration provider work requires one exact funded package and one private MP4 output.',
    )
  }
  const authorizedAtMs = Date.parse(input.authorizedAt)
  const expiresAtMs = Date.parse(input.expiresAt)
  if (
    ...
    expiresAtMs <= authorizedAtMs ||
    expiresAtMs - authorizedAtMs > 60 * 60 * 1_000
  ) throw new Error('Canonical visual-calibration authorization window is invalid.')
  const idempotencyKey = input.idempotencyKey.trim()
  if (idempotencyKey.length < 16 || idempotencyKey.length > 240) {
    throw new Error('Canonical visual-calibration work requires a bounded idempotency key.')
  }
  for (const value of [
    input.maximumAuthorizedProviderCostMicros,
    input.maximumAuthorizedInfrastructureCostMicros,
  ]) {
    if (!Number.isSafeInteger(value) || value <= 0 || value > 5_000_000) {
      throw new Error('Canonical visual-calibration cost ceiling is invalid.')
    }
  }
  const visualCalibrationContext = canonicalVisualCalibrationContextSchema.parse(
    input.visualCalibrationContext,
  )
  const expectedOutputIds = [input.expectedOutputId] as [string]
  const expectedOutputKeys = [expectedOutput.outputKey] as [string]
  const payload = {
    schemaVersion: CANONICAL_PROVIDER_WORK_AUTHORIZATION_V4_VERSION,
    source: 'canonical_approved_package_provider_work_authority' as const,
    authorityClass: 'private_injected_nonprovider_test' as const,
    ownerUserId: input.ownerUserId,
    workspaceId: executionPackage.workspaceId,
    ...
    providerAccountPolicyDigest: input.providerAccountPolicyDigest,
    idempotencyKeyHash: sha256AuthorityValue({
      domain: 'reeditpro:canonical-provider-work-idempotency:v4',
      key: idempotencyKey,
    }),
    requestPolicyHash: sha256AuthorityValue(profile.requestPolicy),
    costPolicyHash: sha256AuthorityValue(profile.costPolicy),
    ...
    boundaries: {
      ...
      rawCredentialAllowed: false as const,
      rawPromptOrRequestBodyPersistedInQueue: false as const,
      providerUrlPersisted: false as const,
  ...
  return canonicalProviderWorkAuthorizationV4Schema.parse({
    ...payload,
    authorityHash: sha256AuthorityValue(payload),
  })
}

export function assertCanonicalProviderWorkAuthorizationV4(input: {
  value: unknown
  queueDefinition: CanonicalPrivatePackageWorkQueueDefinition
  now: string
}): CanonicalProviderWorkAuthorizationV4 {
  const parsed = canonicalProviderWorkAuthorizationV4Schema.parse(input.value)
  const { authorityHash, ...payload } = parsed
  const queueJob = requiredQueueJob(input.queueDefinition, parsed.queueJobId)
  const profile = resolveCanonicalProviderOperationV4(parsed.operationId)
  if (
    authorityHash !== sha256AuthorityValue(payload) ||
    parsed.queueDefinitionHash !== input.queueDefinition.definitionHash ||
    parsed.workspaceId !== input.queueDefinition.identity.workspaceId ||
    ...
    parsed.costPolicyHash !== sha256AuthorityValue(profile.costPolicy) ||
    Date.parse(parsed.expiresAt) <= Date.parse(input.now) ||
    Date.parse(parsed.providerRateAuthority.expiresAt) <= Date.parse(input.now)
  ) throw new Error('Canonical provider-work V4 authorization is stale or changed.')
  return parsed
}

export function canonicalProviderWorkAuthorizationRequestHashV4(
  authorization: CanonicalProviderWorkAuthorizationV4,
): string {
  return sha256AuthorityValue({
    domain: 'reeditpro:canonical-provider-work-authorization-request:v4',
    authorizationHash: authorization.authorityHash,
    queueDefinitionHash: authorization.queueDefinitionHash,
    queueJobDefinitionHash: authorization.queueJobDefinitionHash,
    idempotencyKeyHash: authorization.idempotencyKeyHash,
...
export function canonicalProviderOperationRegistryV4Hash(): string {
  return sha256AuthorityValue(createCanonicalProviderOperationRegistryV4())
}
