...
| Revision and recovery | `private_executable` through accepted second review, superseded-history recovery, and pre-dispatch cancellation | An exact review-bound revision completes snapshot-v2 execution/acceptance. Fresh history-service instances reopen current and superseded QA-passed MP4 bytes. A restart-safe cancellation saga persists a pending fence, releases/expires active never-started leases, then atomically releases a fully unused synthetic reservation while preserving snapshot/job/package/lease evidence. | Cancellation after dispatch or execution-fence start, deployed disaster recovery/retention, and real-user/browser handoff remain gated. |
...
| Authenticated private upload-to-review pipeline | backend planning handoff plus bounded two-pass execution fixture | The authenticated backend binds finalized local/private uploads and current planning inputs into exact plan-publication authority. A server-derived fixture reaches first review, revision, accepted second review, and restart-safe current/superseded history download. Browser consumption and deployed real-user operations remain blocked. |
...
14. **Completed for authenticated local/private pre-dispatch scope:** persist a cancellation-pending fence, terminally release/expire never-started leases, atomically cancel the fully unused synthetic reservation, preserve immutable snapshot/job/package/lease evidence, serialize against lease claim, and deny future active package authority. Post-dispatch and started-execution compensation remain gated.
