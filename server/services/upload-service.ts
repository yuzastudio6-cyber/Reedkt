...
function mediaAssetTypeForUpload(uploadIntent: UploadIntentView): string {
  ...
  if (uploadIntent.uploadPurpose === 'qa_artifact') return 'qa_report'
  if (uploadIntent.uploadPurpose === 'worker_temp') return 'worker_temp'
  if (uploadIntent.uploadPurpose === 'processed_media') return isVideo ? 'processed_video' : isAudio ? 'processed_audio' : 'processed_asset'
