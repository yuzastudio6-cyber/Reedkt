...
Current integration head: `4e0ca864be052008cea86e287a683b687a8139f1`
...
1. `RP-EXTERNAL-BETA-CONTROLLED-PRIVATE-INVITE-IAM-GRANT-1`
2. future provider/model runtime confirmation packet only if a real call is explicitly approved later

Controlled external beta is enabled on the staging API only. The correct status is `controlled_external_beta_private_invite_access_policy_ready` with `external beta enabled in this phase: true`. The next access step requires an explicit invited identity list and guarded IAM grant confirmation.
