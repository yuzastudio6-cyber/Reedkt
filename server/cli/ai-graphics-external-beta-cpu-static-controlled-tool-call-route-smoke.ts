...
import {
  AI_GRAPHICS_EXTERNAL_BETA_CPU_STATIC_CONTROLLED_TOOL_CALL_ROUTE_FLAG,
  AI_GRAPHICS_EXTERNAL_BETA_CPU_STATIC_CONTROLLED_TOOL_CALL_ROUTE_PATH,
  type AiGraphicsExternalBetaCpuStaticControlledToolCallRequest,
  listAiGraphicsExternalBetaCpuStaticControlledToolCallReadinessCases,
} from '../routes/ai-graphics-external-beta-cpu-static-controlled-tool-call-routes'
...
interface RouteSmokeResult {
  ...
  controlledCpuStaticRouteExecutionPerformed: boolean
...
async function postToolCall(
  baseUrl: string,
  request: AiGraphicsExternalBetaCpuStaticControlledToolCallRequest,
) {
  const response = await fetch(
    `${baseUrl}${AI_GRAPHICS_EXTERNAL_BETA_CPU_STATIC_CONTROLLED_TOOL_CALL_ROUTE_PATH}`,
    {
      method: 'POST',
      headers: {
...
async function runRouteSmokeCase(
  ...
  return {
    ...
    controlledCpuStaticRouteExecutionPerformed:
      data.controlledCpuStaticRouteExecutionPerformed === true,
...
async function main() {
  const cases = listAiGraphicsExternalBetaCpuStaticControlledToolCallReadinessCases()
  ...
  for (const result of routeResults) {
    ...
    assert(
      ...
      `${result.toolId} was not marked scoped callable`,
    )
    assert(
      result.controlledCpuStaticRouteExecutionPerformed === true,
  ...
  const report = {
    ...
    controlledCpuStaticRouteExecutedTools: routeResults.length,
    controlledCpuStaticRouteCallableNowTools: routeResults.filter(
      (result) => result.externalAgentCanExecuteCpuStaticControlledToolsNow,
    ).length,
    ...
    routeResults,
    booleans: {
      externalBetaCpuStaticControlledToolCallRouteSmokePassed: true,
