...
export function createSoundEffectPlan(
  ...
  const plan: SoundEffectPlanRecord = {
    ...
    strategy: 'Detailed SFX Director planning happens separately. Default SFX supports edit layers, not every source-footage action.',
    effects: [],
    avoidRules: [
      'Avoid loud SFX under speech.',
      'Do not add fake source-action sounds by default.',
      'Use SFX only when a planned edit layer or story cue benefits from it.',
    ],
    createdAt: nowIso(),
    updatedAt: nowIso(),
    metadata: { mockOnly: true },
