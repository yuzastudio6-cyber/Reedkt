...
function buildNextActions(
  ...
  actions.push('Before staging API deploy, rerun the read-only default-branch input discovery workflow with gh workflow run beta-readiness-api-staging-input-discovery.yml --repo yuzastudio6-cyber/Reedkt --ref codex/reeditpro-web-ui-shell --field confirm_staging_api_input_discovery=READ_STAGING_BETA_API_DEPLOY_INPUTS, or have the owner provide exact Artifact Registry repository, deployer service account, and runtime API service account values; current source truth records Artifact Registry list and service-account list IAM blockers.')
