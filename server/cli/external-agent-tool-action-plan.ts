...
function actionForTool(toolId: string) {
  ...
  if (toolId === 'ai_video_broll_generation_wan') {
    return {
      ...
      externalManualBlocker:
        'B-roll 9S attempted the us-west1-b bounded no-idle IAP wheelhouse transfer proof and stocked out before VM creation; the future 9T no-VM stockout-fix prompt is required before another transfer proof VM attempt, dependency install, or inference; do not create VMs from this rollup',
      afterBlockerClears:
        'run the separate 9T no-VM stockout-fix prompt before selecting another transfer proof target or considering any capacity strategy',
    }
  }
