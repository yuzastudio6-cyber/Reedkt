import {
  ...
  writeSecondControlledCandidateApprovalArtifacts,
...
if (!execute) {
  console.log(summarizeSecondControlledCandidateApproval())
  process.exit(0)
}

if (!metadataOnly) {
  throw new Error(
    'Use --metadata-only. This approval packet cannot execute the second candidate, tools, routes, workers, providers, media, Supabase writes, GCS uploads, public artifacts, signed URLs, raw prompts, or runtime paths.'
  )
}
...
const reports = writeSecondControlledCandidateApprovalArtifacts()
console.log(summarizeSecondControlledCandidateApproval(reports))
