...
| --- | --- | --- | --- | --- |
| `qwen2_5_vl_7b_instruct` | controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference retry gate | auth verified, runtime blocked | read-only gcloud token refresh, Cloud Run service describe, and private caller job describe now pass; bounded private inference retry planning and the 58DS retry gate are recorded, and retry attempt approval remains required before any invocation, model load, or inference | `QWEN2_5_VL_STACK_TOOL_58DT-PRIVATE-INFERENCE-RETRY-ATTEMPT-APPROVAL: approve one bounded approved-fixture private inference retry after gate verification, no inference/no mutation` |
...
1. Qwen: auth/service/job visibility is now verified and the bounded approved-fixture private inference retry plan is recorded; re-run read-only preflight immediately before any future runtime gate.
...
- Qwen is the closest lane to controlled private model inference; auth/service/job visibility is now verified and the bounded private inference retry plan is recorded, but runtime remains blocked until a retry gate is recorded.
