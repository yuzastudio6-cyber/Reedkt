...
- Skill key: `music`
- Skill version: `1.0.0`
- Contract: `music.skill_contract.v1`
- Manifest hash:
  `33b4abdd325fed0d44acaee9818d6ffa027df506d18dd684905e0f94916ba7bd`
...
The service owns request validation, admission, scope enforcement, structured
context study, Music-need and silence decisions, narrative arc, exact cue sheet,
...
The frozen profile is `music.provider.google_lyria_3_pro_preview.v1`, using
...
Injected transport returns real WAV bytes. The canonical graph independently
...
v4 and creates the final handoff. Unknown outcomes reconcile before replay.
...
range, selected asset, rational rate, operation parameters, protected speech,
approval, cost ceiling and idempotency. Music validates the Sound caller
receipt, route and artifacts; Sound mutation receipts determine actual Music
mutation ranges. Nested Sound costs remain separate and are not double-counted.
...
| `npm run test:sound-acceptance` | passed within aggregate and independently |
...
- Items 130–137: dedicated CI, pushed SHA parity, clean worktree and final
  standalone status — verified after the closure commit is pushed; no Head of
  Orchestra or other top-level skill was implemented.
...
- Final acceptance/CI commit — contains provider live boundary, evidence
  registry, UI integration, aggregate validation and this closure report.
