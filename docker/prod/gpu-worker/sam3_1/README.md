...
- A100 80 GB uses `a100_full_gpu_state_v1`. L4 uses the separately qualified
  ...
  fresh CUDA session and streams the upstream postprocessor one frame at a time
  instead of batching 16 frames. This bounds the L4 frame-16 reconditioning
  peak without changing source resolution, temporal coverage, or the required
  object set. Frames, active memory, model inference, and outputs remain on CUDA; CPU
