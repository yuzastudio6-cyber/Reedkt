...
function invokeControlledLimitedNoRealMediaBoundary(envelope) {
  const stopReasons = []
  ...
  return {
    ...
    controlledLimitedNoRealMediaBoundaryInvoked: true,
...
const sourceBoundary = parseJsonBlock(
  sourceBoundaryDoc,
...
assert(source.preflightResult.controlledLimitedNoRealMediaToolExecutionProofMayProceedNext === true, 'source does not allow proof')
assert(source.preflightResult.vagueOwnerProofAccepted === false, 'source accepted vague owner proof')
assert(source.preflightResult.toolCountCovered === 15, 'source proof tool count mismatch')
assert(source.soundCpuTools.readyForControlledLimitedNoRealMediaToolExecutionProof === 15, 'source proof readiness count mismatch')
...
assert(prompt.proofScope.mayRunControlledLimitedNoRealMediaToolExecutionProof === true, 'prompt does not allow controlled proof')
assert(prompt.proofScope.allowedToolCount === 15, 'prompt tool count mismatch')
assert(sourceBoundary.allowedNextProofEnvelope.requiresSanitizedWhatHappenedOutput === true, 'source boundary lacks what-happened proof requirement')

for (const key of Object.keys(runtimeFlags)) {
  const promptKey = `${key}Today`
  if (promptKey in prompt.proofScope) {
    assert(prompt.proofScope[promptKey] === false, `prompt scope ${promptKey} must be false`)
  } else if (key === 'allowRealUserMedia') {
    assert(prompt.proofScope.allowRealUserMedia === false, 'prompt scope allowRealUserMedia must be false')
  }
}

const toolDescriptors = tools.map((toolId, index) => ({
  toolId,
  descriptorId: `phase117-no-real-media-tool-${String(index + 1).padStart(2, '0')}`,
  descriptorKind:
    index % 4 === 0
...
const invocations = jobTypes.map((jobType, index) => {
  const envelope = {
    approvedPlanSnapshotId: 'phase117-synthetic-approved-plan-snapshot',
    workspaceId: 'phase117-synthetic-workspace',
    projectId: 'phase117-synthetic-project',
    toolCallRequestId: `phase117-no-real-media-tool-call-${index + 1}`,
    idempotencyKey: `phase117-controlled-limited-no-real-media-proof-${jobType}`,
  ...
  return {
    ...
    result: invokeControlledLimitedNoRealMediaBoundary(envelope),
  }
})
...
const allInvocationsPassed = invocations.every(
  ({ result }) =>
    result.ok &&
    result.controlledLimitedNoRealMediaBoundaryInvoked &&
...
const proof = {
  ...
  proofKind: 'controlled_limited_no_real_media_tool_execution_boundary',
  sourceDecisionVerified: true,
  controlledLimitedNoRealMediaBoundaryInvoked: allInvocationsPassed,
