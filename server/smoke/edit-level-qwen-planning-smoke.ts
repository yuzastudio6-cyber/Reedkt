...
import {
  ...
  modelRoleAllowsToolCode,
  modelRoleAllowsUserReasoning,
...
import {
  ...
  validateReEditProModelRoleUse as validateBackendExportedReEditProModelRoleUse,
...
const requiredFiles = [
  'src/types/model-role-routing.ts',
  'src/lib/model-role-routing-contract.ts',
...
assert.equal(createEditLevelQwenPlanningUserSummary('premium').includes('deeper planning reasoning'), true)
...
assert.equal(createEditLevelQwenPlanningBoundarySummary().join(' ').includes('no AI planning call'), true)
...
assert.equal(createEditLevelQwenPlanningSummary({ level: 'premium' }).userFacingSummary.includes('deeper planning reasoning'), true)
