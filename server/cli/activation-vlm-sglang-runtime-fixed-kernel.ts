...
if (!execute) {
  console.log([
    'Phase 39C-SG-FIXED SGLang fixed-kernel CLI',
    'No execution performed.',
    'Plan/report/cost/web-research/smoke modes are non-mutating by default.',
    ...
    'This CLI never downloads new Qwen models, uploads model weights, processes real media, accepts raw prompts, calls providers, creates public artifacts, unlocks beta/production, or touches Track A.',
  ].join('\n'))
} else {
  ...
  if (json) console.log(JSON.stringify(result, null, 2))
    ...
    console.log([
      'Phase 39C-SG-FIXED SGLang fixed-kernel compatibility completed with recorded status.',
      `Run: ${result.runId}`,
