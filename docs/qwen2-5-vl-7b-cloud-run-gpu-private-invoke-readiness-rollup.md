...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_real_dispatch_approved_fixture_private_inference_auth_refresh_verified_retry_plan_required`.
...
| Controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference attempt | blocked controlled persisted worker dispatch runtime real-dispatch approved fixture private inference auth verified retry plan required | The 58DQ attempt result and 58DQ auth-refresh result are recorded. Non-interactive access-token refresh, service describe, and job describe now pass in this shell. No service update, CPU caller job execution, identity-token fetch for the service, private request, model load, or inference occurred. | Record a bounded approved-fixture private inference retry plan, then rerun auth/service/job verification immediately before any future runtime gate. |
