...
} from '../services/tracka-three-tool-external-agent-persistent-job-queue-dry-run-1'

const reportRoot = '/tmp/reeditpro-tracka-three-tool-external-agent-persistent-job-queue-dry-run-1'
const reportFileName = 'three-tool-external-agent-persistent-job-queue-dry-run-report.json'
const manifestFileName = 'three-tool-external-agent-persistent-job-queue-dry-run-manifest.json'
...
async function main(): Promise<void> {
  ...
  const result = runThreeToolExternalAgentPersistentJobQueueDryRun()
  ...
  console.log(JSON.stringify({
    ...
    idempotencyKey: result.queueDryRun.idempotencyKey,
