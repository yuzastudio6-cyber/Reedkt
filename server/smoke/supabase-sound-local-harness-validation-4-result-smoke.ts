...
import { SUPABASE_SOUND_LOCAL_HARNESS_VALIDATION_4_RESULT } from '../../src/backend/mock/mock-supabase-sound-local-harness-validation-4-result'

const reportPath = 'docs/supabase-sound-local-harness-validation-4-report.md'
const specPath = 'src/backend/mock/mock-supabase-sound-local-harness-validation-4-result.ts'
const activeDraftMigrationPath = 'supabase/migrations/999_supabase_sound_local_fixture_records_draft.sql'
const scriptName = 'smoke:supabase-sound-local-harness-validation-4-result'
const scriptCommand =
  'tsx server/smoke/supabase-sound-local-harness-validation-4-result-smoke.ts'
...
check(existsSync(reportPath), 'Harness validation 4 report must exist.')
check(existsSync(specPath), 'Harness validation 4 result spec must exist.')
...
const result = SUPABASE_SOUND_LOCAL_HARNESS_VALIDATION_4_RESULT
...
check(result.mode === 'local_supabase_harness_validation_4_result', 'Mode must match.')
...
check(
  result.harness.selectedHarnessPath === 'local_supabase_cli_stack',
  'Selected harness path must be local Supabase CLI stack.',
)
check(result.harness.selectedPortRange === '55420-55429', 'Selected port range must match.')
check(result.harness.supabaseCliUsed === true, 'Supabase CLI must be used only for local start.')
check(result.harness.dockerUsed === true, 'Docker must be inspection-only after failed start.')
...
check(
  result.harness.reeditproLocalStoppedResetRemoved === false,
  'Unrelated local stack must not be stopped, reset, or removed.',
)
check(result.harness.portConflictDetected === false, 'Port conflict must not be present.')
check(result.harness.startupFailedBeforeSql === true, 'Startup failure must be recorded.')
check(
  result.harness.sanitizedStartupFailure ===
    'column reference "description" is ambiguous (SQLSTATE 42702)',
  'Sanitized startup failure must match.',
)
check(result.harness.harnessStartedByThisPrompt === false, 'Harness must not remain started.')
check(result.harness.harnessStoppedByThisPrompt === false, 'Harness must not be stopped.')
check(result.harness.cleanupVerified === true, 'Cleanup must be verified.')
...
check(result.safety.localUrlsOnly === true, 'Safety must require local URLs only.')
check(
  result.safety.selectedPortsVerifiedFreeBeforeStart === true,
  'Selected ports must be verified free before start.',
)
...
scanForbiddenValues(result, 'SUPABASE_SOUND_LOCAL_HARNESS_VALIDATION_4_RESULT')
  ...
  JSON.stringify(
    {
      ...
      smoke: 'supabase-sound-local-harness-validation-4-result',
      mode: result.mode,
      ...
      portConflictDetected: result.harness.portConflictDetected,
      startupFailedBeforeSql: result.harness.startupFailedBeforeSql,
      reeditproLocalStoppedResetRemoved: result.harness.reeditproLocalStoppedResetRemoved,
