...
| Canonical private job execution | `private_executable` for one immutable job | Authenticated job-only route derives the approved work item, output, tool, operation, lease, dispatch, private artifact, QA, reconciliation, and replay from server authority. Before approval, a content-addressed plan-time manifest now reconciles planner declarations and work items with stable identities and proof hashes from the server catalog. Exact source-trim, Remotion final-composition, and dependency-bound final-QA jobs pass through the adapter. | Product, beta, and production flags remain false. A plan-time identity manifest does not replace package, lease, dispatch, artifact, QA, or deployment evidence. |
...
The legacy upload journey still reports its historical fail-closed seam, and the legacy standalone-credit browser journey remains blocked. Separately, the authenticated local/private canonical path now preserves unique incremental source identity and finalized media identity, synchronizes current Exact Edit Preferences into server authority, derives verified source/frame evidence, and can present and approve an exactly representable source-and-caption plan through frontend-safe coordinators. The result for that narrow graph is one immutable snapshot, one synthetic private-test reservation, and derived jobs with no execution. The real two-source/rich named-edit aggregate scenario now proves the earlier authority path and then exits successfully with `blocked_by_exact_multi_source_and_rich_work_item_compilation`; it does not silently simplify the plan or approve it. The canonical backend fixture independently completes both review passes plus authenticated current/superseded history recovery. Browser package/execution/review mutations and deployed real-user infrastructure remain required.
...
| Authenticated private upload-to-review pipeline | narrow exact plan presentation/approval connected; multi-source/rich browser compilation blocked; bounded two-pass execution fixture remains backend-only | The authenticated backend persists a checksum-protected tenant-scoped planning handoff and content-addressed publication candidate, then presents one server-owned canonical plan only when the plan is exactly representable by the narrow source-and-caption graph. The named-edit browser binds the visible plan version/hash and estimate identity/hash/maximum to a frontend-safe approval coordinator. Exact concurrent approval converges on one immutable snapshot and synthetic private-test reservation; jobs are derived but not executed. A read-only `canonical-journey` endpoint recovers the approved snapshot stage without raw authority. The real two-source/rich editor scenario stops before presentation/approval, while independent backend fixtures still prove the bounded work-graph review/revision lifecycle. | Browser multi-source/rich work-item compilation, package request, execution advancement, review mutations, and deployed real-user operations remain blocked. |
...
> ReeditPro has deterministic planning and canonical approval authority plus 50 exact private tool lifecycles. Its named-edit browser can synchronize exact local/private planning authority and, for the currently proven narrow source-and-caption graph, present and approve the exact version, estimate, and visible credit maximum into an immutable snapshot with a synthetic test reservation—without starting execution. Rich multi-source plans persist their verified planning authority but remain approval-blocked until exact work-item compilation is proven. Authenticated backend fixtures independently execute a bounded original review and revision pass with final QA and restart-safe history recovery. Browser execution/review mutations, deployed real-user infrastructure, external beta, public delivery, live billing, Supabase/RLS tenancy, distributed workers, and production promotion remain gated.
