import fs from 'node:fs'
import path from 'node:path'
import { spawnSync } from 'node:child_process'

const root = process.cwd()

const requiredDocs = [
  'docs/supabase-fk-index-migration-plan.md',
  'docs/supabase-fk-index-priority-matrix.md',
  'docs/supabase-fk-index-duplicate-review-contract.md',
  'docs/supabase-fk-index-naming-contract.md',
  'docs/supabase-fk-index-write-amplification-risk.md',
  'docs/supabase-fk-index-future-test-matrix.md',
  'docs/supabase-fk-index-rollback-cleanup-plan.md',
  'docs/supabase-fk-index-staging-evidence-requirements.md',
  'docs/draft-sql/supabase-advisor-remediation/fk-indexes-draft.sql.md',
  'docs/prompt-26h-validation-results.md',
  'docs/implementation-prompts/prompt-26h-fk-index-hardening-migration-plan.md',
]

const trackerFiles = [
  'PRODUCTION_FOUNDATION_STATUS.md',
  'docs/source-of-truth-map.md',
  'docs/production-milestone-plan.md',
  'docs/implementation-prompts/README.md',
  'docs/beta-readiness-scorecard.md',
  'docs/production-beta-blocker-inventory.md',
  'docs/supabase-milestone-sync-matrix.md',
  'docs/supabase-advisor-hardening-prompt-sequence.md',
  'docs/supabase-fk-index-hardening-plan.md',
  'docs/supabase-fk-index-draft-remediation-packet.md',
  'docs/supabase-advisor-hardening-priority-matrix.md',
  'package.json',
  'scripts/validation/run-foundation-validation.mjs',
  '.github/workflows/foundation-validation.yml',
]

const fkFindings = [
  'api_idempotency_keys.user_id',
  'approved_plan_snapshots.approved_by_user_id',
  'approved_plan_snapshots.credit_approval_id',
  'chat_actions.workspace_id',
  'chat_attachments.workspace_id',
  'credit_approvals.chat_action_id',
  'credit_approvals.chat_message_id',
  'credit_estimates.chat_message_id',
  'credit_reservations.chat_session_id',
  'chat_actions.executed_by',
  'chat_attachments.media_asset_id',
  'chat_messages.actor_user_id',
  'credit_ledger_entries.related_chat_action_id',
  'edit_plans.created_by_user_id',
  'ambient_sound_plans.audio_environment_analysis_id',
]

const requiredLabels = [
  'DRAFT ONLY — DO NOT EXECUTE',
  'NOT AN ACTIVE MIGRATION',
  'not validated',
  'not applied to Supabase',
  'requires future prompt',
]

const requiredDocTerms = {
  'docs/supabase-fk-index-migration-plan.md': [
    'FK index migration plan status: `fk_index_migration_plan_created`.',
    'Supabase update required: docs/status only.',
    'Supabase update status: docs_only.',
    'Supabase environment touched: none.',
    'SQL executed: none.',
    'Migration deployed: no.',
    'Active migration files changed: no.',
    'Production capability enabled: none; FK index hardening migration plan only.',
    'Prompt 26H-1 - FK Index Local Migration Candidate',
    'Prompt GD-0 - AI Tools / Graphic Design Stack Repo Audit',
    'SUPABASE_RLS_STORAGE_DATABASE',
  ],
  'docs/supabase-fk-index-priority-matrix.md': [
    'Priority matrix status: `fk_index_priority_matrix_created`.',
    'FK index migration plan status: `fk_index_migration_plan_created`.',
    'Duplicate review required: all candidates.',
    'High',
    'Medium',
    'Defer',
  ],
  'docs/supabase-fk-index-duplicate-review-contract.md': [
    'Duplicate review contract status: `fk_index_duplicate_review_planned`.',
    'Duplicate-index review executed: no.',
    'create_single_column_index_candidate',
    'skip_existing_index_covers',
  ],
  'docs/supabase-fk-index-naming-contract.md': [
    'Naming contract status: `fk_index_naming_contract_created`.',
    'idx_<table>_<column>',
  ],
  'docs/supabase-fk-index-write-amplification-risk.md': [
    'Write-amplification review status: `fk_index_write_amplification_review_planned`.',
    'Low',
    'Medium',
    'High',
  ],
  'docs/supabase-fk-index-future-test-matrix.md': [
    'Future test matrix status: `fk_index_future_tests_planned`.',
    'Local SQL run: no.',
    'Staging SQL run: no.',
  ],
  'docs/supabase-fk-index-rollback-cleanup-plan.md': [
    'Rollback plan status: `fk_index_rollback_requirements_defined`.',
    'Rollback executed: no.',
    'Cleanup executed: no.',
  ],
  'docs/supabase-fk-index-staging-evidence-requirements.md': [
    'Staging evidence status: `evidence_required`.',
    'Prompt 23 state on this base: `pending_human_approval`.',
    'Production readiness approved: no.',
  ],
  'docs/draft-sql/supabase-advisor-remediation/fk-indexes-draft.sql.md': requiredLabels,
  'docs/prompt-26h-validation-results.md': [
    'FK index migration plan status: `fk_index_migration_plan_created`.',
    'Supabase update status: docs_only.',
    'Supabase environment touched: none.',
    'SQL executed: none.',
    'Migration deployed: no.',
    'Active migration files changed: no.',
    'Google Cloud API touched: false.',
    'Secret Manager API touched: false.',
    'Production capability enabled: none; FK index hardening migration plan only.',
  ],
  'docs/implementation-prompts/prompt-26h-fk-index-hardening-migration-plan.md': [
    'Prompt 26H - FK Index Hardening Migration Plan',
    'Exact capability enabled: none; FK index hardening migration plan only.',
  ],
}

const trackerTerms = {
  'PRODUCTION_FOUNDATION_STATUS.md': [
    'FK Index Hardening Migration Plan',
    'docs/supabase-fk-index-migration-plan.md',
    'fk_index_migration_plan_created',
  ],
  'docs/source-of-truth-map.md': [
    'FK index hardening migration plan',
    'docs/supabase-fk-index-migration-plan.md',
  ],
  'docs/production-milestone-plan.md': [
    'Prompt 26H - FK Index Hardening Migration Plan',
    'fk_index_migration_plan_created',
  ],
  'docs/implementation-prompts/README.md': [
    '26H',
    'FK Index Hardening Migration Plan',
  ],
  'docs/beta-readiness-scorecard.md': [
    'Prompt 26H FK index hardening migration plan',
  ],
  'docs/production-beta-blocker-inventory.md': [
    'Prompt 26H records `fk_index_migration_plan_created`',
  ],
  'docs/supabase-milestone-sync-matrix.md': [
    '26H',
    'fk_index_migration_plan_created',
  ],
  'docs/supabase-advisor-hardening-prompt-sequence.md': [
    'Prompt 26H',
    'fk_index_migration_plan_created',
  ],
  'docs/supabase-fk-index-hardening-plan.md': [
    'Prompt 26H Update',
    'fk_index_migration_plan_created',
  ],
  'docs/supabase-fk-index-draft-remediation-packet.md': [
    'Prompt 26H Status',
    'fk_index_migration_plan_created',
  ],
  'docs/supabase-advisor-hardening-priority-matrix.md': [
    'Prompt 26H',
    'Prompt 26H-1',
  ],
  'package.json': [
    'supabase:fk-index:migration-plan:diagnostics',
  ],
  'scripts/validation/run-foundation-validation.mjs': [
    'supabase_fk_index_migration_plan_diagnostics',
  ],
  '.github/workflows/foundation-validation.yml': [
    'codex/rp-foundation-26g-security-definer-exposure-migration-plan',
  ],
}

const redactedProjectRef = 'wmyy****ishd'
const rawProjectRefPatterns = [/\b[a-z0-9]{20}\b/g]

const rawSecretPatterns = [
  /postgres(?:ql)?:\/\/[^@\s]+@/i,
  /\b(?:SUPABASE|POSTGRES|DATABASE|JWT|STRIPE|OPENAI|PROVIDER|GOOGLE|GCP)_[A-Z0-9_]*(?:KEY|SECRET|PASSWORD|TOKEN|URL)\s*=\s*[^<\s][^\s]+/i,
  /\b(service[_-]?role[_-]?key|jwt[_-]?secret|provider[_-]?key|stripe[_-]?key|database[_-]?password|secret manager value|secret payload)\s*[:=]\s*['"`]?[A-Za-z0-9_./+=-]{12,}/i,
  /eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{10,}/,
  /https?:\/\/[^/\s]+\/storage\/v1\/object\/sign\//i,
  /signedUrl\s*[:=]\s*['"`]https?:\/\//i,
  /signed_url\s*[:=]\s*['"`]https?:\/\//i,
]

const unsafeClaimPatterns = [
  /\bFK index (created|applied|deployed|validated|remediated|fixed|resolved)\b/i,
  /\bindex(?:es)? (created|applied|deployed|validated|remediated|fixed|resolved)\b/i,
  /\badvisor findings? (resolved|remediated|fixed|cleared|applied)\b/i,
  /\badvisor remediation (applied|executed|completed|validated)\b/i,
  /\bactive migration (created|added|changed|deployed)\b/i,
  /\bapplied_to_staging\b/i,
  /\bapplied_to_production\b/i,
  /\bvalidated_in_staging\b/i,
  /\bstaging (supabase|sql|rls|migration|index).{0,100}\b(ran|executed|passed|completed|validated|approved|applied)\b/i,
  /\bproduction (supabase|sql|readiness|beta|migration|index).{0,100}\b(ran|executed|passed|completed|validated|approved|ready|unlocked|applied)\b/i,
  /\bGoogle Cloud API touched:\s*yes\b/i,
  /\bSecret Manager API touched:\s*yes\b/i,
  /\bSecret Manager (metadata|values?|payloads?) (fetched|read|printed|retrieved|verified|accessed)\b/i,
  /\bSQL (ran|executed|passed|completed)\b/i,
  /\bSQL executed:\s*(?!none\b)/i,
  /\bMigration deployed:\s*yes\b/i,
  /\bActive migration files changed:\s*(?!no\b)/i,
  /\b(beta|production).{0,40}\bunlocked\b/i,
  /\bhuman approval (granted|approved)\b/i,
  /\bstaging execution approved:\s*yes\b/i,
]

const unsafeCommandPatterns = [
  /\bgcloud\b/i,
  /\bsupabase\s+link\b/i,
  /\bsupabase\s+db\s+push\b/i,
  /\bsupabase\s+db\s+reset\b/i,
  /\bsupabase\s+start\b/i,
  /\bsupabase\s+status\b/i,
  /\bsupabase\s+migration\s+up\b/i,
  /\bsupabase\s+functions\s+deploy\b/i,
  /\bpsql\b/i,
]

function resolvePath(relativePath) {
  return path.join(root, relativePath)
}

function exists(relativePath) {
  return fs.existsSync(resolvePath(relativePath))
}

function readFile(relativePath) {
  return exists(relativePath) ? fs.readFileSync(resolvePath(relativePath), 'utf8') : ''
}

function compact(value) {
  return String(value).replace(/\s+/g, ' ').slice(0, 260)
}

function finding(file, kind, line, excerpt) {
  return { file, kind, line, excerpt: compact(excerpt) }
}

function isSafeContext(line) {
  return /\b(no|not|never|forbid|forbidden|blocked|must not|do not|does not|did not|without|placeholder|reference only|metadata-only|future-only|draft only|template|redacted|none|pending|required|prohibited|warning|missing|not_applicable_no_evidence|evidence_required|access_not_verified|record only|triage only|plan only|planning only|classification only|contract only|not applied|not approved|not deployed|not remediated|not executed|not changed|not created|not fetched|not verified|not an active migration|sketch|future prompt|future migration|future candidate|future executable|future approved|review-only|preserve|candidate only|requirements defined|planned|recommended next prompt|validation result|local-only|sanitized localhost|local validation|local supabase|toolchain|environment-blocked|manual setup|non-mutating|allowing local|deliverables after prompt|commented sketches|candidate examples only)\b/i.test(line)
}

function scanPatterns(files, patterns, kind, { redact = false } = {}) {
  const findings = []
  for (const file of files) {
    readFile(file).split('\n').forEach((line, index) => {
      if (isSafeContext(line)) return
      for (const pattern of patterns) {
        if (pattern.test(line)) {
          findings.push(finding(file, kind, index + 1, redact ? '[redacted unsafe pattern]' : line))
        }
      }
    })
  }
  return findings
}

function scanProjectRefs(files) {
  const findings = []
  for (const file of files) {
    readFile(file).split('\n').forEach((line, index) => {
      if (line.includes(redactedProjectRef) || isSafeContext(line)) return
      for (const pattern of rawProjectRefPatterns) {
        const matches = line.match(pattern) || []
        for (const match of matches) {
          if (/^[0-9]+$/.test(match)) continue
          findings.push(finding(file, 'rawProjectRefRisk', index + 1, `[redacted project-ref-like token length=${match.length}]`))
        }
      }
    })
  }
  return findings
}

function listFiles(directory) {
  const absolute = resolvePath(directory)
  if (!fs.existsSync(absolute)) return []
  return fs.readdirSync(absolute, { withFileTypes: true }).flatMap((entry) => {
    const relative = path.join(directory, entry.name)
    if (entry.isDirectory()) return listFiles(relative)
    return [relative]
  })
}

function diffChangedFiles() {
  const base = process.env.REEDITPRO_FK_INDEX_DIFF_BASE || 'origin/codex/rp-foundation-26g-security-definer-exposure-migration-plan'
  const result = spawnSync('git', ['diff', '--name-only', `${base}...HEAD`], {
    cwd: root,
    encoding: 'utf8',
  })

  if (result.status !== 0) {
    return { available: false, files: [] }
  }

  return {
    available: true,
    files: result.stdout.split('\n').map((line) => line.trim()).filter(Boolean),
  }
}

const filesToScan = [...new Set([...requiredDocs, ...trackerFiles])]

const missingFiles = [...requiredDocs, ...trackerFiles]
  .filter((file) => !exists(file))
  .map((file) => finding(file, 'missingFile', 1, file))

const missingTerms = []
for (const [file, terms] of Object.entries(requiredDocTerms)) {
  const text = readFile(file)
  for (const term of terms) {
    if (!text.includes(term)) {
      missingTerms.push(finding(file, 'missingRequiredTerm', 1, term))
    }
  }
}

const fkCoverageDocs = [
  'docs/supabase-fk-index-migration-plan.md',
  'docs/supabase-fk-index-priority-matrix.md',
  'docs/supabase-fk-index-duplicate-review-contract.md',
  'docs/supabase-fk-index-naming-contract.md',
]

for (const file of fkCoverageDocs) {
  const text = readFile(file)
  for (const fkFinding of fkFindings) {
    if (!text.includes(fkFinding)) {
      missingTerms.push(finding(file, 'missingFkFindingCoverage', 1, fkFinding))
    }
  }
}

for (const [file, terms] of Object.entries(trackerTerms)) {
  const text = readFile(file)
  for (const term of terms) {
    if (!text.includes(term)) {
      missingTerms.push(finding(file, 'missingTrackerTerm', 1, term))
    }
  }
}

const activeMigrationMarkerFindings = listFiles('supabase/migrations')
  .filter((file) => file.endsWith('.sql'))
  .flatMap((file) =>
    readFile(file).split('\n').flatMap((line, index) => {
      if (/Prompt 26H|fk_index_migration_plan_created|FK Index Hardening Migration Plan/i.test(line)) {
        return [finding(file, 'activeMigrationPrompt26HMarker', index + 1, line)]
      }
      return []
    }),
  )

const executableRemediationSqlFindings = listFiles('docs/draft-sql/supabase-advisor-remediation')
  .filter((file) => file.includes('fk-index') && file.endsWith('.sql'))
  .map((file) => finding(file, 'executableRemediationSqlFile', 1, file))

const diffResult = diffChangedFiles()
const activeMigrationDiffFindings = diffResult.files
  .filter((file) => file.startsWith('supabase/migrations/'))
  .map((file) => finding(file, 'activeMigrationFileChangedInDiff', 1, file))

const executableSqlDiffFindings = diffResult.files
  .filter((file) => file.endsWith('.sql') && !file.endsWith('.sql.md'))
  .map((file) => finding(file, 'executableSqlFileChangedInDiff', 1, file))

const unsafeFindings = [
  ...scanPatterns(filesToScan, rawSecretPatterns, 'secretRisk', { redact: true }),
  ...scanPatterns(filesToScan, unsafeClaimPatterns, 'unsafeClaim'),
  ...scanPatterns(filesToScan, unsafeCommandPatterns, 'unsafeCommand'),
  ...scanProjectRefs(filesToScan),
]

const findings = [
  ...missingFiles,
  ...missingTerms,
  ...activeMigrationMarkerFindings,
  ...executableRemediationSqlFindings,
  ...activeMigrationDiffFindings,
  ...executableSqlDiffFindings,
  ...unsafeFindings,
]

const result = {
  status: findings.length === 0 ? 'passed' : 'failed',
  fkIndexPlanStatus: 'fk_index_migration_plan_created',
  supabaseUpdateRequired: 'docs/status only',
  supabaseUpdateStatus: 'docs_only',
  supabaseEnvironmentTouched: 'none',
  sqlExecuted: 'none',
  migrationDeployed: 'no',
  activeMigrationFilesChanged: 'no',
  executableSqlFilesAdded: 'no',
  googleCloudApiTouched: false,
  secretManagerApiTouched: false,
  diffCheckAvailable: diffResult.available,
  findingsCovered: fkFindings.length,
  requiredDocsChecked: requiredDocs.length,
  trackerFilesChecked: trackerFiles.length,
  nextRecommendedPrompt: 'Prompt 26H-1 - FK Index Local Migration Candidate or Prompt GD-0 - AI Tools / Graphic Design Stack Repo Audit',
  findings,
}

console.log(JSON.stringify(result, null, 2))

if (findings.length > 0) {
  process.exitCode = 1
}
