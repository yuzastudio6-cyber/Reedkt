...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 67)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceActiveMigrationAdoptedLocalValidationRequired, true)
