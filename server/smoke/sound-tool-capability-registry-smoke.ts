...
for (const route of routes) {
  assert.equal(route.routeVersion, '2.0.0')
...
const mirelo = getSoundToolRouteManifest('sound.route.generate.video_sfx.mirelo.v1', '2.0.0')!
...
const mmaudio = getSoundToolRouteManifest('sound.route.generate.video_sfx.mmaudio.v1', '2.0.0')!
...
const invalidMiniSkills = {
  ...
  routeKey: 'sound.route.unregistered.mini.v1', routeVersion: '2.0.0', routeHash: '0'.repeat(64),
...
const noSound = getSoundToolRouteManifest('sound.route.no_sound.v1', '2.0.0')!
const {
  routeHash: _hash,
...
const invalidFallback = publishSoundToolRouteManifest({
  ...
  fallbackPolicy: {
    ...
    fallbackRouteRefs: [{ routeKey: 'sound.route.acquire.project_source.v1', routeVersion: '2.0.0' }],
  },
})
