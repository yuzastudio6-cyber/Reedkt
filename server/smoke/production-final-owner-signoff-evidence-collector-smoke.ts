...
} from '../cli/production-final-owner-signoff-evidence-collector'
...
const dryRun = await runProductionFinalOwnerSignoffEvidenceCollectorFromEnv(ownerSignoffEnv(), async () => {
  dryRunFetchCalled = true
  throw new Error('fetch must not run in dry-run mode')
})
assert.equal(dryRun.ok, true, 'complete owner signoff should pass dry-run mode')
assert.equal(dryRun.mode, 'dry_run', 'collector should default to dry-run mode')
assert.equal(dryRun.readyForFinalOwnerSignoffEvidence, true, 'owner signoff evidence should be locally ready')
assert.equal(dryRun.recordConfirmationRequired, true, 'collector should require explicit record confirmation')
assert.equal(Object.values(dryRun.signoff.approvals).every(Boolean), true, 'all owner approvals should be present')
assert.equal(dryRunFetchCalled, false, 'dry-run must not call backend fetch')

const blocked = await runProductionFinalOwnerSignoffEvidenceCollectorFromEnv({
  ...ownerSignoffEnv(),
  REEDITPRO_PRODUCTION_OWNER_PAID_PRODUCTION_APPROVED: 'false',
}, async () => {
  throw new Error('fetch must not run with incomplete owner signoff evidence')
})
assert.equal(blocked.ok, false, 'missing paid-production approval should block')
assert.equal(blocked.signoff.ready, false, 'signoff slice should be blocked')
assert.equal(blocked.signoff.blockers.includes('paidProductionApproved is not approved.'), true)

await assert.rejects(
  () => runProductionFinalOwnerSignoffEvidenceCollectorFromEnv({
    ...ownerSignoffEnv(),
    REEDITPRO_PRODUCTION_OWNER_NOTES: 'owner approval contains service_role_key',
  }, async () => {
    throw new Error('fetch must not run with secret-like owner notes')
  }),
  /secret-like|OWNER_NOTES/,
  'collector should reject secret-like owner signoff fields before backend calls',
)

const blockedAuditCalls: Array<{ url: string; method: string; headers: Record<string, string>; body?: Record<string, unknown> }> = []
const blockedAuditRecord = await runProductionFinalOwnerSignoffEvidenceCollectorFromEnv({
  ...ownerSignoffEnv(),
  ...
  REEDITPRO_PRODUCTION_READINESS_BEARER_TOKEN: 'production-final-owner-signoff-bearer-token',
  REEDITPRO_PRODUCTION_READINESS_IDEMPOTENCY_KEY: 'production-final-owner-signoff-blocked-audit-smoke',
...
const recorded = await runProductionFinalOwnerSignoffEvidenceCollectorFromEnv(completeEnv(), fakeFetch(calls))
...
assert.equal(recorded.signoff.ready, true, 'owner signoff slice should remain ready')
assert.equal(calls.length, 2, 'confirmed collector should POST evidence and GET readback')
assert.equal(calls[0]?.method, 'POST')
assert.equal(calls[1]?.method, 'GET')
assert.equal(calls[0]?.headers['idempotency-key'], 'production-final-owner-signoff-evidence-smoke')
assert.equal(JSON.stringify(recorded).includes('production-final-owner-signoff-bearer-token'), false, 'collector result must not expose bearer token')

console.log(JSON.stringify({
  ok: true,
  dryRunMode: dryRun.mode,
  recordedMode: recorded.mode,
  approvalCount: Object.keys(recorded.signoff.approvals).length,
  ...
  return async (url, init) => {
    ...
    if (init.method === 'POST' && url.includes('/v1/beta-readiness/production-tool-execution-readiness/evidence')) {
      return jsonResponse(201, {
        ...
        warnings: ['Production final owner signoff route smoke response.'],
      })
    }
...
function ownerSignoffEnv(): ProductionFinalOwnerSignoffEvidenceCollectorEnv {
  const yes = 'true'
  return {
    REEDITPRO_PRODUCTION_READINESS_SOURCE_ID: 'production-final-owner-signoff-evidence-smoke',
    REEDITPRO_PRODUCTION_READINESS_SOURCE_SHA: '2ba7829c10d36d0c65ad7d6e3c25a71e4a37267c',
    REEDITPRO_PRODUCTION_READINESS_WORKSPACE_ID: 'workspace-production-final-owner-signoff-smoke',
    REEDITPRO_PRODUCTION_READINESS_PROJECT_ID: 'project-production-final-owner-signoff-smoke',
    REEDITPRO_PRODUCTION_READINESS_CONFIRM_EVIDENCE_REVIEW: yes,
    REEDITPRO_PRODUCTION_EVIDENCE_REVIEWED_BY: 'production-final-owner-signoff-reviewer',
    REEDITPRO_PRODUCTION_EVIDENCE_REVIEWED_AT: '2026-07-03T00:00:00.000Z',
    REEDITPRO_PRODUCTION_OWNER_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:owner-signoff',
    ...
    REEDITPRO_PRODUCTION_OWNER_NOTES: 'Final owner signoff evidence reviewed for deployment, security, storage/privacy, legal, support, billing, operations, real-user-media beta, private media, artifact privacy, paid production, and final delivery/share.',
  }
}

function completeEnv(): ProductionFinalOwnerSignoffEvidenceCollectorEnv {
  const yes = 'true'
  return {
    ...ownerSignoffEnv(),
    REEDITPRO_PRODUCTION_OWNER_SIGNOFF_CONFIRM_RECORD_EVIDENCE: yes,
    REEDITPRO_PRODUCTION_READINESS_API_BASE_URL: 'https://api.production.reeditpro.example',
    REEDITPRO_PRODUCTION_READINESS_BEARER_TOKEN: 'production-final-owner-signoff-bearer-token',
    REEDITPRO_PRODUCTION_READINESS_IDEMPOTENCY_KEY: 'production-final-owner-signoff-evidence-smoke',
    REEDITPRO_PRODUCTION_SUPABASE_ENVIRONMENT: 'production',
    REEDITPRO_PRODUCTION_SUPABASE_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:supabase',
    ...
    REEDITPRO_PRODUCTION_TOOL_COST_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:tool-cost-ledger',
    ...
    REEDITPRO_PRODUCTION_WALLET_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:wallet',
    ...
    REEDITPRO_PRODUCTION_STRIPE_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:stripe',
    ...
    REEDITPRO_PRODUCTION_OBSERVABILITY_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:observability',
    ...
    REEDITPRO_PRODUCTION_OPERATIONS_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:operations',
    ...
    REEDITPRO_PRODUCTION_TOOLS_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:tools',
    REEDITPRO_PRODUCTION_TOOLS_SOURCE_ID: 'production-final-owner-signoff-evidence-smoke:tools',
    REEDITPRO_PRODUCTION_TOOLS_SOURCE_SHA: '2ba7829c10d36d0c65ad7d6e3c25a71e4a37267c',
    ...
    REEDITPRO_PRODUCTION_HARD_EVIDENCE_ARTIFACT_ID: 'prod-final-owner-signoff-artifact:hard-safety',
