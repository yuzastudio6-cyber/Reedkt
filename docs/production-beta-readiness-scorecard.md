...
- Result: the probe covers all 21 AI graphics tools. It reports `externalAgentRouteExecutableNowTools=13`, with six CPU/static controlled tools and seven browser-runtime controlled tools callable in explicit local/mock route mode. It reports the eight GPU/model tools as known canonical route tools with fail-closed `409` outcomes until native NVIDIA L4 runtime proof and required private model-weight evidence are accepted. `gpuModelRuntimeAdmissionBlockedTools=8`, `modelWeightManifestRequiredTools=5`, `gpuRuntimeShouldStartNowTools=0`, `workerDispatchPerformedTools=0`, `publicArtifactCreatedTools=0`, and `signedUrlCreatedTools=0`.
- Runtime/beta/production: this probe does not execute tools, dispatch Workers, call providers/models, start browser/WebGL/canvas or GPU runtime, download model weights, mutate Supabase/GCS, create signed URLs, create public artifacts, unlock broad all-21 execution, unlock external beta traffic, or unlock production. `externalAgentCanExecuteSomeToolsNow=true` and `agentCanExecuteControlledCpuStaticAndBrowserRuntimeToolsNow=true`, while `agentCanExecuteAll21ToolsNow=false`, `agentCanExecuteGpuModelToolsNow=false`, `gpuRuntimeShouldStartNow=false`, `runtimeReadyNow=false`, `externalBetaReadyNow=false`, and `productionReadyNow=false`.
