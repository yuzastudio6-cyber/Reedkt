...
const forbidden = [
  ...
  /Remote Supabase command class:\s*`(?!(none_in_this_phase|readonly_migration_history_and_db_push_dry_run|readonly_migration_history_readback|supabase migration list --db-url \[redacted\])`)/i,
  /SQL execution:\s*`(?!(none|false|read_only_catalog_sql_only)`)/i,
  /SQL mutation:\s*`(?!(none|false)`)/i,
  /Migration deployed:\s*`(?!(no|false|local_only)`)/i,
  /Storage readback:\s*`(?!(none|none_not_run_due_stale_migration_history)`)/i,
