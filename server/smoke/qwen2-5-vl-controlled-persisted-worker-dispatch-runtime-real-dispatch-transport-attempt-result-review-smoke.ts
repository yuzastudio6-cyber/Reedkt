...
const DECISION =
  'qwen2_5_vl_controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_attempt_result_review_accepted_approved_fixture_inference_plan_required'
const NEXT_PROMPT =
...
assert.equal(
  review.upstreamControlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultDecision,
  QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_REAL_DISPATCH_TRANSPORT_ATTEMPT_RESULT.decision,
)
assert.equal(review.acceptedEvidence.transportAttemptResultAccepted, true)
assert.equal(review.acceptedEvidence.transportAttemptRecorded, true)
...
assert.equal(review.acceptedEvidence.serviceTargetNonPersistenceAccepted, true)
assert.equal(review.acceptedEvidence.audienceNonPersistenceAccepted, true)
...
assert.equal(review.acceptedEvidence.postAttemptFailClosedStateAccepted, true)
...
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultReviewRecorded, true)
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchTransportAttemptResultReviewAccepted, true)
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePlanRequired, true)
assert.equal(review.runtimeFlags.transportReachabilityAcceptedForFutureInferencePlan, true)
...
assert.deepEqual(
  ...
  [],
  `Forbidden values in transport attempt result review data: ${forbiddenDataFindings.join('; ')}`,
)

console.log(JSON.stringify({
  ok: true,
  decision: review.decision,
  acceptedTransportEvidence: review.acceptedEvidence.transportAttemptResultAccepted,
  nextBlocker:
