# Model CPU/GPU Runtime Boundary Owner Approval

Decision: `ai_graphics_canonical_agent_selection_runtime_boundary_owner_approved_with_warnings`

Owner approves `torch_torchvision`, `transformers`, `sam2`, `birefnet`, `real_esrgan`, `kornia`, `rembg`, and `transparent_background` as `model_cpu_gpu_runtime_later`. CPU import, model provenance, model-weight, GPU/model runtime, and inference remain blocked.
