...
const EXPECTED_JOB = 'weeditpro-gpu-rate-publisher' as const
...
async function main(): Promise<void> {
  ...
  const environment = z.object({
    WEEDITPRO_GPU_RATE_OPERATOR_ACTION: z.literal('publish_all_routes'),
    WEEDITPRO_GOOGLE_CLOUD_BILLING_ACCOUNT_RESOURCE_NAME:
      billingAccountResourceName,
  }).strict().parse({
    WEEDITPRO_GPU_RATE_OPERATOR_ACTION:
      process.env.WEEDITPRO_GPU_RATE_OPERATOR_ACTION,
    WEEDITPRO_GOOGLE_CLOUD_BILLING_ACCOUNT_RESOURCE_NAME:
  ...
  const receipt = await publishCanonicalCurrentGoogleCloudGpuRateAuthorities({
    publicationId: `gpu-rates-us-central1-${execution}`,
...
function assertCloudOperatorInvocation(): void {
  if (
    ...
    'GPU rate publication is restricted to the exact cloud operator.',
  )
}
...
main().catch((error: unknown) => {
  const code = error instanceof Error
    ? error.message
    : 'gpu_rate_cloud_operator_failed'
