...
- `npm run smoke:beta-readiness-operator-status` verifies the combined operator status report for complete and missing input cases, secret-safe output, current blocker counts, evidence-driven policy, and next-command sequencing without touching a deployed backend.
