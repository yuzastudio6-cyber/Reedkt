...
| `ai_video_broll_generation_wan` | controlled L4 private proof, Wan/Wan2.1 selected, private cache/proof-runner/fast cache readiness, B-roll quota verification result, 9K through 10ZA proof/stockout/fix evidence, 10ZB fixed-delivery dependency/install runner evidence, and external-agent wrapper execution evidence | ready for explicit external-agent gate | The canonical B-roll wrapper delegated to 10ZB and proved the selected zone can create the no-public-IP G2/L4 VM, use IAP SSH, fetch the private GCS wheelhouse by exact object paths, validate the cache marker and manifest, install dependencies offline, import `torch`, `diffusers`, and `transformers`, then delete the prompt-scoped VM and verify cleanup. This is dependency/install readiness only: Wan model import, model load, inference, generated video, generated assets, Supabase, SQL, signed URLs, credits, beta, and production remain blocked. | `AI-VIDEO-BROLL-GEN-11A-MODEL-IMPORT-PLAN: plan Wan model import proof after payload/install readiness, no inference` |
...
- B-roll proof path has quota for one L4 VM, the 9K no-idle prompt, cleanup-verified proof/stockout evidence through 10ZA, and a passed 10ZB fixed-delivery dependency/install runner. The 10ZB runner uses a prompt-scoped no-public-IP L4 VM, exact private GCS object downloads, offline wheel install, dependency import readiness, and mandatory cleanup verification. The next safe step is 11A model import planning; model import, model load, inference, generated video, generated assets, Supabase, SQL, signed URLs, credits, beta, and production remain blocked.
