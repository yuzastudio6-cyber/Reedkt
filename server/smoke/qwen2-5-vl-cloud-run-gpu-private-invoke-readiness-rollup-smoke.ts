...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 37)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceMediaAssetsStatusBaselineFixRequired, true)
