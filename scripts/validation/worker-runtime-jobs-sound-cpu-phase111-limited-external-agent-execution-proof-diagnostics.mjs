...
const nextPrompt = 'WORKER_RUNTIME_JOBS-SOUND-CPU-PHASE111-LIMITED-EXTERNAL-AGENT-EXECUTION-PROOF-OWNER-REVIEW'

const docs = {
  source:
    'docs/worker-runtime-jobs-sound-cpu-phase110-limited-external-agent-execution-owner-review-result.md',
  prompt: 'docs/implementation-prompts/prompt-worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof.md',
  runner: 'scripts/validation/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof-runner.mjs',
  result: 'docs/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof-result.md',
  output: 'docs/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-proof-output-register.md',
  verification:
    'docs/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-no-side-effect-verification-register.md',
  coverage: 'docs/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-tool-coverage-register.md',
  blockers:
    'docs/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof-blocker-register.md',
  policy:
    'docs/worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof-claim-policy.md',
  next: 'docs/implementation-prompts/prompt-worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof-owner-review.md',
}
...
const parsed = {
  ...
  blockers: parseJsonBlock(
    docs.blockers,
    'worker-runtime-jobs-sound-cpu-phase111-limited-external-agent-execution-proof-blocker-register',
  ),
...
assert(parsed.source.sourceVerification.sourcePr === 2061, 'source sourcePr mismatch')
assert(
  parsed.source.sourceVerification.sourceMergeCommit === 'e74d6a44b2a9b442efd40178af91f6230a40f894',
  'source merge mismatch',
)
assert(parsed.source.ownerReview.limitedExternalAgentExecutionProofMayProceedNext === true, 'source proof permission missing')
assert(parsed.source.soundCpuTools.readyForLimitedExternalAgentExecutionProof === 15, 'source proof count mismatch')
...
assert(parsed.prompt.proofScope.runLimitedExternalAgentBoundary === true, 'prompt limited boundary missing')
...
assert(runnerText.includes('invokeLimitedExternalAgentBoundary'), 'runner boundary function missing')
...
assert(parsed.result.sourceVerification.sourcePr === 2062, 'result source PR mismatch')
assert(
  parsed.result.sourceVerification.sourceMergeCommit === '55908b1293a211ab070c05e5e4876be55c309ea7',
  'result source merge mismatch',
)
assert(parsed.result.proofResult.status === 'passed', 'proof status mismatch')
assert(parsed.result.proofResult.limitedBoundaryInvoked === true, 'limited boundary not invoked')
...
assert(parsed.result.soundCpuTools.limitedExternalAgentExecutionProofPassed === 15, 'proof count mismatch')
...
assert(parsed.output.proofCommand === 'npm run worker-runtime-jobs:sound-cpu-phase111-limited-external-agent-execution-proof:proof', 'proof command mismatch')
...
assert(parsed.coverage.soundCpuToolSet.limitedExternalAgentProofComplete === true, 'coverage limited proof missing')
...
assert(
  parsed.blockers.resolvedForThisGate.some((row) => row.blockerId === 'limited_external_agent_execution_proof_pending'),
...
assert(parsed.policy.allowedClaims.limitedExternalAgentExecutionProofPassedClaimed === true, 'proof claim missing')
...
assert(parsed.next.reviewScope.mayAcceptReadinessReconciliationNext === true, 'next reconciliation missing')
  ...
  JSON.stringify(
    {
      ...
      decision,
      sourcePr: 2062,
      limitedExternalAgentExecutionProofPassed: true,
      invocationCount: 4,
