...
import {
  AI_VIDEO_BROLL_GEN_10J_NO_IDLE_L4_IAP_WHEELHOUSE_PAYLOAD_INSTALL_PROOF_US_WEST4_C_RESULT,
  AI_VIDEO_BROLL_GEN_10K_PAYLOAD_INSTALL_STOCKOUT_FIX_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10j-no-idle-l4-iap-wheelhouse-payload-install-proof-us-west4-c-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-10j-no-idle-l4-iap-wheelhouse-payload-install-proof-us-west4-c-result.md'
const NEXT_PROMPT_PATH = 'docs/implementation-prompts/prompt-ai-video-broll-gen-10k-payload-install-stockout-fix.md'
const SPEC_PATH =
...
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10j-no-idle-l4-iap-wheelhouse-payload-install-proof-us-west4-c-result'
const DECISION =
...
function scanForbiddenValues(value: unknown, prefix = 'broll10jResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10j-no-idle-l4-iap-wheelhouse-payload-install-proof-us-west4-c-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `10J result doc missing ${required}`)
}

for (const required of [
  AI_VIDEO_BROLL_GEN_10K_PAYLOAD_INSTALL_STOCKOUT_FIX_PROMPT,
  'read-only and no-VM',
  '`us-west4-c` 10J stockout',
  'Do not create a VM',
  'run-when-used and stop-when-idle',
  '`generated_local_fixture_passed`',
]) {
  assert.equal(prompt.includes(required), true, `10K prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10J_NO_IDLE_L4_IAP_WHEELHOUSE_PAYLOAD_INSTALL_PROOF_US_WEST4_C_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10j_no_idle_l4_iap_wheelhouse_payload_install_proof_us_west4_c_result')
assert.equal(result.attemptedZone, 'us-west4-c')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10K_PAYLOAD_INSTALL_STOCKOUT_FIX_PROMPT)
