...
import {
  AI_VIDEO_BROLL_GEN_9O_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_RESULT,
  AI_VIDEO_BROLL_GEN_9O_RETRY_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-9o-retry-no-idle-l4-iap-wheelhouse-transfer-proof.md'
const ORIGINAL_PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof-result'
const DECISION =
  'ai_video_broll_gen_9o_no_idle_l4_iap_wheelhouse_transfer_blocked_resource_pool_exhausted_cleanup_verified'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll9oResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9o-no-idle-l4-iap-wheelhouse-transfer-proof-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `9O result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9O retry prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9O_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9o_no_idle_l4_iap_wheelhouse_transfer_proof_result')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9O_RETRY_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_PROMPT)
