...
function assertFalseFlags(flags: JsonRecord) {
  for (const key of [
    'backendRuntimePersistenceLocalHarnessValidationRetry4Passed',
...
const doc = read('docs/qwen2-5-vl-7b-backend-runtime-persistence-local-harness-validation-retry-4-result.md')
for (const phrase of [
  DECISION,
  '`55430`',
  '`55431`',
...
const result = QWEN2_5_VL_BACKEND_RUNTIME_PERSISTENCE_LOCAL_HARNESS_VALIDATION_RETRY_4_RESULT
assert.equal(result.decision, DECISION)
assert.equal(
  result.upstreamBackendRuntimePersistenceBaselineMediaAssetsStatusFixDecision,
...
assert.equal(result.attempt.failedStatementCategory, 'idx_edit_plan_segments_plan_order')
assert.equal(
...
assert.equal(result.validationOutcome.status, 'blocked_edit_plan_segments_version_baseline_column')
assert.equal(result.validationOutcome.mediaAssetsStatusBaselineFixVerified, true)
assert.equal(result.validationOutcome.editPlanSegmentsVersionBaselineFixRequired, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry4ResultRecorded, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceLocalHarnessValidationRetry4Attempted, true)
...
assert.equal(result.runtimeFlags.backendRuntimePersistenceMediaAssetsStatusBaselineFixVerified, true)
assert.equal(result.runtimeFlags.backendRuntimePersistenceEditPlanSegmentsVersionBaselineFixRequired, true)

const forbiddenDataFindings = scanValues({ result })
assert.deepEqual(
  forbiddenDataFindings,
  [],
  `Forbidden values in Qwen validation retry 4 data: ${forbiddenDataFindings.join('; ')}`,
)

console.log(JSON.stringify({
  ok: true,
  decision: DECISION,
  localHarnessValidationRetry4Attempted: result.validationOutcome.localHarnessValidationRetry4Attempted,
  localHarnessValidationRetry4Passed: result.validationOutcome.localHarnessValidationRetry4Passed,
  mediaAssetsStatusBaselineFixVerified: result.validationOutcome.mediaAssetsStatusBaselineFixVerified,
