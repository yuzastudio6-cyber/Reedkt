...
export const CANONICAL_GPU_WORKER_REMBG_PYTHON =
  '/opt/reeditpro/gpu-operations/rembg/venv/bin/python' as const
export const CANONICAL_GPU_WORKER_REMBG_RUNNER =
  '/opt/reeditpro/gpu-operations/rembg/runner.py' as const

const MAXIMUM_CAPTURE_BYTES = 128 * 1_024

export function createCanonicalGpuWorkerRembgSubprocessRuntimePort():
CanonicalGpuWorkerOperationRuntimePort {
  return createCanonicalGpuWorkerOperationRuntimePort({
    ...
    async execute(input) {
      if (
        ...
        throw notReady(
          'canonical_gpu_worker_rembg_subprocess_request_invalid',
        )
      }
...
async function runFixedSubprocess(input: {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      ['-I', '-B', CANONICAL_GPU_WORKER_REMBG_RUNNER],
      {
        cwd: '/app',
    ...
    child.once('error', () => {
      ...
      settled = true
      reject(notReady(
        'canonical_gpu_worker_rembg_fixed_subprocess_unavailable',
      ))
    })
    child.once('close', (exitCode) => {
      ...
      if (captureExceeded) {
        reject(notReady(
          'canonical_gpu_worker_rembg_subprocess_capture_exceeded',
        ))
        return
      }
...
function notReady(code: string): ApiError {
  return new ApiError('TOOL_NOT_READY', code, 409, {
    requiredGate:
      'canonical_gpu_worker_rembg_fixed_subprocess_runtime',
  })
}
