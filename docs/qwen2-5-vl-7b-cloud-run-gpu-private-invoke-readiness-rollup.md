...
- controlled persisted worker dispatch runtime real-dispatch execution attempt result review: blocked, result review required
...
| Controlled persisted worker dispatch runtime real-dispatch execution attempt result review | blocked controlled persisted worker dispatch runtime real-dispatch execution attempt result review required | The fail-closed real-dispatch execution attempt result is recorded but has not been reviewed. | Review the fail-closed real-dispatch execution attempt result before any further runtime gate can advance. |
...
The next action is the controlled persisted worker dispatch runtime real-dispatch execution attempt result review. That future prompt must keep generated assets, public artifacts, signed URLs, beta, production, and `generated_local_fixture_passed` blocked now.
