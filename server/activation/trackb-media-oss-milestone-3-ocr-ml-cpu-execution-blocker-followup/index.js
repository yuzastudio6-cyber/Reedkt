...
export const branchName = 'codex/rp-trackb-media-oss-milestone-3-ocr-ml-cpu-execution-blocker-followup'
export const baseBranch = 'codex/rp-github-merge-hygiene-open-pr-stack-audit'
export const expectedSourceSha = 'b17a20898d5c046447905e5a411a733133afa535'
...
export const imageTag = `reeditpro-ocr-runtime:trackb-milestone3-cpu-libgomp-${expectedSourceSha}`
export const approvedRuntimePackage = 'libgomp1'
...
const blockerNextPrompt = 'TRACKB_MEDIA_OSS_MILESTONE_3_OCR_ML_CPU_BLOCKER_FOLLOWUP_RESOLUTION'
...
export function requiredConfirmations() {
  return [
    'REEDITPRO_CONFIRM_TRACKB_MEDIA_OSS_MILESTONE_3_OCR_ML_CPU_EXECUTION_BLOCKER_FOLLOWUP',
    'REEDITPRO_CONFIRM_TRACKB_MEDIA_OSS_STEWARD_SOURCE_OF_TRUTH',
    'REEDITPRO_CONFIRM_OCR_RUNTIME_TARGET',
    'REEDITPRO_CONFIRM_LIBGOMP_PACKAGE_REVIEW',
...
export function buildTrackBMilestone3OcrMlCpuExecutionBlockerFollowupPlan() {
  return {
    branchName,
    baseBranch,
    expectedSourceSha,
    ...
    decisionCandidates: [
      ...
      packageStrategyDecision,
      dockerfilePatchDecision,
...
function buildSourceAudit(generated) {
  ...
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.sourceAudit.v1',
    ...
    sourceEvidence: [
      {
        ...
        blocker: 'missing_libgomp_so_1_while_loading_paddle_base_libpaddle_so',
        dockerBuildPassed: true,
        paddleOcrSkippedBecausePaddlePaddleFailed: true,
...
function buildLibgompRootCauseReview(generated) {
  const dockerfile = existsSync(join(repoRoot, dockerfilePath)) ? readText(dockerfilePath) : ''
  return {
    schema:
      'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.libgompRootCauseReview.v1',
    generatedAt: generated,
    priorEvidencePr: 583,
    priorDecision:
      'trackb_media_oss_milestone3_ocr_ml_cpu_execution_blocked_by_paddlepaddle_import_or_api_shape',
    exactBlocker: 'libgomp.so.1: cannot open shared object file',
    affectedLibrary: '/usr/local/lib/python3.12/site-packages/paddle/base/libpaddle.so',
    baseImage: dockerfile.match(/^FROM\s+(.+)$/m)?.[1] || null,
    packageManager: dockerfile.includes('apt-get') || dockerfile.includes('python:3.12-slim') ? 'apt' : 'unknown',
    selectedPackage: approvedRuntimePackage,
    selectedPackageReason:
      'libgomp1 is the Debian/Ubuntu GCC OpenMP runtime package that provides libgomp.so.1 for Python native libraries such as PaddlePaddle.',
    packageStrategyClear:
      dockerfile.includes('FROM python:3.12-slim') && approvedRuntimePackage === 'libgomp1',
    ocrInferenceRun: false,
    modelAssetOperationsRun: false,
...
function buildSourceTargetCheck(generated) {
  ...
  const packageStrategy = {
    baseImage: dockerfile.match(/^FROM\s+(.+)$/m)?.[1] || null,
    packageManager: dockerfile.includes('apt-get') || dockerfile.includes('FROM python:3.12-slim')
      ? 'apt'
      : 'unknown',
    approvedPackage: approvedRuntimePackage,
    aptLayerPresent: /apt-get update[\s\S]*apt-get install -y --no-install-recommends libgomp1[\s\S]*rm -rf \/var\/lib\/apt\/lists\/\*/.test(
      dockerfile,
    ),
    unrelatedPackagesAdded: /apt-get install -y --no-install-recommends(?!\s+libgomp1\b)/.test(
      dockerfile,
    ),
  }
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.sourceTargetCheck.v1',
    generatedAt: generated,
    targetDockerfile: dockerfilePath,
    targetRequirements: requirementsPath,
    dockerfileExists,
    ...
    sourceTargetValid:
      ...
      packageStrategy.packageManager === 'apt' &&
      packageStrategy.aptLayerPresent === true &&
      packageStrategy.unrelatedPackagesAdded === false,
  }
}
...
function buildDockerfilePatchReport(generated, sourceTargetCheck) {
  return {
    schema:
      'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.dockerfilePatch.v1',
    generatedAt: generated,
    targetDockerfile: dockerfilePath,
    approvedPackage: approvedRuntimePackage,
    patchApplied: sourceTargetCheck.packageStrategy?.aptLayerPresent === true,
    onlyApprovedPackageAdded: sourceTargetCheck.packageStrategy?.unrelatedPackagesAdded === false,
...
function buildBuildContextReview(generated) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.buildContextReview.v1',
    generatedAt: generated,
...
function runDependencyHydration(execute) {
  ...
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.dependencyHydration.v1',
    ...before,
...
function buildBuildContextGenerationReport(generated) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.buildContextGeneration.v1',
    generatedAt: generated,
    skipped: true,
...
function buildGeneratedArtifactScan(generated) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.generatedArtifactScan.v1',
    generatedAt: generated,
    skipped: true,
...
function runLibgompPresenceProof(execute, dockerBuildReport) {
  ...
  const result = run(
    ...
    [
      ...
      imageTag,
      '-c',
      "import ctypes.util, glob, json; resolved=ctypes.util.find_library('gomp'); paths=glob.glob('/usr/lib/**/libgomp.so.1', recursive=True); print(json.dumps({'library':'libgomp','resolved':resolved,'paths':paths[:5]}))",
    ],
    { timeout: 120000 },
  )
  return {
    ...
    libgompPresent:
      result.exitCode === 0 &&
      (/"resolved":\s*"[^"]+"/.test(result.stdoutSummary) || /libgomp\.so\.1/.test(result.stdoutSummary)),
...
`
  ...
  const modelAssetRequired =
    ...
    /(download|model asset|model_dir|model_path|det_model|rec_model|cls_model)/i.test(combinedOutput)
  const blocker =
    importApi.exitCode === 0
      ? null
      : /libgl\.so\.1/i.test(combinedOutput)
        ? 'paddleocr_import_blocked_missing_libGL_so_1'
        : modelAssetRequired
...
function buildModelOcrBoundary(generated, paddleOcrReport) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.modelOcrBoundary.v1',
    generatedAt: generated,
    ocrInferenceRun: false,
    modelDownloadRun: false,
    modelAssetCopyRun: false,
...
function buildLatencyMemoryCostReport(generated, dockerBuildReport, paddlePaddleReport, paddleOcrReport) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.cpuLatencyMemoryCost.v1',
    generatedAt: generated,
...
function cleanupArtifacts(execute, dockerBuildReport) {
  ...
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.artifactCleanup.v1',
    before,
    after,
    cleanupCommand: `rm -rf ${cleanupOutputs.join(' ')}`,
...
function runSafetyScan(generated, cleanupReport) {
  ...
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.safetyScan.v1',
    generatedAt: generated,
...
function deriveDecision({
  sourceTargetCheck,
  rootCauseReport,
  dockerfilePatchReport,
  hydrationReport,
  generationReport,
  scanReport,
  dockerBuildReport,
  libgompPresenceReport,
  paddlePaddleReport,
  paddleOcrReport,
  cleanupReport,
  safetyReport,
  ...
  if (rootCauseReport.packageStrategyClear !== true) return packageStrategyDecision
  if (
    dockerfilePatchReport.patchApplied !== true ||
    dockerfilePatchReport.onlyApprovedPackageAdded !== true
  ) {
    return dockerfilePatchDecision
  }
  ...
  if (libgompPresenceReport.libgompPresent !== true) return dockerfilePatchDecision
...
function buildStatusMatrix(generated, decision, paddlePaddleReport, paddleOcrReport) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.statusMatrix.v1',
    generatedAt: generated,
    decision,
    tools: [
      {
      ...
      },
      {
        ...
        blocker: paddleOcrReport.blocker,
      },
    ],
  }
}
...
function buildDecisionReport(generated, decision, nextPrompt) {
  return {
    schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.decision.v1',
    generatedAt: generated,
    decision,
    ...
    dockerfileMutationAllowed: true,
    unrelatedDockerfileMutationAllowed: false,
    approvedDockerfilePatchPackage: approvedRuntimePackage,
    dockerImagePushRun: false,
...
function buildReports({ execute = false } = {}) {
  ...
  const rootCauseReport = buildLibgompRootCauseReview(generated)
  const sourceTargetCheck = buildSourceTargetCheck(generated)
  const dockerfilePatchReport = buildDockerfilePatchReport(generated, sourceTargetCheck)
  ...
  const libgompPresenceReport = runLibgompPresenceProof(execute, dockerBuildReport)
  ...
  return {
    ...
    rootCauseReport: { ...rootCauseReport, decision },
    sourceTargetCheck: { ...sourceTargetCheck, decision },
    dockerfilePatchReport: { ...dockerfilePatchReport, decision },
    ...
    dockerBuildReport: {
      schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.dockerBuild.v1',
      generatedAt: generated,
      decision,
      ...dockerBuildReport,
    },
    libgompPresenceReport: {
      schema:
        'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.libgompPresence.v1',
      generatedAt: generated,
      decision,
      ...libgompPresenceReport,
    },
    paddlePaddleReport: {
      schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.paddlePaddleProof.v1',
      generatedAt: generated,
      decision,
      ...paddlePaddleReport,
    },
    paddleOcrReport: {
      schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.paddleOcrApiProof.v1',
      generatedAt: generated,
      decision,
      ...paddleOcrReport,
    },
    ...
    decisionReport,
    readinessReport: {
      schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.readiness.v1',
      generatedAt: generated,
      decision,
    ...
    privateArtifactManifest: {
      schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.privateArtifactManifest.v1',
      generatedAt: generated,
      decision,
    ...
    validationResults: {
      schema: 'reeditpro.openSourceToolStack.trackBMilestone3OcrMlCpuExecutionBlockerFollowup.validationResults.v1',
      generatedAt: generated,
      decision,
...
export function writeTrackBMilestone3OcrMlCpuExecutionBlockerFollowupArtifacts(options = {}) {
...
export function readTrackBMilestone3OcrMlCpuExecutionBlockerFollowupArtifacts() {
  return {
    sourceAudit: readJson(`${reportDir}/source-of-truth-audit.json`),
    rootCauseReport: readJson(`${reportDir}/libgomp-root-cause-review.json`),
    ...
    libgompPresenceReport: readJson(`${reportDir}/libgomp-presence-report.json`),
    ...
    boundaryReport: readJson(`${reportDir}/model-ocr-gpu-boundary-verification.json`),
    ...
    statusMatrix: readJson(`${reportDir}/milestone-3-cpu-blocker-followup-status-matrix.json`),
    decisionReport: readJson(`${reportDir}/milestone-3-ocr-ml-cpu-blocker-followup-decision.json`),
...
function writeReports(reports) {
  ...
  writeJson(`${reportDir}/libgomp-root-cause-review.json`, reports.rootCauseReport)
  ...
  writeJson(`${reportDir}/libgomp-presence-report.json`, reports.libgompPresenceReport)
  ...
  writeJson(`${reportDir}/model-ocr-gpu-boundary-verification.json`, reports.boundaryReport)
  ...
  writeJson(`${reportDir}/milestone-3-cpu-blocker-followup-status-matrix.json`, reports.statusMatrix)
  writeJson(`${reportDir}/milestone-3-ocr-ml-cpu-blocker-followup-decision.json`, reports.decisionReport)
  ...
  writeText(
    ...
    `# Track B Milestone 3 OCR/ML CPU Execution Blocker Follow-Up Source Audit\n\nDecision: \`${decision}\`.\n\nSource SHA: \`${reports.sourceAudit.sourceSha}\`.\n\nPR #583 is merged source-of-truth evidence for the missing \`libgomp.so.1\` blocker. PR #578 is merged source-of-truth approval for CPU-only PaddleOCR/PaddlePaddle import/API-shape proof against \`${dockerfilePath}\`. PR #574/#571/#567/#563/#559/#557/#551/#549/#546/#545/#542 remain merged supporting evidence. PR #543 and historical OCR PRs #51/#56 remain context-only.\n`,
  )
  writeText(
    `${reportDir}/libgomp-root-cause-review.md`,
    `# libgomp Root Cause Review\n\nPrior PR: #${reports.rootCauseReport.priorEvidencePr}.\n\nPrior blocker: \`${reports.rootCauseReport.exactBlocker}\` while loading \`${reports.rootCauseReport.affectedLibrary}\`.\n\nBase image: \`${reports.rootCauseReport.baseImage}\`.\n\nSelected package: \`${reports.rootCauseReport.selectedPackage}\`.\n\nPackage strategy clear: \`${reports.rootCauseReport.packageStrategyClear}\`.\n\nOCR inference, model asset operations, and GPU execution remain blocked.\n`,
  )
  writeText(
    `${reportDir}/source-target-check.md`,
    `# Source Target Check\n\nTarget Dockerfile: \`${dockerfilePath}\`.\n\nTarget requirements: \`${requirementsPath}\`.\n\nPinned PaddlePaddle declared: \`${reports.sourceTargetCheck.packagePresence['paddlepaddle==3.0.0']}\`.\n\nPinned PaddleOCR declared: \`${reports.sourceTargetCheck.packagePresence['paddleocr==3.0.0']}\`.\n\nRuntime guard env vars declared: \`${reports.sourceTargetCheck.allRuntimeGuardsDeclared}\`.\n\nApproved package layer present: \`${reports.sourceTargetCheck.packageStrategy.aptLayerPresent}\`.\n`,
  )
  writeText(
    `${reportDir}/dockerfile-patch-report.md`,
    `# Dockerfile Patch Report\n\nTarget Dockerfile: \`${reports.dockerfilePatchReport.targetDockerfile}\`.\n\nApproved package: \`${reports.dockerfilePatchReport.approvedPackage}\`.\n\nPatch applied: \`${reports.dockerfilePatchReport.patchApplied}\`.\n\nOnly approved package added: \`${reports.dockerfilePatchReport.onlyApprovedPackageAdded}\`.\n\nRequirements, package-lock, .dockerignore, and unrelated Dockerfiles were not intentionally mutated.\n`,
  )
  writeText(
  ...
  writeText(
    `${reportDir}/libgomp-presence-report.md`,
    `# libgomp Presence Report\n\nImage tag: \`${reports.libgompPresenceReport.imageTag || imageTag}\`.\n\nCommand: \`${reports.libgompPresenceReport.command || 'not_run'}\`.\n\nExit code: \`${reports.libgompPresenceReport.exitCode ?? 'not_run'}\`.\n\nlibgomp present: \`${reports.libgompPresenceReport.libgompPresent}\`.\n\nNetwork disabled: \`${reports.libgompPresenceReport.dockerRunNetworkNone === true}\`.\n`,
  )
  writeText(
  ...
  writeText(
    `${reportDir}/model-ocr-gpu-boundary-verification.md`,
  ...
  writeText(
    `${reportDir}/milestone-3-cpu-blocker-followup-status-matrix.md`,
  ...
  writeText(
    `${reportDir}/milestone-3-ocr-ml-cpu-blocker-followup-decision.md`,
...
`
  ...
  status.milestone3OcrMlCpuExecutionBlockerFollowup = {
    decision: reports.decisionReport.decision,
    reportKey: 'trackb_milestone_3_ocr_ml_cpu_execution_blocker_followup_reports',
    targetDockerfile: dockerfilePath,
    targetRequirements: requirementsPath,
    imageTag,
