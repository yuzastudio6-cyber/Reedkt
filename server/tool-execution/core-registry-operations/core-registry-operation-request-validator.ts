...
export type CoreRegistryOperationRequestValidationErrorCode =
  | 'unknown_tool'
  | 'policy_blocked'
  | 'invalid_json_object'
  | 'request_too_large'
...
export interface CoreRegistryOperationRequestValidationError {
  code: CoreRegistryOperationRequestValidationErrorCode
  field: string
  message: string
}

export type CoreRegistryOperationRequestValidationResult =
  | {
      ok: true
      spec: CoreRegistryOperationSpec
      request: Readonly<Record<string, unknown>>
    }
  | {
      ok: false
      spec?: CoreRegistryOperationSpec
      errors: CoreRegistryOperationRequestValidationError[]
    }

/**
 * Validates only the immutable request contract. A successful result is not an
 * execution authorization and does not override productReady=false, lease,
 * license, model, deployment, QA, credit, or worker-runtime evidence gates.
 */
export function validateCoreRegistryOperationRequestContract(
  requestedToolName: string,
  value: unknown,
): CoreRegistryOperationRequestValidationResult {
  const spec = resolveCoreRegistryOperationSpec(requestedToolName)
  if (!spec) {
    return failure(
      undefined,
      'unknown_tool',
      'requestedToolName',
      'Requested tool is not an allowlisted registry-only fixed operation identity.',
    )
  }
  if (spec.disposition === 'policy_blocked') {
    return failure(
      ...
      'policy_blocked',
      'requestedToolName',
      `${spec.canonicalToolId} is non-callable under current product policy: ${spec.policyBlockReasons.join(' ')}`,
    )
  }
  ...
  const errors: CoreRegistryOperationRequestValidationError[] = []
  collectProhibitedInputs(
    request,
    '$',
    new Set(spec.requestSchema.prohibitedPropertyNames),
    errors,
  )
  ...
  return { ok: true, spec, request: deepFreeze({ ...request }) }
}

export function assertCoreRegistryOperationRequestContract(
  requestedToolName: string,
  value: unknown,
): { spec: CoreRegistryOperationSpec; request: Readonly<Record<string, unknown>> } {
  const result = validateCoreRegistryOperationRequestContract(requestedToolName, value)
  if (!result.ok) {
...
function validateArtifactBindings(
  ...
  value.forEach((item, index) => {
    ...
    validateStringField(
      item.artifactId,
      schema.items.properties.artifactId,
      `${prefix}.artifactId`,
      errors,
      'artifact_contract_failed',
    )
    ...
    validateStringField(
      item.sha256,
      schema.items.properties.sha256,
      `${prefix}.sha256`,
      errors,
      'artifact_contract_failed',
    )
    validateNumberField(
      item.byteLength,
      schema.items.properties.byteLength,
      `${prefix}.byteLength`,
      errors,
      'artifact_contract_failed',
    )
...
function validateStringField(
  ...
  field: string,
  errors: CoreRegistryOperationRequestValidationError[],
  code: CoreRegistryOperationRequestValidationErrorCode = 'invalid_field',
): void {
...
function validateNumberField(
  value: unknown,
  constraint: ProfessionalToolOperationNumberConstraint,
  field: string,
  errors: CoreRegistryOperationRequestValidationError[],
  code: CoreRegistryOperationRequestValidationErrorCode = 'invalid_field',
): void {
...
function failure(
  spec: CoreRegistryOperationSpec | undefined,
  code: CoreRegistryOperationRequestValidationErrorCode,
  field: string,
  message: string,
): CoreRegistryOperationRequestValidationResult {
...
function error(
  ...
  field: string,
  message: string,
): CoreRegistryOperationRequestValidationError {
