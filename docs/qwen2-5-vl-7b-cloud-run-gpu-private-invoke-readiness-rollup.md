...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_real_dispatch_persisted_job_lease_bridge_required`.
...
| Controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge | blocked controlled persisted worker dispatch runtime real-dispatch persisted job lease bridge required | The missing bridge must create exactly one controlled persisted job/lease/idempotency/event/runtime-message/worker-claim envelope before a future private invoke handoff. | Plan and implement the persisted job and lease bridge before another Qwen inference attempt. |
...
Current blocker clarification: the previous approved-fixture inference attempt approval blocker is superseded by the recorded approval, and the 58DB attempt result is now recorded as blocked before paid runtime execution. The remaining Qwen private-invoke blocker is the controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge.
