...
const priorMergeSha = '9f3d7afc8eb33d93bae0c8728e2666ffbcccceeb'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-GUARDED-LIVE-REGISTRATION-REVIEW-1'

const packetFiles = [
  `${packetDir}/gpac-mp4box-runtime-scaffold-negative-tests-decision.json`,
  `${packetDir}/gpac-mp4box-runtime-scaffold-negative-tests-decision.md`,
...
function fail(message) {
  console.error(`TRACKA-GPAC-MP4BOX-RUNTIME-SCAFFOLD-NEGATIVE-TESTS-1 diagnostics failed: ${message}`)
  process.exit(1)
}
...
if (packageJson.scripts?.['tracka:gpac-mp4box-runtime-scaffold-negative-tests:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-runtime-scaffold-negative-tests-diagnostics.mjs') fail('missing package diagnostics script')
if (packageJson.scripts?.['smoke:tracka-gpac-mp4box-runtime-scaffold-negative-tests'] !== 'tsx server/smoke/tracka-gpac-mp4box-runtime-scaffold-negative-tests-smoke.ts') fail('missing smoke script')
...
const decision = json(`${packetDir}/gpac-mp4box-runtime-scaffold-negative-tests-decision.json`)
if (decision.decision !== decisionText) fail('decision drift')
if (decision.execution !== executionText) fail('execution drift')
if (decision.prior?.disabledRuntimeScaffold !== priorDecision) fail('prior scaffold decision drift')
if (decision.prior?.disabledRuntimeScaffoldMergeSha !== priorMergeSha) fail('prior scaffold merge SHA drift')
if (decision.baselineScaffoldStatus !== 'disabled_scaffold_registered_no_runtime') fail('baseline status drift')
...
for (const input of ['rawChat', 'rawCommandString', 'frontendFilePath', 'publicUrlSourceOfTruth', 'signedUrlSourceOfTruth', 'arbitraryPrivateMedia', 'providerOrModelPromptPayload', 'serviceRoleSecretPayload', 'broadServiceRoleHandlerPayload']) {
  if (matrix.rejectedInputCoverage?.[input] !== 'blocked_rejected_input_present') fail(`matrix rejected input drift ${input}`)
}
for (const attempt of runtimeAttempts) {
  if (matrix.runtimeAttemptCoverage?.[attempt] !== 'blocked_runtime_execution_attempt') fail(`matrix runtime drift ${attempt}`)
}
for (const attempt of deliveryAttempts) {
  if (matrix.deliveryAttemptCoverage?.[attempt] !== 'blocked_storage_or_public_delivery_attempt') fail(`matrix delivery drift ${attempt}`)
}
...
if (readiness.readyForGuardedLiveRegistrationReview !== true) fail('readiness next gate drift')
...
const prior = json(`${priorDir}/gpac-mp4box-disabled-runtime-scaffold-decision.json`)
if (prior.decision !== priorDecision) fail('prior disabled scaffold decision drift')

const smokeText = read('server/smoke/tracka-gpac-mp4box-runtime-scaffold-negative-tests-smoke.ts')
for (const text of [
  'rejectedInputCases',
  'runtimeAttemptCases',
...
for (const file of changedFiles) {
  ...
  if (/^server\//.test(file) && file !== 'server/smoke/tracka-gpac-mp4box-runtime-scaffold-negative-tests-smoke.ts') fail(`unexpected server path ${file}`)
...
console.log('TRACKA-GPAC-MP4BOX-RUNTIME-SCAFFOLD-NEGATIVE-TESTS-1 diagnostics passed')
console.log(`Decision: ${decisionText}`)
console.log('Negative tests cover rejected inputs, runtime attempts, storage/public delivery attempts, and missing references')
