...
- `npm run smoke:beta-readiness-operator-status` verifies the combined CLI operator status report for complete and missing input cases, secret-safe output, current blocker counts, evidence-driven policy, and next-command sequencing without touching a deployed backend. `npm run smoke:beta-readiness-api` verifies the deployed backend operator-status route stays read-only and reflects stored workspace evidence.
