...
- backend runtime persistence credit approval snapshots baseline fix: blocked, migration prerequisite fix required
...
| Backend runtime persistence credit approval snapshots baseline fix | blocked backend runtime persistence credit approval snapshots baseline fix required | Retry 5 proved the edit-plan-segments version baseline fix and identified the next active baseline prerequisite. | Fix active baseline migration `202605180004_reeditpro_credits_approval_snapshots.sql` so it is idempotent when loaded after older credit and approval baseline tables that lack approved snapshot reference columns. |
