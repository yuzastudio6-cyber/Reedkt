...
const lane = 'TRACKA-GPAC-MP4BOX-GUARDED-HANDLER-IMPLEMENTATION-REVIEW-1'
const packetDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-guarded-handler-implementation-review'
const priorDir = 'docs/track-a/native-container-render-tools/gpac-mp4box-handler-registration-scaffold-negative-tests'
const decisionText = 'tracka_gpac_mp4box_guarded_handler_implementation_review_passed_ready_for_disabled_handler_implementation_contract'
const executionText = 'completed_docs_only_guarded_handler_implementation_review_no_runtime_execution'
const priorDecision = 'tracka_gpac_mp4box_handler_registration_scaffold_negative_tests_passed_ready_for_guarded_handler_implementation_review'
const priorMergeSha = 'f3fa8f3d5fb8057b2541c15257c61f6fdbc18596'
const nextPrompt = 'TRACKA-GPAC-MP4BOX-DISABLED-HANDLER-IMPLEMENTATION-CONTRACT-1'

const packetFiles = [
  `${packetDir}/gpac-mp4box-guarded-handler-implementation-review-decision.json`,
  `${packetDir}/gpac-mp4box-guarded-handler-implementation-review-decision.md`,
  `${packetDir}/handler-implementation-review-matrix.json`,
  `${packetDir}/handler-implementation-review-matrix.md`,
...
const priorFiles = [
  `${priorDir}/gpac-mp4box-handler-registration-scaffold-negative-tests-decision.json`,
...
if (packageJson.scripts?.['tracka:gpac-mp4box-guarded-handler-implementation-review:diagnostics'] !== 'node scripts/validation/tracka-gpac-mp4box-guarded-handler-implementation-review-diagnostics.mjs') fail('missing diagnostics script')

const decision = json(`${packetDir}/gpac-mp4box-guarded-handler-implementation-review-decision.json`)
...
if (decision.prior?.handlerRegistrationScaffoldNegativeTests !== priorDecision) fail('prior decision drift')
if (decision.prior?.handlerRegistrationScaffoldNegativeTestsMergeSha !== priorMergeSha) fail('prior merge drift')
if (decision.review?.allowedFutureScope !== 'disabled_handler_implementation_contract_only') fail('future scope drift')
...
if (readiness.guardedHandlerImplementationReviewPassed !== true) fail('review readiness drift')
if (readiness.readyForDisabledHandlerImplementationContract !== true) fail('next readiness drift')
...
const matrix = json(`${packetDir}/handler-implementation-review-matrix.json`)
if (matrix.futureAllowedScope !== 'disabled_handler_implementation_contract_only') fail('matrix scope drift')
...
for (const text of [lane, decisionText, executionText, priorDecision, priorMergeSha, nextPrompt, 'disabled_handler_implementation_contract_only', 'Product-ready local OSS tools: `0`', 'Package-lock: `unchanged`', 'Generated artifacts committed: `none`', 'PR #577 remains open/draft/blocked/conflicting and excluded']) {
