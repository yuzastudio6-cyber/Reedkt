...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 80)
assert.equal(
...
assert.equal(rollup.runtimeFlags.controlledPersistedWorkerDispatchRuntimeExecutionApprovalRequired, true)
