...
| --- | --- | --- | --- | --- |
| `qwen2_5_vl_7b_instruct` | controlled persisted worker dispatch runtime real-dispatch approved-fixture private inference bounded retry strict output fix ready | ready for explicit tool gate | the 58DW bounded retry loaded Qwen and returned parseable JSON, but the fixture metadata failed the required schema; the strict structured-output source fix now rejects that top-level object-row shape and allows only one explicit retry-2 path after live preflight | `QWEN2_5_VL_STACK_TOOL_58DW-RETRY-2: run one bounded approved-fixture private inference retry after strict structured-output fix, no generated assets/no mutation` |
...
1. Qwen: the 58DW bounded retry result remains recorded as schema-invalid runtime evidence, and the 58DW-FIX strict structured-output source fix is now recorded and locally validated. External agents may surface only the explicit 58DW-RETRY-2 bounded fixture prompt after read-only live auth/service/job preflight passes; raw chat execution, unbounded inference, generated assets, signed URLs, beta, and production remain blocked.
...
- Qwen is the closest lane to controlled private model inference; the bounded private inference retry plan/gate/approval, 58DV gate alignment, 58DW bounded retry result, and 58DW-FIX strict structured-output fix are recorded. The retry proved model load/inference reachability but failed the required structured fixture schema; the source now rejects that failed top-level object-row shape and the only allowed follow-up is the explicit 58DW-RETRY-2 bounded approved-fixture prompt after live preflight passes. Raw chat, direct arbitrary Cloud Run invocation, unbounded inference, generated assets, Supabase mutation, signed URLs, credits, beta, and production remain blocked.
