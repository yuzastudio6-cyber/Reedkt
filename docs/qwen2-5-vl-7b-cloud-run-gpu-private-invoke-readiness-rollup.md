...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_runtime_execution_plan_recorded_execution_approval_required`.
...
- controlled persisted worker dispatch runtime execution approval: blocked, execution approval required
...
| Controlled persisted worker dispatch runtime execution approval | blocked controlled persisted worker dispatch runtime execution approval required | Runtime execution plan is recorded, but the exact execution envelope has not been approved for any future controlled no-beta runtime gate. | Approve or reject the execution plan while keeping Cloud Run invocation, Qwen inference, generated assets, beta, and production blocked. |
