...
type RunnerSummary = {
  ...
  wanLatentInferenceCanaryPassed: boolean
  promptEncodingRun: boolean
  denoisingRun: boolean
  transientLatentsCreated: boolean
  inferenceOutputPersisted: boolean
...
const REMOTE_ROOT = '/tmp/reeditpro-broll-11h'
...
function main() {
  ...
  if (preflightOnly) {
    ...
    const summary: RunnerSummary = {
      ...
      mode: 'ai_video_broll_gen_11h_l4_inference_proof_runner_read_only_preflight_only',
      decision: passed
        ? 'ai_video_broll_gen_11h_preflight_passed_vm_create_still_not_attempted'
        : 'ai_video_broll_gen_11h_preflight_blocked_no_vm_created',
      nextPrompt: passed
  ...
  if (!execute) {
    ...
    summary.mode = cacheFillOnly
      ? 'ai_video_broll_gen_11h_l4_inference_proof_runner_cache_fill_confirmation_blocked'
      : 'ai_video_broll_gen_11h_l4_inference_proof_runner_static_plan'
    summary.decision = cacheFillOnly
      ? 'ai_video_broll_gen_11h_model_cache_fill_requires_execute_confirmation'
      : 'ai_video_broll_gen_11h_l4_inference_proof_runner_static_plan_only_no_gcp_commands'
    summary.blockers = cacheFillOnly ? [`confirmation_env_required:${CONFIRM_ENV}=true`] : []
    summary.nextPrompt = 'AI-VIDEO-BROLL-GEN-11H-INFERENCE-PROOF-EXECUTE: run bounded Wan inference proof with mandatory cleanup, no generated video/no persisted assets'
    print(summary)
    return
  }
  ...
  if (process.env[CONFIRM_ENV] !== 'true') {
    ...
    summary.mode = 'ai_video_broll_gen_11h_l4_inference_proof_runner_confirmation_blocked'
    summary.decision = 'ai_video_broll_gen_11h_l4_inference_proof_runner_confirmation_required_no_gcp_commands'
...
function runCacheFill(summaryPath: string) {
  ...
  if (!localModelCache.ok) {
    const summary: RunnerSummary = {
      ...
      mode: 'ai_video_broll_gen_11h_l4_inference_proof_runner_cache_fill_local_preflight_blocked',
      decision: 'ai_video_broll_gen_11h_model_cache_fill_local_preflight_blocked_no_vm_created',
  ...
  if (!preflight.ok) {
    const summary: RunnerSummary = {
      ...baseSummary(summaryPath, 'blocked'),
      mode: 'ai_video_broll_gen_11h_l4_inference_proof_runner_cache_fill_preflight_blocked',
      decision: 'ai_video_broll_gen_11h_model_cache_fill_preflight_blocked_no_vm_created',
  ...
  const summary: RunnerSummary = {
    ...
    mode: 'ai_video_broll_gen_11h_l4_inference_proof_runner_private_gcs_model_cache_fill_only',
    decision: passed
      ? 'ai_video_broll_gen_11h_private_gcs_model_cache_ready_no_vm_created'
      : 'ai_video_broll_gen_11h_private_gcs_model_cache_fill_failed_no_vm_created',
...
function runExecute(summaryPath: string) {
  ...
  let wanLatentInferenceCanaryPassed = false
  let promptEncodingRun = false
  let denoisingRun = false
  let transientLatentsCreated = false
  const inferenceOutputPersisted = false
  ...
  const finishWith = (status: 'passed' | 'failed' | 'blocked', blockers: string[]) => {
    const summary: RunnerSummary = {
      ...
      decision: status === 'passed'
        ? 'ai_video_broll_gen_11h_l4_latent_inference_proof_passed_cleanup_verified_no_generated_video'
        : 'ai_video_broll_gen_11h_l4_inference_proof_blocked_or_failed_cleanup_required',
      blockers: Array.from(new Set(blockers)),
      phaseResults,
      preflightPassed,
      ...
      promptEncodingRun,
      denoisingRun,
      transientLatentsCreated,
      inferenceOutputPersisted,
      cleanupAttempted,
      cleanupVerified,
      runtimeSideEffects: runtimeSideEffects({
        computeVmCreateAttempted,
        ...
        modelInferenceRun: wanLatentInferenceCanaryPassed,
        promptEncodingRun,
        denoisingRun,
        transientLatentsCreated,
      }),
  ...
  try {
    ...
    const remoteGcloud = runSsh(
      ...
      'gcloud --version && echo REEDITPRO_BROLL_11H_REMOTE_GCLOUD_READY',
      120_000,
    )
    ...
    const downloadWheelhouse = runSsh(
      ...
      [
        ...
        '&&',
        'echo REEDITPRO_BROLL_11H_PRIVATE_GCS_WHEELHOUSE_DOWNLOAD_OK',
      ].join(' '),
    ...
    const downloadModel = runSsh(
      ...
      [
        ...
        '&&',
        'echo REEDITPRO_BROLL_11H_PRIVATE_GCS_MODEL_CACHE_DOWNLOAD_OK',
      ].join(' '),
      GCS_PAYLOAD_TIMEOUT_MS,
    )
    ...
    wheelhousePayloadTransferred =
      ...
      Boolean(downloadWheelhouse.stdoutSummary?.includes('REEDITPRO_BROLL_11H_PRIVATE_GCS_WHEELHOUSE_DOWNLOAD_OK'))
    modelCachePayloadTransferred =
      downloadModel.ok &&
      Boolean(downloadModel.stdoutSummary?.includes('REEDITPRO_BROLL_11H_PRIVATE_GCS_MODEL_CACHE_DOWNLOAD_OK'))
    ...
    remoteModelCacheValidated =
      ...
      Boolean(remoteModelValidation.stdoutSummary?.includes('REEDITPRO_BROLL_11H_REMOTE_MODEL_CACHE_READY'))
    ...
    const install = runSsh(
      ...
      [
        ...
        'python3.12 -c',
        JSON.stringify(
          `import pathlib, zipfile; src=pathlib.Path('${REMOTE_WHEELHOUSE}'); dst=pathlib.Path('${REMOTE_TARGET_DEPS}'); wheels=sorted(src.glob('*.whl')); assert len(wheels) == ${EXPECTED_WHEEL_COUNT}, len(wheels); [zipfile.ZipFile(wheel).extractall(dst) for wheel in wheels]; print('REEDITPRO_BROLL_11H_OFFLINE_WHEEL_EXTRACT_OK')`,
        ),
        '&&',
        'echo REEDITPRO_BROLL_11H_OFFLINE_INSTALL_OK',
      ].join(' '),
      25 * 60_000,
    )
    phaseResults.push(install)
    offlineDependencyInstallPassed =
      install.ok && Boolean(install.stdoutSummary?.includes('REEDITPRO_BROLL_11H_OFFLINE_INSTALL_OK'))
    ...
    const imports = runSsh(
      ...
      [
        ...
        'python3.12 -c',
        JSON.stringify(
          "import torch, diffusers, transformers; from diffusers import WanPipeline; print('REEDITPRO_BROLL_11H_WAN_PIPELINE_CLASS_IMPORT_OK')",
        ),
      ].join(' '),
      120_000,
    )
    ...
    dependencyImportReadinessPassed =
      imports.ok && Boolean(imports.stdoutSummary?.includes('REEDITPRO_BROLL_11H_WAN_PIPELINE_CLASS_IMPORT_OK'))
    ...
    denoisingRun = wanLatentInferenceCanaryPassed
    transientLatentsCreated = wanLatentInferenceCanaryPassed

    cleanupPrompt()

    const passed = wanLatentInferenceCanaryPassed && cleanupVerified
    finishWith(
      passed ? 'passed' : 'failed',
      passed
        ? []
        : [
            ...(wanPipelineLocalLoadPassed ? [] : ['wan_pipeline_local_load_failed']),
            ...(wanLatentInferenceCanaryPassed ? [] : ['wan_latent_inference_canary_failed']),
            ...(cleanupVerified ? [] : ['cleanup_not_verified_after_inference_canary']),
          ],
    )
  } catch (error) {
    cleanupPrompt()
...
function uploadModelCacheToPrivateGcs(): PhaseResult[] {
  ...
  const markerPath = path.join('.tmp', 'ai-video-broll-gen-11h-model-cache-ready.json')
...
function waitForIapLookupReadiness() {
  ...
  for (let attempt = 1; attempt <= 8; attempt += 1) {
    const ssh = runSsh(
      ...
      'echo REEDITPRO_BROLL_11H_IAP_LOOKUP_READY',
      60_000,
    )
    phaseResults.push(ssh)
    ok = ssh.ok && Boolean(ssh.stdoutSummary?.includes('REEDITPRO_BROLL_11H_IAP_LOOKUP_READY'))
    if (ok) break
    sleep(10_000)
  }
...
function validateLocalGcloudIapAcceleration(): PhaseResult {
  ...
  const numpy = spawnSync(
    pythonLocation,
    ['-c', "import numpy; print('REEDITPRO_BROLL_11H_GCLOUD_IAP_NUMPY_READY')"],
    {
      cwd: process.cwd(),
      env: process.env,
      encoding: 'utf8',
  ...
  const ok =
    numpy.status === 0 &&
    String(numpy.stdout ?? '').includes('REEDITPRO_BROLL_11H_GCLOUD_IAP_NUMPY_READY')
  return {
    id: 'local_gcloud_iap_numpy_acceleration_ready',
    ok,
...
function buildRemoteWheelhouseDownloadScript() {
  ...
  return buildRemoteExactGcsDownloadScript({
    ...
    successMarker: 'REEDITPRO_BROLL_11H_PRIVATE_GCS_WHEELHOUSE_EXACT_DOWNLOAD_OK',
  })
}
...
function buildRemoteModelCacheDownloadScript() {
  ...
  return buildRemoteExactGcsDownloadScript({
    ...
    successMarker: 'REEDITPRO_BROLL_11H_PRIVATE_GCS_MODEL_CACHE_EXACT_DOWNLOAD_OK',
  })
}
...
function buildRemoteModelCacheValidationScript() {
  const scriptBody = [
    ...
    "print('REEDITPRO_BROLL_11H_REMOTE_MODEL_CACHE_READY')",
  ].join('\n')

  return `exec(${JSON.stringify(scriptBody)})`
}

function buildWanPipelineLatentInferenceCanaryScript() {
  const scriptBody = [
...
function baseSummary(summaryPath: string, status: RunnerSummary['status']): RunnerSummary {
  return {
    ok: status === 'passed',
    mode: 'ai_video_broll_gen_11h_l4_inference_proof_runner_result',
    status,
    decision:
      status === 'passed'
        ? 'ai_video_broll_gen_11h_l4_latent_inference_proof_passed_cleanup_verified_no_generated_video'
...
function runtimeSideEffects(overrides: {
  ...
  modelInferenceRun?: boolean
  promptEncodingRun?: boolean
  denoisingRun?: boolean
  transientLatentsCreated?: boolean
}): JsonRecord {
  return {
    gcpReadOnlyCommandsExecuted: overrides.gcpReadOnlyCommandsExecuted ?? false,
    ...
    modelInferenceRun: overrides.modelInferenceRun ?? false,
    promptEncodingRun: overrides.promptEncodingRun ?? false,
    denoisingRun: overrides.denoisingRun ?? false,
    transientLatentsCreated: overrides.transientLatentsCreated ?? false,
    inferenceOutputPersisted: false,
    vaeDecodeRun: false,
