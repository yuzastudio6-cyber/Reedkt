...
| P0 | SECURITY DEFINER exposure | `has_workspace_role`, `is_workspace_owner_or_admin`, `is_workspace_owner_record`, `set_updated_at`, `can_export_render`, `is_project_editor`, `is_project_member` | Broad execute grants may expose helper behavior or create an RLS bypass if bodies are too permissive. | Prompt 26G creates the migration plan; Prompt 26G-1 is the future local candidate. | `security_definer_exposure_migration_plan_created`; no remediation applied. |
...
| P2 | Unindexed foreign keys | Runtime/foundation FK examples across idempotency, approved snapshots, chat, credits, edit plans, and deferred audio planning. | Slow validation, lock contention, and poor production performance signal. | Prompt 26H creates the migration plan; Prompt 26H-1 is the future local candidate. | `fk_index_migration_plan_created`; no remediation applied. |
...
Prompt 26G advances the SECURITY DEFINER exposure row to `security_definer_exposure_migration_plan_created` only. It classifies the seven functions conservatively, defines future grant review, invoker-decision, test, rollback/cleanup, and staging evidence requirements, and updates the draft-only Markdown sketch. It does not resolve advisor findings, alter functions, change grants, create active migrations, run SQL, or touch Supabase. Prompt 26G-1 is required for a future local migration candidate, while Prompt 26H is the next FK index hardening plan option.

## Prompt 26H update

Prompt 26H advances the unindexed foreign-key row to `fk_index_migration_plan_created` only. It defines FK priority, duplicate review, naming, write-amplification, future test, rollback/cleanup, and staging evidence requirements for fifteen connected-advisor findings and updates the draft-only Markdown sketch. It does not resolve advisor findings, create indexes, create active migrations, run SQL, or touch Supabase. Prompt 26H-1 is required for a future local migration candidate.
