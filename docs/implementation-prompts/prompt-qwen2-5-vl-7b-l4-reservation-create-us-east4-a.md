...
Create one bounded NVIDIA L4 capacity reservation for the Qwen2.5-VL proof path in `us-east4-a`, without creating a VM or running inference.

This prompt is a reservation mutation gate selected by the capacity remediation packet. It must repeat all safety preflight checks immediately before mutation and stop if the target state is not clean.
