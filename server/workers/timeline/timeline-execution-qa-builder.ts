...
import type { SmartCutPreviewResult } from '../smart-cut/smart-cut-execution-types'

export function buildTimelineExecutionQAResults(input: {
  workspaceId: string
  projectId: string
  mediaAssetId: string
  ...
  const transcriptIssues = input.timelineManifest.captionLayers.length > 0
    ? []
    : [issue('caption_or_transcript_ref_missing', 'Timeline has no caption/transcript artifact refs yet.', 'warning')]
  const previewIssues = input.previewResult?.status === 'failed'
    ? [issue('preview_failed', input.previewResult.errorMessage ?? 'Preview failed.', 'blocking')]
    : input.previewResult?.status === 'created'
      ? []
      : [issue('preview_not_created', 'Preview duration sync is not checked because no preview was created.', 'warning')]

  return [
    buildGate(input, 'render_timeline_integrity', timelineIssues),
    buildGate(input, 'cut_smoothness', timelineIssues.filter((item) => item.code.includes('clip') || item.code.includes('overlap'))),
...
function validateTimeline(timelineManifest: TimelineManifest): ProductionToolIssue[] {
  ...
  if (timelineManifest.durationSeconds <= 0) {
    issues.push(issue('timeline_duration_zero', 'Timeline execution manifest duration must be greater than zero.', 'blocking'))
  }
...
function buildGate(
  ...
  return {
    id: `timeline-execution-gate-${gateType}-${input.mediaAssetId}`,
    workspaceId: input.workspaceId,
    projectId: input.projectId,
    mediaAssetId: input.mediaAssetId,
    toolExecutionPlanId: input.toolExecutionPlanId ?? 'timeline-execution-dry-run',
    ...
    recommendations: issues.length > 0
      ? [{ action: 'review_timeline_execution', reason: `Resolve ${gateType} before preview or future render.`, priority: blocking ? 'urgent' : 'normal' }]
      : [{ action: 'continue', reason: `${gateType} passed M14 timeline execution checks.`, priority: 'low' }],
