...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_controlled_persisted_dispatch_smoke_executed_result_review_required`.
...
| Controlled persisted worker dispatch smoke result review | blocked controlled persisted worker dispatch smoke result review required | The smoke execution result is recorded and all real runtime, Cloud Run, inference, Supabase, generated asset, signed URL, credit, beta, and production side-effect gates remain false. | Review and accept the mock-only result before any real worker dispatch readiness claim. |
