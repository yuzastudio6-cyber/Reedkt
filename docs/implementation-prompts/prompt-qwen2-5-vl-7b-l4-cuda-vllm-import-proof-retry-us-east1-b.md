...
Retry the controlled Qwen2.5-VL L4 CUDA visibility and vLLM import proof in `us-east1-b`, after stockouts in `us-central1-b`, `us-central1-a`, `us-central1-c`, and `us-west1-a`.

This prompt may create one bounded no-public-IP NVIDIA L4 / Google Cloud G2 proof VM in `us-east1-b` only after repeating all safety preflight checks. It may transfer the private Qwen wheelhouse and model cache, run CUDA visibility, import `torch`, `transformers`, `qwen_vl_utils`, and `vllm`, and run the metadata loader gate. It must not run inference or start serving.
...
- verify `g2-standard-8` is visible in `us-east1-b`;
- verify `nvidia-l4` is visible in `us-east1-b`;
- verify `us-east1` regional `NVIDIA_L4_GPUS` limit is at least `1` and usage is `0`;
...
- verify no matching proof VM, disk, address, or reservation exists in `us-east1-b` / `us-east1`;
- verify private wheelhouse and private model cache are present locally;
- verify final bounded cost and cleanup plan.

## Allowed Work

- Create one no-public-IP `g2-standard-8` VM in `us-east1-b` with one NVIDIA L4 only if all preflight checks pass.
...
- CUDA visibility result on NVIDIA L4 in `us-east1-b`, or a clean blocked result.
