...
import {
  AI_VIDEO_BROLL_GEN_10A_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_B_RESULT,
  AI_VIDEO_BROLL_GEN_10B_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10a-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-b-result'

const ROOT = process.cwd()
const DOC_PATH =
  'docs/ai-video-broll-gen-10a-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-b-result.md'
const NEXT_PROMPT_PATH = 'docs/implementation-prompts/prompt-ai-video-broll-gen-10b-iap-wheelhouse-transfer-stockout-fix.md'
const SPEC_PATH =
...
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10a-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-b-result'
const DECISION =
  'ai_video_broll_gen_10a_us_east1_b_iap_wheelhouse_transfer_blocked_resource_pool_exhausted_cleanup_verified'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll10aResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10a-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-b-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `10A result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(nextPrompt.includes(required), true, `10B stockout fix prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10A_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_B_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10a_no_idle_l4_iap_wheelhouse_transfer_proof_us_east1_b_result')
...
assert.equal(result.createBlockedReason, 'gpu_availability')
assert.equal(result.createCompletedWithNoVmResource, true)
assert.equal(result.targetRegion, 'us-east1')
assert.equal(result.targetZone, 'us-east1-b')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10B_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT)
