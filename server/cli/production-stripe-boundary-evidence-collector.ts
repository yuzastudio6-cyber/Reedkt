...
export async function runProductionStripeBoundaryEvidenceCollectorFromEnv(
  ...
  if (!record.ok || (record.mode !== 'recorded' && record.mode !== 'blocked_recorded')) {
    throw new Error('Production Stripe boundary evidence could not be recorded as a passing or blocked audit packet through the all-up production readiness evidence collector.')
  }
  ...
  return {
    ...
    warnings: [
      ...
      record.mode === 'blocked_recorded'
        ? 'Stripe boundary evidence was recorded as a blocked audit packet while the all-up production readiness packet remains incomplete.'
        : 'Stripe boundary evidence was recorded as part of the authenticated all-up production readiness evidence packet.',
