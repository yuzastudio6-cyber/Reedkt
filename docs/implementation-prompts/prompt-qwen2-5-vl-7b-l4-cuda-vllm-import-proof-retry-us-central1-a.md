...
Retry the controlled Qwen2.5-VL L4 CUDA visibility and vLLM import proof in `us-central1-a`, after `us-central1-b` returned zonal stockout for the approved `g2-standard-8` plus one NVIDIA L4 shape.

This prompt may create one bounded no-public-IP NVIDIA L4 / Google Cloud G2 proof VM in `us-central1-a` only after repeating all safety preflight checks. It may transfer the private Qwen wheelhouse and model cache, run CUDA visibility, import `torch`, `transformers`, `qwen_vl_utils`, and `vllm`, and run the metadata loader gate. It must not run inference or start serving.
...
- Fallback same-region candidate if stocked out again: `us-central1-c`
...
- Prior stockout plan: `docs/qwen2-5-vl-7b-l4-stockout-plan-result.md`
...
- verify `g2-standard-8` is visible in `us-central1-a`;
- verify `nvidia-l4` is visible in `us-central1-a`;
...
- verify no matching proof VM, disk, address, or reservation exists in `us-central1-a`;
- verify private wheelhouse and private model cache are present locally;
- verify final bounded cost and cleanup plan.

## Allowed Work

- Create one no-public-IP `g2-standard-8` VM in `us-central1-a` with one NVIDIA L4 only if all preflight checks pass.
...
- CUDA visibility result on NVIDIA L4 in `us-central1-a`, or a clean stockout/blocked result.
