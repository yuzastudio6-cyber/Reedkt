...
| Backend runtime persistence generation requests approved snapshot baseline fix | blocked backend runtime persistence generation requests approved snapshot baseline fix required | Retry 7 isolates the next active baseline prerequisite to `generation_requests.approved_plan_snapshot_id` handling in `202605180005_reeditpro_generation_assets_jobs.sql`. | Guard `generation_requests.approved_plan_snapshot_id` before `idx_generation_requests_project_snapshot` without inventing generation requests, approved snapshots, generated assets, jobs, storage records, worker rows, credit rows, or provider outputs. |
...
The next action is fixing the active generation assets/jobs baseline so older local baselines have `generation_requests.approved_plan_snapshot_id` before `idx_generation_requests_project_snapshot` runs. That future prompt must not touch Supabase cloud, staging, production, live data, Cloud Run, Qwen inference, worker dispatch, generated assets, public artifacts, signed URLs, beta, production, or claim `generated_local_fixture_passed`.
