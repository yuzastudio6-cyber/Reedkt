...
for (const tool of rollup.tools) {
  if (tool.toolId === QWEN_TOOL_ID) {
    ...
    assert.equal(
      ...
      false,
      `${tool.toolId} must not be execution-ready unless its runtime blocker is cleared`,
    )
  }
}
...
assert.equal(
  ...
  true,
  'actionPlan.manualBlockers must include B-roll until the 11H pipeline-load timeout is fixed',
)
assert.equal(
  asArray(readiness.blockers, 'readiness.blockers').some(
  ...
  true,
  'readiness.blockers must include B-roll until the 11H pipeline-load timeout is fixed',
)
