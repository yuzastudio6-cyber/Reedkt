...
- controlled persisted worker dispatch runtime real-dispatch transport dependency enablement implementation: blocked, implementation required
...
| Controlled persisted worker dispatch runtime real-dispatch transport dependency enablement implementation | blocked controlled persisted worker dispatch runtime real-dispatch transport dependency enablement implementation required | The approval requires bounded implementation of backend lease, idempotency, Qwen adapter, private invoke envelope, private invoke transport, response classification, QA/audit/cost/credit, cleanup, and lock-preservation dependency surfaces. | Implement the controlled dependency surfaces without enabling generated assets, beta, production, or arbitrary worker execution. |
...
The next action is controlled persisted worker dispatch runtime real-dispatch transport dependency enablement implementation. That future prompt must keep generated assets, public artifacts, signed URLs, beta, production, and `generated_local_fixture_passed` blocked now.
