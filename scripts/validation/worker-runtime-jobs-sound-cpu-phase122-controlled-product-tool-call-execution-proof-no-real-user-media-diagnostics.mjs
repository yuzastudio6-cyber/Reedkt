...
const nextPrompt =
  'WORKER_RUNTIME_JOBS-SOUND-CPU-PHASE122-CONTROLLED-PRODUCT-TOOL-CALL-EXECUTION-PROOF-OWNER-REVIEW-NO-REAL-USER-MEDIA'

const docs = {
  source:
    'docs/worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-review-no-real-user-media-result.md',
  sourceAcceptance:
    'docs/worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-acceptance-register-no-real-user-media.md',
  sourceHandoff:
    'docs/worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-proof-handoff-register-no-real-user-media.md',
  prompt:
    'docs/implementation-prompts/prompt-worker-runtime-jobs-sound-cpu-phase122-controlled-product-tool-call-execution-proof-no-real-user-media.md',
  runner:
    'scripts/validation/worker-runtime-jobs-sound-cpu-phase122-controlled-product-tool-call-execution-proof-no-real-user-media-runner.mjs',
  result:
...
const parsed = {
  source: parseJsonBlock(
    docs.source,
    'worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-review-no-real-user-media-result',
  ),
  sourceAcceptance: parseJsonBlock(
    docs.sourceAcceptance,
    'worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-acceptance-register-no-real-user-media',
  ),
  sourceHandoff: parseJsonBlock(
    docs.sourceHandoff,
    'worker-runtime-jobs-sound-cpu-phase121-controlled-product-tool-call-execution-plan-owner-proof-handoff-register-no-real-user-media',
  ),
  prompt: parseJsonBlock(
    docs.prompt,
    'worker-runtime-jobs-sound-cpu-phase122-controlled-product-tool-call-execution-proof-no-real-user-media',
  ),
...
assert(read(docs.runner).includes('invokeControlledProductToolCallBoundary'), 'runner boundary missing')
assert(read(docs.runner).includes('proofOutputWrittenToDisk: false'), 'runner output policy missing')
assert(
  read(docs.runner).includes("externalAgentAdapterMode: 'disabled_synthetic_boundary'"),
  'runner disabled agent boundary missing',
)

assert(parsed.source.decision === sourceDecision, 'source decision mismatch')
assert(parsed.source.sourceVerification.sourcePr === 2090, 'source source PR mismatch')
assert(
  parsed.source.sourceVerification.sourceMergeCommit === '7f1f2f1b3278b0d1d1e702ce6016888a1647e290',
  'source source merge mismatch',
)
assert(parsed.source.ownerReview.controlledProductToolCallExecutionProofMayProceedNext === true, 'source proof next missing')
assert(parsed.source.ownerReview.acceptedExpectedInvocationCount === 4, 'source invocation count mismatch')
assert(parsed.source.soundCpuTools.readyForControlledProductToolCallExecutionProofNoRealUserMedia === 15, 'source proof count mismatch')
...
assert(parsed.prompt.proofScope.runControlledProductToolCallExecutionProofCommand === true, 'prompt proof command missing')
...
assert(parsed.result.sourceVerification.sourcePr === 2091, 'result source PR mismatch')
assert(
  parsed.result.sourceVerification.sourceMergeCommit === '7c9d383a1798fa7720662417e56bb8b015e09bbe',
...
assert(parsed.result.proofResult.totalSyntheticProductToolCallBoundaryInvocationsObserved === 4, 'proof total invocation mismatch')
...
assert(parsed.result.soundCpuTools.controlledProductToolCallExecutionProofPassed === 15, 'tool proof count mismatch')
assert(parsed.result.soundCpuTools.readyForControlledProductToolCallExecutionProofOwnerReview === 15, 'tool owner review count mismatch')
assert(parsed.result.soundCpuTools.readyForProductToolCallExecutionToday === 0, 'tool product widened')
assert(parsed.result.soundCpuTools.readyForRealExternalAgentExecutionToday === 0, 'tool real agent widened')
...
for (const [index, row] of parsed.output.whatHappened.entries()) {
  assert(row.invocationId === `phase122-product-tool-call-${index + 1}`, 'what-happened invocation mismatch')
...
assert(parsed.coverage.coverageKind === 'controlled_product_tool_call_execution_boundary_no_real_user_media', 'coverage kind mismatch')
...
assert(
  parsed.blockers.resolvedForThisGate.some(
    (row) => row.blockerId === 'controlled_product_tool_call_execution_proof_pending',
  ),
...
assert(parsed.blockers.readyForControlledProductToolCallExecutionProofOwnerReview === true, 'owner review readiness missing')
assert(parsed.blockers.readyForProductToolCallExecutionToday === false, 'product today widened')
assert(parsed.blockers.soundCpuToolsReadyForControlledProductToolCallExecutionProofOwnerReview === 15, 'blocker tool count mismatch')
assert(parsed.blockers.soundCpuToolsReadyForProductToolCallExecutionToday === 0, 'blocker product count widened')

assert(parsed.policy.allowedClaims.controlledProductToolCallExecutionProofPassedClaimed === true, 'policy proof claim missing')
assert(parsed.policy.allowedClaims.controlledProductToolCallExecutionProofOwnerReviewMayProceedClaimed === true, 'policy owner review missing')
...
assert(parsed.policy.nextGateMayRunControlledProductToolCallExecutionProofOwnerReview === true, 'policy next owner review missing')
...
assert(parsed.next.reviewScope.reviewControlledProductToolCallExecutionProofOnly === true, 'next review scope missing')
assert(parsed.next.reviewScope.requireWhatHappenedEvidence === true, 'next evidence rule missing')
assert(parsed.next.reviewScope.mayReconcileProductToolCallExecutionReadinessNext === true, 'next reconciliation missing')
  ...
  JSON.stringify(
    {
      ...
      invocationCount: parsed.result.proofResult.invocationCount,
      controlledProductToolCallExecutionProofPassed:
        parsed.result.soundCpuTools.controlledProductToolCallExecutionProofPassed,
      readyForControlledProductToolCallExecutionProofOwnerReview:
        parsed.result.soundCpuTools.readyForControlledProductToolCallExecutionProofOwnerReview,
