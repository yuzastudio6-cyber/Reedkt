...
const MODE = 'qwen2_5_vl_private_invoke_token_path_fix_result'
const DECISION =
...
const docText = read('docs/qwen2-5-vl-7b-cloud-run-gpu-private-invoke-token-path-fix-result.md')
const runnerSource = read('server/activation/qwen2-5-vl-cloud-run-gpu-private-invoke-smoke-execute.ts')
const packageJson = JSON.parse(read('package.json')) as { scripts?: Record<string, string> }

assert.equal(
  packageJson.scripts?.['smoke:qwen2-5-vl-private-invoke-token-path-fix-result'],
  'tsx server/smoke/qwen2-5-vl-private-invoke-token-path-fix-result-smoke.ts',
)
...
assert.ok(runnerSource.includes('REEDITPRO_GCP_IMPERSONATE_SERVICE_ACCOUNT'))
assert.ok(runnerSource.includes('--impersonate-service-account'))
assert.ok(runnerSource.includes('redactImpersonationArg'))
assert.ok(runnerSource.includes('SERVICE_ACCOUNT_EMAIL_PATTERN'))
assert.ok(runnerSource.includes('token_creator_permission_required'))
assert.ok(runnerSource.includes('service_url_value_stored=false'))
assert.ok(runnerSource.includes('stripProbeRawOutput'))

const result = QWEN25_PRIVATE_INVOKE_TOKEN_PATH_FIX_RESULT
assert.equal(result.mode, MODE)
assert.equal(result.status, 'blocked')
assert.equal(result.decision, DECISION)
assert.equal(result.runId, 'qwen25-private-invoke-smoke-20260627T111828')
assert.equal(result.preflightRunId, 'qwen25-private-invoke-auth-20260627T101708')
assert.deepEqual(result.blockers, ['token_creator_permission_required'])
assert.equal(result.runnerFix.supportsExplicitImpersonationEnv, true)
assert.equal(result.runnerFix.supportsConfiguredGcloudImpersonation, true)
assert.equal(result.runnerFix.validatesServiceAccountEmail, true)
assert.equal(result.runnerFix.serviceAccountKeysCreated, false)
assert.equal(result.runnerFix.iamPolicyMutated, false)
...
assert.equal(result.tokenPathFinding.serviceAccountImpersonationConfigured, true)
assert.equal(result.tokenPathFinding.serviceAccountImpersonationAttempted, true)
assert.equal(result.tokenPathFinding.tokenCreatorPermissionRequired, true)
assert.equal(result.tokenPathFinding.serviceAccountKeyCreated, false)
assert.equal(result.tokenPathFinding.authHeaderCreated, false)
assert.equal(result.tokenPathFinding.cloudRunRequestSent, false)
...
for (const file of [
  'docs/qwen2-5-vl-7b-cloud-run-gpu-private-invoke-token-path-fix-result.md',
...
assert.deepEqual(
  ...
  [],
  `Forbidden values in Qwen private invoke token path fix result data: ${forbiddenDataFindings.join('; ')}`,
)
...
console.log(JSON.stringify({
  ...
  serviceAccountImpersonationAttempted: result.runtimeFlags.serviceAccountImpersonationAttempted,
