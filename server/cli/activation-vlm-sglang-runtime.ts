import { runVlmSglangRuntimeRecovery } from '../activation/vlm-sglang-runtime'
...
if (!execute) {
  console.log([
    'Phase 39C-SG SGLang VLM runtime evaluation CLI',
    'No execution performed.',
    'Plan/report/IAM/cost/smoke modes are non-mutating by default.',
    'Pass --execute with current-shell confirmations for SGLang approval, private GCS read, SGLang image build/push, staging L4 Cloud Run execution, and private artifact upload.',
  ...
  const result = await runVlmSglangRuntimeRecovery({
    execute: true,
    keepTemp,
    runId,
    safeArtifactDir,
  ...
  if (json) console.log(JSON.stringify(result, null, 2))
    ...
    console.log([
      'Phase 39C-SG SGLang VLM runtime evaluation completed with recorded status.',
      `Run: ${result.runId}`,
