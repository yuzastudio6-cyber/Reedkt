...
| Selective deterministic motion | `verified_private_runtime` for the bounded scalar renderer; `verified_source_contract` for role/activation divergence detection | canonical motion compiler, independent sample-digest verification, Remotion sampler, actual pixel-displacement measurement, selected-scene selective-motion reconciliation | The current canonical compiler broadcasts a scene-level `rotate` verb to every component. The new read-only reconciliation proves that static anchors, stable subjects, and environmental effects receive mechanical rotation even when only explicitly activated mechanical components should rotate. The canonical motion owner must derive tracks from component role plus linked activation, bind retained rotation to approved rig pivots, and route environmental motion to a qualified primitive or approved fallback. |
