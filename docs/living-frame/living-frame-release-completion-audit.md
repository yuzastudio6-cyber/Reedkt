...
| Selective deterministic motion | `verified_private_runtime` for the bounded scalar renderer; `verified_source_contract` for role/activation divergence detection, environmental-particle admission, and the subject-neutral particle kernel | canonical motion compiler, independent sample-digest verification, Remotion sampler, actual pixel-displacement measurement, selected-scene selective-motion reconciliation, selected-scene environmental-particle admission, environmental-particle kernel | The current canonical compiler broadcasts a scene-level `rotate` verb to every component. The read-only reconciliation proves that static anchors, stable subjects, and environmental effects receive mechanical rotation even when only explicitly activated mechanical components should rotate. The environmental admission binds the exact selected component, MasterTiming phases, confirmed frame, procedural geometry, existing PixiJS identity, and required particle primitive. The adjacent kernel now provides seven typed subject-neutral profiles plus deterministic, style-bound, motion-density-capped, one-shot particle state tracks without claiming selected-scene or execution authority. Materialization remains blocked because approved selected-scene evidence still has no immutable typed profile reference, the canonical full-segment five-phase domain conflicts with the narrower visual timing range required by the motion compiler, the current PixiJS operation produces one opaque 640×360 static PNG, and the current Remotion Living Frame overlay accepts one static PNG. Canonical owners must reconcile the phase domain, adopt the typed profile reference, add the bounded time-sampled PixiJS operation and Remotion adapter, and bind work/cost/asset/QA/private-review evidence. |
