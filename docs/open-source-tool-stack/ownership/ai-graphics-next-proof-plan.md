...
| CPU/static spec validation execution | `d3`, `vega_lite`, `vega`, `satori`, `svgdotjs_svg_js`, `viz_js` | Blocked at dependency lockfile gate | `d3`, `vega-lite`, `vega`, `satori`, `@svgdotjs/svg.js`, and `@viz-js/viz` are not declared in package manifests; dependency mutation, static execution, runtime, public artifacts, beta, and production remain blocked |
| CPU/static dependency reconciliation | `d3`, `vega_lite`, `vega`, `satori`, `svgdotjs_svg_js`, `viz_js` | Ready for refreshed execution from dependency-bearing package-proof base | Dependency install, package-lock mutation in this lane, CPU/static execution, runtime, public artifacts, beta, and production remain blocked |
...
Next recommended prompt: `AI_GRAPHICS_DRAFT_PACKAGE_PROOF_CPU_STATIC_SPEC_VALIDATION_REFRESHED_EXECUTION`.
