import {
  ...
  writeOpenSourceToolStackMissingOptionalInstallReviewArtifacts,
...
if (!execute) {
  console.log(summarizeOpenSourceToolStackMissingOptionalInstallReview())
  process.exit(0)
}

if (!metadataOnly) {
  throw new Error('Use --metadata-only. This review packet may not install or prove tools.')
}
...
const reports = writeOpenSourceToolStackMissingOptionalInstallReviewArtifacts()
console.log(summarizeOpenSourceToolStackMissingOptionalInstallReview(reports))
