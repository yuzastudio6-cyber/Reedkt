...
export const privateSearxngServiceConfig: PrivateSearxngServiceConfig = {
  phase: '49F',
  projectId: 'reeditpro',
  region: 'us-central1',
  env: 'staging',
...
export const privateSearxngSafetyFlags: PrivateSearxngSafetyFlags = {
  paidProvidersAllowed: false,
  publicSearxngInstanceAllowed: false,
  publicUnauthenticatedAccessAllowed: false,
  browserCaptureAllowed: false,
...
export function validatePrivateSearxngServiceExecutionEnv(input: {
  ...
  if (input.projectId !== privateSearxngServiceConfig.projectId) blockers.push('GCP_PROJECT_ID must be reeditpro.')
  if (input.activeProject && input.activeProject !== privateSearxngServiceConfig.projectId) blockers.push('Active gcloud project must be reeditpro.')
  if (input.region !== privateSearxngServiceConfig.region) blockers.push('GCP_REGION must be us-central1.')
  if (input.env !== privateSearxngServiceConfig.env) blockers.push('REEDITPRO_ENV must be staging.')
  ...
  if ((input.publicUnauthenticatedAccessAllowed ?? 'false') !== 'false') blockers.push('Public unauthenticated Cloud Run access must remain blocked.')
  if ((input.broadCrawlingAllowed ?? 'false') !== 'false') blockers.push('Broad crawling must remain blocked.')
  if ((input.browserCaptureAllowed ?? 'false') !== 'false') blockers.push('Browser capture must remain blocked in Phase 49F.')
  if ((input.readabilityExtractionAllowed ?? 'false') !== 'false') blockers.push('Readability extraction must remain blocked in Phase 49F.')
  if ((input.publicArtifactAllowed ?? 'false') !== 'false') blockers.push('Public artifacts must remain blocked.')
  if ((input.productionReady ?? 'false') !== 'false') blockers.push('Production readiness is not approved by the Phase 49F private SearXNG service gate.')
  if ((input.externalBetaReady ?? 'false') !== 'false') blockers.push('External beta readiness is not approved by the Phase 49F private SearXNG service gate.')
  if ((input.paidProductionReady ?? 'false') !== 'false') blockers.push('Paid production readiness is not approved by the Phase 49F private SearXNG service gate.')
  if ((input.broadMediaReady ?? 'false') !== 'false') blockers.push('Broad media readiness is not approved by the Phase 49F private SearXNG service gate.')
