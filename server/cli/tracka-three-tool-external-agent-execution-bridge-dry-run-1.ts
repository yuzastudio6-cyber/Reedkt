...
import {
  TRACKA_THREE_TOOL_EXTERNAL_AGENT_EXECUTION_BRIDGE_DRY_RUN_PACKET,
  runThreeToolExternalAgentExecutionBridgeDryRun,
} from '../services/tracka-three-tool-external-agent-execution-bridge-dry-run-1'

const reportRoot = '/tmp/reeditpro-tracka-three-tool-external-agent-execution-bridge-dry-run-1'
const reportFileName = 'three-tool-external-agent-execution-bridge-dry-run-report.json'
const manifestFileName = 'three-tool-external-agent-execution-bridge-dry-run-manifest.json'
...
async function main(): Promise<void> {
  ...
  const result = runThreeToolExternalAgentExecutionBridgeDryRun()
