...
|---|---|---|---|---|
| source truth | `tool.ffprobe.inspect_approved_media.v1` | canonical-private execution bound into the final Track receipt | source facts | fail closed |
| bounded media | `tool.ffmpeg.execute_approved_media_recipe.v1` | canonical-private proxy/redaction execution bound into the final Track receipt | private proxy/frames/redaction preview | fail closed |
| shot discovery | `tool.pyscenedetect.detect_scene_boundaries.v1` | confined content-detector execution bound into the final Track receipt | shot candidates | deterministic explicit boundaries or review |
| visual geometry | `tool.opencv.analyze_approved_visual_artifacts.v1` with fixed `track_all_camera_motion_v1` and `track_all_planar_homography_v1` profiles | confined per-operation native runtime; optical-flow/homography execution bound into the final Track receipt | camera/planar/flow/QA evidence | no model fallback |
