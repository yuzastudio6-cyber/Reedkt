...
readonly CONFIRMATION='ingest-one-weeditpro-sam31-qualification-einops-wheel-v1'
readonly CONTEXT='docker/prod/gpu-worker/sam3_1'
readonly CONFIG="${CONTEXT}/cloudbuild.qualification-einops-ingest.yaml"
readonly APPLE_COMMAND_LINE_TOOLS='/Library/Developer/CommandLineTools'

[[ $# -eq 0 ]] || { printf 'ERROR: caller arguments are forbidden.\n' >&2; exit 64; }
[[ "${WEEDITPRO_CONFIRM_SAM31_QUALIFICATION_EINOPS_INGEST:-}" = "${CONFIRMATION}" ]] \
  || { printf 'ERROR: exact einops-ingest confirmation is required.\n' >&2; exit 64; }
