...
const readiness = buildAiGraphicsInternalBetaProductionWorkerJobReadiness({
  evidenceBundleInput,
...
if (
  hasFlag('--require-owner-approved-production-worker-jobs-ready') &&
  !readiness.ownerApprovedProductionWorkerJobEvidenceAccepted
) {
  process.exitCode = 2
}
