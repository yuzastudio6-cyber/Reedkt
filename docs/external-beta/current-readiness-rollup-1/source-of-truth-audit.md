...
- `RP-EXTERNAL-BETA-CONTROLLED-TESTER-UI-FLOW-SMOKE-1`: source-of-truth for the controlled tester browser/UI surface probe using `aiediting@reeditpro.com`; it records blocker `blocked_deployed_browser_ui_surface_not_present` because no deployed browser UI service was found and authenticated `/`, `/dashboard`, `/projects`, and `/editor` probes on `reeditpro-staging-api` returned API JSON `404` responses.
