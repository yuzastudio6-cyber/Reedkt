...
| Gap | Status | Notes |
| --- | --- | --- |
| Auth/profile/workspace bootstrap missing | Partially fixed | Google-first Supabase OAuth handoff, exact same-origin/base-path callback construction, protected-route recovery, email/password fallback, and local browser regression evidence now exist. The browser-to-private-Cloud-Run source contract now adds safe API-origin validation, Google API Gateway JWT validation, a dedicated original-token revalidation header, exact gateway/token/Supabase-user matching, and noncredentialed exact-origin CORS. A read-only 2026-07-17 check found the hosted app stale, the Cloud Run API private, and the API Gateway API disabled. The Supabase Google provider/callback/redirect allowlists, asymmetric signing-key compatibility, reviewed hosted deployment, real Gmail session, gateway/IAM deployment, canonical RLS-backed profile/workspace provisioning, and two-user staging readback remain unproven. Backend/admin fallback is still needed for RLS-blocked creation. |
| Backend runtime missing | Partially fixed | API route contracts, registry, mock router, frontend client, IAM-private Cloud Run target, and a fail-closed Google API Gateway browser-transport/OpenAPI source contract exist. The Cloud Run service is not a proven current reviewed revision, API Gateway is disabled, and no signed-in browser-to-staging route has passed. |
