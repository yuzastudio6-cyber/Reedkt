...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'controlled_persisted_worker_dispatch_runtime_real_dispatch_approved_fixture_private_inference_bounded_retry_prompt_live_preflight_verified_exact_prompt_required'
...
export function getQwenVlPlannerRoutingUiData(): QwenVlPlannerRoutingUiData {
  return {
    ...
    privateInvokeClient: {
      ...
      currentBlocker:
        'Controlled private invoke, structured-output, persistence, active migration, persisted worker dispatch, private invoke, and private inference planning evidence are recorded through the historical 58DU blocked retry result and 58DV gate alignment. The current live next-command guard verifies local auth, service describe, and caller job describe before surfacing the exact 58DW bounded retry prompt. Generated assets, signed URLs, beta, and production remain blocked.',
      boundaryNotes: [
        ...
        'The live next-command guard verifies local auth, Cloud Run service describe, and caller job describe before surfacing the exact 58DW bounded retry prompt. Generated assets, published artifacts, signed URLs, beta, and production remain blocked now.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
