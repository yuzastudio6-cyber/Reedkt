...
import {
  AI_VIDEO_BROLL_GEN_9Q_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_A_RESULT,
  AI_VIDEO_BROLL_GEN_9R_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9q-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-a-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9q-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-a-result.md'
const PROMPT_PATH = 'docs/implementation-prompts/prompt-ai-video-broll-gen-9r-iap-wheelhouse-transfer-stockout-fix.md'
const SPEC_PATH =
...
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9q-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-a-result'
const DECISION =
  'ai_video_broll_gen_9q_us_west1_a_iap_wheelhouse_transfer_blocked_resource_pool_exhausted_cleanup_verified'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll9qResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9q-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-a-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `9Q result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9R stockout fix prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9Q_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_A_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9q_no_idle_l4_iap_wheelhouse_transfer_proof_us_west1_a_result')
...
assert.equal(result.operationCompletedWithNoVmResource, true)
assert.equal(result.targetZone, 'us-west1-a')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9R_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT)
