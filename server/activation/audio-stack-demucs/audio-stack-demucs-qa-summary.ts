...
export function buildAudioStackDemucsQaGates(): AudioStackDemucsQaGate[] {
  ...
  return [
    gate('tool_routing', 'passed', 'DeepFilterNet owns speech cleanup; Demucs is the separation candidate; RNNoise is not product-routed.'),
    gate('rnnoise_removal', 'passed', 'RNNoise is removed from active labels, fallback plans, and Phase 36G execution scope.'),
    gate('demucs_source_evidence', 'warning', 'Official Demucs repo/model documentation exists, but the repo is archived and model provenance needs stricter review.'),
    gate('demucs_license_provenance', blockers.length ? 'blocked' : 'passed', blockers.length ? blockers.join(' ') : 'Demucs pretrained-model evidence is clear.'),
    gate('download_runtime_block', 'passed', 'Demucs download/runtime commands are explicitly disabled while model licensing is ambiguous.'),
