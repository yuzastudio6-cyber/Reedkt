...
The Qwen 3.7 beta transport is config-driven. Supported profiles are `openai_chat_completions` and `generic_json_post`.
...
Qwen2.5-VL visual context uses the separate transport docs in `qwen25vl-provider-transport.md`. Qwen 3.7 Marker Chat transport must not receive sampled frame payloads or raw video/audio; it may receive compact visual summaries only in a future context-aware Marker Chat milestone.
