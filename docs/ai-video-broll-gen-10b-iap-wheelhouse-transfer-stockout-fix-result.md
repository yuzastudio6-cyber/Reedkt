...
| Zone | Region | Status | `g2-standard-4` visible | `nvidia_l4` visible | Regional L4 quota | Regional CPU quota | Regional SSD quota | 10B disposition |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `us-east1-b` | `us-east1` | `UP` | true | true | limit `1`, usage `0` | limit `200`, usage `0` | limit `500`, usage `0` | rejected_recent_stockout |
...
These checks do not reserve real-time capacity and do not guarantee the next VM create will succeed. They prove only that the checked candidate zones expose the requested shape, quota is visible and unused, no matching proof resources are currently present, and the private cache plus wheelhouse remain ready.
...
- `us-east1-c` is the first untried same-region cross-zone candidate with visible `g2-standard-4`, visible `nvidia_l4`, zone status `UP`, and unused regional L4 quota.
- `us-east1-d` remains an available same-region fallback if a future `us-east1-c` proof also stocks out.
...
- A delayed retry is not selected because untried no-idle same-region transfer targets remain available.
...
- `us-east1-c` zone status;
- `g2-standard-4` visibility in `us-east1-c`;
- `nvidia_l4` visibility in `us-east1-c`;
...
## Future 10C Guardrails

The next execution prompt may attempt exactly one no-public-IP `g2-standard-4` VM with one NVIDIA L4 in `us-east1-c` only after repeated preflight passes.
