...
for (const required of [
  ...
  'Decision: `ai_video_broll_wan_gpu_global_quota_verified_iap_oslogin_fix_required`',
...
assert.equal(
  spec.nextActionIfQuotaCleared.includes(
    '10Q-IAP-OSLOGIN-ACCESS-FIX',
  ),
  true,
)
assert.equal(spec.allowedReadOnlyCommands.length, 7)
assert.equal(resultSpec.decision, 'ai_video_broll_wan_gpu_global_quota_verified_iap_oslogin_fix_required')
...
assert.equal(resultSpec.noIdleLifecycleGate.readyForIapOsLoginAccessFixPrompt, true)
...
assert.equal(resultSpec.readyForIapOsLoginAccessFixPrompt, true)
...
assert.equal(live.readyForIapOsLoginAccessFixPrompt, live.quotaSufficientForOneL4Vm)
  ...
  JSON.stringify(
    {
      ...
      readyForIapOsLoginAccessFixPrompt: live.readyForIapOsLoginAccessFixPrompt,
