...
export function createEditLevelEstimateItem(input: {
  ...
  return {
    ...
    displayName: hideInternalToolNamesInCopy(definitionItem.displayName),
    ...
    technicalNotes: [
      ...
      'No credits are reserved or spent; no credit records, backend steps, render jobs, progress, AI asset calls, model calls, planner execution, media processing, file-byte reads, or external fetches occur.',
    ],
    estimateOnly: true,
...
function levelEstimateCopy(level: ReEditProCanonicalEditLevel) {
  if (level === 'normal') {
    return {
      ...
      qwenReason: 'Normal uses a single future planning pass policy.',
      qwenSummary: 'Single planning pass policy; no planning service call.',
      visualReason: 'Normal uses targeted visual understanding only when needed.',
  ...
  if (level === 'premium') {
    return {
      ...
      qwenReason: 'Premium uses two-pass future planning policy.',
      qwenSummary: 'Two planning passes; no planning service call.',
      visualReason: 'Premium plans key visual moments and marker windows.',
  ...
  return {
    ...
    qwenReason: 'Ultra Premium uses multi-pass future planning policy.',
    qwenSummary: 'Multi-pass planning policy; no planning service call.',
    visualReason: 'Ultra Premium plans scene-level visual understanding.',
