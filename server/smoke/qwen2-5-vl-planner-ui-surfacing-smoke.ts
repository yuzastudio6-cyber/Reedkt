...
check(
  ...
  'Private invoke client status must record the controlled persisted worker dispatch runtime real-dispatch transport preflight approval blocker.',
)
check(
...
check(
  docText.includes('approved_worker_integration_review_required') ||
    ...
    docText.includes('controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_preflight_approval_required') ||
    data.privateInvokeClient.currentStatus ===
      'controlled_persisted_worker_dispatch_runtime_real_dispatch_transport_preflight_approval_required',
...
check(
  docText.includes('Retry 14 verified that fix') &&
    ...
    docText.includes('QWEN2_5_VL_STACK_TOOL_58CS-CONTROLLED-PERSISTED-WORKER-DISPATCH-RUNTIME-REAL-DISPATCH-TRANSPORT-PREFLIGHT-APPROVAL'),
  'Doc must record retry-14 evidence, the storage upload pipeline policy-comment fix, retry-15 pass, and controlled persisted worker dispatch runtime real-dispatch transport preflight approval blocker.',
)
check(
