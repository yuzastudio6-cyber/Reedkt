...
export const canonicalSourceLedCaptionRevisionPresentationSchema = z.object({
  ...
  derivation: z.object({
    ...
    chatDirectionReread: z.literal(true),
    chatDirectionCount: z.number().int().min(0).max(200),
    chatThreadRevision: z.number().int().min(0),
    chatDirectionAuthorityDigestSha256: sha256,
