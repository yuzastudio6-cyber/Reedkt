...
| Authenticated private upload-to-review pipeline | discoverable one-publication persisted backend handoff plus bounded two-pass execution fixture | The authenticated backend persists a checksum-protected, tenant-scoped upload/planning handoff, serializes preparation, and maintains a checksum-protected latest pointer per edit session. An authenticated client can persist a content-addressed publication-request candidate; candidate submission is serialized and a checksum-protected latest pointer supports refresh/restart recovery. Safe inspection returns hashes/state only, while the internal route loads the full candidate and revalidates the handoff before publication. The canonical plan binding durably limits each handoff to one full publication request/idempotency key while allowing exact restart-safe replay. All authenticated canonical HTTP fixtures and the full initial/review/revision lifecycle publish through persisted handoff authority; the former caller-authority HTTP route fails closed. The fixture reaches first review, accepted second review, and restart-safe current/superseded history download. Frontend consumption and deployed real-user operations remain blocked. |
...
15. **Backend bridge complete; frontend consumption remains:** bind the browser journey to the authenticated canonical planning handoff and content-addressed publication-request candidate without reviving legacy caller-authored authority. The authenticated submit/inspect and internal publish-from-candidate routes now pass local/private canonical evidence; no frontend call site is added by this backend-only task.
