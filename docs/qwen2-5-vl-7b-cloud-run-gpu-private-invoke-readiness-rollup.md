...
| Backend runtime persistence local harness validation retry after generation requests approved snapshot fix | blocked backend runtime persistence local harness validation retry after generation requests approved snapshot fix required | The generation requests approved snapshot baseline fix is recorded. | Run the next approved local harness retry to determine whether the active baseline reaches Qwen draft SQL. |
