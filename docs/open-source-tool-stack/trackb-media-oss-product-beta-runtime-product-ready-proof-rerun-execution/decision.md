...
Decision passes the bounded rerun execution gate and advances to QA review only.
...
```json
{
  ...
  "blockedScopeConfirmations": [
    "no live product calls",
    "no real route dispatch",
    "no worker dispatch to real tools",
    "no real tools",
    "no Docker",
    "no installs",
    "no media processing",
