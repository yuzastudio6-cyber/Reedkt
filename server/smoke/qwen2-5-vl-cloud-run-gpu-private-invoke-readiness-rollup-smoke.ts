...
for (const phrase of [
  ...
  'controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge: blocked, bridge required',
  ...
  'Controlled persisted worker dispatch runtime real-dispatch persisted job and lease bridge | blocked controlled persisted worker dispatch runtime real-dispatch persisted job lease bridge required',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRequired=false`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceApprovalRecorded=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferenceAcceptedForPreflight=true`',
  '`controlledPersistedWorkerDispatchRuntimeRealDispatchApprovedFixtureInferencePreflightRequired=false`',
...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 113)
assert.equal(
...
assert.equal(rollup.runtimeFlags.persistedJobLeaseBridgeRequired, true)
