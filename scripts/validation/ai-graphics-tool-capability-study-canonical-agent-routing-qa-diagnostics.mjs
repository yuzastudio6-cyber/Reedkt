...
const baseRef =
  "origin/codex/rp-ai-graphics-tool-capability-study-canonical-agent-routing-approval";
...
const expectedScriptCommand =
  "node scripts/validation/ai-graphics-tool-capability-study-canonical-agent-routing-qa-diagnostics.mjs";
...
const requiredDocs = [
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-qa-review.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-qa-source-lockfile.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-schema-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-capability-map-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-ranking-policy-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-elimination-policy-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-fallback-policy-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-safety-boundary-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-planning-only-policy-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-examples-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-blocked-use-register-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-next-lane-qa.md",
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-qa-decision.md",
  "docs/prompt-ai-graphics-tool-capability-study-canonical-agent-routing-qa-review-results.md",
  "docs/implementation-prompts/prompt-ai-graphics-tool-capability-study-canonical-agent-routing-qa-review.md",
];
...
const requiredJson = [
  ...
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-planning-only-policy-qa.json",
  ...
  "docs/tool-intelligence/ai-graphics/canonical-agent-routing-qa-decision.json",
];
...
const capabilityDocs = capabilityDocNames.map(
  (name) => `docs/tool-intelligence/ai-graphics/canonical-routing/qa/${name}.md`,
);
...
try {
  ...
  sourceApproval = parseJson(
    "docs/tool-intelligence/ai-graphics/canonical-agent-routing-approval.json",
  );
} catch (error) {
  fail(`Unable to parse QA/source JSON: ${error.message}`);
}

if (qa.decision !== expectedDecision) fail(`Unexpected decision: ${qa.decision}`);
if (
  sourceApproval.decision !==
  "ai_graphics_tool_capability_study_canonical_agent_routing_approved_with_warnings"
) {
  fail("Source canonical routing approval decision missing or unexpected.");
}

const trueBooleans = [
  "canonicalAgentRoutingQaCompleted",
  "sourceCanonicalRoutingApprovalAccepted",
  "sourceCapabilityStudyOwnerApprovalQaAccepted",
  "all21ToolsCoveredByRoutingQa",
  "allRequiredCapabilitiesCoveredQa",
  "canonicalRoutingSchemaQaAccepted",
  "capabilityMapQaAccepted",
  "rankingPolicyQaAccepted",
  "eliminationPolicyQaAccepted",
  "fallbackPolicyQaAccepted",
  "planningOnlyPolicyQaAccepted",
  "safetyBoundaryQaAccepted",
  "routingExamplesQaAccepted",
...
const schema = qa.schemaQa?.sourceSchema || {};
...
const capabilityQa = qa.capabilityQa || {};
for (const capability of requiredCapabilities) {
  ...
  for (const key of [
    "preferredPlanningToolsAccepted",
    "conditionalPlanningToolsAccepted",
    "fallbackPlanningToolsAccepted",
    "eliminatedToolsAccepted",
    "blockedRuntimeReasonsAccepted",
  ]) {
...
for (const tool of allTools) {
  if (!tools.has(tool)) fail(`Missing QA tool row: ${tool}`);
}
...
const ranking = qa.rankingPolicyQa || {};
...
const eliminationText = (
  qa.eliminationPolicyQa?.eliminationPolicy?.rules || []
).join("\n");
...
const fallback = qa.fallbackPolicyQa?.fallbackPolicy || {};
for (const capability of requiredCapabilities) {
...
const examples = qa.routingExamplesQa?.routeExamples || {};
for (const capability of requiredCapabilities) {
  if (!examples[capability]) fail(`Routing examples QA missing: ${capability}`);
}

const blocked = qa.blockedUseRegisterQa?.blockedUseRegister || {};
for (const phrase of [
...
const safety = qa.safetyBoundaryQa?.safetyBoundary || {};
...
if (failures.length > 0) {
  console.error("AI graphics canonical routing QA diagnostics failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("AI graphics canonical routing QA diagnostics passed.");
