...
const [cli, vite, dockerfile, buildConfig, buildScript, deployScript,
  ...
  read('server/cli/canonical-vertex-a100-account-effective-rate-cloud-job.ts'),
  read('vite.vertex-a100-rate-publisher.config.ts'),
  read('docker/prod/vertex-a100-rate-publisher/Dockerfile'),
  read('scripts/gcp/prod/cloudbuild-vertex-a100-rate-publisher.yaml'),
  read('scripts/gcp/prod/44-build-vertex-a100-rate-publisher-image.sh'),
  read('scripts/gcp/prod/45-deploy-vertex-a100-rate-publisher-job.sh'),
  read('scripts/gcp/prod/46-run-vertex-a100-rate-publisher-job.sh'),
  read('package.json'),
])

assert.match(cli, /weeditpro-vertex-a100-rate-publisher/u)
assert.match(cli, /z\.literal\('publish_one'\)/u)
assert.match(cli, /CLOUD_RUN_TASK_INDEX !== '0'/u)
assert.match(cli, /process\.argv\[2\] !== '--execute'/u)
assert.match(cli, /vertex-a100-us-central1-\$\{execution\}/u)
...
assert.match(runScript,
  /WEEDITPRO_VERTEX_A100_RATE_OPERATOR_ACTION=publish_one/u)
assert.match(runScript,
...
assert.doesNotMatch(runScript,
  /WEEDITPRO_VERTEX_A100_RATE_PUBLICATION_ID|ffmpeg|ffprobe|python|docker|--gpu/u)
assert.doesNotMatch(runScript, /printf[^\n]*linked_account/u)

assert.match(packageJson,
  /"smoke:vertex-a100-account-effective-rate-cloud-publisher"/u)
assert.match(packageJson,
  /"build:vertex-a100-rate-publisher-bundle"/u)
assert.match(packageJson,
  /"build:vertex-a100-rate-publisher-image"/u)
assert.match(packageJson,
  /"deploy:vertex-a100-rate-publisher-job"/u)
assert.match(packageJson,
  /"run:vertex-a100-rate-publisher-job"/u)

console.log(JSON.stringify({
  smoke: 'canonical-vertex-a100-rate-cloud-publisher',
  checks: 67,
