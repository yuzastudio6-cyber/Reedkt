...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'controlled_persisted_worker_dispatch_runtime_smoke_execution_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58BQ-CONTROLLED-PERSISTED-WORKER-DISPATCH-RUNTIME-SMOKE-EXECUTION: run controlled persisted Qwen worker dispatch runtime smoke, no Cloud Run invocation/no inference/no assets/no beta'
}
  ...
  return {
    ...
    privateInvokeClient: {
      ...
      boundaryNotes: [
        ...
        'The next runtime gate is controlled persisted worker dispatch runtime smoke execution; it must keep the implemented runtime fail-closed and must not deploy migrations, invoke Cloud Run, run Qwen inference, create generated assets, publish artifacts, create signed URLs, unlock beta, or unlock production.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
