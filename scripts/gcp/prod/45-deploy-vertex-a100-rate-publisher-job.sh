...
# Deploys the private scale-from-zero account-price publisher permanently
...
readonly IMAGE_NAME='weeditpro-vertex-a100-rate-publisher'
readonly REPOSITORY='reeditpro-workers'
readonly CONFIRMATION='deploy-weeditpro-vertex-a100-rate-publisher-v1'
...
[[ "${WEEDITPRO_CONFIRM_VERTEX_A100_RATE_PUBLISHER_JOB_DEPLOY:-}" \
...
build_id="${WEEDITPRO_VERTEX_A100_RATE_PUBLISHER_BUILD_ID:-}"
[[ "${build_id}" =~ ^[a-f0-9-]{36}$ ]] || fail 'exact Cloud Build ID is missing'
image_tag="vertex-a100-rate-publisher-${source_commit:0:16}"
...
for role in roles/storage.objectCreator roles/storage.objectViewer; do
  ...
  jq -e --arg role "${role}" \
    ...
    || fail 'rate publisher bucket IAM changed'
done
...
gcloud run jobs deploy "${JOB}" \
  ...
  --set-env-vars='WEEDITPRO_VERTEX_A100_RATE_OPERATOR_ACTION=disabled' \
  --labels='app=weeditpro,operation=vertex-a100-rate-publisher,scale=zero' \
  --quiet
...
if jq -e 'any(.bindings[]?.members[]?;
  ...
  fail 'Vertex A100 rate publisher has a public IAM principal'
fi
...
printf '{"operation":"weeditpro_vertex_a100_rate_publisher_job_deploy_v1",'
...
printf '"billingAccountCoordinatePersisted":false,'
