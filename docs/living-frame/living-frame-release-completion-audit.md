...
| --- | --- | --- | --- |
| ComfyUI | One shared NVIDIA L4 controlled-image host | Pinned source, locked wheel closure, strict protocol, canonical five-model mount session, digest-bound non-root runtime-confinement requirement, operation-scoped `sam2` import guard, exact 761-package legacy-candidate SPDX inventory, 1,582-file static source-corpus scan, reproducible model-free local confinement/startup, exact private-local five-model atomic read-only mount evidence, exact canonical offline-image build/layout proof, bounded new-image package inventory, checksum-verified partial Trivy metadata vulnerability evidence, the official shared-parent Torch `2.6.0`/TorchVision `0.21.0`/CUDA 12.4 core hardening matrix, and rehashed private-local core wheel bytes | `blocked_hardened_rebuild_and_complete_scan_required`; the exact canonical 35-wheel/three-source closure builds into an 11,392,910,414-byte `linux/amd64` image with default UID/GID 65532, fixed runner, two reviewed custom-node trees, no baked model weights, and passing confinement/layout verification. The five-model mount proof separately rereads all 11,700,367,157 bytes and stops at hard CUDA refusal. Full-image Docker export did not complete in the bounded scanner window. The partial 186-package Trivy SPDX projection covers Python metadata only and found 40 unique findings: one critical, 16 high, 17 medium, and six low. The non-executable hardening candidate binds official Torch `2.6.0+cu124`, TorchVision `0.21.0+cu124`, Triton `3.2.0`, and cuSPARSELt `0.6.2` wheel identities. All four archives and embedded metadata are now rehashed locally, but the host cache remains mutable and is neither canonically ingested nor atomically mounted read-only. The complete offline closure and compatibility proof remain open. The image must still be rebuilt and completely scanned before L4 admission; OS/filesystem coverage, inherited direct-VCS disposition, signature, distributed mounts, complete graph load, and actual L4 generation remain open. |
