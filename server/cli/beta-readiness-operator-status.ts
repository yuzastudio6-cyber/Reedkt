...
function buildNextActions(
  ...
  actions.push('Before staging API deploy, a higher-privilege GCP owner must apply the exact staging IAM/runtime prerequisites directly: Artifact Registry get/upload access for the deployer on us-central1/reeditpro-staging-workers, creation or selection of reeditpro-api-staging@reeditpro.iam.gserviceaccount.com, deployer iam.serviceAccounts.actAs on that runtime account, deployer describe access for fixed staging API Secret Manager entries, and runtime secret accessor on only those fixed entries. Cloud Run deploy permissions are still proven by read-only audit run 28321041675, but the guarded owner-remediation workflow beta-readiness-api-staging-owner-remediation.yml still proves through source-truth run 28321557589 that the deployer cannot self-remediate the remaining owner-side permissions.')
  actions.push('Use npm run beta:readiness:owner-command-handoff plus docs/beta-readiness/api-staging-input-discovery/2026-06-28-api-staging-owner-remediation-command-packet.md as the current higher-privilege owner command packet; it intentionally omits Cloud Run role mutation because run 28321041675 proves Cloud Run deploy permissions, and it keeps deployer secret access metadata-only through owner-supplied fixed secret names.')
