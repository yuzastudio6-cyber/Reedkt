...
export interface ProductionToolCostEstimate {
  ...
  actualUsagePricingSnapshot: ToolCostPricingSnapshot
  serviceFeeIncluded: false
...
export interface BuildToolCostEventIdempotencyKeyInput {
  workspaceId: string
  projectId: string
  toolId: ProductionToolId | string
  jobId?: string | null
...
export function estimateProductionToolCost(
  ...
  return okProductionToolCost({
    ...
    actualUsagePricingSnapshot: calculated.data.pricingSnapshot,
    serviceFeeIncluded: false,
...
export function estimateToolCost(
  ...
  return estimateProductionToolCost(input)
}

export function buildToolCostEventIdempotencyKey(
  input: BuildToolCostEventIdempotencyKeyInput,
): string {
...
export function emitProductionToolCostEvent(
  ...
  const billableToUser = deriveToolCostEventBillableToUser(input.billableToUser, failureCategory)
  ...
  const event = createMockToolCostEvent({
    ...
    nonBillableReason: billableToUser
      ? undefined
      : input.nonBillableReason ?? deriveToolCostEventNonBillableReason(failureCategory, estimate.data.creditPrerequisiteStatus),
...
export function emitToolCostEvent(
  input: EmitProductionToolCostEventInput,
): ProductionToolCostResult<ProductionToolCostEventEmission> {
  return emitProductionToolCostEvent(input)
}
...
function deriveToolCostEventBillableToUser(
  explicitBillableToUser: boolean | undefined,
  failureCategory: ToolCostFailureCategory,
): boolean {
  if (explicitBillableToUser !== undefined) return explicitBillableToUser
  return !isNonBillableFailureCategory(failureCategory)
}

function deriveToolCostEventNonBillableReason(
  failureCategory: ToolCostFailureCategory,
  fallbackStatus: ToolCreditPrerequisiteStatus,
): string {
  if (isNonBillableFailureCategory(failureCategory)) return failureCategory
  return fallbackStatus
}

function isNonBillableFailureCategory(failureCategory: ToolCostFailureCategory): boolean {
  return failureCategory !== 'none' && failureCategory !== 'user_requested_retry'
}
...
function buildEventMetadata(
  ...
  return {
    ...
    costEventKeyShape: 'workspace_project_work_tool_retry',
    costEventWorkKind: workIdentity.kind,
    costEventWorkId: workIdentity.id,
    costEventRetryAttempt: input.retryAttempt ?? 0,
...
function resolveToolCostEventWorkIdentity(input: {
  jobId?: string | null
  generationRequestId?: string | null
  renderJobId?: string | null
  jobBatchId?: string | null
