...
- backend runtime persistence local harness validation retry after credit estimates plan version fix: blocked, retry required
...
| Backend runtime persistence local harness validation retry after credit estimates plan version fix | blocked backend runtime persistence local harness validation retry after credit estimates plan version fix required | The credit estimates plan-version baseline fix is recorded. | Retry local harness after the fix to determine whether the active baseline reaches Qwen draft SQL. |
