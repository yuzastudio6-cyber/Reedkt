...
check(
  data.privateInvokeClient.currentStatus === 'controlled_persisted_worker_dispatch_runtime_approval_plan_required',
  'Private invoke client status must record the controlled persisted worker dispatch runtime approval-plan blocker.',
)
check(
...
check(
  docText.includes('Retry 14 verified that fix') &&
    ...
    docText.includes('QWEN2_5_VL_STACK_TOOL_58BS-CONTROLLED-PERSISTED-WORKER-DISPATCH-RUNTIME-APPROVAL-PLAN'),
  'Doc must record retry-14 evidence, the storage upload pipeline policy-comment fix, retry-15 pass, and controlled persisted worker dispatch runtime approval-plan blocker.',
)
check(
