...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 57)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceStorageObjectsPolicyCommentBaselineFixRequired, true)
