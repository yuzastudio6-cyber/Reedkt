...
Current integration head: `d45219ba83b45d0d3e3d313f915d5aa71947971c`
...
External product beta readiness: `controlled_external_beta_smoke_validated_authenticated_staging_api`
...
1. `RP-EXTERNAL-BETA-CONTROLLED-PRIVATE-INVITE-ACCESS-1`
2. future provider/model runtime confirmation packet only if a real call is explicitly approved later

Controlled external beta is enabled on the staging API only. The correct status is `controlled_external_beta_smoke_validated_authenticated_staging_api` with `external beta enabled in this phase: true`.
