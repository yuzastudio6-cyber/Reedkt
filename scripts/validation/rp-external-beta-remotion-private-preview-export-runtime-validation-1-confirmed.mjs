...
const confirmationVar = 'REEDITPRO_CONFIRM_EXTERNAL_BETA_REMOTION_PRIVATE_PREVIEW_EXPORT_RUNTIME_VALIDATION'
const rootDir = '/tmp/reeditpro-rp-external-beta-remotion-private-preview-export-runtime-validation-1'
...
function failClosed(blocker, message, extra = {}) {
  ...
  const report = {
    ...
    remotionRendererMediaEncoding: false,
...
const outputLocation = join(outputDir, 'reeditpro-external-beta-generated-local-preview.mp4')
...
const GeneratedLocalPreview = () => {
  ...
  const progress = Math.min(1, frame / 29);
  return React.createElement(
    ...
    React.createElement('div', {
      style: {
        ...
        borderRadius: 10,
        boxShadow: '0 8px 22px rgba(23,32,42,0.12)',
        padding: 22,
        textAlign: 'center',
        transform: \`scale(\${0.96 + progress * 0.04})\`,
      },
    },
      React.createElement('div', { style: { fontSize: 26, fontWeight: 700 } }, 'ReEditPro External Beta Gate'),
      React.createElement('div', { style: { fontSize: 15, marginTop: 10 } }, 'Generated local private preview fixture'),
...
const Root = () => React.createElement(Composition, {
  id: 'ReeditProExternalBetaGeneratedLocalPreview',
...
try {
  ...
  const manifest = {
    ...
    nextMilestone: 'RP-EXTERNAL-BETA-PROVIDER-MODEL-CALL-POLICY-CLOSURE-1',
  }

  const qa = {
    packet,
    runId,
    status: 'passed_external_beta_generated_local_private_preview_fixture',
  ...
  const blocker = /browser|chrome|chromium|executable|spawn/i.test(message)
    ? 'blocked_external_beta_remotion_browser_runtime_unavailable'
    : 'blocked_external_beta_remotion_private_preview_export_runtime_validation_failed'
  const blockedReport = {
    packet,
    runId,
    decision: blocker,
