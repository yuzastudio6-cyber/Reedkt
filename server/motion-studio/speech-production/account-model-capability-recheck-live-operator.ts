...
const AUTHORIZATION_ID = 'MS-012C2-ACCOUNT-MODEL-CAPABILITY-EXT-005' as const
const PACKET_RELATIVE_PATH =
  'tasks/motion-studio/MS-012C/c2-account-model-capability-recheck-live-authorization-request-ext-005.json' as const
const AUTHORIZATION_RECORD_RELATIVE_PATH =
  'tasks/motion-studio/MS-012C/c2-account-model-capability-recheck-live-authorization-record-ext-005.json' as const
const PRIVATE_RUN_ROOT_RELATIVE_PATH =
  '.reeditpro-local-storage-ms012c2-account-model-capability-recheck-ext-005' as const
const CONSUMPTION_RECORD_FILENAME = 'authority-consumed.json' as const
const EVIDENCE_FILENAME = 'private-account-model-capability-recheck-evidence.json' as const
...
const packetSchema = z.object({
  schemaVersion: z.literal('motion-studio.speech-account-model-capability-recheck-live-authorization.v1'),
...
const authorizationRecordSchema = z.object({
  schemaVersion: z.literal(
    'motion-studio.speech-account-model-capability-recheck-live-authorization-record.v1',
  ),
...
export interface MotionStudioSpeechAccountModelCapabilityRecheckLiveOperatorResultV1 {
  schemaVersion: 'motion-studio.speech-account-model-capability-recheck-live-operator-result.v1'
...
export interface MotionStudioSpeechAccountModelCapabilityRecheckFixtureInput {
...
export const MOTION_STUDIO_SPEECH_ACCOUNT_MODEL_CAPABILITY_RECHECK_LIVE_OPERATOR_PATHS =
  Object.freeze({
...
export async function executeMotionStudioSpeechAccountModelCapabilityRecheckLiveOperator(input: {
  repositoryRoot: string
  now: string
}): Promise<MotionStudioSpeechAccountModelCapabilityRecheckLiveOperatorResultV1> {
...
export async function executeMotionStudioSpeechAccountModelCapabilityRecheckFixtureOperator(
  input: MotionStudioSpeechAccountModelCapabilityRecheckFixtureInput,
): Promise<MotionStudioSpeechAccountModelCapabilityRecheckLiveOperatorResultV1> {
...
async function executeOperator(input: ({
  ...
  repositoryRoot: string
  now: string
} | ({ mode: 'fixture' } & MotionStudioSpeechAccountModelCapabilityRecheckFixtureInput))): Promise<
  MotionStudioSpeechAccountModelCapabilityRecheckLiveOperatorResultV1
> {
  const now = exactIso(input.now, 'account-model-capability-recheck operator time')
  const repositoryRoot = await realpath(input.repositoryRoot)
  const packetBytes = await readRegularBoundedFile(
    inside(repositoryRoot, PACKET_RELATIVE_PATH),
    MAXIMUM_PACKET_BYTES,
    'account-model-capability-recheck authority packet',
  )
  const packetDigest = sha256Bytes(packetBytes)
  const packet = packetSchema.parse(parseJson(packetBytes, 'account-model-capability-recheck authority packet'))
  ...
  const authorizationBytes = await readRegularBoundedFile(
    ...
    'account-model-capability-recheck authorization record',
  )
  const authorization = authorizationRecordSchema.parse(
    parseJson(authorizationBytes, 'account-model-capability-recheck authorization record'),
  )
  if (authorization.authorityPacketDigest !== packetDigest) {
    blocked('Speech account-model-capability-recheck authorization does not bind the exact packet.')
  }
  ...
  if (nowMs < recordedAt || nowMs >= expiresAt || expiresAt - recordedAt > 86_400_000) {
    blocked('Speech account-model-capability-recheck authority is outside its exact execution window.')
  }
  if (
    authorization.maximumInternalProductionCostMicros !==
      packet.costPolicy.maximumInternalProductionCostMicros
  ) {
    blocked('Speech account-model-capability-recheck cost authority changed after packet review.')
  }
  ...
  if (catalogEvidence.evidenceDigest !== packet.acceptedVoiceDecision.catalogEvidenceDigest) {
    blocked('Speech account-model-capability-recheck lane lost its accepted catalog binding.')
  }
  ...
  await writePrivateJsonCreateOnly(consumptionPath, {
    schemaVersion: 'motion-studio.speech-account-model-capability-recheck-authority-consumption.v1',
    ...
    consumedAt: now,
    immutable: true,
  }, 'Speech account-model-capability-recheck authority is already consumed.')
  ...
  try {
    ...
    const evidenceBase = {
      schemaVersion: 'motion-studio.speech-account-model-capability-recheck-private-evidence.v1' as const,
    ...
    await writePrivateJsonCreateOnly(
      evidencePath,
      evidence,
      'Speech account-model-capability-recheck evidence already exists.',
    )
    ...
    await writePrivateJsonCreateOnly(failurePath, {
      schemaVersion: 'motion-studio.speech-account-model-capability-recheck-terminal-failure.v1',
      ...
      failedAt: now,
      immutable: true,
    }, 'Speech account-model-capability-recheck terminal failure already exists.')
    blocked('Speech account-model-capability-recheck lane stopped terminally without exposing provider material.')
  }
}
...
function validateApiKey(value: string): string {
  ...
  if (normalized.length < 16 || normalized.length > 4_096 || /\s/.test(normalized)) {
    blocked('Speech account-model-capability-recheck credential is malformed.')
  }
  return normalized
}
  ...
  return new Promise((resolveRequest, rejectRequest) => {
    ...
    const finishError = () => {
      ...
      rejectRequest(new ApiError(
        ...
        'Speech account-model-capability-recheck provider request failed.',
        409,
      ))
    }
...
function fixtureResponse(
  ...
  if (
    ...
    blocked('Speech account-model-capability-recheck fixture response violates the exact response boundary.')
  }
  return {
...
function parseExactModel(response: ModelCatalogResponse): {
  ...
  if (response.statusCode !== 200 || response.contentType !== 'application/json') {
    blocked('Speech account-model-capability-recheck response is not an accepted model catalog.')
  }
  const value = parseJson(response.bytes, 'account model catalog')
  if (!Array.isArray(value)) blocked('Speech account-model-capability-recheck catalog is malformed.')
...
function selectOnePublicIpv4(addresses: readonly string[]): string {
  ...
  if (unique.length === 0 || unique.some((address) => !isPublicIpv4(address))) {
    blocked('Speech account-model-capability-recheck DNS results are empty or unsafe.')
  }
  return unique[0]!
}
...
async function verifyAcceptedVoiceDecision(repositoryRoot: string, packet: Packet): Promise<void> {
  ...
  if (sha256Bytes(decisionBytes) !== packet.acceptedVoiceDecision.fileSha256) {
    blocked('Speech account-model-capability-recheck accepted voice decision changed after review.')
  }
  ...
  ) blocked('Speech account-model-capability-recheck accepted voice decision lost its exact binding.')
}
...
async function verifyReviewedFiles(repositoryRoot: string, files: readonly {
  ...
  for (const file of files) {
    if (seen.has(file.relativePath)) blocked('Speech account-model-capability-recheck packet repeats a reviewed file.')
    seen.add(file.relativePath)
    const bytes = await readRegularBoundedFile(
      inside(repositoryRoot, file.relativePath),
      MAXIMUM_REVIEWED_FILE_BYTES,
      'reviewed account-model-capability-recheck file',
    )
    if (bytes.byteLength !== file.byteLength || sha256Bytes(bytes) !== file.sha256) {
      blocked('Speech account-model-capability-recheck reviewed file identity changed.')
    }
  }
}
...
async function assertPrivateDirectory(path: string, repositoryRoot: string): Promise<void> {
  ...
  ) blocked('Speech account-model-capability-recheck private run root is not owner-private.')
}
...
async function writePrivateJsonCreateOnly(path: string, value: unknown, conflict: string): Promise<void> {
  try {
    ...
    blocked('Speech account-model-capability-recheck private evidence could not be persisted.')
  }
  const metadata = await lstat(path)
  if (
    !metadata.isFile() || metadata.isSymbolicLink() || (metadata.mode & 0o777) !== 0o600 ||
    (typeof process.getuid === 'function' && metadata.uid !== process.getuid())
  ) blocked('Speech account-model-capability-recheck evidence is not owner-private.')
}

function inside(repositoryRoot: string, relativePath: string): string {
  if (relativePath.startsWith('/') || relativePath.includes('..') || relativePath.includes('\\')) {
    blocked('Speech account-model-capability-recheck path is unsafe.')
  }
  const target = resolve(repositoryRoot, relativePath)
  if (!target.startsWith(`${repositoryRoot}${sep}`)) blocked('Speech account-model-capability-recheck path escaped its repository.')
  return target
}
