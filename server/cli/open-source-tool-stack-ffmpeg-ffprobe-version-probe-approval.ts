import {
  ...
  writeOpenSourceToolStackFfmpegFfprobeVersionProbeApprovalArtifacts,
...
if (!execute) {
  console.log(summarizeOpenSourceToolStackFfmpegFfprobeVersionProbeApproval())
  process.exit(0)
}

if (!metadataOnly) {
  throw new Error('Use --metadata-only. This approval may not run probes, build Docker, process media, or mutate packages.')
}
...
const reports = writeOpenSourceToolStackFfmpegFfprobeVersionProbeApprovalArtifacts()
console.log(summarizeOpenSourceToolStackFfmpegFfprobeVersionProbeApproval(reports))
