...
- Projects (`/projects`): scan and create project containers across the product without pretending every project is a normal video edit.
- Edit Videos (`/edit-videos`): resume normal named video edits and open the existing exact-edit route in normal Edit Chat.
- Motion Studio (`/motion-studio`): retain the existing specialized Storytelling library/workspace/chat when the Motion source is reconciled; do not redirect it into normal Edit Chat or select it from a content category.
