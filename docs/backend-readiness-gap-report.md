...
| Gap | Status | Notes |
| --- | --- | --- |
| Auth/profile/workspace bootstrap missing | Partially fixed | Google-first Supabase OAuth handoff, exact same-origin/base-path callback construction, protected-route recovery, email/password fallback, and local browser regression evidence now exist. A read-only 2026-07-17 check reached the existing GitHub Pages URL but found the current sign-in surface missing, so the host is stale and not a Gmail test target. The Supabase Google provider, Google callback, Supabase redirect allowlist, reviewed hosted deployment, real Gmail session readback, canonical RLS-backed profile/workspace provisioning, and browser-to-staging API authorization are still unproven. Backend/admin fallback is still needed for RLS-blocked creation. |
