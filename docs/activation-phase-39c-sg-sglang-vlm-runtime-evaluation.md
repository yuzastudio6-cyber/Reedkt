...
## Kernel Compatibility Follow-up

Run `phase39c-sg-kernel-20260602T0132` did not clear the SGLang runtime blocker. Import-only Cloud Run L4 smoke failed for the baseline and issue-compatible kernel profiles before model copy or generated fixture inference. The blocker is still SGLang/sgl-kernel CUDA driver symbol compatibility, specifically missing `cuGreenCtxDestroy`. The new kernel QA prefix also needs scoped private artifact write/readback permission before future reruns can upload private smoke reports.
