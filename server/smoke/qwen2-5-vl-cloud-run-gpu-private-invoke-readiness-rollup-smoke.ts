...
assert.equal(rollup.readinessGates.filter((gate) => gate.status === 'ready').length, 59)
assert.equal(
...
assert.equal(rollup.runtimeFlags.backendRuntimePersistenceStorageUploadPipelinePolicyCommentBaselineFixRequired, true)
