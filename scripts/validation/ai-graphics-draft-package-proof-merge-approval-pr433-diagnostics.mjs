...
const baseRef = 'origin/codex/rp-ai-graphics-draft-package-proof-merge-execution-pr425'
const expectedDecision = 'ai_graphics_draft_package_proof_pr433_merge_approval_passed_with_warnings'
const allowedDecisions = new Set([
  expectedDecision,
  'ai_graphics_draft_package_proof_pr433_merge_approval_passed',
  'blocked_pending_pr425_post_merge_review',
  'blocked_pending_pr433_not_open',
  'blocked_pending_pr433_not_mergeable_clean',
  'blocked_pending_pr433_head_changed_review',
  'blocked_pending_pr433_scope_mismatch',
  'blocked_pending_pr433_package_lock_risk_review',
  'blocked_pending_pr433_merge_approval_scope_review',
])

const requiredDocs = [
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433-source-lockfile.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433-matrix.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr425-post-merge-review-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr433-merge-approval.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-pr441-deferred-after-pr433-merge-approval.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-stack-order-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-package-lock-risk-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-conflict-risk-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-boundary-review-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-blocked-use-register-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-decision-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-next-lane-recommendation-pr433.md',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433-matrix.json',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433-source-lockfile.json',
  'docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-stack-order-pr433.json',
  'docs/prompt-ai-graphics-draft-package-proof-merge-approval-pr433-results.md',
  'docs/implementation-prompts/prompt-ai-graphics-draft-package-proof-merge-approval-pr433.md',
]

const expectedSources = ['PR #573', 'PR #425', 'PR #572', 'PR #569', 'PR #568', 'PR #564', 'PR #561', 'PR #433', 'PR #441', 'PR #543', 'PR #542', 'PR #544']
...
const expectedMergeSha = 'a055ef045db2a6ce127a044bee6219d5933532c3'

const requiredTrueBooleans = [
  'pr433MergeApprovalCompleted',
  'pr425PostMergeStateChecked',
  'pr425MergeShaRecorded',
  'pr433LiveStateChecked',
  'pr433OpenNonDraftClean',
  'pr433MergeApprovedForFutureExecution',
  'pr441Deferred',
  'stackOrderAccepted',
]
...
const forbiddenPatterns = [
  ['pr433_or_pr441_merged_now', /\b(pr433MergedNow|pr441MergedNow|prMergedNow|sourcePrsMergedNow)\b\s*[:=]\s*`?true`?/i],
...
for (const required of ['TRACK_B_MEDIA_OSS_STEWARD', 'Track A', 'PR #425 -> PR #433 -> PR #441', 'AI_GRAPHICS_DRAFT_PACKAGE_PROOF_MERGE_EXECUTION_PR433', expectedMergeSha]) {
...
let stackOrder = {}
try {
  sourceLockfile = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433-source-lockfile.json')
  matrix = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-pr433-matrix.json')
  stackOrder = readJson('docs/open-source-tool-stack/ownership/ai-graphics-draft-package-proof-merge-approval-stack-order-pr433.json')
} catch (error) {
...
if (stackOrder.firstMergeExecutionTarget !== 433) failures.push(`first_merge_execution_target_unexpected:${stackOrder.firstMergeExecutionTarget}`)
if (stackOrder.nextLane !== 'AI_GRAPHICS_DRAFT_PACKAGE_PROOF_MERGE_EXECUTION_PR433') failures.push('next_lane_unexpected')
...
if (rows[425]?.state !== 'merged' || rows[425]?.merged !== true || rows[425]?.mergeSha !== expectedMergeSha) failures.push('pr425_post_merge_state_unexpected')
if (rows[433]?.state !== 'open' || rows[433]?.draft !== false || rows[433]?.mergeable !== 'MERGEABLE' || rows[433]?.merged !== false) failures.push('pr433_state_unexpected')
...
if (rows[433]?.mergeApprovedForFutureExecution !== true) failures.push('pr433_future_merge_approval_not_true')
if (rows[441]?.mergeApprovedForFutureExecution !== false) failures.push('pr441_should_be_deferred')
if (rows[433]?.evidenceLevel !== 'install_import_synthetic_static_proof') failures.push('pr433_evidence_level_unexpected')

for (const source of ['pr573', 'pr572', 'pr569', 'pr568', 'pr564', 'pr561', 'pr543']) {
...
if (sourceLockfile.sources?.pr425?.mergeSha !== expectedMergeSha) failures.push('source_lockfile_pr425_merge_sha_unexpected')
for (const source of ['pr433', 'pr441']) {
  const data = sourceLockfile.sources?.[source]
  if (!data || data.state !== 'open' || data.draft !== false || data.mergeable !== 'MERGEABLE' || data.merged !== false) failures.push(`source_lockfile_source_pr_unexpected:${source}`)
}
...
try {
  ...
  const expectedScript = 'node scripts/validation/ai-graphics-draft-package-proof-merge-approval-pr433-diagnostics.mjs'
  if (packageAfter.scripts?.['ai-graphics:draft-package-proof-merge-approval-pr433:diagnostics'] !== expectedScript) failures.push('missing_package_script')
} catch (error) {
...
if (failures.length) {
  console.error('AI graphics draft package proof PR433 merge approval diagnostics failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('AI graphics draft package proof PR433 merge approval diagnostics passed.')
