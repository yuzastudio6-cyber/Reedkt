...
import {
  ...
  AI_VIDEO_BROLL_GEN_9X_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9w-no-idle-l4-iap-wheelhouse-transfer-proof-us-east4-a-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9w-no-idle-l4-iap-wheelhouse-transfer-proof-us-east4-a-result.md'
const PROMPT_PATH = 'docs/implementation-prompts/prompt-ai-video-broll-gen-9x-iap-wheelhouse-transfer-stockout-fix.md'
const SPEC_PATH =
...
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9w-no-idle-l4-iap-wheelhouse-transfer-proof-us-east4-a-result'
const DECISION =
  'ai_video_broll_gen_9w_us_east4_a_iap_wheelhouse_transfer_blocked_resource_pool_exhausted_cleanup_verified'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll9wResult'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9w-no-idle-l4-iap-wheelhouse-transfer-proof-us-east4-a-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `9W result doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9X stockout fix prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9W_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST4_A_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9w_no_idle_l4_iap_wheelhouse_transfer_proof_us_east4_a_result')
...
assert.equal(result.targetZone, 'us-east4-a')
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9X_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_PROMPT)
