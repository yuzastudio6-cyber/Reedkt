...
The focused smoke constructs a valid in-memory contract-shape candidate only to
exercise the parser and projection builder. It does not persist that candidate,
does not consume a provider/GPU/model result, and does not change the current
preflight. The smoke output names this distinction explicitly.
...
inputs, and inconsistent preflights.
...
Safe work completed: The terminal admission and per-job/output projection seam
is fully implemented and adversarially tested.
