...
const canonicalSteps: VerificationStep[] = [
  ...
  step(
    ...
    'A bounded two-chunk canonical graph reopens both independently QA-passed private 4K object chunks, executes strict adjacent-boundary color policy, continuous lossless program audio, private review finalization, and objective review-master QA through all 11 jobs. A separate immutable nine-job customer-delivery package completes its root, both 4K H.264 High encodes, both independent chunk probes, one H.264-stream-copy plus one-time FLAC-to-AAC private MP4 mux, and separately leased full decoded-video plus decoded-audio/sync QA through 8/9 jobs with attempt-level internal cost and restart-safe exact replay. Both synthetic-fixture objective outcomes honestly require user review, so private-download reconciliation remains queued and unattempted. This retained 129-second fixture does not mark the six-hour 255-job graph, cloud, product, or production executed.',
  ),
  step(
