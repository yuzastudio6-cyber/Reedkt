...
import {
  AI_VIDEO_BROLL_GEN_9P_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_9Q_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_A_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9p-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9p-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-9q-no-idle-l4-iap-wheelhouse-transfer-proof-us-west1-a.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-9p-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-9p-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9p-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
  'ai_video_broll_gen_9p_iap_wheelhouse_transfer_stockout_fix_ready_for_us_west1_a_transfer_proof_no_vm_no_inference'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll9pStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9p-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  'Select `us-west1-a` as the next bounded no-idle IAP wheelhouse transfer proof target.',
  ...
  assert.equal(doc.includes(required), true, `9P stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9Q prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9P_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9p_iap_wheelhouse_transfer_stockout_fix_result')
assert.deepEqual(result.priorTransferProofZones, ['us-central1-c', 'us-central1-c'])
assert.equal(result.selectedRetryZone, 'us-west1-a')
assert.equal(result.selectedRetryRegion, 'us-west1')
assert.equal(result.sameZoneRetryRejected, true)
...
assert.equal(
  result.selectionRationale.includes('us_central1_c_transfer_proof_stocked_out_twice_before_vm_creation'),
  true,
)
assert.equal(
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9Q_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_WEST1_A_PROMPT)
  ...
  JSON.stringify(
    {
      ...
      sameZoneRetryRejected: result.sameZoneRetryRejected,
