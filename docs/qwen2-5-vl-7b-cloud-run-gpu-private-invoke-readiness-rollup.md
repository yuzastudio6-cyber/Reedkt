...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_persistence_local_harness_result_review_accepted_active_migration_plan_required`.
...
- backend runtime persistence active migration plan: blocked, validated draft still needs active migration planning
...
| Backend runtime persistence active migration plan | blocked backend runtime persistence active migration plan required | The validated Qwen persistence SQL is still draft-only. | Plan how to promote the validated draft to an active migration without deploying it; keep private invoke, real worker dispatch, assets, signed URLs, beta, and production blocked. |
