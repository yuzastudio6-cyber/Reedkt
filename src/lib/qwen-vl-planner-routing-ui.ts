...
export type QwenVlPlannerRoutingUiPrivateInvokeClient = {
  ...
  routeRuntime: 'mock'
  currentStatus: 'backend_runtime_persistence_active_migration_deploy_execution_required'
...
export type QwenVlPlannerRoutingUiData = {
  ...
  nextPrompt: 'QWEN2_5_VL_STACK_TOOL_58BE-BACKEND-RUNTIME-PERSISTENCE-ACTIVE-MIGRATION-DEPLOY-EXECUTE: deploy validated Qwen persistence migration to approved Supabase target, no assets/no beta'
}
...
export function getQwenVlPlannerRoutingUiData(): QwenVlPlannerRoutingUiData {
  return {
    ...
    privateInvokeClient: {
      ...
      currentStatus: 'backend_runtime_persistence_active_migration_deploy_execution_required',
      currentBlocker:
        'Controlled private invoke auth works, the dedicated Direct VPC private route subnet is configured, the no-model CPU-only caller source is defined, the controlled caller job is deployed, one caller contract smoke observed the expected fail-closed response with inference disabled, runtime readiness review is recorded, structured-output retry/result review passed, private runtime and approved worker integration reviews are accepted, fail-closed backend dispatch and controlled dispatch dry-run are recorded, and backend runtime persistence has advanced through local harness validation, active migration creation, active migration validation, deploy planning, and target-class deploy approval. User-facing readiness remains blocked until the future deploy-execution prompt applies or confirms the migration on the approved Supabase target and later runtime approval is recorded.',
      boundaryNotes: [
        ...
        'The next runtime gate is deployment execution for the locally validated Qwen persistence migration; it must not invoke Cloud Run, run Qwen inference, dispatch workers, create generated assets, publish artifacts, create signed URLs, unlock beta, or unlock production.',
      ],
      runtimeFlags: {
        usesCentralApiClient: true,
        serviceUrlResolvedNow: false,
