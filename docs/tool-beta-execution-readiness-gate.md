...
- `npm run beta:tools:libass-synthetic-burnin-qa-preflight` is the local synthetic-only QA lane after filter proof. It creates temp synthetic video and ASS captions, runs one bounded libass burn-in/probe, records checksums/sizes/duration and safe ASS style status, deletes temp artifacts, and emits a `libass` product-ready local OSS evidence preview only when explicit production and product-ready confirmations are supplied. It records open PR #73 as historical activation/private-media context, not as a current-source duplicate to merge.
...
- `npm run smoke:beta-readiness-operator-status` verifies the combined CLI operator status report for complete and missing input cases, secret-safe output, current blocker counts, evidence-driven policy, and next-command sequencing without touching a deployed backend. `npm run smoke:beta-readiness-operator-status-api` verifies the deployed readback CLI uses GET, keeps bearer tokens out of summaries, supports stored-workspace status, and fails closed when required readiness is absent. `npm run smoke:beta-readiness-api` verifies the deployed backend operator-status route stays read-only and reflects stored workspace evidence.
