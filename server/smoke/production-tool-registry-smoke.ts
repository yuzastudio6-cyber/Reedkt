...
const gstreamer = requireProfile('gstreamer')
check(gstreamer.category === 'qa', 'GStreamer must be registered as a backend validation/QA capability.')
check(gstreamer.workerType === 'tool_readiness_worker', 'GStreamer validation must route through the tool readiness worker.')
check(gstreamer.executionMode === 'readiness_check', 'GStreamer validation must remain a readiness check, not render execution.')
assertToolAllowedForWorker('gstreamer', 'tool_readiness_worker')

const mkvtoolnix = requireProfile('mkvtoolnix')
check(mkvtoolnix.category === 'qa', 'MKVToolNix must be registered as a backend validation/QA capability.')
check(mkvtoolnix.workerType === 'tool_readiness_worker', 'MKVToolNix validation must route through the tool readiness worker.')
check(mkvtoolnix.qaResponsibilities.includes('export_codec_format'), 'MKVToolNix validation must carry container/export codec QA.')
assertToolAllowedForWorker('mkvtoolnix', 'tool_readiness_worker')

const gpacMp4Box = requireProfile('gpac_mp4box')
check(gpacMp4Box.category === 'qa', 'GPAC/MP4Box must be registered as a backend validation/QA capability.')
check(gpacMp4Box.workerType === 'tool_readiness_worker', 'GPAC/MP4Box validation must route through the tool readiness worker.')
check(gpacMp4Box.notBestFor.includes('Bento4 fallback selection'), 'GPAC/MP4Box must keep Bento4 separate and not selected.')
assertToolAllowedForWorker('gpac_mp4box', 'tool_readiness_worker')
