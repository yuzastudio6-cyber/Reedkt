...
```json
{
  ...
  "plannedRouteHarnessType": "bounded_local_staging_product_route_harness",
  "routeIds": [
  ...
  "allowedFutureChanges": [
    "local/staging-only route harness entrypoints",
    "dry-run validator that returns sanitized receipts",
    "fail-closed queue guard that never dispatches real workers",
    "status readback for sanitized local receipts only"
  ],
  "forbiddenFutureChanges": [
    "real tool execution",
    "Docker or installs",
    "media processing",
    "Supabase/GCS writes",
  ...
  "noRuntimeApiEnabledThisPhase": true
}
```
