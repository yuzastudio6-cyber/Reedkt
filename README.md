...
RP-CREDITPURCHASE-01 adds mock purchased credit packs and local purchased grants. Mock top-up adds available credits to the in-memory wallet only, keeps purchased credits separate from weekly bonus credits, and requires callers to retry reservation, revised-credit approval, or export gate after top-up. See `docs/credit-top-up-purchased-grants.md` and `smoke:credit-purchase`; boundaries remain no live billing, no Stripe/payment, no real checkout, no Supabase write, no provider call, no production wallet or ledger mutation, no render/export execution, and no export unlock.

RP-STRIPE-FOUNDATION-01 adds the mock/config-only Stripe billing foundation for future paid top-ups. It defines disabled/test/live Stripe config, Secret Manager reference validation, customer/payment method link contracts, SetupIntent and Checkout stubs, webhook raw-body/idempotency boundaries, and live-mode readiness checks. See `docs/stripe-billing-foundation.md` and `smoke:stripe-foundation`; boundaries remain no live Stripe calls, no charges, no real Checkout/SetupIntent/PaymentIntent, no webhook credit grants, no Supabase write, no production wallet or ledger mutation, no provider/render/export execution, and no package-lock change.

RP-STRIPE-TESTMODE-01 adds the first real Stripe test-mode path for customer links, saved payment SetupIntents, credit-pack Checkout Sessions, and verified raw-body webhook processing. Test checkout grants local mock purchased credits only after a verified test webhook; live Stripe, production wallet/ledger persistence, Supabase writes, provider/render/export work, export unlock, and package-lock changes remain out of scope. See `docs/stripe-testmode-billing-path.md` and `smoke:stripe-testmode`.

RP-STRIPE-LIVE-READINESS-01 adds a no-charge live readiness gate. A complete live config can report `ready_no_charge`, but live Checkout Sessions, live SetupIntents, live PaymentIntents, live webhooks, live credit grants, production wallet/ledger mutation, Supabase writes, provider/render/export work, export unlock, and package-lock changes remain blocked. See `docs/stripe-live-readiness.md` and `smoke:stripe-live-readiness`.

RP-CREDITUI-01 adds fixture-driven credit lifecycle cards on the wallet page. Users can inspect wallet balance, estimate range, required hold, reservation state, runtime pause, revised-credit choices, settlement receipt, export lock, mock top-up packs, and Stripe readiness without live billing, server mutations, provider calls, render/export execution, export unlock, or raw secret exposure. See `docs/credit-ui-lifecycle-cards.md` and `smoke:credit-ui`.

RP-CREDITAUDIT-01 adds read-only support/admin credit audit evidence. Operators can trace estimates, reservations, runtime pauses, tool costs, settlements, export locks, mock top-ups, grants, and safe Stripe test/mock events without live billing, server mutation, provider calls, render/export execution, export unlock, raw secret exposure, or card data. See `docs/credit-audit-support-timeline.md` and `smoke:credit-audit`.

RP-EXTERNALBETA-01 adds the mock/test-safe external beta credit launch gate. It reports the end-to-end scenario matrix, mock/test readiness, no-charge live-readiness blocker, and support/audit evidence without live billing, checkout/top-up execution, settlement execution, provider calls, render/export execution, export unlock, production persistence, raw secret exposure, or package-lock changes. See `docs/external-beta-credit-launch-gate.md` and `smoke:external-beta-credit`.

RP-PERSISTENCE-PLAN-01 adds the production persistence architecture and Supabase schema gap plan for the external-beta credit lifecycle. It maps mock/test wallets, grants, reservations, settlements, revised-credit actions, export locks, purchased grants, Stripe links, audit evidence, idempotency, and tool-cost records onto existing/proposed tables without creating migrations, running Supabase CLI, applying SQL, connecting to Supabase, mutating production wallets/ledgers, processing Stripe webhooks, or changing package-lock. See `docs/credit-production-persistence-plan.md` and `smoke:credit-production-persistence-plan`.

RP-SUPABASE-MIGRATION-01 adds review-only Supabase credit persistence draft SQL under `supabase/review/`, plus RLS/RPC draft notes and a smoke check. The drafts stay outside `supabase/migrations/` and do not run Supabase CLI, apply SQL, connect to a database, generate DB types, wire persistence repositories, mutate wallets/ledgers, process Stripe webhooks, or change package-lock. See `docs/credit-supabase-migration-draft.md` and `smoke:credit-supabase-migration-draft`.
