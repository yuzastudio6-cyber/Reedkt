...
test.describe('Edit Preferences route entrypoint', () => {
  test('opens the clean Preferences page safely', async ({ page }) => {
    ...
    await expect(page.getByTestId('edit-references-panel')).toContainText('Teach ReEditPro your editing judgment')
    await page.getByRole('tab', { name: 'Workspace Defaults' }).click()
  ...
  test('keeps legacy edit preferences route as a redirect', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/preferences$/)
    await expect(page.getByTestId('edit-preferences-page')).toBeVisible()
    await expect(page.getByRole('tab', { name: 'Edit References' })).toHaveAttribute('aria-selected', 'true')
    await expectNoHorizontalOverflow(page)
  })
})
