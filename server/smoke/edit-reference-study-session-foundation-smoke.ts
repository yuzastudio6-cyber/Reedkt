...
try {
  ...
  try {
    ...
    assert.equal(message.body.data.detail.nextAction, 'add_reference_evidence')
    assert.match(message.body.data.detail.messages.at(-1)?.content ?? '', /DNA and quality review remain unavailable/i)
  ...
  try {
    ...
    assert.equal(blocked.code, 'EDIT_REFERENCE_PERSISTENCE_BLOCKED')
  } finally {
