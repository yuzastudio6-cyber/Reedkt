...
const DECISION =
  'qwen2_5_vl_controlled_persisted_worker_dispatch_runtime_execution_attempt_result_review_accepted_real_dispatch_approval_plan_required'
const NEXT_PROMPT =
...
function assertFalseRuntimeFlags(flags: JsonRecord) {
  for (const key of [
    'controlledPersistedWorkerDispatchRuntimeExecutionAttemptResultReviewRequired',
...
const doc = read('docs/qwen2-5-vl-7b-controlled-persisted-worker-dispatch-runtime-execution-attempt-result-review.md')
for (const phrase of [
  DECISION,
...
const review = QWEN2_5_VL_CONTROLLED_PERSISTED_WORKER_DISPATCH_RUNTIME_EXECUTION_ATTEMPT_RESULT_REVIEW
assert.equal(review.decision, DECISION)
assert.equal(review.nextPrompt, NEXT_PROMPT)
assert.equal(
  review.upstreamControlledPersistedWorkerDispatchRuntimeExecutionAttemptResultDecision,
...
assert.equal(review.acceptedEvidence.realDispatchNonExecutionAccepted, true)
...
assert.equal(
  review.remainingBlockers.controlledPersistedWorkerDispatchRuntimeRealDispatchApprovalPlanRequired,
  true,
)
...
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeExecutionAttemptResultReviewRecorded, true)
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeExecutionAttemptResultReviewAccepted, true)
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchApprovalPlanRequired, true)
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeExecutionAttemptRecorded, true)
assert.equal(review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeExecutionAttemptPassedFailClosed, true)
...
assert.deepEqual(
  ...
  [],
  `Forbidden values in controlled persisted runtime execution attempt result review data: ${forbiddenDataFindings.join('; ')}`,
)
...
console.log(JSON.stringify({
  ...
  realDispatchApprovalPlanRequired:
    review.runtimeFlags.controlledPersistedWorkerDispatchRuntimeRealDispatchApprovalPlanRequired,
