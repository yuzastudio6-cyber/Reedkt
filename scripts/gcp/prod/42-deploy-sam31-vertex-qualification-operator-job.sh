...
# Deploys the private scale-from-zero Vertex operator permanently unarmed.
# Exact start/reconcile data is supplied only as one-execution overrides.
...
readonly WORKER_SERVICE_ACCOUNT='weeditpro-sam31-qual-sa@reeditpro.iam.gserviceaccount.com'
readonly IMAGE_NAME='weeditpro-sam31-vertex-qualification-operator'
readonly REPOSITORY='reeditpro-workers'
readonly CONFIRMATION='deploy-weeditpro-sam31-vertex-qualification-operator-v1'
...
[[ "${WEEDITPRO_CONFIRM_SAM31_VERTEX_QUALIFICATION_OPERATOR_JOB_DEPLOY:-}" \
...
build_id="${WEEDITPRO_SAM31_VERTEX_QUALIFICATION_OPERATOR_BUILD_ID:-}"
[[ "${build_id}" =~ ^[a-f0-9-]{36}$ ]] || fail 'exact Cloud Build ID is missing'
image_tag="sam31-vertex-operator-${source_commit:0:16}"
...
' <<<"${project_policy}" >/dev/null || fail 'operator project IAM changed'
worker_policy="$(gcloud iam service-accounts get-iam-policy \
  "${WORKER_SERVICE_ACCOUNT}" --project="${PROJECT_ID}" --format=json)"
jq -e --arg member "serviceAccount:${SERVICE_ACCOUNT}" '
  any(.bindings[]?;
    .role == "roles/iam.serviceAccountUser"
    and (((.members // []) | index($member)) != null))
' <<<"${worker_policy}" >/dev/null || fail 'worker act-as IAM changed'
for binding in \
  'reeditpro-production-reeditpro-control-plane-state:roles/storage.objectCreator' \
  'reeditpro-production-reeditpro-control-plane-state:roles/storage.objectViewer' \
  'reeditpro-production-sam31-qualification-private:roles/storage.objectCreator' \
  'reeditpro-production-sam31-qualification-private:roles/storage.objectViewer' \
  'reeditpro-production-reeditpro-model-artifacts:roles/storage.objectViewer'; do
  bucket="${binding%%:*}"
  role="${binding#*:}"
  ...
  jq -e --arg role "${role}" \
    ...
    || fail 'operator bucket IAM changed'
done
...
gcloud run jobs deploy "${JOB}" \
  ...
  --max-retries=0 --task-timeout=30m --execution-environment=gen2 \
  --set-env-vars='WEEDITPRO_SAM31_VERTEX_OPERATOR_ACTION=disabled' \
  --labels='app=weeditpro,operation=sam31-vertex-operator,scale=zero' \
  --quiet
...
if jq -e 'any(.bindings[]?.members[]?;
  ...
  fail 'Vertex qualification operator has a public IAM principal'
fi
...
' <<<"${job}" >/dev/null || fail 'deployed operator is not exactly unarmed'

printf '{"operation":"weeditpro_sam31_vertex_qualification_operator_job_deploy_v1",'
...
printf '"vertexJobStarted":false,"gpuOrModelRuntimeStarted":false,'
