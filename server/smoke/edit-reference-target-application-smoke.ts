...
import { editReferenceScopeHash, PrivateEditReferenceRepository } from '../edit-references/private-edit-reference-repository'
import type {
...
try {
  ...
  try {
    ...
    const tutorialUnderstanding = await persistReadyTargetVideoUnderstandingFixture({
      localStorageRoot: root,
      ownerUserId,
      workspaceId,
      editReferenceId: referenceId,
      studySessionId: studyId,
      targetContext: tutorialTarget,
      fixtureKey: 'gate5-voice-tutorial',
    })
    ...
    const missingTargetUnderstanding = await requestError(runtime.baseUrl, `/v1/edit-reference-studies/${studyId}/preference-dna/${dnaVersion.id}/applications`, {
      method: 'POST', key: 'gate5-missing-target-understanding', body: {
        workspaceId,
        expectedReferenceRevision: approved.body.data.detail.reference.revision,
        expectedDNAContentDigest: dnaVersion.contentDigest,
        acknowledgeAdaptNotCopy: true,
        targetContext: tutorialTarget,
      },
    })
    assert.equal(missingTargetUnderstanding.status, 400)
    assert.equal(missingTargetUnderstanding.code, 'VALIDATION_FAILED')

    const staleTargetUnderstanding = await requestError(runtime.baseUrl, `/v1/edit-reference-studies/${studyId}/preference-dna/${dnaVersion.id}/applications`, {
      method: 'POST', key: 'gate5-stale-target-understanding', body: {
        ...applicationRequest(approved.body.data.detail.reference.revision, dnaVersion.contentDigest, tutorialTarget, tutorialUnderstanding),
        targetUnderstandingPackageDigestSha256: 'f'.repeat(64),
      },
    })
    assert.equal(staleTargetUnderstanding.status, 409)
    assert.equal(staleTargetUnderstanding.code, 'VERSION_CONFLICT')

    const callerSummaryMismatch = await requestError(runtime.baseUrl, `/v1/edit-reference-studies/${studyId}/preference-dna/${dnaVersion.id}/applications`, {
      method: 'POST', key: 'gate5-caller-summary-mismatch', body: {
        ...applicationRequest(approved.body.data.detail.reference.revision, dnaVersion.contentDigest, tutorialTarget, tutorialUnderstanding),
        targetContext: { ...tutorialTarget, sourceSummary: 'Caller-authored summary must not replace studied target evidence.' },
      },
    })
    assert.equal(callerSummaryMismatch.status, 409)
    assert.equal(callerSummaryMismatch.code, 'VERSION_CONFLICT')
    ...
    const riskyTarget = { ...tutorialTarget, currentUserInstruction: 'Copy the exact shot order and use the same timing as the reference.' }
    const riskyUnderstanding = await persistReadyTargetVideoUnderstandingFixture({
      localStorageRoot: root,
      ownerUserId,
      workspaceId,
      editReferenceId: referenceId,
      studySessionId: studyId,
      targetContext: riskyTarget,
      fixtureKey: 'gate5-copy-risk-target',
    })
    ...
    assert.equal(tutorialApplication.applicationVersion, 'edit-reference-target-application-v2')
    assert.equal(tutorialApplication.runtimeSource, 'verified_live')
    assert.equal(tutorialApplication.targetIdentityStatus, 'verified_target_video_understanding')
    assert.equal(tutorialApplication.targetUnderstanding?.packageId, tutorialUnderstanding.packageId)
    assert(tutorialApplication.decisions.every((decision) => (
      decision.targetUnderstandingPackageId === tutorialUnderstanding.packageId
      && (decision.targetEvidenceIds?.length ?? 0) > 0
    )))
    ...
    assert(tutorialApplication.decisions.some((decision) => decision.layerId === 'sfx_sound_design' && decision.decision === 'ignored' && decision.precedence === 'current_user_instruction'))
    ...
    assert(tutorialApplication.decisions.filter((decision) => decision.layerId === 'do_not_copy_rules').every((decision) => decision.decision === 'blocked'))
    ...
    const montageUnderstanding = await persistReadyTargetVideoUnderstandingFixture({
      localStorageRoot: root,
      ownerUserId,
      workspaceId,
      editReferenceId: referenceId,
      studySessionId: studyId,
      targetContext: montageTarget,
      fixtureKey: 'gate5-silent-lifestyle',
    })
    ...
    assert(montageApplication.decisions.some((decision) => decision.layerId === 'speech_caption_behavior' && decision.decision === 'ignored' && decision.precedence === 'current_user_instruction'))
    ...
    const auditEvents = await new PrivateEditReferenceRepository().readAuditEvents({ localStorageRoot: root, ownerUserId, workspaceId })
    assert(auditEvents.some((event) => event.eventType === 'preference_application_prepared'))
