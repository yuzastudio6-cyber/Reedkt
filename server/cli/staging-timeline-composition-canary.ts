...
import { clean } from '../cloud-run/render-canary-config'
...
const previousSmokeRunIds = readTimelinePreviousSmokeRunIds(process.env)
...
const context: ServiceContext = {
  ...
  requestId: 'cli-staging-timeline-composition-canary',
  auth: {
...
const result = await runStagingTimelineCompositionCanary(context)
console.log(JSON.stringify(result, null, 2))
if (!result.ok || result.status !== 'passed' || !result.strictValidation.ok) process.exitCode = 1

function readTimelinePreviousSmokeRunIds(sourceEnv: Record<string, string | undefined>): Array<{
  label: string
  smokeRunId?: string
}> {
  return [
    ...
    {
      label: 'previous_media_analysis_smoke',
      smokeRunId: clean(sourceEnv.SUPABASE_E2E_PREVIOUS_MEDIA_ANALYSIS_SMOKE_RUN_ID),
    },
  ]
}
