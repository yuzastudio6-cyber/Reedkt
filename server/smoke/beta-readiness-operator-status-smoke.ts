...
assert.equal(readyReport.sourceTruth.localAcceptedEvidence.productReadyLocalOssCount, 0, 'deployed evidence has not yet recorded product-ready local OSS')
assert.equal(readyReport.sourceTruth.localAcceptedEvidence.productReadyLocalOssRequiredForCurrentTrackB, 16, 'current Track B lane must require product-ready readback count 16')
...
assert.equal(readyReport.toolEvidence.localDefaultsPreviewCommand, 'npm run beta:tools:core-real-check-preview -- --local-defaults', 'operator status should name the local-defaults preview command')
assert.equal(readyReport.toolEvidence.hydratedLocalDefaultsPreviewCommand, 'npm run beta:tools:core-real-check-preview:hydrated -- --local-defaults', 'operator status should name the hydrated local-defaults preview command')
...
assert.ok(readyReport.nextActions.some((action) => action.includes('current 16-tool product-ready bundle')), 'ready report should preserve the current bundle context')
assert.ok(readyReport.nextActions.some((action) => action.includes('beta:tools:trackb-product-ready-deployed-evidence-collector')), 'ready report should name the Track B product-ready deployed evidence collector command')
...
assert.ok(readyReport.nextActions.some((action) => action.includes('beta:readiness:external-beta-sequence-preflight')), 'ready report should name the all-up external beta sequence preflight before the collector')
...
assert.ok(emptyReport.nextActions.some((action) => action.includes('trackb-product-ready-deployed-evidence-collector')), 'empty report should direct operator to the Track B product-ready deployed evidence collector')
assert.ok(emptyReport.nextActions.some((action) => action.includes('REEDITPRO_BETA_TOOLS_LOCAL_BUNDLE_REQUIRED_PRODUCT_READY_LOCAL_OSS_COUNT=16')), 'empty report should require product-ready deployed evidence readback count 16')
assert.ok(emptyReport.nextActions.some((action) => action.includes('core-real-check-preview -- --local-defaults')), 'empty report should name the no-secret local defaults preview when source evidence is refreshed')
...
assert.ok(emptyReport.nextActions.some((action) => action.includes('external-beta-sequence-preflight')), 'empty report should direct operator to the external beta sequence preflight before the collector')
