...
const canonicalSteps: VerificationStep[] = [
  ...
  step(
    ...
    'A fixed server-owned FFmpeg recipe reopens two checksum-bound synthetic 4K MP4 sources, executes exact hard-cut H.264 MP4-slice concat by stream copy into one server-committed Matroska object-mezzanine chunk, proves same-input byte-deterministic reexecution, and invokes FFprobe separately to verify frame count, duration, timestamps, dimensions, codec, and BT.709 color. This isolated runner proof grants no canonical job, artifact-persistence, cloud, object-store, product, or production authority.',
  ),
  step(
