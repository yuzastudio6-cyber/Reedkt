...
RP-RUNTIME-GUARD-01 uses rate-card high estimates at paid worker/provider/render start to decide whether the reserved max still covers the next tool. Current billable mock tool-cost events count toward the projection, non-billable events are excluded, and `serviceFeeIncluded = false` remains mandatory for every tool-cost event. Projected overage pauses before provider or render/export work and creates only a mock revised-credit action.
