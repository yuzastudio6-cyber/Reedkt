...
for (const required of [
  ...
  '`broll_no_idle_l4_iap_wheelhouse_transfer_us_west1_c_prompt_required_before_dependency_or_inference`',
...
assert.equal(rollup.recommendedNextPrompt, BROLL_9U_US_WEST1_C_TRANSFER_PROMPT)
...
assert.equal(broll?.nextAction, BROLL_9U_US_WEST1_C_TRANSFER_PROMPT)
...
assert.equal(broll?.noIdleLifecycleGate?.targetZone, 'us-west1-c')
