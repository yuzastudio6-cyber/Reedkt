...
import {
  loadCanonicalLivingFrameWorkGraphProjection,
  ...
  return {
    async publishCanonicalPlan(input: PublishCanonicalEditPlanInput) {
      ...
      try {
        ...
        throw new ApiError(
          'VALIDATION_FAILED',
          'Canonical Caption rendered-media work could not be bound to its exact planning and final-canvas authority.',
          400,
          {
            reason: error instanceof Error
              ? error.message : 'invalid_caption_rendered_media_work',
          },
        )
      }
        ...
        warnings: [
          ...
          ...(captionPlanning.projection
            ? [captionPlanning.projection.disposition ===
                ...
                : `Canonical plan authority adds ${captionPlanning.workItems.length} server-owned Caption planning work item(s); ${captionRenderedMediaWorkBinding ? 'exact libass and Remotion rendered-media work is bound' : 'rendered-media work remains unbound'}, while deterministic rendered QA, qualified complete-time visual review, and independent private review remain separate closed gates.`]
            : []),
      ...
      const response = await mutatePrivateEditAuthorityAggregate({
        ...
        mutation: async (aggregate) => {
          ...
          const lockedPlanWorkItems =
            plan.workItemIds.map((workItemId) =>
              requirePlanWorkItem(
                aggregate,
                workItemId,
                plan.id,
          ...
          if (
            ...
            throw new ApiError(
              'IDEMPOTENCY_CONFLICT',
              'Canonical Living Frame estimate/work/asset projection changed before approval.',
              409,
            )
          }
          if (
          ...
          if (
            ...
            throw new ApiError(
              'IDEMPOTENCY_CONFLICT',
              'Canonical Living Frame work-graph projection changed before approval.',
              409,
            )
          }
          assertLivingFrameExecutionAuthorityReady(
    ...
    async loadApprovedExecutionAuthority(
      ...
      const livingFramePlanWorkItems =
        lineage.plan.workItemIds.map((workItemId) =>
          requirePlanWorkItem(
            aggregate,
            workItemId,
            lineage.plan.id,
...
async function assertLivingFrameWorkGraphProjectionMatchesPlan(
  ...
  if (!projection) {
    if (pendingPlanItems.length > 0) {
      throw new ApiError(
        'IDEMPOTENCY_CONFLICT',
        'Canonical plan contains Living Frame pending work without its server-derived projection.',
        409,
      )
    }
    return
  ...
  if (
    ...
    throw new ApiError(
      'IDEMPOTENCY_CONFLICT',
      'Canonical Living Frame work-item count no longer matches its immutable projection.',
      409,
    )
  }
  const byKey = new Map(
  ...
  for (const expected of projection.workItems) {
    const actual = byKey.get(expected.workItemKey)
    if (
      !actual
      || actual.workItemType !== expected.workItemType
      || actual.workerClass !== expected.workerClass
      || actual.executionInputHash !==
        sha256AuthorityValue(expected.executionInput)
      || actual.executionInputRef.sha256 !==
        sha256AuthorityValue(expected.executionInput)
      || actual.fallbackPolicyRef.sha256 !==
        sha256AuthorityValue(expected.fallbackPolicy)
      || stableAuthorityStringify(
      ...
      || actual.attemptTimeoutSeconds !==
        expected.attemptTimeoutSeconds
      || actual.scheduledDelaySeconds !==
        expected.scheduledDelaySeconds
      ...
      throw new ApiError(
        'IDEMPOTENCY_CONFLICT',
        'Canonical Living Frame work item no longer matches its immutable source, dependency, output, budget, or blocked-operation authority.',
        409,
        {
          workItemKey: expected.workItemKey,
          requiredGate:
...
function assertLivingFrameExecutionAuthorityReady(
  ...
  if (!estimateWorkAssetProjection) {
    throw new ApiError(
      'IDEMPOTENCY_CONFLICT',
      'Canonical Living Frame selected-scene lineage is missing its estimate/work/asset projection.',
      409,
    )
  }
  if (!workGraphProjection) {
    throw new ApiError(
      'IDEMPOTENCY_CONFLICT',
      'Canonical Living Frame selected-scene lineage is missing its server-derived canonical work graph.',
      409,
    )
  }
  if (
    ...
    if (
      ...
      || workGraphProjection.projectedItems.length !== 0
      || customerEstimateAuthority
        .projectedLivingFrameToolCostLineItemCount !== 0
      || customerEstimateAuthority
        .projectedLivingFrameMaximumInternalToolCostCredits !== 0
