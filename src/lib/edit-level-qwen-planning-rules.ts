...
export const EDIT_LEVEL_QWEN_PLANNING_DIMENSION_DEFINITIONS: EditLevelQwenPlanningDimensionDefinition[] = [
  dimension('qwen_reasoning_depth', 'Planning reasoning depth', 'How deeply the future main planning agent should reason for the selected level.', 'runtime_disabled'),
  dimension('planning_pass_count', 'Planning pass count', 'Expected future planning pass shape.', 'available_mock'),
  ...
  dimension('source_context_depth', 'Source context depth', 'How source-understanding context is summarized for future planning.', 'available_mock'),
  ...
  dimension('edit_brief_marker_priority', 'Edit Brief marker priority', 'How much weight Edit Brief markers receive in future planning.', 'available_mock'),
  dimension('preference_dna_usage', 'Preference DNA usage', 'How strongly Edit Preference/DNA informs future planning.', 'available_mock'),
  dimension('visual_intelligence_summary_usage', 'Visual summary usage', 'How future planning consumes visual summaries produced elsewhere.', 'provider_required'),
  dimension('transcript_usage', 'Transcript usage', 'How future planning consumes transcript summaries when speech exists.', 'worker_required'),
  dimension('audio_context_usage', 'Audio context usage', 'How future planning consumes audio, music, and SFX context.', 'worker_required'),
  dimension('graphic_text_context_usage', 'Graphic/text context usage', 'How future planning consumes graphic, text, card, and layout context.', 'available_mock'),
  dimension('qa_warning_usage', 'QA warning usage', 'How future planning uses QA warnings.', 'available_mock'),
  ...
  dimension('fallback_behavior', 'Fallback behavior', 'How deterministic fallback behaves when planning context is unavailable.', 'available_mock'),
  dimension('usage_estimate_policy', 'Usage estimate policy', 'Low, medium, or high estimate-only policy for future planning.', 'available_mock'),
...
export function createEditLevelQwenPlanningDimensionRoute(input: {
  ...
  return {
    ...
    technicalNotes: [
      ...
      'RP-EDITLEVEL-07 resolves planning policy only.',
      'No model call, coding assistant call, external service, planner, edit-plan, worker, render, or credit operation runs.',
    ],
    ...sideEffectFlags,
    sideEffectFlags,
  }
...
export function createEditLevelQwenPromptPolicy(
  ...
  if (level === 'normal') {
    return {
      ...
      includeVisualIntelligenceSummary: false,
      ...
      userFacingSummary: 'Compact future planning context: user prompt, selected level, export target, basic source metadata, optional preference summary, and important Edit Brief markers only.',
      mockOnly: true,
    }
  }
  ...
  if (level === 'premium') {
    return {
      ...
      includeVisualIntelligenceSummary: true,
      ...
      userFacingSummary: 'Enhanced future planning context: source key moments, marker windows, transcript/audio/graphic summaries, Preference DNA, Edit Brief priorities, and QA warnings.',
      mockOnly: true,
    }
  }
  ...
  return {
    ...
    userFacingSummary: 'Studio future planning context: scene-level source context, visual summaries, transcript and speech timing, audio/sound design, graphic/layout context, Preference DNA, QA guardrails, marker conflicts, and plan-hint history.',
    mockOnly: true,
  }
}
...
export function createEditLevelQwenPlanningBoundarySummary(): string[] {
  return [
    'Planning profile only; no AI planning call is made.',
    'No AI planning call, technical assembly call, asset service call, plan generation, media processing, render, progress, storage, file-byte, external-fetch, or credit operation runs.',
...
function createNormalQwenPlanningDimensionRoutes(): EditLevelQwenPlanningDimensionRoute[] {
  ...
  return [
    route(level, 'qwen_reasoning_depth', 'required', 'runtime_disabled', 'standard', 'Normal uses standard future planning reasoning for a clean professional edit.', 'Use deterministic professional fallback.', 'Standard reasoning.'),
    ...
    route(level, 'visual_intelligence_summary_usage', 'optional', 'provider_required', 'targeted when needed', 'Normal consumes visual summaries only when ambiguity requires it.', 'Use source metadata and marker note fallback.', 'Targeted visual summary usage.'),
    ...
    route(level, 'fallback_behavior', 'required', 'available_mock', 'deterministic fallback acceptable', 'Normal can use deterministic professional fallback if the planning agent is unavailable.', 'Use deterministic professional fallback.', 'Deterministic fallback acceptable.'),
    route(level, 'usage_estimate_policy', 'required', 'available_mock', 'low estimate', 'Normal planning profile uses low estimate-only policy.', 'Show estimate-only copy.', 'Low estimate only.'),
...
function createPremiumQwenPlanningDimensionRoutes(): EditLevelQwenPlanningDimensionRoute[] {
  ...
  return [
    route(level, 'qwen_reasoning_depth', 'required', 'runtime_disabled', 'deep', 'Premium uses deeper future planning reasoning for creative planning.', 'Use deterministic fallback with degraded capability notice.', 'Deep creative reasoning.'),
    ...
    route(level, 'visual_intelligence_summary_usage', 'recommended', 'provider_required', 'key visual moments and marker windows', 'Premium consumes key visual summaries when available.', 'Use key-moment fallback and ask clarification if needed.', 'Visual summaries recommended.'),
    ...
    route(level, 'usage_estimate_policy', 'required', 'available_mock', 'medium estimate', 'Premium planning profile uses medium estimate-only policy.', 'Show estimate-only copy.', 'Medium estimate only.'),
...
function createUltraQwenPlanningDimensionRoutes(): EditLevelQwenPlanningDimensionRoute[] {
  ...
  return [
    route(level, 'qwen_reasoning_depth', 'required', 'runtime_disabled', 'multi_pass', 'Ultra Premium uses studio-level multi-pass future planning reasoning.', 'Degrade to Premium-safe reasoning and show clear notice.', 'Studio multi-pass reasoning.'),
    ...
    route(level, 'visual_intelligence_summary_usage', 'recommended', 'provider_required', 'scene-level visual summaries', 'Ultra Premium consumes scene-level visual summaries when available.', 'Degrade to Premium visual context and show notice.', 'Scene-level visual summary usage.'),
    ...
    route(level, 'usage_estimate_policy', 'required', 'available_mock', 'high estimate', 'Ultra Premium planning profile uses high estimate-only policy.', 'Show estimate-only copy.', 'High estimate only.'),
...
function createPackage(
  ...
  return {
    ...
    technicalSummary: `${mapCanonicalEditLevelToPublicLabel(level)} planning profile resolves ${dimensions.length} dimensions with ${reasoningDepthForLevel(level)} reasoning and ${planningPassForLevel(level)} policy; all side-effect flags remain false and no planners execute.`,
    warnings: [
      'Local planning profile only.',
      'No model call, coding assistant call, external service, planner, edit-plan, worker, render, progress, Supabase, file-byte, external-fetch, or credit operation is executed.',
...
function fallbackPolicyForLevel(level: ReEditProCanonicalEditLevel): string[] {
  if (level === 'normal') {
    return [
      ...
      'Use source metadata and marker note fallback when visual summaries are unavailable.',
      'Do not claim model, transcript, audio, graphic, or planner execution ran.',
    ]
  }
  ...
  if (level === 'premium') {
    return [
      ...
      'Use key-moment fallback and ask clarification when visual summaries or transcript context are unavailable.',
...
function userFacingSummaryForLevel(level: ReEditProCanonicalEditLevel): string {
  if (level === 'normal') {
    return 'Normal uses standard planning reasoning for a clean professional edit, with compact context and targeted clarification when needed.'
  }

  if (level === 'premium') {
    return 'Premium uses deeper planning reasoning with source context, marker windows, Edit Preference DNA, and stronger creative planning.'
  }

  return 'Ultra Premium uses studio-level planning reasoning with scene-level context, Preference DNA, strict QA, and multi-layer creative planning.'
}

function warningsForLevel(level: ReEditProCanonicalEditLevel): string[] {
  if (level === 'normal') return ['Normal planning policy is a compact future reasoning profile only.']
  if (level === 'premium') return ['Premium planning policy is deeper future reasoning metadata only and shows degraded notices when runtime is unavailable.']
  return ['Ultra Premium planning policy is studio multi-pass future reasoning metadata only and degrades to Premium-safe reasoning when runtime tools are unavailable.']
}
