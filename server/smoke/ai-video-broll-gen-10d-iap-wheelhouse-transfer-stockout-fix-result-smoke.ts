...
import {
  AI_VIDEO_BROLL_GEN_10D_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_10E_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_D_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-10d-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-10d-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-10e-no-idle-l4-iap-wheelhouse-transfer-proof-us-east1-d.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-10d-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-10d-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-10d-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
  'ai_video_broll_gen_10d_iap_wheelhouse_transfer_stockout_fix_ready_for_us_east1_d_transfer_proof_no_vm_no_inference'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll10dStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-10d-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `10D stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `10E prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_10D_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_10d_iap_wheelhouse_transfer_stockout_fix_result')
...
assert.equal(result.selectedCapacityStrategy, 'bounded_same_region_remaining_cross_zone_no_idle_transfer_proof_retry')
assert.equal(result.selectedRetryZone, 'us-east1-d')
...
assert.equal(
  result.readOnlyChecks.candidates.some(
    (candidate) =>
      ...
      candidate.selected &&
      candidate.disposition === 'selected_remaining_same_region_cross_zone_retry' &&
...
assert.equal(result.futureRetryPreflightRequired.includes('us_east1_d_zone_status'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_10E_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST1_D_PROMPT)
