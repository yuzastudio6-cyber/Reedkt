...
# Deploys but never executes the exact-byte SAM 3.1 review job. The job is a
# scale-zero private control-plane scanner, not a model or media runtime.
...
readonly BUILD_ID='4234171e-3518-496a-a20a-356984786f21'
readonly SOURCE_COMMIT='b0bd49f0b424d2937a20682020b7957f05312af2'
readonly SOURCE_TREE='fc3b5b5e63a759de8838520bda84aa0df7dd16bc'
readonly IMAGE_TAG="sam31-private-review-${SOURCE_COMMIT:0:16}"
readonly IMAGE_DIGEST='sha256:51c995ea5e6ef0ee43e2f011f45657acd4ce038dc5d6510852630fa1f5543a20'
readonly IMAGE="${REGION}-docker.pkg.dev/${PROJECT_ID}/reeditpro-workers/weeditpro-sam31-private-artifact-review@${IMAGE_DIGEST}"
readonly IMAGE_TAGGED="${REGION}-docker.pkg.dev/${PROJECT_ID}/reeditpro-workers/weeditpro-sam31-private-artifact-review:${IMAGE_TAG}"
...
readonly CONFIRMATION='deploy-weeditpro-sam31-private-artifact-review-v1'
...
[[ "${publication_id}" =~ ^[A-Za-z0-9][A-Za-z0-9._:-]{0,239}$ \
  ...
  || fail 'canonical publication reference is invalid'
...
jq -e --arg id "${publication_id}" --arg hash "${publication_hash}" '
  ...
  and .sourceArchive.coordinate.sha256
    == "5138f0e396de40a40ef0168c106e089aacbbf1dc7651be2f81c76f89c2f67f2a"
  and .sourceArchive.coordinate.byteLength == 73605120
  and .checkpoint.coordinate.sha256
    == "0567debeec80ba4ac6369540c6c248025283cb3ff2b92827509e57e2b3541cb6"
  and .checkpoint.coordinate.byteLength == 3502755717
...
gcloud run jobs deploy "${JOB}" \
  ...
  --cpu=4 --memory=16Gi --tasks=1 --parallelism=1 \
  --max-retries=0 --task-timeout=4h --execution-environment=gen2 \
  --network="${NETWORK}" --subnet="${SUBNET}" --vpc-egress=all-traffic \
  --set-env-vars="WEEDITPRO_SAM31_PRIVATE_ARTIFACT_REVIEW_CONFIRM=review-exact-published-sam31-private-artifacts,WEEDITPRO_SAM31_PUBLICATION_ID=${publication_id},WEEDITPRO_SAM31_PUBLICATION_SHA256=${publication_hash}" \
  --quiet
...
if jq -e 'any(.bindings[]?.members[]?;
  ...
  fail 'private artifact review job has a public IAM principal'
fi

printf '{"operation":"weeditpro_sam31_private_artifact_review_job_deploy_v1",'
