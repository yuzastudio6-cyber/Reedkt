...
Decision: `qwen2_5_vl_cloud_run_gpu_private_invoke_readiness_rollup_active_migration_remote_satisfaction_accepted_runtime_dispatch_readiness_required`.
...
| Persisted worker dispatch readiness review | blocked persisted worker dispatch readiness review required | Backend runtime persistence no-deploy satisfaction is accepted, fail-closed coordinator evidence remains recorded, and approved worker integration review accepted metadata-only local queue and private invoke contract shapes. | Review persisted job, lease, idempotency, backend runtime message, job event, worker claim, private storage record, signed URL audit, QA, audit, and credit boundaries before any real dispatch path. |
