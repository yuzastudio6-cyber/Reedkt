...
const confirmationVar = 'REEDITPRO_CONFIRM_SUPABASE_CLEAN_STAGING_BRANCH_MIGRATION_CHAIN_APPLY'
...
const reportDir = 'docs/activation-supabase-clean-staging-branch-migration-chain-apply-1-reports'
const reportPath = path.join(reportDir, 'clean_staging_branch_migration_chain_apply_report.json')
const manifestPath = path.join(reportDir, 'clean_staging_branch_migration_chain_apply_manifest.json')
const baseOutputDir = '/tmp/reeditpro-supabase-clean-staging-branch-migration-chain-apply-1'
...
function runMigrationList(dbUrl) {
  ...
  return {
    ...
    untrackedRemoteMigrationIds: textIds.remoteOnlyMigrationIds.filter((id) => !textIds.localMigrationIds.includes(id)),
...
if (process.env[confirmationVar] !== 'true') {
  finish({
    decision: 'blocked_pending_clean_staging_branch_migration_chain_apply_confirmation',
    execution: 'blocked_confirmation_absent_no_secret_payload_access',
    blocker: 'blocked_pending_clean_staging_branch_migration_chain_apply_confirmation',
...
const beforeHistory = runMigrationList(dbUrl)
const beforeMissing = missingRequired(beforeHistory)
const dryRun = runDryRun(dbUrl)
if (dryRun.status !== 'passed') {
  const untrackedRemoteMigrationIds = beforeHistory.untrackedRemoteMigrationIds ?? []
  const dryRunBlocker = untrackedRemoteMigrationIds.length > 0
    ? 'blocked_clean_branch_remote_migration_history_has_untracked_versions'
    : 'blocked_clean_branch_migration_chain_dry_run_failed'
  finish({
    decision: dryRunBlocker,
...
const apply = runApply(dbUrl)
if (apply.status !== 'passed') {
  const afterFailedHistory = runMigrationList(dbUrl)
  finish({
    decision: 'blocked_clean_branch_migration_chain_apply_failed',
    execution: 'blocked_migration_apply_failed_readback_recorded',
    blocker: 'blocked_clean_branch_migration_chain_apply_failed',
    confirmation: 'present_true',
    credentialMetadata: {
      supabaseAccessTokenSecret: accessTokenSecret,
      supabaseAccessTokenLatestVersion: accessTokenMetadata,
...
const afterHistory = runMigrationList(dbUrl)
const afterMissing = missingRequired(afterHistory)
const catalog = afterMissing.length === 0 ? runReadOnlyCatalogSql(dbUrl) : null
...
let decision = 'completed_clean_staging_branch_migration_chain_apply_and_readback'
let blocker = 'none'
if (afterHistory.status !== 'passed' || afterMissing.length > 0) {
  decision = 'blocked_clean_branch_post_apply_migration_history_not_current'
  blocker = 'blocked_clean_branch_post_apply_migration_history_not_current'
} else if (!catalog || catalog.status !== 'passed') {
  decision = 'blocked_clean_branch_post_apply_catalog_readback_failed'
  blocker = 'blocked_clean_branch_post_apply_catalog_readback_failed'
} else if (missingTables.length > 0 || rlsMissingTables.length > 0) {
  decision = 'blocked_clean_branch_post_apply_schema_rls_readback_failed'
  blocker = 'blocked_clean_branch_post_apply_schema_rls_readback_failed'
} else if (missingPrivateBuckets.length > 0 || publicPrivateBuckets.length > 0) {
  decision = 'blocked_clean_branch_post_apply_private_storage_bucket_metadata_readback_failed'
  blocker = 'blocked_clean_branch_post_apply_private_storage_bucket_metadata_readback_failed'
}

finish({
  decision,
  execution: decision === 'completed_clean_staging_branch_migration_chain_apply_and_readback'
    ? 'completed_guarded_clean_branch_migration_chain_apply_readback_no_beta_unlock'
  ...
  readiness: {
    workerRuntimeTransactionalContract2: decision === 'completed_clean_staging_branch_migration_chain_apply_and_readback'
      ? 'ready_for_clean_staging_worker_rpc_readback'
      : 'blocked_pending_clean_staging_post_apply_readback_closure',
    workerRuntimeTrackaPrivateE2eExecutionGate2r: decision === 'completed_clean_staging_branch_migration_chain_apply_and_readback'
      ? 'blocked_pending_worker_rpc_readback_and_service_role_runtime_validation'
      : 'blocked_pending_clean_staging_post_apply_readback_closure',
    internalBetaReadinessRollup: decision === 'completed_clean_staging_branch_migration_chain_apply_and_readback'
  ...
  packageLock: 'unchanged',
  generatedArtifactsCommitted: 'none',
}, decision === 'completed_clean_staging_branch_migration_chain_apply_and_readback' ? 0 : 1)
