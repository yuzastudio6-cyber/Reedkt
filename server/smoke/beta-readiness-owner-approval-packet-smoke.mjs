...
assert.equal(report.sourceTruth.sourceSha, 'ee177046bfb07868c4eb0ebd04f4eaff42c811ce')
assert.equal(report.sourceTruth.deployedEvidenceSourceSha, 'ee177046bfb07868c4eb0ebd04f4eaff42c811ce')
assert.equal(report.sourceTruth.sourceFreshnessDecision, 'beta_readiness_source_freshness_preflight_passed_current_source_matches_deploy_evidence')
assert.equal(report.sourceTruth.currentSourceApiDeployPacket, 'docs/beta-readiness/api-staging-deploy-current-source/2026-06-30-ee177-api-staging-deploy.json')
assert.equal(report.sourceTruth.deployedEvidenceInputManifest, 'docs/beta-readiness/deployed-evidence-input-manifest/2026-06-30-ee177-deployed-evidence-input-manifest.json')
assert.equal(report.sourceTruth.normalApiRevision, 'reeditpro-api-staging-00015-skq')
...
assert.equal(report.sourceTruth.normalApiDeployRunId, '28419763433')
...
assert.ok(report.postApprovalCommands.includes('REEDITPRO_BETA_OPERATOR_ENV_FILE=.env.reeditpro-beta-operator.local npm run beta:readiness:external-beta-operator-value-progress -- --markdown'))
assert.ok(report.postApprovalCommands.includes('npm run beta:readiness:external-beta-operator-autofill-env'))
assert.ok(report.postApprovalCommands.includes('npm run beta:readiness:external-beta-operator-autofill-local-env'))
