...
- Scope: first scoped callable execution path for the AI graphics lane. The route is mounted only behind `AI_GRAPHICS_EXTERNAL_BETA_CPU_STATIC_CONTROLLED_TOOL_CALL_ROUTE_ENABLED=true` and accepts the five CPU/static tools with installed package execution proof: `d3`, `vega_lite`, `vega`, `svgdotjs_svg_js`, and `viz_js`.
- Result: the real Express app returns `200` for all five scoped route calls when the dedicated flag is enabled, returns `404` when disabled, invokes the controlled CPU/static adapter for all five tools, and returns private output metadata plus SHA-256 hashes only. `controlledCpuStaticRouteCallableNowTools=5`, `controlledAdapterExecutedTools=5`, `remainingToolsStillBlockedForRuntime=16`, `gpuRuntimeShouldStartNowTools=0`, `publicArtifactCreatedTools=0`, and `signedUrlCreatedTools=0`.
- Runtime/beta/production: scoped CPU/static route execution is now callable for those five tools only. It does not unlock all 21 tools, external beta traffic, production, GPU/model runtime, browser/WebGL/canvas runtime, provider/model calls, Supabase/GCS mutation, signed URLs, public artifacts, or media processing. `externalAgentCanExecuteCpuStaticControlledToolsNow=true`, while `agentCanExecuteAll21ToolsNow=false`, `gpuRuntimeShouldStartNow=false`, `externalBetaReadyNow=false`, and `productionReadyNow=false`.
