...
import {
  AI_VIDEO_BROLL_GEN_9V_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT,
  AI_VIDEO_BROLL_GEN_9W_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST4_A_PROMPT,
} from '../../src/backend/mock/mock-ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix-result'

const ROOT = process.cwd()
const DOC_PATH = 'docs/ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix-result.md'
const PROMPT_PATH =
  'docs/implementation-prompts/prompt-ai-video-broll-gen-9w-no-idle-l4-iap-wheelhouse-transfer-proof-us-east4-a.md'
const SPEC_PATH = 'src/backend/mock/mock-ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix-result.ts'
const SMOKE_PATH = 'server/smoke/ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts'
const PACKAGE_SCRIPT = 'smoke:ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix-result'
const DECISION =
  'ai_video_broll_gen_9v_iap_wheelhouse_transfer_stockout_fix_ready_for_us_east4_a_transfer_proof_no_vm_no_inference'

function read(relativePath: string): string {
  return readFileSync(path.join(ROOT, relativePath), 'utf8')
}

function scanForbiddenValues(value: unknown, prefix = 'broll9vStockoutFix'): string[] {
...
assert.equal(
  ...
  'tsx server/smoke/ai-video-broll-gen-9v-iap-wheelhouse-transfer-stockout-fix-result-smoke.ts',
...
for (const required of [
  ...
  assert.equal(doc.includes(required), true, `9V stockout fix doc missing ${required}`)
}
...
for (const required of [
  ...
  assert.equal(prompt.includes(required), true, `9W prompt missing ${required}`)
}

const result = AI_VIDEO_BROLL_GEN_9V_IAP_WHEELHOUSE_TRANSFER_STOCKOUT_FIX_RESULT
assert.equal(result.decision, DECISION)
assert.equal(result.mode, 'ai_video_broll_gen_9v_iap_wheelhouse_transfer_stockout_fix_result')
...
assert.equal(result.selectedRetryZone, 'us-east4-a')
...
assert.equal(result.strategyRationale.includes('all_us_west1_transfer_proof_zones_stocked_out_before_vm_creation'), true)
assert.equal(
  result.strategyRationale.includes('us_east4_a_is_first_untried_cross_region_fallback_with_visible_shape_and_unused_l4_quota'),
  true,
)
...
assert.equal(result.futureRetryPreflightRequired.includes('us_east4_a_zone_status'), true)
...
assert.equal(result.nextPrompt, AI_VIDEO_BROLL_GEN_9W_NO_IDLE_L4_IAP_WHEELHOUSE_TRANSFER_PROOF_US_EAST4_A_PROMPT)
