...
If the local faster-whisper runtime/model prerequisites are not present, use the explicit internal-testing bootstrap in `docs/internal-testing-faster-whisper-local-bootstrap.md`. That command requires `REEDITPRO_CONFIRM_INTERNAL_TESTING_FASTER_WHISPER_LOCAL_BOOTSTRAP=true`, writes only under `/private/tmp`, and produces the local Python command, model path, and manifest path for rerunning the real-video acceptance command. Normal acceptance commands still do not install packages or download models.
