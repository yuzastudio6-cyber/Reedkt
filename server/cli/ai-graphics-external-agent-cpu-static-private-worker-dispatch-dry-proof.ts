...
} from '../tool-registry/ai-graphics-external-agent-cpu-static-private-worker-dispatch-dry-proof'

const sourceClaimDryProofPath =
...
function makeMarkdown(
  report: ReturnType<typeof buildAiGraphicsExternalAgentCpuStaticPrivateWorkerDispatchDryProof>,
): string {
  const dispatchableTools = report.rows
    .filter((row) => row.dryWorkerDispatchProofPrepared)
    .map((row) => `\`${row.toolId}\``)
    .join(', ')
  ...
  const rows = report.rows
    .map(
      (row) =>
        `| \`${row.toolId}\` | \`${row.runtimeTarget}\` | \`${row.dispatchDryProofStatus}\` | \`${row.dryWorkerDispatchProofPrepared}\` | \`${row.dryWorkerDispatchEnvelopePrepared}\` | \`${row.externalAgentCanDispatchPrivateWorkerJobNow}\` | \`${row.workerClaimApprovedNow}\` | \`${row.workerDispatchApprovedNow}\` | \`${row.toolExecutionApprovedNow}\` | ${row.blocker} |`,
    )
    .join('\n')
  const dispatchEnvelopes = report.rows
    .filter((row) => row.dryDispatchContract)
    .map((row) => {
      const dispatch = row.dryDispatchContract
      return `- \`${row.toolId}\`: queue=\`${dispatch?.queueName}\`, snapshot=\`${dispatch?.approvedPlanSnapshotRef}\`, reservation=\`${dispatch?.creditReservationRef}\`, manifest=\`${dispatch?.privateArtifactManifestRef}\`, queueIdempotency=\`${dispatch?.queuePayloadIdempotencyKey}\`, dispatchIdempotency=\`${dispatch?.dryDispatchIdempotencyKey}\`, dispatchAttempt=\`${dispatch?.workerDispatchAttemptRef}\``
    })
    .join('\n')
  const liveRequirements = report.dryDispatchPolicy.requiredBeforeAnyLiveWorkerDispatch
    .map((requirement) => `- ${requirement}`)
    .join('\n')

  return `# AI Graphics External Agent CPU Static Private Worker Dispatch Dry Proof
...
- Private worker claim dry-proof packet: \`${sourceClaimDryProofPath}\`
- Source claim dry-proof decision: \`${report.sourceClaimDryProofDecision}\`
...
- Private worker dispatch dry proofs prepared: \`${report.counts.privateWorkerDispatchDryProofPreparedTools}\` tools: ${dispatchableTools}
- Dry worker dispatch envelopes prepared: \`${report.counts.dryWorkerDispatchEnvelopesPreparedTools}\`
- CPU/static blocked tools: \`${report.counts.cpuStaticPrivateWorkerDispatchDryBlockedTools}\`
...
function makePromptResult(
  ...
  return `# Prompt AI Graphics External Agent CPU Static Private Worker Dispatch Dry Proof Results
...
- Decision: \`${report.decision}\`
- Private worker dispatch dry proofs prepared: \`${report.counts.privateWorkerDispatchDryProofPreparedTools}\`
- Dry worker dispatch envelopes prepared: \`${report.counts.dryWorkerDispatchEnvelopesPreparedTools}\`
- CPU/static blocked tools: \`${report.counts.cpuStaticPrivateWorkerDispatchDryBlockedTools}\`
...
function makeImplementationPrompt(
  ...
  return `# AI Graphics External Agent CPU Static Private Worker Dispatch Dry Proof Implementation Record

Implemented the private worker dispatch dry-proof contract from accepted CPU/static private worker claim dry-proof evidence.
...
- \`${report.counts.privateWorkerDispatchDryProofPreparedTools}\` CPU/static tools have private worker dispatch dry-proof envelopes prepared.
- \`${report.counts.dryWorkerDispatchEnvelopesPreparedTools}\` dry dispatch envelopes preserve approved plan snapshot fixture refs, credit reservation fixture refs, private artifact manifest refs, queue idempotency keys, dry dispatch idempotency keys, request trace refs, checkback refs, fallback refs, QA gate refs, and private artifact visibility.
...
async function main() {
  const source = readJson(sourceClaimDryProofPath)

  const report = buildAiGraphicsExternalAgentCpuStaticPrivateWorkerDispatchDryProof({
    sourceClaimDryProofDecision: source.decision,
    sourceClaimDryProofStatus: source.status,
    sourceClaimDryProofRows: source.rows,
    sourcePrivateWorkerClaimDryProofPreparedTools:
      source.counts?.privateWorkerClaimDryProofPreparedTools,
    sourceCpuStaticPrivateWorkerClaimDryReadyTools:
      source.counts?.cpuStaticPrivateWorkerClaimDryReadyTools,
    sourceCpuStaticPrivateWorkerClaimDryBlockedTools:
      source.counts?.cpuStaticPrivateWorkerClaimDryBlockedTools,
    ...
    sourceDryWorkerClaimEnvelopesPreparedTools:
      source.counts?.dryWorkerClaimEnvelopesPreparedTools,
    sourceExternalAgentCanClaimPrivateWorkerJobNowTools:
      source.counts?.externalAgentCanClaimPrivateWorkerJobNowTools,
    ...
    sourceWorkerClaimApprovedNowTools:
      source.counts?.workerClaimApprovedNowTools,
  ...
  assert(
    report.decision ===
      AI_GRAPHICS_EXTERNAL_AGENT_CPU_STATIC_PRIVATE_WORKER_DISPATCH_DRY_PROOF_DECISION,
    'Unexpected private worker dispatch dry-proof decision',
  )
  assert(
    report.counts.totalAiGraphicsTools === 21,
    'Private worker dispatch dry proof must cover all 21 tools',
  )
  assert(
    report.counts.privateWorkerDispatchDryProofPreparedTools === 5,
    'Expected five private worker dispatch dry proofs',
  )
  assert(
    report.counts.dryWorkerDispatchEnvelopesPreparedTools === 5,
    'Expected five private worker dry dispatch envelopes',
  )
  assert(
  ...
  assert(
    ...
    'Source claim dry proof was not accepted',
  )
