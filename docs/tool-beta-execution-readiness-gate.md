...
- `POST /v1/beta-readiness/platform-billing-qa` runs an idempotent platform billing QA harness. In mock mode it records a controlled in-memory tool-cost event, replays the same idempotency key, reads the project summary, confirms wallet settlement and Stripe remain separate, and names the staging/production evidence still missing. In non-mock mode, persistent-store QA is blocked unless `allowPersistentStoreQa` is explicitly true for an approved staging/production billing QA run.
